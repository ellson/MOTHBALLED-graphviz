...
static void vml_begin_graph(GVJ_t *job) {
  ...
  gvputs(job,
         "      if ( msie > 0 ){      // If Internet Explorer, return version "
         "number\n");
  gvputs(job,
         "         ievers= parseInt (ua.substring (msie+5, ua.indexOf ('.', "
         "msie )))\n");
  gvputs(job, "      }\n");
  ...
  gvputs(job,
         "<DIV id='_VML1_' style=\"position:relative; display:inline; "
         "visibility:hidden");
  ...
  gvprintf(job, " coordorigin=\"0,0\" coordsize=\"%d,%d\" >", graphWidth,
           graphHeight);
}
...
static void vml_end_graph(GVJ_t *job) {
  ...
  gvputs(job,
         "<H2>Sorry, this diagram will only display correctly on Internet "
         "Explorer 5 (and up) browsers.</H2>\n");
  gvputs(job, "</DIV>\n");
...
static void vml_begin_anchor(GVJ_t *job, char *href, char *tooltip,
                             char *target, char *id) {
  gvputs(job, "<a");
  if (href && href[0]) gvprintf(job, " href=\"%s\"", html_string(href));
  if (tooltip && tooltip[0])
    gvprintf(job, " title=\"%s\"", html_string(tooltip));
  if (target && target[0]) gvprintf(job, " target=\"%s\"", html_string(target));
  gvputs(job, ">\n");
}

static void vml_end_anchor(GVJ_t *job) { gvputs(job, "</a>\n"); }
...
static void vml_textspan(GVJ_t *job, pointf p, textspan_t *span) {
  ...
  /* text "y" was too high
   * Graphviz uses "baseline", VML seems to use bottom of descenders - so we
   * fudge a little
  ...
  gvputs(job,
         "<v:textbox inset=\"0,0,0,0\" style=\"position:absolute; "
         "v-text-wrapping:'false';padding:'0';");

  pA = span->font->postscript_alias;
  if (pA) {
    gvprintf(job, "font-family: '%s';", pA->family);
    if (pA->weight) gvprintf(job, "font-weight: %s;", pA->weight);
    if (pA->stretch) gvprintf(job, "font-stretch: %s;", pA->stretch);
    if (pA->style) gvprintf(job, "font-style: %s;", pA->style);
  } else {
  ...
  switch (obj->pencolor.type) {
    case COLOR_STRING:
      ...
      break;
    case RGBA_BYTE:
      gvprintf(job, "color:#%02x%02x%02x;", obj->pencolor.u.rgba[0],
...
static void vml_bezier(GVJ_t *job, pointf *A, int n, int arrow_at_start,
...
static void vml_polyline(GVJ_t *job, pointf *A, int n) {
  ...
  gvputs(job, " <v:shape style=\"position:absolute; ");
  gvprintf(job, " width: %d; height: %d\" filled=\"false\">", graphWidth,
           graphHeight);
  gvputs(job, "<v:path v=\"");
  for (i = 0; i < n; i++) {
    if (i == 0) gvputs(job, " m ");
...
gvrender_engine_t vml_engine = {
    vml_begin_job,    0,                 /* vml_end_job */
    vml_begin_graph,  vml_end_graph,  0, /* vml_begin_layer */
    0,                                   /* vml_end_layer */
    0,                                   /* vml_begin_page */
    0,                                   /* vml_end_page */
    0,                                   /* vml_begin_cluster */
    ...
    0,                                   /* vml_begin_edge */
    0,                                   /* vml_end_edge */
    vml_begin_anchor, vml_end_anchor, 0, /* vml_begin_label */
    0,                                   /* vml_end_label */
    vml_textspan,     0,                 /* vml_resolve_color */
    vml_ellipse,      vml_polygon,    vml_bezier,
    vml_polyline,     vml_comment,    0, /* vml_library_shape */
};
...
gvdevice_features_t device_features_vmlz = {
    GVDEVICE_DOES_TRUECOLOR | GVDEVICE_COMPRESSED_FORMAT, /* flags */
    {0., 0.},   /* default margin - points */
    {0., 0.},   /* default page width, height - points */
    {96., 96.}, /* default dpi */
};
