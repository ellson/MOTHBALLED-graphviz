...
static void map_output_shape(GVJ_t *job, map_shape_t map_shape, pointf *AF,
                             int nump, char *url, char *tooltip, char *target,
                             char *id) {
  int i;
  ...
  if (job->render.id == FORMAT_IMAP && url && url[0]) {
    switch (map_shape) {
      ...
      case MAP_POLYGON:
        ...
        for (i = 0; i < nump; i++) gvprintf(job, " %d,%d", A[i].x, A[i].y);
        gvputs(job, "\n");
        break;
      default:
        assert(0);
    ...
    switch (map_shape) {
      ...
      case MAP_POLYGON:
        ...
        for (i = 1; i < nump; i++) gvprintf(job, ",%d,%d", A[i].x, A[i].y);
        break;
      default:
        break;
    }
...
static void map_begin_anchor(GVJ_t *job, char *url, char *tooltip, char *target,
  ...
  map_output_shape(job, obj->url_map_shape, obj->url_map_p, obj->url_map_n, url,
                   tooltip, target, id);
}

static gvrender_engine_t map_engine = {0, /* map_begin_job */
                                       0, /* map_end_job */
                                       0, /* map_begin_graph */
                                       0, /* map_end_graph */
                                       0, /* map_begin_layer */
                                       ...
                                       0, /* map_begin_edge */
                                       0, /* map_end_edge */
                                       map_begin_anchor, 0, /* map_end_anchor */
                                       0, /* map_begin_label */
                                       0, /* map_end_label */
                                       0, /* map_textpara */
                                       0, /* map_resolve_color */
...
static gvrender_features_t render_features_map = {
    EMIT_CLUSTERS_LAST | GVRENDER_Y_GOES_DOWN | GVRENDER_DOES_MAPS |
        GVRENDER_DOES_LABELS | GVRENDER_DOES_TOOLTIPS | GVRENDER_DOES_TARGETS |
...
static gvdevice_features_t device_features_map = {
    GVRENDER_DOES_MAP_CIRCLE | GVRENDER_DOES_MAP_POLYGON, /* flags */
