...
static void svg_begin_graph(GVJ_t *job) {
  ...
  gvprintf(job, "<svg width=\"%dpt\" height=\"%dpt\"\n", job->width,
           job->height);
  gvprintf(job, " viewBox=\"%.2f %.2f %.2f %.2f\"", job->canvasBox.LL.x,
           job->canvasBox.LL.y, job->canvasBox.UR.x, job->canvasBox.UR.y);
  /* namespace of svg */
...
static void svg_begin_page(GVJ_t *job) {
  ...
  gvprintf(job, " transform=\"scale(%g %g) rotate(%d) translate(%g %g)\">\n",
           job->scale.x, job->scale.y, -job->rotation, job->translation.x,
           -job->translation.y);
  /* default style */
  if (agnameof(obj->u.g)[0]) {
    gvputs(job, "<title>");
...
static void svg_begin_cluster(GVJ_t *job) {
  ...
  gvputs(job, "<g id=\"");
  gvputs(job, xml_string(obj->id));
  if( job->events.onfocusin != NULL )
    gvputs(job, printf("onfocusin="%s"", job->events.onfocusin));
  if( job->events.onfocusout != NULL )
    gvputs(job, printf("onfocusout="%s"", job->events.onfocusout));
  if( job->events.onactivate != NULL )
    gvputs(job, printf("onactivate="%s"", job->events.onactivate));
  if( job->events.onclick != NULL )
    gvputs(job, printf("onclick="%s"", job->events.onclick));
  if( job->events.onmousedown != NULL )
    gvputs(job, printf("onmousedown="%s"", job->events.onmousedown));
  if( job->events.onmouseup != NULL )
    gvputs(job, printf("onmouseup="%s"", job->events.onmouseup));
  if( job->events.onmouseover != NULL )
    gvputs(job, printf("onmouseover="%s"", job->events.onmouseover));
  if( job->events.onmousemove != NULL )
    gvputs(job, printf("onmousemove="%s"", job->events.onmousemove));
  if( job->events.onmouseout != NULL )
    gvputs(job, printf("onmouseout="%s"", job->events.onmouseout));
  if( job->events.onunload != NULL )
    gvputs(job, printf("onunload="%s"", job->events.onunload));
  if( job->events.onabort != NULL )
    gvputs(job, printf("onabort="%s"", job->events.onabort));
  if( job->events.onerror != NULL )
    gvputs(job, printf("onerror="%s"", job->events.onerror));
  if( job->events.onresize != NULL )
    gvputs(job, printf("onresize="%s"", job->events.onresize));
  if( job->events.onscroll != NULL )
    gvputs(job, printf("onscroll="%s"", job->events.onscroll));
  if( job->events.onzoom != NULL )
    gvputs(job, printf("onzoom="%s"", job->events.onzoom));
  if( job->events.onbegin != NULL )
    gvputs(job, printf("onbegin="%s"", job->events.onbegin));
  if( job->events.onend != NULL )
    gvputs(job, printf("onend="%s"", job->events.onend));
  if( job->events.onrepeat != NULL )
    gvputs(job, printf("onrepeat="%s"", job->events.onrepeat));
...
static void svg_textspan(GVJ_t *job, pointf p, textspan_t *span) {
  ...
  if (!obj->labeledgealigned) gvprintf(job, " x=\"%g\" y=\"%g\"", p.x, -p.y);
  ...
  if ((span->font) && (flags = span->font->flags)) {
    ...
    if ((flags & (HTML_UL | HTML_S | HTML_OL))) {
      ...
      if ((flags & HTML_S)) gvprintf(job, "%sline-through", (comma ? "," : ""));
      gvprintf(job, "\"");
    }
  ...
  if (obj->labeledgealigned) {
    gvprintf(job, "<textPath xlink:href=\"#%s_p\" startOffset=\"50%%\">",
             xml_string(obj->id));
    gvprintf(job, "<tspan x=\"0\" dy=\"%g\">", -p.y);
  }
  gvputs(job, xml_string0(span->str, TRUE));
  if (obj->labeledgealigned) gvprintf(job, "</tspan></textPath>");
  gvputs(job, "</text>\n");
}
...
static int svg_gradstyle(GVJ_t *job, pointf *A, int n) {
  ...
  if (obj->gradient_frac > 0)
    gvprintf(job, "<stop offset=\"%.03f\" style=\"stop-color:",
             obj->gradient_frac - 0.001);
  else
...
static int svg_rgradstyle(GVJ_t *job, pointf *A, int n) {
  ...
  gvprintf(job,
           "<defs>\n<radialGradient id=\"r_%d\" cx=\"50%%\" cy=\"50%%\" "
           "r=\"75%%\" fx=\"%d%%\" fy=\"%d%%\">\n",
           id, ifx, ify);
  gvputs(job, "<stop offset=\"0\" style=\"stop-color:");
  svg_print_color(job, obj->fillcolor);
...
static void svg_ellipse(GVJ_t *job, pointf *A, int filled) {
  ...
  gvprintf(job, " rx=\"%g\" ry=\"%g\"", A[1].x - A[0].x, A[1].y - A[0].y);
  gvputs(job, "/>\n");
}

static void svg_bezier(GVJ_t *job, pointf *A, int n, int arrow_at_start,
...
static void svg_polygon(GVJ_t *job, pointf *A, int n, int filled) {
  ...
  svg_grstyle(job, filled, gid);
  gvputs(job, " points=\"");
  for (i = 0; i < n; i++) gvprintf(job, "%g,%g ", A[i].x, -A[i].y);
...
gvrender_engine_t svg_engine = {
    ...
    svg_begin_graph,  svg_end_graph,  svg_begin_layer,   svg_end_layer,
    svg_begin_page,   svg_end_page,   svg_begin_cluster, svg_end_cluster,
    0, /* svg_begin_nodes */
    0, /* svg_end_nodes */
    0, /* svg_begin_edges */
    0, /* svg_end_edges */
    svg_begin_node,   svg_end_node,   svg_begin_edge,    svg_end_edge,
    svg_begin_anchor, svg_end_anchor, 0, /* svg_begin_anchor */
    0,                                   /* svg_end_anchor */
    svg_textspan,     0,                 /* svg_resolve_color */
    svg_ellipse,      svg_polygon,    svg_bezier,        svg_polyline,
    svg_comment,      0, /* svg_library_shape */
};
