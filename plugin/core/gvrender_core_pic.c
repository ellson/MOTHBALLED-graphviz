...
static char *picfontname(char *psname) {
  ...
  for (p = fonttab; p->psname; p++)
    if (strcmp(p->psname, psname) == 0) break;
  if (p->psname)
    rv = p->trname;
  else {
...
static void pic_line_style(obj_state_t *obj, int *line_style,
                           double *style_val) {
  switch (obj->pen) {
    case PEN_DASHED:
      *line_style = 1;
...
static void pic_begin_graph(GVJ_t *job) {
  ...
  gvprintf(job, "%s save point size and font\n.nr .S \\n(.s\n.nr DF \\n(.f\n",
           EscComment);
}

static void pic_end_graph(GVJ_t *job) {
  gvprintf(job, "%s restore point size and font\n.ps \\n(.S\n.ft \\n(DF\n",
           EscComment);
}
...
static void pic_begin_page(GVJ_t *job) {
  ...
  gvprintf(job,
           "%s to change drawing size, multiply the width and height on the "
           ".PS line above and the number on the two lines below (rounded to "
           "the nearest integer) by a scale factor\n",
           EscComment);
  if (width > 0.0) {
  ...
  gvprintf(job, "%s don't change anything below this line in this drawing\n",
           EscComment);
  gvprintf(job, "%s non-fatal run-time pic version determination, version 2\n",
           EscComment);
  gvprintf(job, "boxrad=2.0 %s will be reset to 0.0 by gpic only\n",
           EscComment);
  gvprintf(job, "scale=1.0 %s required for comparisons\n", EscComment);
  gvprintf(job, "%s boxrad is now 0.0 in gpic, else it remains 2.0\n",
           EscComment);
  gvprintf(job,
           "%s dashwid is 0.1 in 10th Edition, 0.05 in DWB 2 and in gpic\n",
           EscComment);
  gvprintf(job,
           "%s fillval is 0.3 in 10th Edition (fill 0 means black), 0.5 in "
           "gpic (fill 0 means white), undefined in DWB 2\n",
           EscComment);
  gvprintf(job,
           "%s fill has no meaning in DWB 2, gpic can use fill or filled, 10th "
           "Edition uses fill only\n",
           EscComment);
  gvprintf(job, "%s DWB 2 doesn't use fill and doesn't define fillval\n",
           EscComment);
  gvprintf(
      job,
  ...
  gvprintf(job,
           "%s GNU pic supports a linethick variable to set line thickness; "
           "DWB and 10th Ed. do not\n",
           EscComment);
...
static void pic_end_page(GVJ_t *job) { gvprintf(job, "]\n.PE\n"); }
...
static void pic_textspan(GVJ_t *job, pointf p, textspan_t *span) {
  ...
  if ((sz = (int)span->font->size) < 1) sz = 1;
  if (sz != lastsize) {
    gvprintf(job, ".ps %d*\\n(SFu/%.0fu\n", sz, Fontscale);
    lastsize = sz;
  }
...
static void pic_ellipse(GVJ_t *job, pointf *A, int filled) {
  ...
  gvprintf(job, "ellipse attrs%d %swid %.5f ht %.5f at (%.5f,%.5f);\n", 1,
           filled ? "fill " : "", PS2INCH(2 * (A[1].x - A[0].x)),
           PS2INCH(2 * (A[1].y - A[0].y)), PS2INCH(A[0].x), PS2INCH(A[0].y));
}
...
gvrender_engine_t pic_engine = {
    0,                                 /* pic_begin_job */
    0,                                 /* pic_end_job */
    pic_begin_graph, pic_end_graph, 0, /* pic_begin_layer */
    0,                                 /* pic_end_layer */
    pic_begin_page,  pic_end_page,  0, /* pic_begin_cluster */
    0,                                 /* pic_end_cluster */
    0,                                 /* pic_begin_nodes */
    0,                                 /* pic_end_nodes */
    0,                                 /* pic_begin_edges */
    ...
    0,                                 /* pic_begin_label */
    0,                                 /* pic_end_label */
    pic_textspan,    0,                /* pic_resolve_color */
    pic_ellipse,     pic_polygon,   pic_bezier,
    pic_polyline,    pic_comment,   0, /* pic_library_shape */
};
