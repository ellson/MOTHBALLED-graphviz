...
static void mif_ptarray(GVJ_t *job, pointf *A, int n) {
  ...
  for (i = 0; i < n; i++) gvprintf(job, " <Point %.2f %.2f>\n", A[i].x, A[i].y);
}
...
static mif_color_t mif_colors[] = {{"Black", {0, 0, 0, 100}},
                                   {"White", {0, 0, 0, 0}},
                                   {"Red", {0, 100, 100, 0}},
                                   {"Green", {100, 0, 100, 0}},
                                   {"Blue", {100, 100, 0, 0}},
...
static void mif_comment(GVJ_t *job, char *str) { gvprintf(job, "# %s\n", str); }
...
static void mif_begin_job(GVJ_t *job) {
  gvprintf(job, "<MIFFile 3.00> # Generated by %s version %s (%s)\n",
           job->common->info[0], job->common->info[1], job->common->info[2]);
}

static void mif_end_job(GVJ_t *job) { gvputs(job, "# end of MIFFile\n"); }
...
static void mif_begin_graph(GVJ_t *job) {
  ...
  gvprintf(job, "<BRect %g %g %g %g>\n", job->canvasBox.LL.x,
           job->canvasBox.UR.y, job->canvasBox.UR.x - job->canvasBox.LL.x,
           job->canvasBox.UR.y - job->canvasBox.LL.y);
}

static void mif_begin_page(GVJ_t *job) {
  gvprintf(job,
...
static void mif_set_color(GVJ_t *job, char *name) {
  ...
  for (i = 0; i < (sizeof(mif_colors) / sizeof(mif_color_t)); i++) {
    if (strcasecmp(mif_colors[i].name, name) == 0) mif_color(job, i);
  }
...
static void mif_textpara(GVJ_t *job, pointf p, textpara_t *para) {
  ...
  gvprintf(job, "<TextLine <Angle %d> <TLOrigin %.2f %.2f> <TLAlignment %s>",
...
static void mif_bezier(GVJ_t *job, pointf *A, int n, int arrow_at_start,
  ...
  gvprintf(job, "<PolyLine <Fill 15> <Smoothed Yes> <HeadCap Square>\n");
  mif_ptarray(job, A, n);
  gvputs(job, ">\n");
}
...
static void mif_ellipse(GVJ_t *job, pointf *A, int filled) {
  ...
  gvprintf(job, "<Ellipse %s <BRect %.2f %.2f %.1f %.1f>>\n",
           filled ? FillStr : NoFillStr, A[1].x - dia.x, A[1].y, dia.x, -dia.y);
}
...
gvrender_engine_t mif_engine = {
    mif_begin_job,  mif_end_job, mif_begin_graph, 0, /* mif_end_graph */
    0,                                               /* mif_begin_layer */
    0,                                               /* mif_end_layer */
    ...
    0,                                               /* mif_begin_label */
    0,                                               /* mif_end_label */
    mif_textpara,   0,                               /* mif_resolve_color */
    mif_ellipse,    mif_polygon, mif_bezier,      mif_polyline,
    mif_comment,    0, /* mif_library_shape */
};

gvrender_features_t mif_features = {GVRENDER_Y_GOES_DOWN, /* flags */
