...
#define POV_COLORS                                                             \
  "aquamarine", "bakerschoc", "black", "blue", "blueviolet", "brass",          \
      "brightgold", "bronze", "bronze2", "brown", "cadetblue", "clear",        \
      "coolcopper", "copper", "coral", "cornflowerblue", "cyan", "darkbrown",  \
      "darkgreen", "darkolivegreen", "darkorchid", "darkpurple",               \
      "darkslateblue", "darkslategray", "darkslategrey", "darktan",            \
      "darkturquoise", "darkwood", "dkgreencopper", "dustyrose", "feldspar",   \
      "firebrick", "flesh", "forestgreen", "gold", "goldenrod", "gray05",      \
      "gray10", "gray15", "gray20", "gray25", "gray30", "gray35", "gray40",    \
      "gray45", "gray50", "gray55", "gray60", "gray65", "gray70", "gray75",    \
      "gray80", "gray85", "gray90", "gray95", "green", "greencopper",          \
      "greenyellow", "huntersgreen", "indianred", "khaki", "lightblue",        \
      "light_purple", "lightsteelblue", "lightwood", "limegreen", "magenta",   \
      "mandarinorange", "maroon", "mediumaquamarine", "mediumblue",            \
      "mediumforestgreen", "mediumgoldenrod", "mediumorchid",                  \
      "mediumseagreen", "mediumslateblue", "mediumspringgreen",                \
      "mediumturquoise", "mediumvioletred", "mediumwood", "med_purple",        \
      "mica", "midnightblue", "navy", "navyblue", "neonblue", "neonpink",      \
      "newmidnightblue", "newtan", "oldgold", "orange", "orangered", "orchid", \
      "palegreen", "pink", "plum", "quartz", "red", "richblue", "salmon",      \
      "scarlet", "seagreen", "semiSweetChoc", "sienna", "silver", "skyblue",   \
      "slateblue", "spicypink", "springgreen", "steelblue", "summersky",       \
      "tan", "thistle", "turquoise", "verydarkbrown", "very_light_purple",     \
...
#endif
      c = el(job, POV_COLOR_RGB, color.u.rgba[0] / 256.0,
             color.u.rgba[1] / 256.0, color.u.rgba[2] / 256.0, transparency);
      break;
    default:
      fprintf(stderr, "oops, internal error: unhandled color type=%d %s\n",
              color.type, color.u.string);
...
static void pov_begin_graph(GVJ_t *job) {
  ...
  gvprintf(job, "// graph_index = %d, pages = %d, layer = %d/%d\n",
           job->graph_index, job->numPages, job->layerNum, job->numLayers);
  ...
  gvprintf(job, "// boundingBox (all pages) LL,UR = %d,%d, %d,%d\n",
           job->boundingBox.LL.x, job->boundingBox.LL.y, job->boundingBox.UR.x,
  ...
  gvprintf(job, POV_CAMERA, x, y, d, x, y, 0.0, (px > py ? px : py) * 1.2);
  gvputs(job, POV_SKY_AND_GND);
  gvputs(job, POV_LIGHT);
}

static void pov_end_graph(GVJ_t *job) { gvputs(job, "//*** end_graph\n"); }

static void pov_begin_layer(GVJ_t *job, char *layername, int layerNum,
                            int numLayers) {
...
static void pov_end_layer(GVJ_t *job) { gvputs(job, "//*** end_layer\n"); }

static void pov_begin_page(GVJ_t *job) { gvputs(job, "//*** begin_page\n"); }

static void pov_end_page(GVJ_t *job) { gvputs(job, "//*** end_page\n"); }
...
static void pov_end_cluster(GVJ_t *job) { gvputs(job, "//*** end_cluster\n"); }

static void pov_begin_node(GVJ_t *job) {
  gvprintf(job, "//*** begin_node: %s\n", agnameof(job->obj->u.n));
}

static void pov_end_node(GVJ_t *job) { gvputs(job, "//*** end_node\n"); }
...
static void pov_textspan(GVJ_t *job, pointf c, textspan_t *span) {
  ...
  pov = el(job, POV_TEXT "    %s    %s    %s    %s    %s" END, span->font->name,
           0.25, 0.0,  // font, depth (0.5 ... 2.0), offset
           span->str, "    no_shadow\n", s, r, t, p);
...
static void pov_ellipse(GVJ_t *job, pointf *A, int filled) {
  ...
  pov = el(job, POV_TORUS "    %s    %s    %s    %s" END, 1.0,
           w,  // radius, size of ring
           s, r, t, p);
  ...
  if (filled) {
    ...
    pov = el(job, POV_SPHERE "    %s    %s    %s    %s" END, 0.0, 0.0, 0.0, s,
             r, t, p);
...
static void pov_bezier(GVJ_t *job, pointf *A, int n, int arrow_at_start,
  ...
  for (i = 0; i < n; i++) {
    v = el(job, POV_VECTOR3 ", %.3f\n", A[i].x + job->translation.x,
           ...
           job->obj->penwidth);       // z coordinate, thickness
...
static void pov_polygon(GVJ_t *job, pointf *A, int n, int filled) {
  ...
  if (filled) {
    ...
    for (i = 0; i < n; i++) {
      ...
      v = el(job, POV_VECTOR3, A[i].x + job->translation.x,
             A[i].y + job->translation.y, 0.0);
...
gvrender_engine_t pov_engine = {
    pov_begin_job,     0, /* pov_end_job */
    pov_begin_graph,   pov_end_graph,   pov_begin_layer,
    pov_end_layer,     pov_begin_page,  pov_end_page,
    pov_begin_cluster, pov_end_cluster, 0, /* pov_begin_nodes */
    0,                                     /* pov_end_nodes */
    0,                                     /* pov_begin_edges */
    0,                                     /* pov_end_edges */
    pov_begin_node,    pov_end_node,    pov_begin_edge,
    pov_end_edge,      0, /* pov_begin_anchor */
    0,                    /* pov_end_anchor */
    0,                    /* pov_begin_label */
    0,                    /* pov_end_label */
    pov_textspan,      0, /* pov_resolve_color */
    pov_ellipse,       pov_polygon,     pov_bezier,
    pov_polyline,      pov_comment,     0, /* pov_library_shape */
};

gvrender_features_t render_features_pov = {
    /* flags */
    GVDEVICE_DOES_LAYERS | GVRENDER_DOES_MAP_RECTANGLE |
        GVRENDER_DOES_MAP_CIRCLE | GVRENDER_DOES_MAP_POLYGON |
        GVRENDER_DOES_MAP_ELLIPSE | GVRENDER_DOES_MAP_BSPLINE |
        GVRENDER_NO_WHITE_BG | GVRENDER_DOES_TRANSFORM | GVRENDER_DOES_Z |
