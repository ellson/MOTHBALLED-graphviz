  ...
  if (job->common->show_boxes == NULL)
    if (job->render.id != FORMAT_EPS)
      gvprintf(job, "%%%%BoundingBox: %d %d %d %d\n", job->boundingBox.LL.x,
               job->boundingBox.LL.y, job->boundingBox.UR.x,
...
static void psgen_begin_layer(GVJ_t *job, char *layername, int layerNum,
                              int numLayers) {
...
static void psgen_begin_page(GVJ_t *job) {
  ...
  gvprintf(job, "%d %d %d beginpage\n", job->pagesArrayElem.x,
           job->pagesArrayElem.y, job->numPages);
  if (job->common->show_boxes == NULL)
    gvprintf(job, "gsave\n%d %d %d %d boxprim clip newpath\n", pbr.LL.x,
             pbr.LL.y, pbr.UR.x - pbr.LL.x, pbr.UR.y - pbr.LL.y);
  gvprintf(job, "%g %g set_scale %d rotate %g %g translate\n", job->scale.x,
           job->scale.y, job->rotation, job->translation.x, job->translation.y);
...
static void psgen_end_cluster(GVJ_t *job) { gvputs(job, "grestore\n"); }

static void psgen_begin_node(GVJ_t *job) { gvputs(job, "gsave\n"); }

static void psgen_end_node(GVJ_t *job) { gvputs(job, "grestore\n"); }

static void psgen_begin_edge(GVJ_t *job) { gvputs(job, "gsave\n"); }

static void psgen_end_edge(GVJ_t *job) { gvputs(job, "grestore\n"); }

static void psgen_begin_anchor(GVJ_t *job, char *url, char *tooltip,
                               char *target, char *id) {
  obj_state_t *obj = job->obj;
...
static void ps_set_pen_style(GVJ_t *job) {
  ...
  while (s && (p = line = *s++)) {
    if (strcmp(line, "setlinewidth") == 0) continue;
    while (*p) p++;
    p++;
    while (*p) {
      gvprintf(job, "%s ", p);
    ...
    if (strcmp(line, "invis") == 0) job->obj->penwidth = 0;
    gvprintf(job, "%s\n", line);
  }
}
...
static void ps_set_color(GVJ_t *job, gvcolor_t *color) {
  ...
  if (color) {
    ...
    gvprintf(job, "%.5g %.5g %.5g %scolor\n", color->u.HSVA[0],
             color->u.HSVA[1], color->u.HSVA[2], objtype);
  }
}
...
static void psgen_textspan(GVJ_t *job, pointf p, textspan_t *span) {
  ...
  if (job->obj->pencolor.u.HSVA[3] < .5) return; /* skip transparent text */
...
static void psgen_bezier(GVJ_t *job, pointf *A, int n, int arrow_at_start,
...
static void psgen_library_shape(GVJ_t *job, char *name, pointf *A, int n,
                                int filled) {
  if (filled && job->obj->fillcolor.u.HSVA[3] > .5) {
    ps_set_color(job, &(job->obj->fillcolor));
    gvputs(job, "[ ");
...
static gvrender_engine_t psgen_engine = {
    psgen_begin_job,   psgen_end_job,      psgen_begin_graph,
    0,                    /* psgen_end_graph */
    psgen_begin_layer, 0, /* psgen_end_layer */
    psgen_begin_page,  psgen_end_page,     psgen_begin_cluster,
    psgen_end_cluster, 0, /* psgen_begin_nodes */
    0,                    /* psgen_end_nodes */
    0,                    /* psgen_begin_edges */
    0,                    /* psgen_end_edges */
    psgen_begin_node,  psgen_end_node,     psgen_begin_edge,
    psgen_end_edge,    psgen_begin_anchor, 0, /* psgen_end_anchor */
    0,                                        /* psgen_begin_label */
    0,                                        /* psgen_end_label */
    psgen_textspan,    0,                     /* psgen_resolve_color */
    psgen_ellipse,     psgen_polygon,      psgen_bezier,
    psgen_polyline,    psgen_comment,      psgen_library_shape, };

static gvrender_features_t render_features_ps = {
    GVRENDER_DOES_TRANSFORM | GVRENDER_DOES_MAPS | GVRENDER_NO_WHITE_BG |
...
static gvdevice_features_t device_features_ps = {
    GVDEVICE_DOES_PAGES | GVDEVICE_DOES_LAYERS, /* flags */
