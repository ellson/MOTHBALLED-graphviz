...
static void core_loadimage_fig(GVJ_t *job, usershape_t *us, boxf bf,
  ...
  gvprintf(job, " %d %d %d %d %d %d %d %d %d %d\n", b.LL.x, b.LL.y, b.LL.x,
           b.UR.y, b.UR.x, b.UR.y, b.UR.x, b.LL.y, b.LL.x, b.LL.y);
}
...
static void core_loadimage_ps(GVJ_t *job, usershape_t *us, boxf b,
  ...
  if (!us->data) {/* read file into cache */
    ...
    if (!gvusershape_file_access(us)) return;
    fd = fileno(us->f);
    switch (us->type) {
      case FT_PS:
      case FT_EPS:
    ...
    if (us->data) us->datafree = ps_freeimage;
...
static void core_loadimage_pslib(GVJ_t *job, usershape_t *us, boxf b,
  ...
  if ((shape = (shape_desc *)us->data)) {
    ...
    if (filled) {
      ...
      gvprintf(job, "[ ");
      for (i = 0; i < 4; i++) gvprintf(job, "%g %g ", AF[i].x, AF[i].y);
...
static void core_loadimage_vml(GVJ_t *job, usershape_t *us, boxf b,
                               boolean filled) {
  unsigned int graphHeight;
  graphHeight = (int)(job->bb.UR.y - job->bb.LL.y);
  gvprintf(job,
           "<v:image src=\"%s\" style=\" position:absolute; width:%.2f; "
           "height:%.2f; left:%.2f ; top:%.2f\"",
           us->name, b.UR.x - b.LL.x, b.UR.y - b.LL.y, b.LL.x,
           graphHeight - b.UR.y);
  gvputs(job, " />\n");
}
...
static void core_loadimage_tk(GVJ_t *job, usershape_t *us, boxf b,
  ...
  gvprintf(job, "$c create image %.2f %.2f -image \"photo_%s\"\n", us->name,
           (b.UR.x + b.LL.x) / 2, (b.UR.y + b.LL.y) / 2);
}
...
static gvloadimage_engine_t engine_vml = {core_loadimage_vml};
