...
static char *vtxcolor[] = {"black",   "blue",   "green", "cyan",      "red",
                           "magenta", "yellow", "white", (char *)NULL};
...
static void vtx_resolve_color(GVJ_t *job, gvcolor_t *color) {
  ...
  switch (color->type) {
    case COLOR_STRING:
      ...
      break;
    case RGBA_BYTE:
      i = 32 + vtxColorResolve(&new, color->u.rgba[0], color->u.rgba[1],
                               color->u.rgba[2]);
      if (new)
        gvprintf(job, "%d %d #%02x%02x%02x\n", object_code, i, color->u.rgba[0],
                 color->u.rgba[1], color->u.rgba[2]);
      color->u.index = i;
      break;
    default:
...
static void vtx_line_style(obj_state_t *obj, int *line_style,
                           double *style_val) {
  switch (obj->pen) {
    case PEN_DASHED:
      *line_style = 1;
...
static void vtx_comment(GVJ_t *job, char *str) { gvprintf(job, "# %s\n", str); }
...
static void vtx_end_graph(GVJ_t *job) { gvputs(job, "# end of FIG file\n"); }

static void vtx_begin_page(GVJ_t *job) { Depth = 2; }

static void vtx_begin_node(GVJ_t *job) { Depth = 1; }

static void vtx_end_node(GVJ_t *job) { Depth = 2; }

static void vtx_begin_edge(GVJ_t *job) { Depth = 0; }

static void vtx_end_edge(GVJ_t *job) { Depth = 2; }
...
static void vtx_textpara(GVJ_t *job, pointf p, textpara_t *para) {
  ...
  gvprintf(job, "%d %d %d %d %d %d %.1f %.4f %d %.1f %.1f %d %d %s\\001\n",
           object_code, sub_type, color, depth, pen_style, font, font_size,
           angle, font_flags, height, length, ROUND(p.x), ROUND(p.y),
           vtx_string(para->str));
}
...
static void vtx_ellipse(GVJ_t *job, pointf *A, int filled) {
  ...
  gvprintf(job,
           ...
           depth, pen_style, area_fill, style_val, direction, angle, center_x,
           center_y, radius_x, radius_y, start_x, start_y, end_x, end_y);
}
...
static void vtx_bezier(GVJ_t *job, pointf *A, int n, int arrow_at_start,
  ...
  gvprintf(job, "%d %d %d %d %d %d %d %d %d %.1f %d %d %d %d\n", object_code,
           ...
           pen_style, area_fill, style_val, cap_style, forward_arrow,
           backward_arrow, count);
...
static void vtx_polygon(GVJ_t *job, pointf *A, int n, int filled) {
  ...
  gvprintf(job, "%d %d %d %d %d %d %d %d %d %.1f %d %d %d %d %d %d\n",
           ...
           radius, forward_arrow, backward_arrow, npoints);
...
gvrender_engine_t vtx_engine = {
    0,                                     /* vtx_begin_job */
    0,                                     /* vtx_end_job */
    vtx_begin_graph, vtx_end_graph,     0, /* vtx_begin_layer */
    0,                                     /* vtx_end_layer */
    ...
    0,                                     /* vtx_begin_edges */
    0,                                     /* vtx_end_edges */
    vtx_begin_node,  vtx_end_node,      vtx_begin_edge,
    vtx_end_edge,    0, /* vtx_begin_anchor */
    0,                  /* vtx_end_anchor */
    0,                  /* vtx_begin_label */
    0,                  /* vtx_end_label */
    vtx_textpara,    vtx_resolve_color, vtx_ellipse,
    vtx_polygon,     vtx_bezier,        vtx_polyline,
    vtx_comment,     0, /* vtx_library_shape */
};
