...
static char *mpcolor[] = {"black",   "blue",   "green", "cyan",      "red",
...
static void mp_resolve_color(GVJ_t *job, gvcolor_t *color) {
  ...
  switch (color->type) {
    case COLOR_STRING:
      ...
      break;
    case RGBA_BYTE:
      i = 32 + mpColorResolve(&new, color->u.rgba[0], color->u.rgba[1],
                              color->u.rgba[2]);
      if (new)
...
static void mp_line_style(obj_state_t *obj, int *line_style,
                          double *style_val) {
  switch (obj->pen) {
    case PEN_DASHED:
      *line_style = 1;
...
static void mp_comment(GVJ_t *job, char *str) { gvprintf(job, "# %s\n", str); }
...
static void mp_end_graph(GVJ_t *job) { gvputs(job, "# end of FIG file\n"); }

static void mp_begin_page(GVJ_t *job) { Depth = 2; }

static void mp_begin_node(GVJ_t *job) { Depth = 1; }

static void mp_end_node(GVJ_t *job) { Depth = 2; }

static void mp_begin_edge(GVJ_t *job) { Depth = 0; }

static void mp_end_edge(GVJ_t *job) { Depth = 2; }
...
gvrender_engine_t mp_engine = {
    0,                                   /* mp_begin_job */
    0,                                   /* mp_end_job */
    mp_begin_graph, mp_end_graph,     0, /* mp_begin_layer */
    0,                                   /* mp_end_layer */
    ...
    0,                                   /* mp_begin_edges */
    0,                                   /* mp_end_edges */
    mp_begin_node,  mp_end_node,      mp_begin_edge, mp_end_edge,
    0, /* mp_begin_anchor */
    0, /* mp_end_anchor */
    0, /* mp_begin_label */
    0, /* mp_end_label */
    mp_textpara,    mp_resolve_color, mp_ellipse,    mp_polygon,
    mp_bezier,      mp_polyline,      mp_comment,    0, /* mp_library_shape */
};
