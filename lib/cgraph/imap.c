...
static Dtdisc_t LookupByName = {
    ...
    0, /* size (ignored) */
    offsetof(IMapEntry_t, namedict_link), NIL(Dtmake_f), NIL(Dtfree_f),
    namecmpf,                             NIL(Dthash_f), agdictobjmem,
    NIL(Dtevent_f)};

static Dtdisc_t LookupById = {
    0, /* object ptr is passed as key */
    0, /* size (ignored) */
    offsetof(IMapEntry_t, iddict_link), NIL(Dtmake_f), NIL(Dtfree_f), idcmpf,
...
char *aginternalmapprint(Agraph_t *g, int objtype, unsigned long id) {
  ...
  if ((isym = find_isym(g, objtype, id))) return isym->str;
  return NILstr;
}
  ...
  for (i = 0; i < 3; i++) {
    if (d_name[i]) {
      for (sym = dtfirst(d_name[i]); sym; sym = nxt) {
        ...
        if (sym->str[0] == LOCALNAMEPREFIX) aginternalmapdelete(g, i, sym->id);
      }
    }
  }
}
