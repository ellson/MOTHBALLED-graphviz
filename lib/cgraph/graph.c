...
Agraph_t *agopen(char *name, Agdesc_t desc, Agdisc_t *arg_disc) {
  ...
  if (agmapnametoid(g, AGRAPH, name, &gid, TRUE)) AGID(g) = gid;
...
Agraph_t *agopen1(Agraph_t *g) {
  ...
  g->e_seq = agdtopen(
      g, g == agroot(g) ? &Ag_mainedge_seq_disc : &Ag_subedge_seq_disc, Dttree);
  g->e_id = agdtopen(
      g, g == agroot(g) ? &Ag_mainedge_id_disc : &Ag_subedge_id_disc, Dttree);
  ...
  if (!par || par->desc.has_attrs) agraphattr_init(g);
  agmethod_init(g, g);
  return g;
}
...
int agclose(Agraph_t *g) {
  ...
  if (par) {
    ...
    Agmemdisc_t *memdisc;
    void *memclos, *clos;
    while (g->clos->cb) agpopdisc(g, g->clos->cb->f);
...
int agnnodes(Agraph_t *g) { return dtsize(g->n_id); }
...
int agnsubg(Agraph_t *g) { return dtsize(g->g_dict); }

int agisdirected(Agraph_t *g) { return g->desc.directed; }

int agisundirected(Agraph_t *g) { return NOT(agisdirected(g)); }

int agisstrict(Agraph_t *g) { return g->desc.strict; }

int agissimple(Agraph_t *g) { return (g->desc.strict && g->desc.no_loop); }
...
int agcountuniqedges(Agraph_t *g, Agnode_t *n, int want_in, int want_out) {
  ...
  if (want_in) {
    if (!want_out)
      rv += cnt(g->e_seq, &(sn->in_seq)); /* cheap */
    else {                                /* less cheap */
...
Dtdisc_t Ag_subgraph_id_disc = {0,                        /* pass object ptr  */
                                0,                        /* size (ignored)   */
                                offsetof(Agraph_t, link), /* link offset */
                                NIL(Dtmake_f),            NIL(Dtfree_f),
                                agraphidcmpf,             NIL(Dthash_f),
