...
Dtdisc_t AgDataDictDisc = {
    ...
    -1,                           (int)offsetof(Agsym_t, link), NIL(Dtmake_f),
    freesym,                      NIL(Dtcompar_f),              NIL(Dthash_f)};
...
static void agcopydict(Dict_t *src, Dict_t *dest, Agraph_t *g, int kind) {
  ...
  for (sym = (Agsym_t *)dtfirst(src); sym; sym = (Agsym_t *)dtnext(src, sym)) {
...
static void addattr(Agraph_t *g, Agobj_t *obj, Agsym_t *sym) {
  ...
  if (sym->id >= MINATTR)
    attr->str = (char **)AGDISC(g, mem)->resize(AGCLOS(g, mem), attr->str,
                                                sym->id * sizeof(char *),
...
static Agsym_t *setattr(Agraph_t *g, int kind, char *name, char *value) {
  ...
  if (rv && (kind == AGRAPH)) agxset(g, rv, value);
...
void agnodeattr_init(Agraph_t *g, Agnode_t *n) {
  ...
  data = agattrrec(n);
  if ((!data) || (!data->dict)) (void)agmakeattrs(g, n);
}
...
void agedgeattr_init(Agraph_t *g, Agedge_t *e) {
  ...
  data = agattrrec(e);
  if ((!data) || (!data->dict)) (void)agmakeattrs(g, e);
}
...
static void init_all_attrs(Agraph_t *g) {
  ...
  root = agroot(g);
  agapply(root, (Agobj_t *)root, (agobjfn_t)agraphattr_init, NIL(Agdisc_t *),
          TRUE);
  for (n = agfstnode(root); n; n = agnxtnode(root, n)) {
    agnodeattr_init(g, n);
    for (e = agfstout(root, n); e; e = agnxtout(root, e)) {
...
int agcopyattr(void *oldobj, void *newobj) {
  ...
  while ((sym = agnxtattr(g, AGTYPE(oldobj), sym))) {
    ...
    /* FIX(?): Each graph has its own string cache, so a whole new refstr is
     * possibly
     * allocated. If the original was an html string, make sure the new one is
     * as well.
