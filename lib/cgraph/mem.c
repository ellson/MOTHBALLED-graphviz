...
static void memfree(void *heap, void *ptr) { vmfree((Vmalloc_t *)heap, ptr); }

static void memclose(void *heap) { vmclose((Vmalloc_t *)heap); }
...
static void *memopen(Agdisc_t *disc) { return NIL(void *); }
...
static void *memresize(void *heap, void *ptr, size_t oldsize, size_t request) {
  ...
  if (request > oldsize) memset((char *)rv + oldsize, 0, request - oldsize);
  return rv;
}
...
#define memclose 0
#else
static void memclose(void *heap) { NOTUSED(heap); }
#endif
...
void *agalloc(Agraph_t *g, size_t size) {
  ...
  if (mem == NIL(void *)) agerr(AGERR, "memory allocation failure");
  return mem;
}
...
void *agrealloc(Agraph_t *g, void *ptr, size_t oldsize, size_t size) {
  ...
  if (size > 0) {
    if (ptr == 0)
      ...
      mem = AGDISC(g, mem)->resize(AGCLOS(g, mem), ptr, oldsize, size);
    if (mem == NIL(void *)) agerr(AGERR, "memory re-allocation failure");
  } else
    mem = NIL(void *);
  return mem;
}
