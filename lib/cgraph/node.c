...
static void installnode(Agraph_t *g, Agnode_t *n) {
  ...
  if (g == agroot(g))
    ...
    sn = agalloc(g, sizeof(Agsubnode_t));
  sn->node = n;
  dtinsert(g->n_id, sn);
  dtinsert(g->n_seq, sn);
  assert(dtsize(g->n_id) == dtsize(g->n_seq));
...
static void installnodetoroot(Agraph_t *g, Agnode_t *n) {
  Agraph_t *par;
  installnode(g, n);
  if ((par = agparent(g))) installnodetoroot(par, n);
}

static void initnode(Agraph_t *g, Agnode_t *n) {
  if (agroot(g)->desc.has_attrs) agnodeattr_init(g, n);
  agmethod_init(g, n);
}
...
int agdelnode(Agraph_t *g, Agnode_t *n) {
  ...
  if (!agfindnode_by_id(g, AGID(n))) return FAILURE; /* bad arg */
  if (g == agroot(g)) {
    for (e = agfstedge(g, n); e; e = f) {
      f = agnxtedge(g, e, n);
      agdeledge(g, e);
  ...
  if (agapply(g, (Agobj_t *)n, (agobjfn_t)agdelnodeimage, NILnode, FALSE) ==
      SUCCESS) {
    if (g == agroot(g)) agfree(g, n);
    return SUCCESS;
  } else
    return FAILURE;
}
...
Agnode_t *agsubnode(Agraph_t *g, Agnode_t *n0, int cflag) {
  ...
  if (agroot(g) != n0->root) return NILnode;
...
static void free_subnode(Dt_t *d, Agsubnode_t *sn, Dtdisc_t *disc) {
  ...
  if (!AGSNMAIN(sn)) agfree(sn->node->root, sn);
}

Dtdisc_t Ag_subnode_id_disc = {0, /* pass object ptr  */
                               0, /* size (ignored)   */
                               offsetof(Agsubnode_t, id_link), /* link offset */
                               NIL(Dtmake_f),                  NIL(Dtfree_f),
                               agsubnodeidcmpf,                NIL(Dthash_f),
...
Dtdisc_t Ag_subnode_seq_disc = {
    ...
    NIL(Dtmake_f),                   (Dtfree_f)free_subnode, agsubnodeseqcmpf,
