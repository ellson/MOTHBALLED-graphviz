...
static int indent(Agraph_t *g, iochan_t *ofile) {
  int i;
  for (i = Level; i > 0; i--) CHKRV(ioput(g, ofile, "\t"));
  return 0;
}
...
static char *_agstrcanon(char *arg, char *buf) {
  ...
  *p++ = '\"';
  ...
  while (uc) {
    ...
    *p++ = (char)uc;
     ...
     * non-id char, not a backslash, where the next char is an
     * id char.
     */
    if (Max_outputline) {
  ...
  *p = '\0';
  ...
  for (tok = tokenlist; *tok; tok++)
    if (!strcasecmp(*tok, arg)) return buf;
  return arg;
}
 ...
 */
char *agcanonStr(char *str) { return agstrcanon(str, getoutputbuffer(str)); }
...
static int write_hdr(Agraph_t *g, iochan_t *ofile, int top) {
  ...
  if (name[0]) CHKRV(write_canonstr(g, ofile, name));
...
static int irrelevant_subgraph(Agraph_t *g) {
  ...
  if (!dd) return TRUE;
  if ((dtsize(dd->dict.n) > 0) || (dtsize(dd->dict.e) > 0)) return FALSE;
  return TRUE;
}
...
static int write_edge_name(Agedge_t *e, iochan_t *ofile, int terminate) {
  ...
  if (NOT(EMPTY(p))) {
    ...
    if (terminate) CHKRV(ioput(g, ofile, "]"));
    rv = TRUE;
  } else
    rv = FALSE;
  return rv;
...
static int attrs_written(void *obj) { return (AGATTRWF((Agobj_t *)obj)); }
...
static int write_node(Agnode_t *n, iochan_t *ofile, Dict_t *d) {
  ...
  if (NOT(attrs_written(n))) CHKRV(write_nondefault_attrs(n, ofile, d));
...
static int write_node_test(Agraph_t *g, Agnode_t *n, unsigned long pred_id) {
  if (NOT(node_in_subg(g, n)) && has_no_predecessor_below(g, n, pred_id)) {
    if (has_no_edges(g, n) || not_default_attrs(g, n)) return TRUE;
  }
  return FALSE;
}
...
static void set_attrwf(Agraph_t *g, int toplevel, int value) {
  ...
  if (toplevel) {
    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
      AGATTRWF(n) = value;
      for (e = agfstout(g, n); e; e = agnxtout(g, e)) AGATTRWF(e) = value;
    }
  }
}
...
int agwrite(Agraph_t *g, void *ofile) {
  ...
  if ((s = agget(g, "linelength")) && isdigit(*s)) {
    ...
    if ((len == 0) || (len >= MIN_OUTPUTLINE)) Max_outputline = len;
  }
