...
extern Agraph_t *agidsubg(Agraph_t *g, unsigned long id,
...
#define AGLEFTPOINTER(rep)                                     \
  ((Agnoderef_t *)((rep)->seq_link.hl._left                    \
                       ? ((void *)((rep)->seq_link.hl._left) - \
                          offsetof(Agsubnode_t, seq_link))     \
                       : 0))
...
#define PREVNREF(g, rep) \
  (((rep) == AGHEADPOINTER(g)) ? 0 : (AGLEFTPOINTER(rep)))

#define LASTNREF(g) \
  (agflatten(g, 1), AGHEADPOINTER(g) ? AGLEFTPOINTER(AGHEADPOINTER(g)) : 0)
