...
static void *idopen(Agraph_t *g, Agdisc_t *disc) { return g; }
...
Agiddisc_t AgIdDisc = {idopen,  idmap,   idalloc,   idfree,
                       idprint, idclose, idregister};

/* aux functions incl. support for disciplines with anonymous IDs */

int agmapnametoid(Agraph_t *g, int objtype, char *str, unsigned long *result,
                  int createflag) {
  int rv;
...
char *agnameof(void *obj) {
  ...
  g = agraphof(obj);
  if ((rv = aginternalmapprint(g, AGTYPE(obj), AGID(obj)))) return rv;
