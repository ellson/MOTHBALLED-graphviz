...
static void mp_reset(void) { onetime = TRUE; }
 ...
 */
static void mp_cat_libfile(FILE *ofp, const char **arglib,
                           const char **stdlib) {
  FILE *fp;
  ...
  if (arglib) {
    for (i = 0; (p = arglib[i]) != 0; i++) {
      if (*p == '\0') continue; /* ignore empty string */
...
static void mp_begin_job(FILE *ofp, graph_t *g, const char **lib, char *info[],
                         point pages) {
...
static void mp_comment(char *str) { fprintf(Output_file, "%% %s\n", str); }
...
static void mp_begin_page(graph_t *g, point page, double scale, int rot,
                          point offset) {
  assert(SP == 0);
  S[SP].font = "";
  S[SP].color = "black";
...
static void mp_bezier(point *A, int n, int arrow_at_start, int arrow_at_end,
                      int filled) {
  int j;
  ...
  for (j = 1; j < n; j += 3)
    fprintf(Output_file,
            "\n  ..controls (%dbp,%dbp) and (%dbp,%dbp).. (%dbp,%dbp)", A[j].x,
            A[j].y, A[j + 1].x, A[j + 1].y, A[j + 2].x, A[j + 2].y);
...
static void mp_ellipse(point p, int rx, int ry, int filled) {
  if (filled)
    fprintf(Output_file,
            "  fill fullcircle xscaled %dbp yscaled %dbp shifted (%dbp,%dbp) "
            "withcolor %s;\n",
...
static void mp_usershape(usershape_t *us, boxf b, point *A, int n,
                         boolean filled) {
  int j;
  fprintf(Output_file, "%%GV USER SHAPE [ ");
  for (j = 0; j < n; j++) fprintf(Output_file, "%d %d ", A[j].x, A[j].y);
...
codegen_t MP_CodeGen = {
    mp_reset,                mp_begin_job,             mp_end_job,
    ...
    mp_begin_context,        mp_end_context,           0,
    /* mp_begin_anchor */ 0, /* mp_end_anchor */
    mp_set_font,             mp_textpara,              mp_set_color,
    mp_set_color,            mp_set_style,             mp_ellipse,
    mp_polygon,              mp_bezier,                mp_polyline,
    0, /* bezier_has_arrows */
    mp_comment,              mp_usershape};
