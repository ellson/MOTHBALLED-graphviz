...
static shapemap_t shapemap[] = {
    {"box", "\"Rectangle\""}, {"ellipse", "\"Ellipse\""},
    {"circle", "\"Ellipse\""}, {"triangle", "\"Triangle\""},
    {"diamond", "\"Diamond\""}, {"trapezium", "\"Trapezoid\""},
    {"parallelogram", "\"Parallelogram\""}, {"hexagon", "\"Hexagon\""},
...
static void vtx_reset(void) { onetime = TRUE; }
...
static void vtx_comment(char *str) { fprintf(Output_file, "; %s\n", str); }

static void vtx_begin_job(FILE *ofp, graph_t *g, const char **lib, char *info[],
                          point pages) {
  char *date = "";
#ifdef SUPPORT_WRITEDATE
  time_t when;
...
static void vtx_begin_page(graph_t *g, point page, double scale, int rot,
                           point offset) {
  int page_number;
  /* point        sz; */
...
static void vtx_begin_nodes(void) { fprintf(Output_file, "(shapes\n"); }
...
static void vtx_end_node(void) { fprintf(Output_file, "  )\n"); }
...
static void vtx_end_edge(void) { fprintf(Output_file, "  )\n"); }
...
static void vtx_style(void) {
  ...
  fprintf(Output_file,
          ...
          "    )\n",
          cp->fill ? "T" : "F", cp->color_r, cp->color_g, cp->color_b,
...
static void vtx_usershape(usershape_t *us, boxf b, point *A, int n,
                          boolean filled) {
  /* FIXME */
  int i;
  pointf mp, max, min;
...
codegen_t VTX_CodeGen = {
    ...
    vtx_set_color,     vtx_set_color,
    vtx_set_style,     vtx_ellipse,
    vtx_polygon,       vtx_bezier,
    vtx_polyline,      1, /* bezier_has_arrows */
    vtx_comment,       vtx_usershape};
