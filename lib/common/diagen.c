...
static char *dia_resolve_color(char *name) {
  ...
  if (!SVG_COLORS_P || (bsearch(&tok, svg_known_colors,
    ...
    if (streq(tok, "transparent")) {
      ...
      sprintf(buf, "#%02x%02x%02x", color.u.rgba[0], color.u.rgba[1],
              color.u.rgba[2]);
      tok = buf;
    }
  }
...
static void dia_reset(void) { onetime = TRUE; }
...
static void dia_begin_job(FILE *ofp, graph_t *g, const char **lib, char *info[],
                          point pages) {
#if HAVE_LIBZ
  int fd;
...
static void dia_begin_page(graph_t *g, point page, double scale, int rot,
                           point offset) {
  /* int          page_number; */
  /* point        sz; */
...
static void dia_end_page(void) { dia_fputs("  </dia:layer>\n"); }
...
static void dia_end_cluster(void) { dia_printf("</dia:group>\n"); }
...
static void dia_end_node(void) { dia_printf("</dia:group>\n"); }

static void dia_begin_edge(edge_t *e) { Curedge = e; }

static void dia_end_edge(void) { Curedge = NULL; }
...
static void dia_set_pencolor(char *name) { cstk[SP].pencolor = name; }

static void dia_set_fillcolor(char *name) { cstk[SP].fillcolor = name; }
...
static void dia_bezier(point *A, int n, int arrow_at_start, int arrow_at_end,
                       int filled) {
  int i, conn_h, conn_t;
  ...
  dia_printf(
      "    <dia:object type=\"Standard - BezierLine\" version=\"0\" "
      "id=\"%s\">\n",
      "00");
  ...
  if (Curedge) {
    ...
    if ((strcmp(shape_t, "ellipse") == 0) || (strcmp(shape_t, "circle") == 0) ||
    ...
    if (arrow_at_start) {
      dia_printf(
          "        <dia:connection handle=\"0\" to=\"%d\" "
          "connection=\"%d\"/>\n",
          AGID(head), conn_h);
      dia_printf(
          "        <dia:connection handle=\"%d\" to=\"%d\" "
          "connection=\"%d\"/>\n",
...
static void dia_polygon(point *A, int n, int filled) {
  ...
  switch (Obj) {
    case NODE:
      dia_printf(
          "    <dia:object type=\"Standard - Polygon\" version=\"0\" "
          "id=\"%d\">\n",
          AGID(Curnode));
      break;
    case EDGE:
...
static void dia_usershape(usershape_t *us, boxf b, point *A, int n,
                          boolean filled) {
  char *imagefile;
...
codegen_t DIA_CodeGen = {
    dia_reset,               dia_begin_job,
    dia_end_job,             dia_begin_graph,
    dia_end_graph,           dia_begin_page,
    dia_end_page,            0,
    /* dia_begin_layer */ 0, /* dia_end_layer */
    ...
    dia_set_pencolor,        dia_set_fillcolor,
    dia_set_style,           dia_ellipse,
    dia_polygon,             dia_bezier,
    dia_polyline,            1, /* bezier_has_arrows */
    dia_comment,             dia_usershape};
