...
static void printboxes(int boxn, boxf *boxes) {
  ...
  for (bi = 0; bi < boxn; bi++) {
    ...
    sprintf(buf, "%d %d %d %d pathbox", (int)ll.x, (int)ll.y, (int)ur.x,
            (int)ur.y);
...
static int debugleveln(edge_t *realedge, int i) {
  return (GD_showboxes(agraphof(aghead(realedge))) == i ||
          ...
          ED_showboxes(realedge) == i || ND_showboxes(aghead(realedge)) == i ||
 ...
 */
pointf *simpleSplineRoute(pointf tp, pointf hp, Ppoly_t poly, int *n_spl_pts,
                          int polyline) {
  Ppolyline_t pl, spl;
  Ppoint_t eps[2];
  Pvector_t evs[2];
...
#endif
    evs[1].x = evs[1].y = 0;
    if (Proutespline(edges, poly.pn, pl, evs, &spl) < 0) return NULL;
  }

  if (mkspacep(spl.pn)) return NULL;
  for (i = 0; i < spl.pn; i++) {
    ps[i] = spl.ps[i];
  }
  *n_spl_pts = spl.pn;
...
int routesplinesinit() {
  ...
  if (Show_boxes) {
    int i;
    for (i = 0; Show_boxes[i]; i++) free(Show_boxes[i]);
    free(Show_boxes);
    Show_boxes = NULL;
    Show_cnt = 0;
  }
...
#endif
  if (Verbose)
    fprintf(stderr, "routesplines: %d edges, %d boxes %.2f sec\n", nedges,
            nboxes, elapsed_sec());
}
...
#define FUDGE .0001
        if (sp[0].y <= boxes[bi].UR.y + FUDGE &&
            sp[0].y >= boxes[bi].LL.y - FUDGE) {
          if (boxes[bi].LL.x > sp[0].x) boxes[bi].LL.x = sp[0].x;
          if (boxes[bi].UR.x < sp[0].x) boxes[bi].UR.x = sp[0].x;
        }
      }
    }
  }
...
#define LOOP_TRIES                                                      \
  15 /* number of times to try to limiting boxes to regain space, using \
        smaller divisions */
 ...
 * are converted into a containing polygon. A shortest path is constructed
 *within
 * the polygon from between the terminal points. If polyline is true, this path
 * is converted to a spline representation. Otherwise, we call the path planner
 *to
 * convert the polyline into a smooth spline staying within the polygon. In both
 * cases, the function returns an array of the computed control points. The
 *number
 * of these points is given in npoints.
 *
 * Note that the returned points are stored in a single array, so the points
 *must be
 * used before another call to this function.
 *
 * During cleanup, the function determines the x-extent of the spline in the
 *box, so
 * the box can be shrunk to the minimum width. The extra space can then be used
 *by other
 * edges.
 *
...
static pointf *_routesplines(path *pp, int *npoints, int polyline) {
  ...
  if (checkpath(boxn, boxes, pp)) return NULL;

#ifdef DEBUG
  if (debugleveln(realedge, 1)) printboxes(boxn, boxes);
  ...
  if (agtail(realedge) != aghead(realedge)) {
    ...
    for (bi = 0, pi = 0; bi < boxn; bi++) {
      next = prev = 0;
      if (bi > 0) prev = (boxes[bi].LL.y > boxes[bi - 1].LL.y) ? -1 : 1;
      if (bi < boxn - 1) next = (boxes[bi + 1].LL.y > boxes[bi].LL.y) ? 1 : -1;
      if (prev != next) {
        if (next == -1 || prev == 1) {
        ...
        if (!(prev == -1 && next == -1)) {
          agerr(AGERR,
                "in routesplines, illegal values of prev %d and next %d, line "
                "%d\n",
                prev, next, __LINE__);
          return NULL;
        }
    ...
    for (bi = boxn - 1; bi >= 0; bi--) {
      next = prev = 0;
      if (bi < boxn - 1) prev = (boxes[bi].LL.y > boxes[bi + 1].LL.y) ? -1 : 1;
      if (bi > 0) next = (boxes[bi - 1].LL.y > boxes[bi].LL.y) ? 1 : -1;
      if (prev != next) {
        if (next == -1 || prev == 1) {
          polypoints[pi].x = boxes[bi].LL.x;
          polypoints[pi++].y = boxes[bi].UR.y;
  ...
  if (flip) {
    ...
    for (i = 0; i < pi; i++) polypoints[i].y *= -1;
  }
  ...
  if (mkspacep(spl.pn)) return NULL; /* Bailout if no memory left */
  ...
  for (loopcnt = 0; unbounded && (loopcnt < LOOP_TRIES); loopcnt++) {
    ...
    for (bi = 0; bi < boxn; bi++) {
      ...
      if ((boxes[bi].LL.x == INT_MAX) || (boxes[bi].UR.x == INT_MIN)) {
        ...
        if (delta > INT_MAX /
                        boxn) /* in limitBoxes, boxn*delta must fit in an int,
                                 so give up */
          loopcnt = LOOP_TRIES;
        break;
      }
    }
    if (bi == boxn) unbounded = FALSE;
  }
  if (unbounded) {
    /* Either an extremely short, even degenerate, box, or some failure with the
     * path
     * planner causing the spline to miss some boxes. In any case, use the
     * shortest path
     * to bound the boxes. This will probably mean a bad edge, but we avoid an
     * infinite
    ...
    agerr(AGWARN,
          ...
          "\"%s\". Something is probably seriously wrong.\n",
  ...
  *npoints = spl.pn;
  ...
  if (GD_showboxes(agraphof(aghead(realedge))) == 2 ||
      ...
      ED_showboxes(realedge) == 2 || ND_showboxes(aghead(realedge)) == 2 ||
...
static int overlap(int i0, int i1, int j0, int j1) {
  ...
  if (i1 <= j0) return 0;
  if (i0 >= j1) return 0;
  if ((j0 <= i0) && (i0 <= j1)) return (j1 - i0);
  if ((j0 <= i1) && (i1 <= j1)) return (i1 - j0);
...
static int checkpath(int boxn, boxf *boxes, path *thepath) {
  ...
  i = 0;
  for (bi = 0; bi < boxn; bi++) {
    if (ABS(boxes[bi].LL.y - boxes[bi].UR.y) < .01) continue;
    if (ABS(boxes[bi].LL.x - boxes[bi].UR.x) < .01) continue;
  ...
  for (bi = 0; bi < boxn - 1; bi++) {
    ...
    if (errs > 0) {
      ...
      for (i = 0; i < errs - 1; i++) {
        if (l == 1)
          xy = (ba->UR.x + bb->LL.x) / 2.0 + 0.5, ba->UR.x = bb->LL.x = xy,
          l = 0;
        else if (r == 1)
          xy = (ba->LL.x + bb->UR.x) / 2.0 + 0.5, ba->LL.x = bb->UR.x = xy,
          r = 0;
        else if (d == 1)
          xy = (ba->UR.y + bb->LL.y) / 2.0 + 0.5, ba->UR.y = bb->LL.y = xy,
          d = 0;
        else if (u == 1)
          xy = (ba->LL.y + bb->UR.y) / 2.0 + 0.5, ba->LL.y = bb->UR.y = xy,
          u = 0;
      }
    }
#else
...
#ifndef DONTFIXPATH
    if (thepath->start.p.x < boxes[0].LL.x) thepath->start.p.x = boxes[0].LL.x;
    if (thepath->start.p.x > boxes[0].UR.x) thepath->start.p.x = boxes[0].UR.x;
    if (thepath->start.p.y < boxes[0].LL.y) thepath->start.p.y = boxes[0].LL.y;
    if (thepath->start.p.y > boxes[0].UR.y) thepath->start.p.y = boxes[0].UR.y;
#else
      abort();
#endif
  }
...
static void printpath(path *pp) {
  ...
  for (bi = 0; bi < pp->nbox; bi++)
    fprintf(stderr, "%d (%.5g, %.5g), (%.5g, %.5g)\n", bi, pp->boxes[bi].LL.x,
            pp->boxes[bi].LL.y, pp->boxes[bi].UR.x, pp->boxes[bi].UR.y);
...
void makeStraightEdges(graph_t *g, edge_t **edges, int e_cnt, int et,
                       splineInfo *sinfo) {
  pointf dumb[4];
  node_t *n;
  node_t *head;
