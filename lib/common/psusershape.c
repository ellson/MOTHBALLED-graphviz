...
static Dtdisc_t ImageDictDisc = {
    ...
    -1,                          /* size */
    0,                           /* link offset */
    NIL(Dtmake_f),               (Dtfree_f)ps_image_free, NIL(Dtcompar_f),
...
static usershape_t *user_init(const char *str) {
  ...
  if (!EPSF_contents) EPSF_contents = dtopen(&ImageDictDisc, Dtoset);
  ...
  if (arglib) {
    for (i = 0; (p = arglib[i]) != 0; i++) {
      ...
      if (!path) {
        ...
        while ((bp = Fgets(fp))) gvputs(job, bp);
...
void epsf_emit_body(GVJ_t *job, usershape_t *us) {
  ...
  while (*p) {
    /* skip %%EOF lines */
    if ((p[0] == '%') && (p[1] == '%') &&
        (!strncasecmp(&p[2], "EOF", 3) || !strncasecmp(&p[2], "BEGIN", 5) ||
         !strncasecmp(&p[2], "END", 3) || !strncasecmp(&p[2], "TRAILER", 7))) {
...
#else
void epsf_emit_body(GVJ_t *job, usershape_t *us) { gvputs(job, us->data); }
#endif

void epsf_define(GVJ_t *job) {
  usershape_t *us;

  if (!EPSF_contents) return;
...
char *ps_string(char *ins, int chset) {
    ...
    default:
      switch (charsetOf(ins)) {
        ...
        case NONLATIN:
          if (!warned) {
            agerr(AGWARN,
                  "UTF-8 input uses non-Latin1 characters which cannot be "
  ...
  if (xb.buf == NULL) agxbinit(&xb, 0, NULL);
