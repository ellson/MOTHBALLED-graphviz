...
double yDir(double y) { return YDIR(y); }

static int (*putstr)(void* chan, const char* str);

static void agputs(const char* s, FILE* fp) { putstr((void*)fp, s); }
...
static void writenodeandport(FILE* f, node_t* node, char* port) {
  ...
  if (port && *port) printstring(f, ":", agcanonStr(port));
}
  ...
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
    for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
      ...
      if (extend) {  // assuming these two attrs have already been created by
                     // cgraph
        if (!(tport = agget(e, "tailport"))) tport = "";
        if (!(hport = agget(e, "headport"))) hport = "";
      } else
        tport = hport = "";
      if (ED_spl(e)) {
        splinePoints = 0;
        ...
        for (i = 0; i < ED_spl(e)->size; i++) {
          bz = ED_spl(e)->list[i];
          for (j = 0; j < bz.size; j++) printpoint(f, bz.list[j]);
        }
      }
      if (ED_label(e)) {
...
static void set_record_rects(node_t* n, field_t* f, agxbuf* xb) {
  ...
  if (f->n_flds == 0) {
    sprintf(buf, "%.5g,%.5g,%.5g,%.5g ", f->b.LL.x + ND_coord(n).x,
            YDIR(f->b.LL.y + ND_coord(n).y), f->b.UR.x + ND_coord(n).x,
            YDIR(f->b.UR.y + ND_coord(n).y));
    agxbput(xb, buf);
  }
  for (i = 0; i < f->n_flds; i++) set_record_rects(n, f->fld[i], xb);
}

static void rec_attach_bb(graph_t* g, Agsym_t* bbsym, Agsym_t* lpsym,
                          Agsym_t* lwsym, Agsym_t* lhsym) {
  int c;
  char buf[BUFSIZ];
  pointf pt;
...
void attach_attrs_and_arrows(graph_t* g, int* sp, int* ep) {
  ...
  if (GD_has_labels(g) & NODE_XLABEL) safe_dcl(g, AGNODE, "xlp", "");
  if (GD_has_labels(g) & EDGE_LABEL) safe_dcl(g, AGEDGE, "lp", "");
  if (GD_has_labels(g) & EDGE_XLABEL) safe_dcl(g, AGEDGE, "xlp", "");
  if (GD_has_labels(g) & HEAD_LABEL) safe_dcl(g, AGEDGE, "head_lp", "");
  if (GD_has_labels(g) & TAIL_LABEL) safe_dcl(g, AGEDGE, "tail_lp", "");
  ...
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
    if (dim3) {
      ...
      sprintf(buf, "%.5g,%.5g,%.5g", ND_coord(n).x, YDIR(ND_coord(n).y),
    ...
    if (strcmp(ND_shape(n)->name, "record") == 0) {
      ...
      if (N_vertices && isPolygon(n)) {
        ...
        for (i = 0; i < sides; i++) {
          if (i > 0) agxbputc(&xb, ' ');
          if (poly->sides >= 3)
            sprintf(buf, "%.5g %.5g", PS2INCH(poly->vertices[i].x),
      ...
      for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
        ...
        for (i = 0; i < ED_spl(e)->size; i++) {
          ...
          if (ED_spl(e)->list[i].sflag) {
            s_arrows = 1;
            sprintf(buf, "s,%.5g,%.5g ", ED_spl(e)->list[i].sp.x,
                    YDIR(ED_spl(e)->list[i].sp.y));
            agxbput(&xb, buf);
          }
          if (ED_spl(e)->list[i].eflag) {
            e_arrows = 1;
            sprintf(buf, "e,%.5g,%.5g ", ED_spl(e)->list[i].ep.x,
