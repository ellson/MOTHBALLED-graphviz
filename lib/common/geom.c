...
int lineToBox(pointf p, pointf q, boxf b) {
  ...
  inside1 =
      (p.x >= b.LL.x) && (p.x <= b.UR.x) && (p.y >= b.LL.y) && (p.y <= b.UR.y);
  inside2 =
      (q.x >= b.LL.x) && (q.x <= b.UR.x) && (q.y >= b.LL.y) && (q.y <= b.UR.y);
  if (inside1 != inside2) {
    return 0;
  }
  if (inside1 & inside2) {
  ...
  if (p.x == q.x) {
    ...
    if (((p.y >= b.LL.y) ^ (q.y >= b.LL.y)) && (p.x >= b.LL.x) &&
        (p.x <= b.UR.x)) {
      return 0;
    }
  } else if (p.y == q.y) {
     ...
     * Horizontal line.
     */
    if (((p.x >= b.LL.x) ^ (q.x >= b.LL.x)) && (p.y >= b.LL.y) &&
        (p.y <= b.UR.y)) {
      return 0;
    }
  } else {
...
point cwrotatep(point p, int cwrot) {
  ...
  switch (cwrot) {
    ...
    case 270:
      ...
      break;
    default:
      if (cwrot < 0) return ccwrotatep(p, -cwrot);
      if (cwrot > 360) return cwrotatep(p, cwrot % 360);
...
pointf cwrotatepf(pointf p, int cwrot) {
  ...
  switch (cwrot) {
    ...
    case 270:
      ...
      break;
    default:
      if (cwrot < 0) return ccwrotatepf(p, -cwrot);
      if (cwrot > 360) return cwrotatepf(p, cwrot % 360);
...
point ccwrotatep(point p, int ccwrot) {
  ...
  switch (ccwrot) {
    ...
    case 270:
      ...
      break;
    default:
      if (ccwrot < 0) return cwrotatep(p, -ccwrot);
      if (ccwrot > 360) return ccwrotatep(p, ccwrot % 360);
...
pointf ccwrotatepf(pointf p, int ccwrot) {
  ...
  switch (ccwrot) {
    ...
    case 270:
      ...
      break;
    default:
      if (ccwrot < 0) return cwrotatepf(p, -ccwrot);
      if (ccwrot > 360) return ccwrotatepf(p, ccwrot % 360);
