...
static void freePair(Dt_t *d, pair *pp, Dtdisc_t *disc) { free(pp); }
...
static Dtdisc_t intPairDisc = {
    offsetof(pair, id), sizeof(point),      offsetof(pair, link),
    0,                  (Dtfree_f)freePair, (Dtcompar_f)cmppair,
    0,                  0,                  0};

PointSet *newPS(void) { return (dtopen(&intPairDisc, Dtoset)); }
...
int sizeOf(PointSet *ps) { return dtsize(ps); }
...
static Dtdisc_t intMPairDisc = {
    offsetof(mpair, id), sizeof(point),       offsetof(mpair, link),
    (Dtmake_f)mkMPair,   (Dtfree_f)freeMPair, (Dtcompar_f)cmppair,
    0,                   0,                   0};
