...
int colorCvt(gvcolor_t *ocolor, gvcolor_t *ncolor) {
  ...
  switch (ocolor->type) {
    case HSVA_DOUBLE:
      sprintf(buf, "%.03f %.03f %.03f %.03f", ocolor->u.HSVA[0],
              ocolor->u.HSVA[1], ocolor->u.HSVA[2], ocolor->u.HSVA[3]);
      break;
    case RGBA_BYTE:
      sprintf(buf, "#%02x%02x%02x%02x", ocolor->u.rgba[0], ocolor->u.rgba[1],
              ocolor->u.rgba[2], ocolor->u.rgba[3]);
      break;
    case RGBA_WORD:
    ...
    case COLOR_STRING:
      ...
      break;
    case CMYK_BYTE:
      /* agerr (AGWARN, "Input color type 'CMYK_BYTE' not supported for
       * conversion\n"); */
      return COLOR_UNKNOWN;
      break;
    case COLOR_INDEX:
      /* agerr (AGWARN, "Input color type 'COLOR_INDEX' not supported for
       * conversion\n"); */
      return COLOR_UNKNOWN;
      break;
    default:
 ...
 */
void setColorScheme(char *s) { colorscheme = s; }
