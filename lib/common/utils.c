 ...
 * this indicates it is web-active. In this case, it requires that the
 *GV_FILE_PATH
...
static char *findPath(char **dirs, int maxdirlen, const char *str) {
  ...
  for (dp = dirs; *dp; dp++) {
    ...
    if (access(safefilename, R_OK) == 0) return safefilename;
  }
  return NULL;
}
...
const char *safefile(const char *filename) {
  ...
  if (HTTPServerEnVar) {/* If used as a server */
    ...
    if (!Gvfilepath || (*Gvfilepath == '\0')) {
      if (onetime) {
        agerr(AGWARN,
              "file loading is disabled because the environment contains "
              "SERVER_NAME=\"%s\"\n"
    ...
    if (onetime && str != filename) {
      agerr(AGWARN,
            ...
            " because files are only permitted to be loaded from the "
            "directories in \"%s\""
  ...
  if (pathlist != Gvimagepath) {
    ...
    if (pathlist && *pathlist) dirs = mkDirlist(pathlist, &maxdirlen);
  }

  if ((*filename == DIRSEP[0]) || !dirs) return filename;
...
boolean mapbool(char *p) { return mapBool(p, FALSE); }
...
pointf dotneato_closest(splines *spl, pointf pt) {
  ...
  do {
    ...
    if (fabs(dlow2 - dhigh2) < 1.0) break;
    if (fabs(high - low) < .00001) break;
    if (dlow2 < dhigh2) {
      high = t;
      dhigh2 = DIST2(pt2, pt);
    } else {
...
/* this caching seems to prevent p.x from getting set from bz.list[0].x
  ...
  for (i = 0; i < spl->size; i++) {
    bz = spl->list[i];
    if (BETWEEN(bz.list[bz.size - 1].y, y, bz.list[0].y)) break;
  }
#if 0
    }
#endif
  if (y > bz.list[0].y)
    ...
    for (i = 0; i < bz.size; i += 3) {
      for (j = 0; j < 3; j++) {
        if ((bz.list[i + j].y <= y) && (y <= bz.list[i + j + 1].y)) break;
        if ((bz.list[i + j].y >= y) && (y >= bz.list[i + j + 1].y)) break;
      }
      if (j < 3) break;
    }
...
void common_init_node(node_t *n) {
  ...
  ND_shape(n) = bind_shape(late_nnstring(n, N_shape, DEFAULT_NODESHAPE), n);
  ...
  ND_label(n) = make_label((void *)n, str,
                           ((aghtmlstr(str) ? LT_HTML : LT_NONE) |
                            ((shapeOf(n) == SH_RECORD) ? LT_RECD : LT_NONE)),
                           fi.fontsize, fi.fontname, fi.fontcolor);
  if (N_xlabel && (str = agxget(n, N_xlabel)) && (str[0])) {
    ND_xlabel(n) =
        make_label((void *)n, str, (aghtmlstr(str) ? LT_HTML : LT_NONE),
                   fi.fontsize, fi.fontname, fi.fontcolor);
...
static void initFontLabelEdgeAttr(edge_t *e, struct fontinfo *fi,
                                  struct fontinfo *lfi) {
...
int common_init_edge(edge_t *e) {
  ...
  if (E_label && (str = agxget(e, E_label)) && (str[0])) {
    ...
    ED_label(e) =
        make_label((void *)e, str, (aghtmlstr(str) ? LT_HTML : LT_NONE),
                   fi.fontsize, fi.fontname, fi.fontcolor);
  ...
  if (E_taillabel && (str = agxget(e, E_taillabel)) && (str[0])) {
    if (!lfi.fontname) initFontLabelEdgeAttr(e, &fi, &lfi);
    ED_tail_label(e) =
        make_label((void *)e, str, (aghtmlstr(str) ? LT_HTML : LT_NONE),
                   lfi.fontsize, lfi.fontname, lfi.fontcolor);
...
static Dtdisc_t mapDisc = {
    offsetof(item, p), sizeof(2 * sizeof(void *)), offsetof(item, link),
 ...
 */
#define MAPC(n) \
  (strncmp(agnameof(n), "cluster", 7) ? NULL : findCluster(cmap, agnameof(n)))

static void checkCompound(edge_t *e, graph_t *clg, agxbuf *xb, Dt_t *map,
                          Dt_t *cmap) {
  graph_t *tg;
  graph_t *hg;
  node_t *cn;
...
static node_t *mapN(node_t *n, graph_t *clg) {
  ...
  if (!(IS_CLUST_NODE(n))) return n;
  agsubnode(clg, n, 1);
  name = strchr(agnameof(n), ':');
  assert(name);
  name++;
  if ((nn = agfindnode(g, name))) return nn;
  nn = agnode(g, name, 1);
  agbindrec(nn, "Agnodeinfo_t", sizeof(Agnodeinfo_t), TRUE);

  /* Set all attributes to default */
  for (sym = agnxtattr(g, AGNODE, NULL); sym;
       (sym = agnxtattr(g, AGNODE, sym))) {
    if (agxget(nn, sym) != sym->defval) agxset(nn, sym, sym->defval);
  }
  return nn;
}
...
static void undoCompound(edge_t *e, graph_t *clg) {
  ...
  if (!(IS_CLUST_NODE(t) || IS_CLUST_NODE(h))) return;
  ntail = mapN(t, clg);
  nhead = mapN(h, clg);
...
static int comp_entities(const void *e1, const void *e2) {
  return strcmp(((struct entities_s *)e1)->name,
 ...
 * check for invalid utf8. If found, treat a single byte as Latin-1, convert it
 * to
...
char *htmlEntityUTF8(char *s, graph_t *g) {
  ...
  while ((c = *(unsigned char *)s++)) {
    if (c < 0xC0)
      ...
      if (!warned) {
        agerr(AGWARN,
              "UTF8 codes > 4 bytes are not currently supported (graph %s) - "
              "treated as Latin-1. Perhaps \"-Gcharset=latin1\" is needed?\n",
              agnameof(g));
        warned = 1;
      }
      c = cvtAndAppend(c, &xb);
    ...
    if (uc == 0 && c == '&') {
      ...
      for (ui = 0; ui < uc; ++ui)
        if ((*s & 0xC0) == 0x80) {
          ...
          if (!warned) {
            agerr(AGWARN,
                  "Invalid %d-byte UTF8 found in input of graph %s - treated "
...
boolean overlap_node(node_t *n, boxf b) {
  ...
  if (!OVERLAP(b, ND_bb(n))) return FALSE;
...
static boolean overlap_arrow(pointf p, pointf u, double scale, int flag,
                             boxf b) {
...
static boolean overlap_bezier(bezier bz, boxf b) {
  ...
  for (i = 1; i < bz.size; i++) {
    p = bz.list[i];
    if (lineToBox(p, u, b) != -1) return TRUE;
    u = p;
  }

  /* check arrows */
  if (bz.sflag) {
    if (overlap_arrow(bz.sp, bz.list[0], 1, bz.sflag, b)) return TRUE;
  }
  if (bz.eflag) {
    if (overlap_arrow(bz.ep, bz.list[bz.size - 1], 1, bz.eflag, b)) return TRUE;
  }
  return FALSE;
}
...
boolean overlap_edge(edge_t *e, boxf b) {
  ...
  if (spl && boxf_overlap(spl->bb, b))
    for (i = 0; i < spl->size; i++)
      if (overlap_bezier(spl->list[i], b)) return TRUE;
...
int edgeType(char *s, int dflt) {
  ...
  switch (*s) {
    ...
    case 'f':
    case 'F':
      if (!strcasecmp(s + 1, "alse")) et = ET_LINE;
      break;
    case 'l':
    case 'L':
      if (!strcasecmp(s + 1, "ine")) et = ET_LINE;
      break;
    case 'n':
    case 'N':
    ...
    case 'o':
    case 'O':
      if (!strcasecmp(s + 1, "rtho")) et = ET_ORTHO;
      break;
    case 'p':
    case 'P':
      if (!strcasecmp(s + 1, "olyline")) et = ET_PLINE;
      break;
    case 's':
    case 'S':
      if (!strcasecmp(s + 1, "pline")) et = ET_SPLINE;
      break;
    case 't':
    case 'T':
      if (!strcasecmp(s + 1, "rue")) et = ET_SPLINE;
      break;
    case 'y':
    case 'Y':
      if (!strcasecmp(s + 1, "es")) et = ET_SPLINE;
      break;
  }
  if (!et) {
 ...
 * point;
 * else uses the angle parameter to identify two points on a line that defines
 * the
 * gradient direction
 * By default, this assumes a left-hand coordinate system (for svg); if RHS = 2
 * flag
...
void gv_cleanup_node(node_t *n) {
  ...
  if (ND_shape(n)) ND_shape(n)->fns->freefn(n);
...
void fix_fc(void) {
  ...
  if ((fp = fopen("fix-fc.exe", "r")) == NULL) return;
  fclose(fp);
...
static void free_clust(Dt_t *dt, clust_t *clp, Dtdisc_t *disc) { free(clp); }

static Dtdisc_t strDisc = {
    offsetof(clust_t, name), -1,                   offsetof(clust_t, link),
    NIL(Dtmake_f),           (Dtfree_f)free_clust, NIL(Dtcompar_f),
    NIL(Dthash_f),           NIL(Dtmemory_f),      NIL(Dtevent_f)};
