...
static int sidesfn(htmldata_t *p, char *v) {
  ...
  if (flags != BORDER_MASK) p->flags |= flags;
  return 0;
}
...
static int stylefn(htmldata_t *p, char *v) {
  ...
  for (tk = strtok(buf, DELIM); tk; tk = strtok(NULL, DELIM)) {
    c = toupper(*tk);
    if (c == 'R') {
      if (!strcasecmp(tk + 1, "OUNDED"))
        p->style |= ROUNDED;
      else if (!strcasecmp(tk + 1, "ADIAL"))
        p->style |= RADIAL;
      else {
        agerr(AGWARN, "Illegal value %s for STYLE - ignored\n", tk);
        rv = 1;
      }
    } else if (!strcasecmp(tk, "SOLID"))
      p->style &= ~(DOTTED | DASHED);
    else if (!strcasecmp(tk, "INVISIBLE") || !strcasecmp(tk, "INVIS"))
      p->style |= INVISIBLE;
    else if (!strcasecmp(tk, "DOTTED"))
      p->style |= DOTTED;
    else if (!strcasecmp(tk, "DASHED"))
      p->style |= DASHED;
    else {
      agerr(AGWARN, "Illegal value %s for STYLE - ignored\n", tk);
      rv = 1;
...
static int gradientanglefn(htmldata_t *p, char *v) {
  ...
  if (doInt(v, "GRADIENTANGLE", 0, 360, &u)) return 1;
...
static int cell_halignfn(htmldata_t *p, char *v) {
  ...
  if ((c == 'L') && !strcasecmp(v + 1, "EFT"))
    ...
    rv = 1;
  if (rv) agerr(AGWARN, "Illegal value %s for ALIGN in TD - ignored\n", v);
  return rv;
}
...
static int balignfn(htmldata_t *p, char *v) {
  ...
  if ((c == 'L') && !strcasecmp(v + 1, "EFT"))
    ...
    rv = 1;
  if (rv) agerr(AGWARN, "Illegal value %s for BALIGN in TD - ignored\n", v);
  return rv;
}
...
static int rowspanfn(htmlcell_t *p, char *v) {
  ...
  if (doInt(v, "ROWSPAN", 0, MAX_USHORT, &u)) return 1;
  if (u == 0) {
...
static int colspanfn(htmlcell_t *p, char *v) {
  ...
  if (doInt(v, "COLSPAN", 0, MAX_USHORT, &u)) return 1;
  if (u == 0) {
...
static int ptsizefn(textfont_t *p, char *v) {
  ...
  if (doInt(v, "POINT-SIZE", 0, MAX_UCHAR, &u)) return 1;
  p->size = (double)u;
  return 0;
}
...
static attr_item tbl_items[] = {{"align", (attrFn)halignfn},
...
static attr_item cell_items[] = {{"align", (attrFn)cell_halignfn},
...
static attr_item font_items[] = {{"color", (attrFn)fontcolorfn},
                                 {"face", (attrFn)facefn},
                                 {"point-size", (attrFn)ptsizefn}, };

static attr_item img_items[] = {{"scale", (attrFn)scalefn},
                                {"src", (attrFn)srcfn}, };

static attr_item br_items[] = {{"align", (attrFn)alignfn}, };
 ...
 */
static void doAttrs(void *tp, attr_item *items, int nel, char **atts, char *s) {
  char *name;
  char *val;
  attr_item *ip;
  attr_item key;
...
int initHTMLlexer(char *src, agxbuf *xb, htmlenv_t *env) {
  ...
  XML_SetElementHandler(state.parser, (XML_StartElementHandler)startElement,
                        endElement);
