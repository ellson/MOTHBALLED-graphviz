...
static void map_edge(edge_t* e) {
  ...
  for (j = 0; j < ED_spl(e)->size; j++) {
    bz = ED_spl(e)->list[j];
    for (k = 0; k < bz.size; k++) bz.list[k] = map_point(bz.list[k]);
    if (bz.sflag) ED_spl(e)->list[j].sp = map_point(ED_spl(e)->list[j].sp);
    if (bz.eflag) ED_spl(e)->list[j].ep = map_point(ED_spl(e)->list[j].ep);
  }
  if (ED_label(e)) ED_label(e)->pos = map_point(ED_label(e)->pos);
  if (ED_xlabel(e)) ED_xlabel(e)->pos = map_point(ED_xlabel(e)->pos);
  /* vladimir */
  if (ED_head_label(e))
...
static void translate_drawing(graph_t* g) {
  ...
  for (v = agfstnode(g); v; v = agnxtnode(g, v)) {
    ...
    if (ND_xlabel(v)) ND_xlabel(v)->pos = map_point(ND_xlabel(v)->pos);
    if (State == GVSPLINES)
...
static int printData(object_t* objs, int n_objs, xlabel_t* lbls, int n_lbls,
  ...
  for (i = 0; i < n_objs; i++) {
    ...
    fprintf(stderr, " [%d] (%.02f,%.02f) (%.02f,%.02f) %p \"%s\"\n", i,
            objs->pos.x, objs->pos.y, objs->sz.x, objs->sz.y, objs->lbl,
  ...
  for (i = 0; i < n_lbls; i++) {
    fprintf(stderr, " [%d] %p set %d (%.02f,%.02f) (%.02f,%.02f) %s\n", i, lbls,
            lbls->set, lbls->pos.x, lbls->pos.y, lbls->sz.x, lbls->sz.y,
 ...
 */
static void addXLabel(textlabel_t* lp, object_t* objp, xlabel_t* xlp,
                      int initObj, pointf pos) {
  if (initObj) {
    objp->sz.x = 0;
    objp->sz.y = 0;
    objp->pos = pos;
...
static int countClusterLabels(Agraph_t* g) {
  int c, i = 0;
  if ((g != agroot(g)) && (GD_label(g)) && GD_label(g)->set) i++;
  for (c = 1; c <= GD_n_cluster(g); c++)
...
static void addXLabels(Agraph_t* gp) {
  ...
  if (!(GD_has_labels(gp) & NODE_XLABEL) &&
      !(GD_has_labels(gp) & EDGE_XLABEL) && !(GD_has_labels(gp) & TAIL_LABEL) &&
  ...
  if (GD_has_labels(gp) & GRAPH_LABEL) n_clbls = countClusterLabels(gp);
  ...
  if (Verbose) printData(objs, n_objs, lbls, n_lbls, &params);
...
void gv_postprocess(Agraph_t* g, int allowTranslation) {
  ...
  if (GD_label(g) && !GD_label(g)->set) place_root_label(g, dimen);
  ...
  if (Show_boxes) {
    ...
    if (Flip)
      ...
      sprintf(buf, M1, Offset.y, Offset.x, Offset.y, Offset.x, -Offset.x,
              -Offset.y);
 ...
 */
void dotneato_postprocess(Agraph_t* g) { gv_postprocess(g, 1); }
...
static void place_flip_graph_label(graph_t* g) {
  ...
  for (c = 1; c <= GD_n_cluster(g); c++) place_flip_graph_label(GD_clust(g)[c]);
}
...
void place_graph_label(graph_t* g) {
  ...
  for (c = 1; c <= GD_n_cluster(g); c++) place_graph_label(GD_clust(g)[c]);
}
