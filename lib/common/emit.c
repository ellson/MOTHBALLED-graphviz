 ...
 */
int initMapData(GVJ_t *job, char *lbl, char *url, char *tooltip, char *target,
                char *id, void *gobj) {
  obj_state_t *obj = job->obj;
  int flags = job->flags;
  int assigned = 0;
...
static void map_point(GVJ_t *job, pointf pf) {
  ...
  if (flags & (GVRENDER_DOES_MAPS | GVRENDER_DOES_TOOLTIPS)) {
    ...
    if (!(flags & GVRENDER_DOES_TRANSFORM)) gvrender_ptf_A(job, p, p, 2);
    if (!(flags & GVRENDER_DOES_MAP_RECTANGLE)) rect2poly(p);
  }
}
...
typedef struct {
  ...
  colorseg_t *segs; /* array of segments; real segments always followed by a
                       sentinel */
} colorsegs_t;
 ...
 *distributed
 * to all colors without an explicit float. If no such colors exist, the
 *remainder
...
int wedgedEllipse(GVJ_t *job, pointf *pf, char *clrs) {
  ...
  semi.x = pf[1].x - ctr.x;
  semi.y = pf[1].y - ctr.y;
  if (save_penwidth > THIN_LINE) gvrender_set_penwidth(job, THIN_LINE);
...
static void psmapOutput(pointf *ps, int n) {
  ...
  for (i = 1; i < n; i++) fprintf(stdout, "%f %f lineto\n", ps[i].x, ps[i].y);
 ...
 */
static void map_bspline_poly(pointf **pbs_p, int **pbs_n, int *pbs_poly_n,
                             int n, pointf *p1, pointf *p2) {
  int i = 0, nump = 0, last = 2 * n - 1;

  for (; i < *pbs_poly_n; i++) nump += (*pbs_n)[i];
 ...
 * in chunks of 100.
 */
static void map_output_bspline(pointf **pbs, int **pbs_n, int *pbs_poly_n,
                               bezier *bp, double w2) {
...
static int layer_index(GVC_t *gvc, char *str, int all) {
  ...
  if (gvc->layerIDs)
    for (i = 1; i <= gvc->numLayers; i++)
      if (streq(str, gvc->layerIDs[i])) return i;
  return -1;
}

static boolean selectedLayer(GVC_t *gvc, int layerNum, int numLayers,
                             char *spec) {
  int n0, n1;
  unsigned char buf[SMALLBUF];
  char *w0, *w1;
  ...
  while ((rval == FALSE) &&
    ...
    if (w0) w1 = strtok_r(NULL, gvc->layerDelims, &buf_p);
...
static int *parse_layerselect(GVC_t *gvc, graph_t *g, char *p) {
  ...
  if (cnt) {
    ...
    agerr(AGWARN,
          "The layerselect attribute \"%s\" does not match any layer specifed "
          "by the layers attribute - ignored.\n",
          p);
    laylist[0] = cnt;
    free(laylist);
    laylist = NULL;
...
static int parse_layers(GVC_t *gvc, graph_t *g, char *p) {
  ...
  if (!gvc->layerDelims) gvc->layerDelims = DEFAULT_LAYERSEP;
  gvc->layerListDelims = agget(g, "layerlistsep");
  if (!gvc->layerListDelims) gvc->layerListDelims = DEFAULT_LAYERLISTSEP;
  if ((tok =
           strpbrk(gvc->layerDelims,
                   gvc->layerListDelims))) {/* conflict in delimiter strings */
    agerr(AGWARN,
          "The character \'%c\' appears in both the layersep and layerlistsep "
          "attributes - layerlistsep ignored.\n",
          *tok);
    gvc->layerListDelims = "";
  }
  ...
  if (ntok) {
    gvc->layerIDs =
        RALLOC(ntok + 2, gvc->layerIDs, char *); /* shrink to minimum size */
    gvc->layerIDs[0] = NULL;
...
static void init_job_pagination(GVJ_t *job, graph_t *g) {
  ...
  if (job->rotation) imageSize = exch_xyf(imageSize);
  ...
  /* set up pagedir */
  job->pagesArrayMajor.x = job->pagesArrayMajor.y = job->pagesArrayMinor.x =
      job->pagesArrayMinor.y = 0;
  ...
  if (GD_drawing(g)->centered) {
    if (pageSize.x > imageSize.x) centering.x = (pageSize.x - imageSize.x) / 2;
    if (pageSize.y > imageSize.y) centering.y = (pageSize.y - imageSize.y) / 2;
  }
...
static pointf *copyPts(pointf *pts, int *ptsize, xdot_point *inpts,
                       int numpts) {
  int i, sz = *ptsize;
...
static void emit_xdot(GVJ_t *job, xdot *xd) {
  ...
  for (i = 0; i < xd->cnt; i++) {
    switch (op->op.kind) {
      ...
      case xd_unfilled_polygon:
        if (boxf_overlap(op->bb, job->clip)) {
          pts =
              copyPts(pts, &ptsize, op->op.u.polygon.pts, op->op.u.polygon.cnt);
      ...
      case xd_polyline:
        if (boxf_overlap(op->bb, job->clip)) {
          pts = copyPts(pts, &ptsize, op->op.u.polyline.pts,
  ...
  if (styles) gvrender_set_style(job, job->gvc->defaultlinestyle);
  free(pts);
}
...
static void emit_background(GVJ_t *job, graph_t *g) {
  ...
  if ((xd = (xdot *)GD_drawing(g)->xdots)) emit_xdot(job, xd);
}
...
static boolean edge_in_layer(GVJ_t *job, graph_t *g, edge_t *e) {
  ...
  if (job->numLayers <= 1) return TRUE;
  pe = late_string(e, E_layer, "");
  if (selectedlayer(job, pe)) return TRUE;
  if (pe[0]) return FALSE;
  for (cnt = 0; cnt < 2; cnt++) {
    pn = late_string(cnt < 1 ? agtail(e) : aghead(e), N_layer, "");
    if ((pn[0] == '\0') || selectedlayer(job, pn)) return TRUE;
  }
  return FALSE;
}
...
static boolean clust_in_layer(GVJ_t *job, graph_t *sg) {
  ...
  if (selectedlayer(job, pg)) return TRUE;
  if (pg[0]) return FALSE;
  for (n = agfstnode(sg); n; n = agnxtnode(sg, n))
    if (node_in_layer(job, sg, n)) return TRUE;
  return FALSE;
}
...
static void emit_begin_node(GVJ_t *job, node_t *n) {
  ...
  if ((flags & (GVRENDER_DOES_MAPS | GVRENDER_DOES_TOOLTIPS)) &&
    ...
    if (shape == SH_POLY || shape == SH_POINT) {
      ...
      if (isRect(poly) && (poly->peripheries || filled)) rect = 1;
    }
    ...
    if (poly && !rect && (flags & GVRENDER_DOES_MAP_POLYGON)) {
      ...
      if ((s = agget(n, "samplepoints"))) nump = atoi(s);
       ...
       */
      if ((nump < 4) || (nump > 60)) nump = DFLT_SAMPLE;
    ...
    if (!(flags & GVRENDER_DOES_TRANSFORM)) gvrender_ptf_A(job, p, p, nump);
    obj->url_map_p = p;
    obj->url_map_n = nump;
  }
...
static void splitBSpline(bezier *bz, float t, bezier *left, bezier *right) {
  ...
  for (j = 0; j < left->size; j++) left->list[j] = bz->list[j];
  k = j - 4;
  for (j = 0; j < right->size; j++) right->list[j] = bz->list[k++];
...
static int multicolor(GVJ_t *job, edge_t *e, char **styles, char *colors,
                      int num, double arrowsize, double penwidth) {
  bezier bz;
...
#define SEP 2.0
  ...
  if (ED_spl(e)) {
    ...
    if (ED_gui_state(e) & GUI_STATE_ACTIVE) {
      pencolor =
          late_nnstring(e, E_activepencolor,
      ...
      pencolor =
          late_nnstring(e, E_selectedpencolor,
                        default_pencolor(pencolor, DEFAULT_SELECTEDPENCOLOR));
      fillcolor =
          late_nnstring(e, E_selectedfillcolor, DEFAULT_SELECTEDFILLCOLOR);
    } else if (ED_gui_state(e) & GUI_STATE_DELETED) {
      pencolor =
          late_nnstring(e, E_deletedpencolor,
                        default_pencolor(pencolor, DEFAULT_DELETEDPENCOLOR));
      fillcolor =
          late_nnstring(e, E_deletedfillcolor, DEFAULT_DELETEDFILLCOLOR);
    } else if (ED_gui_state(e) & GUI_STATE_VISITED) {
      pencolor =
          late_nnstring(e, E_visitedpencolor,
                        default_pencolor(pencolor, DEFAULT_VISITEDPENCOLOR));
      fillcolor =
          late_nnstring(e, E_visitedfillcolor, DEFAULT_VISITEDFILLCOLOR);
    } else
      fillcolor = late_nnstring(e, E_fillcolor, color);
    if (pencolor != color) gvrender_set_pencolor(job, pencolor);
    if (fillcolor != color) gvrender_set_fillcolor(job, fillcolor);
    color = pencolor;
    ...
    /* if more than one color - then generate parallel beziers, one per color */
      ...
      for (cnum = 0, color = strtok(colors, ":"); color;
        ...
        if (cnum == 0) headcolor = tailcolor = color;
        ...
        for (i = 0; i < tmpspl.size; i++) {
          ...
          gvrender_beziercurve(job, tmplist, tmpspl.list[i].size, FALSE, FALSE,
                               FALSE);
        }
      }
      if (bz.sflag) {
      ...
      for (i = 0; i < ED_spl(e)->size; i++) {
        ...
        if (job->flags & GVRENDER_DOES_ARROWS) {
          gvrender_beziercurve(job, bz.list, bz.size, bz.sflag, bz.eflag,
                               FALSE);
        } else {
...
static boolean edge_in_box(edge_t *e, boxf b) {
  ...
  spl = ED_spl(e);
  if (spl && boxf_overlap(spl->bb, b)) return TRUE;

  lp = ED_label(e);
  if (lp && overlap_label(lp, b)) return TRUE;

  lp = ED_xlabel(e);
  if (lp && lp->set && overlap_label(lp, b)) return TRUE;
...
static void emit_begin_edge(GVJ_t *job, edge_t *e, char **styles) {
  ...
  if (flags & GVRENDER_DOES_LABELS) {
    if ((lab = ED_label(e))) obj->label = lab->text;
    obj->taillabel = obj->headlabel = obj->xlabel = obj->label;
    if ((tlab = ED_xlabel(e))) obj->xlabel = tlab->text;
    if ((tlab = ED_tail_label(e))) obj->taillabel = tlab->text;
    if ((hlab = ED_head_label(e))) obj->headlabel = hlab->text;
  }
  ...
  if (flags & (GVRENDER_DOES_MAPS | GVRENDER_DOES_TOOLTIPS)) {
    if (ED_spl(e) && (obj->url || obj->tooltip) &&
      ...
      if (!(flags & GVRENDER_DOES_TRANSFORM)) {
        for (nump = 0, i = 0; i < pbs_poly_n; i++) nump += pbs_n[i];
 ...
 */
static void nodeIntersect(GVJ_t *job, pointf p, boolean explicit_iurl,
                          char *iurl, boolean explicit_itooltip, char *itooltip,
...
static void emit_end_edge(GVJ_t *job) {
  ...
  if (obj->url || obj->explicit_tooltip) {
    ...
    if (obj->url_bsplinemap_poly_n) {
      for (nump = obj->url_bsplinemap_n[0], i = 1;
  ...
  emit_edge_label(
      job, ED_label(e), EMIT_ELABEL, obj->explicit_labeltooltip, obj->labelurl,
      obj->labeltooltip, obj->labeltarget, obj->id,
      ((mapbool(late_string(e, E_decorate, "false")) && ED_spl(e)) ? ED_spl(e)
                                                                   : 0));
  emit_edge_label(
      job, ED_xlabel(e), EMIT_ELABEL, obj->explicit_labeltooltip, obj->labelurl,
      obj->labeltooltip, obj->labeltarget, obj->id,
      ((mapbool(late_string(e, E_decorate, "false")) && ED_spl(e)) ? ED_spl(e)
                                                                   : 0));
...
static void emit_edge(GVJ_t *job, edge_t *e) {
  ...
  if (edge_in_box(e, job->clip) && edge_in_layer(job, agraphof(aghead(e)), e)) {
    ...
    s = malloc(strlen(agnameof(agtail(e))) + 2 + strlen(agnameof(aghead(e))) +
               1);
...
static void freePara(exdot_op *op) {
  if (op->op.kind == xd_text) free_textspan(op->span, 1);
}
  ...
  for (i = 0; i < xd->cnt; i++) {
    ...
    switch (op->op.kind) {
      ...
      case xd_text:
        ...
        expandBB(&bb, bb0.LL);
        expandBB(&bb, bb0.UR);
        if (!xd->freefunc) xd->freefunc = (freefunc_t)freePara;
        break;
      case xd_font:
...
static void init_gvc(GVC_t *gvc, graph_t *g) {
  ...
  if ((p = agget(g, "margin"))) {
    ...
    if (i > 0) {
      ...
      if (i > 1) gvc->margin.y = yf * POINTS_PER_INCH;
  ...
  if ((p = agget(g, "pad"))) {
    ...
    if (i > 0) {
      ...
      if (i > 1) gvc->pad.y = yf * POINTS_PER_INCH;
  ...
  /* pagedir */
  gvc->pagedir = "BL";
  if ((p = agget(g, "pagedir")) && p[0]) gvc->pagedir = p;
...
static void init_job_viewport(GVJ_t *job, graph_t *g) {
  ...
  LL = gvc->bb.LL;
  job->bb.LL.x =
      LL.x - job->pad.x; /* job->bb is bb of graph and padding - graph units */
  job->bb.LL.y = LL.y - job->pad.y;
  job->bb.UR.x = UR.x + job->pad.x;
  job->bb.UR.y = UR.y + job->pad.y;
  sz.x =
      job->bb.UR.x - job->bb.LL.x; /* size, including padding - graph units */
    ...
    if (((str = agget(n, "fillcolor")) != 0) && str[0]) {
      if (strchr(str, ':')) {
        ...
        for (str = strtok(colors, ":"); str; str = strtok(0, ":")) {
          if (str[0]) gvrender_set_pencolor(job, str);
        }
        free(colors);
      } else {
        gvrender_set_pencolor(job, str);
...
static void emit_view(GVJ_t *job, graph_t *g, int flags) {
  ...
  if (!(flags & EMIT_CLUSTERS_LAST)) emit_clusters(job, g, flags);
  if (flags & EMIT_SORTED) {
    /* output all nodes, then all edges */
    gvrender_begin_nodes(job);
    for (n = agfstnode(g); n; n = agnxtnode(g, n)) emit_node(job, n);
    gvrender_end_nodes(job);
    gvrender_begin_edges(job);
    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
      for (e = agfstout(g, n); e; e = agnxtout(g, e)) emit_edge(job, e);
    }
    ...
    gvrender_begin_nodes(job);
    for (n = agfstnode(g); n; n = agnxtnode(g, n))
      if (write_node_test(g, n)) emit_node(job, n);
    gvrender_end_nodes(job);
    gvrender_begin_edges(job);

    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
      for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
        if (write_edge_test(g, e)) emit_edge(job, e);
      }
    }
    gvrender_end_edges(job);
  } else {
...
static void emit_page(GVJ_t *job, graph_t *g) {
  ...
  if (GD_label(g)) emit_label(job, EMIT_GLABEL, GD_label(g));
  if (!(flags & EMIT_CLUSTERS_LAST) && (obj->url || obj->explicit_tooltip))
    gvrender_end_anchor(job);
...
void emit_graph(GVJ_t *job, graph_t *g) {
  ...
  if ((job->flags & GVRENDER_Y_GOES_DOWN) || (Y_invert)) job->devscale.y *= -1;
...
fprintf(stderr,"focus=%g,%g view=%g,%g\n",
  ...
  if (flags & EMIT_COLORS) emit_colors(job, g);

  /* reset node state */
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) ND_state(n) = 0;
  /* iterate layers */
  for (firstlayer(job, &lp); validlayer(job); nextlayer(job, &lp)) {
    if (numPhysicalLayers(job) > 1) gvrender_begin_layer(job);

    /* iterate pages */
    for (firstpage(job); validpage(job); nextpage(job)) emit_page(job, g);

    if (numPhysicalLayers(job) > 1) gvrender_end_layer(job);
  }
...
void emit_clusters(GVJ_t *job, Agraph_t *g, int flags) {
  ...
  for (c = 1; c <= GD_n_cluster(g); c++) {
    sg = GD_clust(g)[c];
    if (clust_in_layer(job, sg) == FALSE) continue;
    /* when mapping, detect events on clusters after sub_clusters */
    if (flags & EMIT_CLUSTERS_LAST) emit_clusters(job, sg, flags);
    ...
    if (GD_gui_state(sg) & GUI_STATE_ACTIVE) {
       ...
       */
      if ((!filled || !fillcolor) && ((color = agget(sg, "bgcolor")) != 0) &&
          color[0]) {
        fillcolor = color;
        filled = FILL;
      }
    ...
    if (istyle & ROUNDED) {
      ...
      if (rv > 1) agerr(AGPREV, "in cluster %s\n", agnameof(sg));
    ...
    free(clrs[0]);
    if ((lab = GD_label(sg))) emit_label(job, EMIT_CLABEL, lab);
    ...
    if (flags & EMIT_PREORDER) {
      for (n = agfstnode(sg); n; n = agnxtnode(sg, n)) {
        emit_node(job, n);
        for (e = agfstout(sg, n); e; e = agnxtout(sg, e)) emit_edge(job, e);
      }
    }
...
static void init_bb_node(graph_t *g, node_t *n) {
  ...
  for (e = agfstout(g, n); e; e = agnxtout(g, e)) init_bb_edge(e);
...
static void init_bb(graph_t *g) {
  ...
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) init_bb_node(g, n);
}
...
#define FINISH() \
  if (Verbose)   \
  fprintf(stderr, "gvRenderJobs %s: %.2f secs.\n", agnameof(g), elapsed_sec())
  ...
  for (job = gvjobs_first(gvc); job; job = gvjobs_next(gvc)) {
    ...
    switch (job->output_lang) {
      ...
      case DIA:
        ...
        job->flags |= EMIT_PREORDER | GVDEVICE_BINARY_FORMAT;
        break;
      default:
     ...
     * mutiple output files, or we are about to write to a different output
     * device */
    ...
    if (prevjob) {
      ...
      if (gvrender_begin_job(job)) continue;
