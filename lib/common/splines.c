 ...
 */
static void arrow_clip(edge_t *fe, node_t *hn, pointf *ps, int *startp,
                       int *endp, bezier *spl, splineInfo *info) {
  edge_t *e;
  int i, j, sflag, eflag;
  ...
  do {
    ...
    if (inside(inside_context, pt)) {
      *idir = t;
      ...
      for (i = 0; i < 4; i++) best[i] = seg[i];
      found = TRUE;
      *odir = t;
    }
  } while (ABS(opt.x - pt.x) > .5 || ABS(opt.y - pt.y) > .5);
  if (found)
    for (i = 0; i < 4; i++) sp[i] = best[i];
  else
    for (i = 0; i < 4; i++) sp[i] = seg[i];
}
...
void shape_clip(node_t *n, pointf curve[4]) {
  ...
  if (ND_shape(n) == NULL || ND_shape(n)->fns->insidefn == NULL) return;
...
bezier *new_spline(edge_t *e, int sz) {
  ...
  if (ED_spl(e) == NULL) ED_spl(e) = NEW(splines);
 ...
 * edge.
 */
void clip_and_install(edge_t *fe, node_t *hn, pointf *ps, int pn,
                      splineInfo *info) {
  pointf p2;
  bezier *newspl;
  node_t *tn;
  ...
  if (clipTail && ND_shape(tn) && ND_shape(tn)->fns->insidefn) {
    ...
    for (start = 0; start < pn - 4; start += 3) {
      ...
      if (ND_shape(tn)->fns->insidefn(&inside_context, p2) == FALSE) break;
    }
  ...
  if (clipHead && ND_shape(hn) && ND_shape(hn)->fns->insidefn) {
    ...
    for (end = pn - 4; end > 0; end -= 3) {
      ...
      if (ND_shape(hn)->fns->insidefn(&inside_context, p2) == FALSE) break;
    }
    shape_clip0(&inside_context, hn, &ps[end], FALSE);
  } else
    end = pn - 4;
  for (; start < pn - 4; start += 3)
    if (!APPROXEQPT(ps[start], ps[start + 3], MILLIPOINT)) break;
  for (; end > 0; end -= 3)
    if (!APPROXEQPT(ps[end], ps[end + 3], MILLIPOINT)) break;
...
void add_box(path *P, boxf b) {
  if (b.LL.x < b.UR.x && b.LL.y < b.UR.y) P->boxes[P->nbox++] = b;
}
...
void beginpath(path *P, edge_t *e, int et, pathend_t *endp, boolean merge) {
  ...
  if (pboxfn && (mask = (*pboxfn)(n, &ED_tail_port(e), side, &endp->boxes[0],
                                  &endp->boxn)))
    endp->sidemask = mask;
  else {
...
void endpath(path *P, edge_t *e, int et, pathend_t *endp, boolean merge) {
  ...
  if (et == REGULAREDGE)
    ...
    side = endp->sidemask; /* for flat edges */
  if (pboxfn && (mask = (*pboxfn)(n, &ED_head_port(e), side, &endp->boxes[0],
                                  &endp->boxn)))
    endp->sidemask = mask;
  else {
    endp->boxes[0] = endp->nb;
...
static int convert_sides_to_points(int tail_side, int head_side) {
  int vertices[] = {12, 4, 6, 2, 3,
                    1,  9, 8};  // the cumulative side value of each node point
  int i, tail_i, head_i;
...
static void selfBottom(edge_t *edges[], int ind, int cnt, double sizex,
  ...
  for (i = 0; i < cnt; i++) {
    ...
    if (ED_label(e)) {
      ...
      ED_label(e)->set = TRUE;
      if (height > stepy) dy += height - stepy;
    }
    clip_and_install(e, aghead(e), points, pointn, sinfo);
#ifdef DEBUG
    if (debugleveln(e, 1)) showPoints(points, pointn);
#endif
  }
}

static void selfTop(edge_t *edges[], int ind, int cnt, double sizex,
                    double stepy, splineInfo *sinfo) {
  int i, sgn, point_pair;
  double hy, ty, stepx, dx, dy, width, height;
  pointf tp, hp, np;
  ...
  switch (point_pair) {
    ...
    case 58:
      dx = sgn *
           ((((ND_lw(n) - (np.x - tp.x)) + (ND_rw(n) - (hp.x - np.x))) / 3.));
      break;
    case 73:
    ...
    case 84:
      dx = sgn *
           ((((ND_lw(n) - (np.x - tp.x)) + (ND_rw(n) - (hp.x - np.x))) / 2.) +
            stepx);
      break;
    case 74:
    case 75:
...
static void selfRight(edge_t *edges[], int ind, int cnt, double stepx,
...
static void selfLeft(edge_t *edges[], int ind, int cnt, double stepx,
                     double sizey, splineInfo *sinfo) {
  int i, sgn, point_pair;
  double hx, tx, stepy, dx, dy, width, height;
  pointf tp, hp, np;
 ...
 */
void makeSelfEdge(path *P, edge_t *edges[], int ind, int cnt, double sizex,
                  double sizey, splineInfo *sinfo) {
  edge_t *e;
...
int place_portlabel(edge_t *e, boolean head_p) {
  ...
  if (ED_edge_type(e) == IGNORED) return 0;
  /* add label here only if labelangle or labeldistance is defined; else, use
   * external label */
  ...
  if (!head_p) {
    ...
    if (bez->eflag) {
      ...
      for (i = 0; i < 4; i++) c[i] = bez->list[bez->size - 4 + i];
