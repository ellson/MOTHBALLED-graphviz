...
textlabel_t *make_label(void *obj, char *str, int kind, double fontsize,
  ...
  if (kind & LT_RECD) {
     ...
     * formatting escape
     * sequences (\n, \l, \r) are processed in make_simple_label. That call also
     * replaces \\ with \.
     */
...
void free_textspan(textspan_t *tl, int cnt) {
  ...
  if (!tl) return;
  for (i = 0; i < cnt; i++) {
    if ((i == 0) && tlp->str) free(tlp->str);
    if (tlp->layout && tlp->free_layout) tlp->free_layout(tlp->layout);
    tlp++;
  }
  free(tl);
}
...
void emit_label(GVJ_t *job, emit_state_t emit_state, textlabel_t *lp) {
  ...
  if (obj->labeledgealigned) p.y -= lp->pos.y;
...
static char *strdup_and_subst_obj0(char *str, void *obj, int escBackslash) {
  ...
  char *g_str = "\\G", *n_str = "\\N", *e_str = "\\E", *h_str = "\\H",
       *t_str = "\\T", *l_str = "\\L";
  int g_len = 2, n_len = 2, e_len = 2, h_len = 2, t_len = 2, l_len = 2,
      tp_len = 0, hp_len = 0;
  int newlen = 0;
  int isEdge = 0;
  textlabel_t *tl;
  ...
  switch (agobjkind(obj)) {
    ...
    case AGEDGE:
      ...
      if ((tp_str = pt.name)) tp_len = strlen(tp_str);
      h_str = agnameof(aghead(((edge_t *)obj)));
      h_len = strlen(h_str);
      pt = ED_head_port((edge_t *)obj);
      if ((hp_str = pt.name)) hp_len = strlen(hp_str);
      h_len = strlen(h_str);
      ...
      if (agisdirected(agroot(agraphof(agtail(((edge_t *)obj))))))
        ...
        e_str = "--";
      e_len = t_len + (tp_len ? tp_len + 1 : 0) + 2 + h_len +
              (hp_len ? hp_len + 1 : 0);
      break;
  }
