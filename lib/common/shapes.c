...
static poly_desc_t star_gen = {star_size, star_vertices, };
...
static shape_desc Shapes[] =
    {/* first entry is default for no such shape */
     {"box", &poly_fns, &p_box}, {"polygon", &poly_fns, &p_polygon},
     {"ellipse", &poly_fns, &p_ellipse}, {"oval", &poly_fns, &p_ellipse},
     {"circle", &poly_fns, &p_circle}, {"point", &point_fns, &p_circle},
     {"egg", &poly_fns, &p_egg}, {"triangle", &poly_fns, &p_triangle},
     {"none", &poly_fns, &p_plaintext}, {"plaintext", &poly_fns, &p_plaintext},
     {"plain", &poly_fns, &p_plain}, {"diamond", &poly_fns, &p_diamond},
     {"trapezium", &poly_fns, &p_trapezium},
     {"parallelogram", &poly_fns, &p_parallelogram},
     {"house", &poly_fns, &p_house}, {"pentagon", &poly_fns, &p_pentagon},
     {"hexagon", &poly_fns, &p_hexagon}, {"septagon", &poly_fns, &p_septagon},
     {"octagon", &poly_fns, &p_octagon}, {"note", &poly_fns, &p_note},
     {"tab", &poly_fns, &p_tab}, {"folder", &poly_fns, &p_folder},
     {"box3d", &poly_fns, &p_box3d}, {"component", &poly_fns, &p_component},
     {"rect", &poly_fns, &p_box}, {"rectangle", &poly_fns, &p_box},
     ...
     {"Mdiamond", &poly_fns, &p_Mdiamond}, {"Msquare", &poly_fns, &p_Msquare},
     {"Mcircle", &poly_fns, &p_Mcircle},
     /* biological circuit shapes, as specified by SBOLv*/
     /** gene expression symbols **/
     {"promoter", &poly_fns, &p_promoter}, {"cds", &poly_fns, &p_cds},
     {"terminator", &poly_fns, &p_terminator}, {"utr", &poly_fns, &p_utr},
     {"insulator", &poly_fns, &p_insulator},
     {"ribosite", &poly_fns, &p_ribosite}, {"rnastab", &poly_fns, &p_rnastab},
     ...
     {"rpromoter", &poly_fns, &p_rpromoter}, {"larrow", &poly_fns, &p_larrow},
     {"rarrow", &poly_fns, &p_rarrow}, {"lpromoter", &poly_fns, &p_lpromoter},
     /*  *** shapes other than polygons  *** */
     {"record", &record_fns, NULL}, {"Mrecord", &record_fns, NULL},
     {"epsf", &epsf_fns, NULL}, {"star", &star_fns, &p_star},
     {NULL, NULL, NULL}};
...
static int stylenode(GVJ_t *job, node_t *n) {
  ...
  if ((pstyle = checkStyle(n, &istyle))) gvrender_set_style(job, pstyle);
...
void round_corners(GVJ_t *job, pointf *AF, int sides, int style, int filled) {
  ...
  for (seg = 0; seg < sides; seg++) {
    ...
    if (mode == ROUNDED) B[i++] = interpolate_pointf(1.0 - RBCURVE * t, p0, p1);
  }
  B[i++] = B[0];
  B[i++] = B[1];
  B[i++] = B[2];
  ...
  switch (mode) {
    ...
    case DOGEAR:
      ...
      for (seg = 1; seg < sides; seg++) D[seg] = AF[seg];
      D[0] = B[3 * (sides - 1) + 4];
    ...
    case TAB:
      ...
      for (seg = 4; seg < sides + 2; seg++) D[seg] = AF[seg - 2];
    ...
    case FOLDER:
      ...
      D[4].x = B[3].x;
      D[4].y = B[3].y;
      for (seg = 4; seg < sides + 3; seg++) D[seg] = AF[seg - 3];
    ...
    case PROMOTER:
      ...
      // the thickness is subituted with (AF[0].x - AF[1].x)/8 to make it
      // scalable in the y with label length
      D = N_NEW(sides + 5, pointf);
      D[0].x = AF[1].x + (AF[0].x - AF[1].x) / 2 +
               (AF[0].x - AF[1].x) / 8;  // x_center + width
      D[0].y = AF[2].y + (AF[1].y - AF[2].y) / 2 +
               (B[3].y - B[4].y) * 3 / 2;  // D[4].y + width
      D[1].x = AF[1].x + (AF[0].x - AF[1].x) / 2 -
               (AF[0].x - AF[1].x) / 4;  // x_center - 2*width
      D[1].y = D[0].y;
      D[2].x = D[1].x;
      ...
      D[4].x = D[3].x;
      D[4].y = AF[2].y + (AF[1].y - AF[2].y) / 2 +
               (B[3].y - B[4].y);  // highest cds point
      D[5].x = D[0].x;
    ...
    case TERMINATOR:
      ...
      */
      ...
      D[0].x = AF[1].x + (AF[0].x - AF[1].x) / 2 +
               (B[2].x - B[3].x) / 4;  // x_center + width/2
      ...
      D[4].x = AF[1].x + (AF[0].x - AF[1].x) / 2 -
               (B[2].x - B[3].x) * 3 / 4;  // D[3].y mirrowed across the center
      D[4].y = D[3].y;
      D[5].x = D[4].x;
      D[5].y = D[2].y;
      D[6].x = AF[1].x + (AF[0].x - AF[1].x) / 2 -
               (B[2].x - B[3].x) / 4;  // D[1].x mirrowed across the center
      D[6].y = D[1].y;
      D[7].x = D[6].x;
      D[7].y = D[0].y;
    ...
    case UTR:
      ...
      D = N_NEW(sides + 2, pointf);
      D[0].x = AF[1].x + (AF[0].x - AF[1].x) / 2 +
               (B[2].x - B[3].x) * 3 / 4;  // x_center+width
      D[0].y = AF[2].y + (AF[1].y - AF[2].y) / 2;  // y_center
      D[1].x = D[0].x;
      D[1].y = D[0].y + (B[3].y - B[4].y) / 4;  // D[0].y+width/2
      D[2].x = AF[1].x + (AF[0].x - AF[1].x) / 2 +
               (B[2].x - B[3].x) / 4;  // x_center+width/2
      D[2].y = D[1].y + (B[3].y - B[4].y) / 2;  // D[1].y+width
      D[3].x = AF[1].x + (AF[0].x - AF[1].x) / 2 -
               (B[2].x - B[3].x) / 4;  // D[2].x mirrowed across the center
      D[3].y = D[2].y;
    ...
    case PRIMERSITE:
      ...
      */
      ...
      D = N_NEW(sides + 1, pointf);
      D[0].x = AF[1].x + (AF[0].x - AF[1].x) / 2 +
               (B[2].x - B[3].x);  // x_center + width*2
      D[0].y = AF[2].y + (AF[1].y - AF[2].y) / 2 +
               (B[3].y - B[4].y) / 4;  // y_center + 1/2 width
      D[1].x = D[0].x - (B[2].x - B[3].x);  // x_center
      D[1].y = D[0].y + (B[3].y - B[4].y);
      D[2].x = D[1].x;
      D[2].y = D[0].y + (B[3].y - B[4].y) / 2;
      D[3].x = AF[1].x + (AF[0].x - AF[1].x) / 2 -
               (AF[0].x - AF[1].x) / 4;  // x_center - 2*(scalable width)
      D[3].y = D[2].y;
      D[4].x = D[3].x;
      D[4].y = D[0].y;
      gvrender_polygon(job, D, sides + 1, filled);
    ...
    case RESTRICTIONSITE:
      ...
      */
      ...
      // scalable in the y with label length
      D = N_NEW(sides + 4, pointf);
      D[0].x = AF[1].x + (AF[0].x - AF[1].x) / 2 + (AF[0].x - AF[1].x) / 8 +
    ...
    case FIVEPOVERHANG:
      ...
      */
      ...
      D[0].y = AF[2].y + (AF[1].y - AF[2].y) / 2 +
               (B[3].y - B[4].y) / 8;  // y_center + 1/4 width
      ...
      D[0].y = AF[2].y + (AF[1].y - AF[2].y) / 2 -
               (B[3].y - B[4].y) * 5 / 8;  // y_center - 5/4 width
    ...
    case NOVERHANG:
      ...
      */
      ...
      D = N_NEW(sides, pointf);
      D[0].x = AF[1].x + (AF[0].x - AF[1].x) / 2 -
               (B[2].x - B[3].x) * 9 / 8;  // x_center - 2*width - 1/4*width
      D[0].y = AF[2].y + (AF[1].y - AF[2].y) / 2 +
               (B[3].y - B[4].y) / 8;  // y_center + 1/4 width
      D[1].x = D[0].x + (B[2].x - B[3].x);
      D[1].y = D[0].y;
      ...
      D = N_NEW(sides, pointf);
      D[0].x = AF[1].x + (AF[0].x - AF[1].x) / 2 +
               (B[2].x - B[3].x) / 8;  // x_center + 1/4*width
      D[0].y = AF[2].y + (AF[1].y - AF[2].y) / 2 -
               (B[3].y - B[4].y) * 5 / 8;  // y_center - width - 1/4 width
      D[1].x = D[0].x + (B[2].x - B[3].x);
      D[1].y = D[0].y;
      ...
      D[0].x = AF[1].x + (AF[0].x - AF[1].x) / 2 +
               (B[2].x - B[3].x) / 8;  // x_center + 1/4*width
      D[0].y = AF[2].y + (AF[1].y - AF[2].y) / 2 +
               (B[3].y - B[4].y) / 8;  // y_center - width - 1/4 width
      D[1].x = D[0].x + (B[2].x - B[3].x);
      D[1].y = D[0].y;
      D[2].x = D[1].x;
      D[2].y = D[1].y + (B[3].y - B[4].y) / 2;
    ...
    case INSULATOR:
      ...
      D = N_NEW(sides, pointf);
      D[0].x = AF[1].x + (AF[0].x - AF[1].x) / 2 +
               (B[2].x - B[3].x) / 2;  // x_center+width
      D[0].y = AF[2].y + (AF[1].y - AF[2].y) / 2 +
               (B[2].x - B[3].x) / 2;  // y_center
      D[1].x = D[0].x;
      D[1].y = AF[2].y + (AF[1].y - AF[2].y) / 2 -
               (B[2].x - B[3].x) / 2;  // D[0].y- width
      D[2].x = AF[1].x + (AF[0].x - AF[1].x) / 2 -
               (B[2].x - B[3].x) / 2;  // x_center-width
      D[2].y = D[1].y;
      D[3].x = D[2].x;
      ...
      /*outer square line*/
      C[0].x = AF[1].x + (AF[0].x - AF[1].x) / 2 +
               (B[2].x - B[3].x) * 3 / 4;  // x_center+1.5*width
      C[0].y = AF[2].y + (AF[1].y - AF[2].y) / 2 +
               (B[2].x - B[3].x) * 3 / 4;  // y_center
      C[1].x = C[0].x;
      C[1].y = AF[2].y + (AF[1].y - AF[2].y) / 2 -
               (B[2].x - B[3].x) * 3 / 4;  // y_center- 1.5*width
      C[2].x = AF[1].x + (AF[0].x - AF[1].x) / 2 -
               (B[2].x - B[3].x) * 3 / 4;  // x_center-1.5*width
      C[2].y = C[1].y;
      C[3].x = C[2].x;
      C[3].y = C[0].y;
    ...
    case RIBOSITE:
      ...
      D[0].x = AF[1].x + (AF[0].x - AF[1].x) / 2 +
               (B[2].x - B[3].x) /
                   4;  // x_center+widtht/2 , lower right corner of the x
      D[0].y = AF[2].y + (AF[1].y - AF[2].y) / 2 +
               (B[3].y - B[4].y) / 2;  // y_center + width
      D[1].x = D[0].x;
      D[1].y = D[0].y + (B[3].y - B[4].y) / 8;  // D[0].y +width/4
      D[2].x = D[0].x -
               (B[2].x - B[3].x) / 8;  // D[0].x- width/4 //right nook of the x
      D[2].y =
          D[1].y + (B[3].y - B[4].y) / 8;  // D[0].y+width/2 or D[1].y+width/4
      D[3].x = D[0].x;
    ...
    case RNASTAB:
      ...
      D[0].x = AF[1].x + (AF[0].x - AF[1].x) / 2 +
               (B[2].x - B[3].x) /
                   8;  // x_center+widtht/8 , lower right corner of the hexagon
      D[0].y = AF[2].y + (AF[1].y - AF[2].y) / 2 +
               (B[3].y - B[4].y) / 2;  // y_center + width
...
static void poly_init(node_t *n) {
  ...
  if (ND_shape(n)->usershape) {
    ...
    if (streq(ND_shape(n)->name, "custom")) {
      ...
      if ((imagesize.x == -1) && (imagesize.y == -1)) {
        agerr(AGWARN, "No or improper shapefile=\"%s\" for node \"%s\"\n",
    ...
    imagesize = gvusershape_size(agraphof(n), sfile);
    if ((imagesize.x == -1) && (imagesize.y == -1)) {
      agerr(AGWARN, "No or improper image=\"%s\" for node \"%s\"\n",
            (sfile ? sfile : "<nil>"), agnameof(n));
      imagesize.x = imagesize.y = 0;
    } else {
      GD_has_images(agraphof(n)) = TRUE;
  ...
  isBox = (sides == 4 && (ROUND(orientation) % 90) == 0 && distortion == 0. &&
           skew == 0.);
  if (isBox) {
...
static boolean poly_inside(inside_t *inside_context, pointf p) {
  ...
  for (j = 1; j < sides; j++) {/* iterate over remaining segments */
    ...
    if (!(same_side(P, O, vertex[i + outp],
                    vertex[i1 + outp]))) {/* false if outside any other
                                             segment's face */
      last = i;
      return FALSE;
    }
...
static pointf compassPoint(inside_t *ictxt, double y, double x) {
  ...
  if (rd) curve[0] = ccwrotatepf(curve[0], 90 * rd);
  return curve[0];
}
 ...
 *
 */
static int compassPort(node_t *n, boxf *bp, port *pp, char *compass, int sides,
                       inside_t *ictxt) {
  boxf b;
  pointf p, ctr;
  int rv = 0;
...
static void poly_gencode(GVJ_t *job, node_t *n) {
  ...
  for (j = 0; j < peripheries; j++) {
    ...
    if (sides <= 2) {
      if ((style & WEDGED) && (j == 0) && multicolor(fillcolor)) {
        ...
        if (rv > 1) agerr(AGPREV, "in node %s\n", agnameof(n));
        filled = 0;
      }
  ...
  if (ND_shape(n)->usershape) {
    ...
    if (streq(name, "custom")) {
      if ((name = agget(n, "shapefile")) && name[0]) usershape_p = TRUE;
    } else
      usershape_p = TRUE;
...
static boolean point_inside(inside_t *inside_context, pointf p) {
  ...
  /* inside bounding box? */
  if ((fabs(P.x) > radius) || (fabs(P.y) > radius)) return FALSE;
...
static void point_gencode(GVJ_t *job, node_t *n) {
  ...
  if (peripheries == 0) {
    peripheries = 1;
    if (color[0]) gvrender_set_pencolor(job, color);
  }
...
static field_t *parse_reclbl(node_t *n, int LR, int flag, char *text) {
  ...
  while (wflag) {
    ...
    switch (*reclblp) {
      case '<':
        if (mode & (HASTABLE | HASPORT)) return parse_error(rv, tmpport);
      ...
      case '>':
        if (lbl->html) goto dotext;
        if (!(mode & INPORT)) return parse_error(rv, tmpport);
        if (psp > text + 1 && psp - 1 != hspsp && *(psp - 1) == ' ') psp--;
        *psp = '\000';
        tmpport = strdup(text);
        mode &= ~INPORT;
        reclblp++;
      ...
      case '{':
        reclblp++;
        if (mode != 0 || !*reclblp) return parse_error(rv, tmpport);
        mode = HASTABLE;
      ...
      case '\000':
        if ((!*reclblp && !flag) || (mode & INPORT))
          return parse_error(rv, tmpport);
        if (!(mode & HASTABLE)) fp = rv->fld[fi++] = NEW(field_t);
        if (tmpport) {
          fp->id = tmpport;
          tmpport = NULL;
        }
        if (!(mode & (HASTEXT | HASTABLE))) mode |= HASTEXT, *tsp++ = ' ';
        if (mode & HASTEXT) {
          if (tsp > text + 1 && tsp - 1 != hstsp && *(tsp - 1) == ' ') tsp--;
          *tsp = '\000';
          fp->lp = make_label((void *)n, strdup(text),
                              (lbl->html ? LT_HTML : LT_NONE), lbl->fontsize,
                              lbl->fontname, lbl->fontcolor);
          fp->LR = TRUE;
          hstsp = tsp = text;
        }
        if (*reclblp) {
      ...
      dotext:
        ...
        if (mode & INTEXT) {
          if (!(*reclblp == ' ' && !ishardspace && *(tsp - 1) == ' ' &&
                !lbl->html))
            *tsp++ = *reclblp;
          if (ishardspace) hstsp = tsp - 1;
        } else if (mode & INPORT) {
          if (!(*reclblp == ' ' && !ishardspace &&
                (psp == text || *(psp - 1) == ' ')))
            *psp++ = *reclblp;
          if (ishardspace) hspsp = psp - 1;
        }
        reclblp++;
        while (*reclblp & 128) *tsp++ = *reclblp++;
        break;
    }
  }
  rv->n_flds = fi;
...
static void record_init(node_t *n) {
  ...
  ul = pointfof(-sz.x / 2.,
                sz.y / 2.); /* FIXME - is this still true:    suspected to
                               introduce ronding error - see Kluge below */
  pos_reclbl(info, ul, sides);
  ND_width(n) = PS2INCH(info->size.x);
  ND_height(n) =
      PS2INCH(info->size.y + 1); /* Kluge!!  +1 to fix rounding diff between
...
shape_desc *bind_shape(char *name, node_t *np) {
  ...
  if (str && !streq(name, "epsf")) name = "custom";
  if (!streq(name, "custom")) {
