...
static node_t *incident(edge_t *e) {
  if (ND_mark(agtail(e))) {
    if (ND_mark(aghead(e)) == FALSE) return agtail(e);
  } else {
    if (ND_mark(aghead(e))) return aghead(e);
  }
  return NULL;
}
  ...
  while (tight_tree() < N_nodes) {
    ...
    for (n = GD_nlist(G); n; n = ND_next(n)) {
      for (i = 0; (f = ND_out(n).list[i]); i++) {
        if ((TREE_EDGE(f) == FALSE) && incident(f) &&
            ((e == NULL) || (SLACK(f) < SLACK(e))))
          e = f;
      }
    }
    if (e) {
...
void tchk(void) {
  ...
  for (n = agfstnode(G); n; n = agnxtnode(G, n)) {
    ...
    for (i = 0; (e = ND_tree_out(n).list[i]); i++) {
      e_cnt++;
      if (SLACK(e) > 0) fprintf(stderr, "not a tight tree %p", e);
    }
  }
  ...
  for (i = 0; (e = ND_out(n).list[i]); i++) {
    ...
    if (ND_onstack(w)) {
      ...
      fprintf(stderr, "cycle: last edge %lx %s(%lx) %s(%lx)\n",
              (unsigned long int)e, agnameof(n), (unsigned long int)n,
...
void check_cycles(graph_t *g) {
  ...
  for (n = GD_nlist(g); n; n = ND_next(n)) checkdfs(g, n);
}
#endif /* DEBUG */
