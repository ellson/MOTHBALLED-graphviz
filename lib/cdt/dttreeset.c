...
#else
static Dtlink_t* treebalance(list, size) Dtlink_t* list;
int size;
#endif
{
  int n;
  ...
  for (l = list, n = size / 2 - 1; n > 0; n -= 1) l = l->right;
...
#else
int dttreeset(dt, minp, balance) Dt_t* dt;
int minp;
int balance;
#endif
{
  ...
  if (dt->meth->type != DT_OSET) return -1;

  size = dtsize(dt);

  if (minp < 0) {
    for (minp = 0; minp < DT_MINP; ++minp)
      if ((1 << minp) >= size) break;
    if (minp <= DT_MINP - 4) /* use log(size) + 4 */
      minp += 4;
  }

  if ((dt->data->minp = minp + (minp % 2)) > DT_MINP) dt->data->minp = DT_MINP;

  if (balance) dt->data->here = treebalance(dtflatten(dt), size);
