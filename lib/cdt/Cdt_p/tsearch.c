...
static Treedisc_t Treedisc = {
    {sizeof(Dtlink_t), -1, /* object is key		*/
     0,                NIL(Dtmake_f),   NIL(Dtfree_f), treecompare,
...
#endif
{
  ...
  if (!(o = (Tree_t*)dtmatch(dt, key))) {
    if (!(o = (Tree_t*)malloc(sizeof(Tree_t)))) return NIL(Void_t*);
    o->key = (Void_t*)key;
    dtinsert(dt, o);
  }
...
#endif
{
  ...
  if (!rootp || !(dt = *((Dt_t**)rootp))) return NIL(Void_t*);
  Treedisc.comparf = comparf;
...
#endif
{
  if (!obj->lchild && !obj->rchild)
    ...
    if (obj->lchild) _twalk((Tree_t*)obj->lchild, action, level + 1);
    (*action)((Void_t*)obj, postorder, level);
    if (obj->rchild) _twalk((Tree_t*)obj->rchild, action, level + 1);
...
#endif
{
  ...
  if (root && (o = (Tree_t*)dtfinger((Dt_t*)root))) _twalk(o, action, 0);
}
