...
#define DTDISC(dc, ky, sz, lk, mkf, frf, cmpf, hshf, memf, evf)      \
  ((dc)->key = (ky), (dc)->size = (sz), (dc)->link = (lk),           \
   ...
   (dc)->hashf = (hshf), (dc)->memoryf = (memf), (dc)->eventf = (evf))
...
#define _DTMTCH(dt, key, action)                                        \
  do {                                                                  \
    ...
    Void_t* _o, *_k, *_key;                                             \
    Dtdisc_t* _dc;                                                      \
    int _ky, _sz, _lk, _cmp;                                            \
    Dtcompar_f _cmpf;                                                   \
    _dc = (dt)->disc;                                                   \
    _DTDSC(_dc, _ky, _sz, _lk, _cmpf);                                  \
    _key = (key);                                                       \
    for (_e = (dt)->data->here; _e;                                     \
         _e = _cmp < 0 ? _e->hl._left : _e->right) {                    \
      _o = _DTOBJ(_e, _lk);                                             \
      _k = _DTKEY(_o, _ky, _sz);                                        \
      if ((_cmp = _DTCMP((dt), _key, _k, _dc, _cmpf, _sz)) == 0) break; \
    }                                                                   \
...
#define dtfinger(d) \
  (_DT(d)->data->here ? dtobj((d), _DT(d)->data->here) : (Void_t*)(0))
