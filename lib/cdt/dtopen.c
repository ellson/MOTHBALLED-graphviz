...
#endif
{
  ...
  if (!disc || !meth) return NIL(Dt_t*);

  /* allocate space for dictionary */
  if (!(dt = (Dt_t*)malloc(sizeof(Dt_t)))) return NIL(Dt_t*);
  ...
  if (disc->eventf) {/* if shared/persistent dictionary, get existing data */
    ...
    if ((e = (*disc->eventf)(dt, DT_OPEN, (Void_t*)(&data), disc)) < 0)
      ...
      if (!disc->memoryf) goto err_open;

      free((Void_t*)dt);
      if (!(dt = (*disc->memoryf)(0, 0, sizeof(Dt_t), disc))) return NIL(Dt_t*);
      dt->searchf = NIL(Dtsearch_f);
      dt->meth = NIL(Dtmethod_t*);
      dt->disc = NIL(Dtdisc_t*);
      dtdisc(dt, disc, 0);
  ...
  if (!(data = (Dtdata_t*)(dt->memoryf)(dt, NIL(Void_t*), sizeof(Dtdata_t),
                                        disc))) {
  err_open:
    free((Void_t*)dt);
    return NIL(Dt_t*);
  ...
  if (disc->eventf) (*disc->eventf)(dt, DT_ENDOPEN, (Void_t*)dt, disc);
