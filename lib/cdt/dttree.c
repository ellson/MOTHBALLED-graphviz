...
#endif
{
  ...
  root = dt->data->here;
  if (!obj) {
    if (!root || !(type & (DT_CLEAR | DT_FIRST | DT_LAST))) return NIL(Void_t*);

    if (type & DT_CLEAR) /* delete all objects */
    {
      if (disc->freef || disc->link < 0) {
        do {
          while ((t = root->left)) RROTATE(root, t);
          t = root->right;
          if (disc->freef) (*disc->freef)(dt, _DTOBJ(root, lk), disc);
          if (disc->link < 0) (*dt->memoryf)(dt, (Void_t*)root, 0, disc);
        } while ((root = t));
      }
    ...
    {
      if (type & DT_LAST) {
        while ((t = root->right)) LROTATE(root, t);
      } else /* type&DT_FIRST */
      {
        while ((t = root->left)) RROTATE(root, t);
      }
  ...
  if (type & (DT_MATCH | DT_SEARCH | DT_INSERT | DT_ATTACH)) {
    ...
    key = _DTKEY(obj, ky, sz);
  do_search:
    if (dt->meth->type == DT_OSET && (minp = dt->data->minp) != 0 &&
        (type & (DT_MATCH | DT_SEARCH))) {/* simple search, note that minp
                                             should be even */
      for (t = root, n = 0; n < minp; ++n) {
        ...
        if ((cmp = _DTCMP(dt, key, k, disc, cmpf, sz)) == 0)
          ...
          turn[n] = cmp;
          if (!(t = cmp < 0 ? t->left : t->right)) return NIL(Void_t*);
        }
      }
  ...
  if (root) {/* found it, now isolate it */
    ...
    has_root:
      ...
      if ((dt->meth->type & DT_OBAG) && (type & (DT_SEARCH | DT_MATCH))) {
        key = _DTOBJ(root, lk);
        key = _DTKEY(key, ky, sz);
        while ((t = root->left)) {/* find max of left subtree */
          while ((r = t->right)) LROTATE(t, r);
          root->left = t;
    ...
    dt_prev:
      if ((root = link.right)) {
        ...
        goto no_root;
                       ...
                       DT_DETACH)) {/* taking an object out of the dictionary */
    dt_delete:
      obj = _DTOBJ(root, lk);
      if (disc->freef && (type & DT_DELETE)) (*disc->freef)(dt, obj, disc);
      if (disc->link < 0) (*dt->memoryf)(dt, (Void_t*)root, 0, disc);
      if ((dt->data->size -= 1) < 0) dt->data->size = -1;
      goto no_root;
    ...
    } else if (type & DT_RENEW) /* a duplicate */
    {
      if (dt->meth->type & DT_OSET) {
        ...
        if (disc->link < 0) (*dt->memoryf)(dt, (Void_t*)me, 0, disc);
      } else {
    ...
    dt_insert:
      ...
      if (obj) {
        if (lk >= 0)
          ...
          root = (Dtlink_t*)(*dt->memoryf)(dt, NIL(Void_t*), sizeof(Dthold_t),
                                           disc);
          if (root)
            ((Dthold_t*)root)->obj = obj;
          else if (disc->makef && disc->freef && (type & DT_INSERT))
            (*disc->freef)(dt, obj, disc);
        }
      }
      if (root) {
        if (dt->data->size >= 0) dt->data->size += 1;
        goto has_root;
      } else
        goto no_root;
    } else if (type & DT_RENEW) {
