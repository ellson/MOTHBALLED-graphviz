...
static alphacmp(Dt_t* dt, reg Void_t* arg_s1, reg Void_t* arg_s2,
                Dtdisc_t* disc)
#else
static alphacmp(dt, arg_s1, arg_s2, disc) Dt_t* dt;
reg Void_t* arg_s1;
reg Void_t* arg_s2;
Dtdisc_t* disc;
#endif
...
#else
static Void_t* newstring(dt, s, disc) Dt_t* dt;
reg Void_t* s;
Dtdisc_t* disc;
#endif
{
  ...
  if (!(news = malloc(n))) return NIL(Void_t*);
  memcpy(news, s, n);
...
main() {
  ...
  if (!(dt = dtopen(&Disc, Dtset))) return -1;
  ...
  */
  ...
  if (!dtdisc(dt, &Disc, DT_SAMECMP | DT_SAMEHASH)) return -1;

  /* now order strings by switching to Dttree */
  if (!dtmethod(dt, Dtorder)) return -1;
