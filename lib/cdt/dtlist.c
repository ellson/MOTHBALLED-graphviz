...
#endif
{
    ...
    if (type & (DT_LAST | DT_FIRST)) {
      ...
      if (disc->freef || disc->link < 0) {
        for (r = dt->data->head; r; r = t) {
          t = r->right;
          if (disc->freef) (*disc->freef)(dt, _DTOBJ(r, lk), disc);
  ...
  if (type & (DT_INSERT | DT_ATTACH)) {
    ...
    if (dt->data->type & DT_DEQUE) {
      ...
      if (type & DT_APPEND) {
        if (!(t = dt->data->here) || !t->right) goto dt_queue;
        r->right = t->right;
        r->right->left = r;
        r->left = t;
        r->left->right = r;
      } else {
        if (!(t = dt->data->here) || t == dt->data->head) goto dt_stack;
        r->left = t->left;
        r->left->right = r;
        r->right = t;
        r->right->left = r;
  ...
  if (!r) return NIL(Void_t*);
  dt->type |= DT_FOUND;

  if (type & (DT_DELETE | DT_DETACH)) {
  dt_delete:
    if (r->right) r->right->left = r->left;
    if (r == (t = dt->data->head)) {
      dt->data->head = r->right;
      if (dt->data->head) dt->data->head->left = t->left;
    } else {
      r->left->right = r->right;
      if (r == t->left) t->left = r->left;
    }
