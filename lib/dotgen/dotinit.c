...
static void dot_init_subg(graph_t* g, graph_t* droot) {
  ...
  if (g == droot) GD_dotroot(agroot(g)) = droot;
...
static void dot_init_node(node_t* n) {
  agbindrec(n, "Agnodeinfo_t", sizeof(Agnodeinfo_t), TRUE);  // graph custom
                                                             // data
  common_init_node(n);
...
static void dot_init_edge(edge_t* e) {
  ...
  agbindrec(e, "Agedgeinfo_t", sizeof(Agedgeinfo_t), TRUE);  // graph custom
                                                             // data
  common_init_edge(e);
...
static void dot_cleanup_graph(graph_t* g) {
  ...
  free_list(GD_comp(g));
  if (GD_rank(g)) {
    for (i = GD_minrank(g); i <= GD_maxrank(g); i++) free(GD_rank(g)[i].av);
    if (GD_minrank(g) == -1)
      free(GD_rank(g) - 1);
    else
      free(GD_rank(g));
...
static void dotLayout(Agraph_t* g) {
  ...
  do {
    ...
    if (aspect.badGraph) {
      agerr(AGWARN,
            "dot does not support the aspect attribute for disconnected graphs "
            "or graphs with clusters\n");
      asp = NULL;
      aspect.nextIter = 0;
    }
  ...
  if (GD_flags(g) & NEW_RANK) removeFill(g);
  dot_sameports(g);
  dot_splines(g);
 ...
 */
static void copyClusterInfo(int ncc, Agraph_t** ccs, Agraph_t* root) {
  int j, i, nclust = 0;
  Agraph_t* sg;
  Agraph_t* cg;

  for (i = 0; i < ncc; i++) nclust += GD_n_cluster(ccs[i]);
...
Agraph_t* dot_root(void* p) { return GD_dotroot(agroot(p)); }
