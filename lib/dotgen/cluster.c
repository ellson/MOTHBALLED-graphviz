...
static void map_path(node_t *from, node_t *to, edge_t *orig, edge_t *ve,
                     int type) {
  int r;
  node_t *u, *v;
  edge_t *e;
  ...
  if ((agtail(ve) == from) && (aghead(ve) == to)) return;
  ...
  if (ED_count(ve) > 1) {
    ...
    if (ND_rank(to) - ND_rank(from) > 1) {
      ...
      if (agtail(ve) != from) {
        ...
        e = ve;
      while (ND_rank(aghead(e)) != ND_rank(to)) e = ND_out(aghead(e)).list[0];
      if (aghead(e) != to) {
        ve = e;
...
static void make_interclust_chain(graph_t *g, node_t *from, node_t *to,
                                  edge_t *orig) {
  int newtype;
  node_t *u, *v;
  ...
  for (n = agfstnode(subg); n; n = agnxtnode(subg, n)) {
    ...
    for (e = agfstedge(g, n); e; e = next) {
      ...
      /* backward edges */
        ...
        /*
        I think that make_interclust_chain should create call other_edge(e)
        anyway
...
static void merge_ranks(graph_t *subg) {
  ...
  root = dot_root(subg);
  if (GD_minrank(subg) > 0) GD_rank(root)[GD_minrank(subg) - 1].valid = FALSE;
  for (r = GD_minrank(subg); r <= GD_maxrank(subg); r++) {
    ...
    for (i = 0; i < GD_rank(subg)[r].n; i++) {
      ...
      if (ND_node_type(v) == VIRTUAL) v->root = agroot(root);
  ...
  for (c = 1; c <= GD_n_cluster(g); c++) {
    ...
    for (n = agfstnode(clust); n; n = nn) {
      ...
      if (ND_ranktype(n) != NORMAL) {
        agerr(AGWARN, "%s was already in a rankset, deleted from cluster %s\n",
