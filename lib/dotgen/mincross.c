...
static int getComp(graph_t *g, node_t *n, graph_t *comp, int *indices) {
  ...
  for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
    ...
    if (!ND_x(aghead(e))) backedge += getComp(g, aghead(e), comp, indices);
  }
  for (e = agfstin(g, n); e; e = agnxtin(g, e)) {
    if (isBackedge(e)) backedge++;
    if (!ND_x(agtail(e))) backedge += getComp(g, agtail(e), comp, indices);
  }
  return backedge;
}
...
#ifdef DEBUG
    for (c = 1; c <= GD_n_cluster(g); c++) check_vlists(GD_clust(g)[c]);
#endif
  }
  cleanup2(g, nc);
}
...
static void ordered_edges(graph_t *g) {
  ...
  if (!G_ordering && !N_ordering) return;
...
static int in_cross(node_t *v, node_t *w) {
  ...
  for (e2 = ND_in(w).list; *e2; e2++) {
    ...
    for (e1 = ND_in(v).list; *e1; e1++) {
      ...
      if ((t > 0) ||
          ((t == 0) && (ED_tail_port(*e1).p.x > ED_tail_port(*e2).p.x)))
...
static int out_cross(node_t *v, node_t *w) {
  ...
  for (e2 = ND_out(w).list; *e2; e2++) {
    ...
    for (e1 = ND_out(v).list; *e1; e1++) {
      ...
      if ((t > 0) ||
          ((t == 0) && ((ED_head_port(*e1)).p.x > (ED_head_port(*e2)).p.x)))
        cross += ((ED_xpenalty(*e1)) * cnt);
    }
  }
  return cross;
...
static void balanceNodes(graph_t *g, int r, node_t *v, node_t *w) {
  ...
  if (ND_node_type(v) == ND_node_type(w)) return;
  ...
  for (i = 0; i < GD_rank(g)[r].n; i++) {
    if (GD_rank(g)[r].v[i] == s) sepIndex = i;
  }
...
static int mincross(graph_t *g, int startpass, int endpass, int doBalance) {
  ...
  for (pass = startpass; pass <= endpass; pass++) {
    if (pass <= 1) {
      ...
      if (g == dot_root(g)) build_ranks(g, pass);
  ...
  if (doBalance) {
    for (iter = 0; iter < maxthispass; iter++) balance(g);
  }
...
static void cleanup2(graph_t *g, int nc) {
  ...
  for (c = 1; c <= GD_n_cluster(g); c++) rec_reset_vlists(GD_clust(g)[c]);
...
static int is_a_vnode_of_an_edge_of(graph_t *g, node_t *v) {
  if ((ND_node_type(v) == VIRTUAL) && (ND_in(v).size == 1) &&
      (ND_out(v).size == 1)) {
...
void rec_save_vlists(graph_t *g) {
  ...
  save_vlist(g);
  for (c = 1; c <= GD_n_cluster(g); c++) rec_save_vlists(GD_clust(g)[c]);
}
 ...
 */
static Agraph_t *realFillRanks(Agraph_t *g, int rnks[], int rnks_sz,
                               Agraph_t *sg) {
  int i, c;
  Agedge_t *e;
  Agnode_t *n;
  ...
  if (dot_root(g) == g) return sg;
  memset(rnks, 0, sizeof(int) * rnks_sz);
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
    rnks[ND_rank(n)] = 1;
    for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
      for (i = ND_rank(n) + 1; i <= ND_rank(aghead(e)); i++) rnks[i] = 1;
    }
  }
...
static void init_mincross(graph_t *g) {
  ...
  mincross_options(g);
  if (GD_flags(g) & NEW_RANK) fillRanks(g);
  class2(g);
  decompose(g, 1);
  allocate_ranks(g);
  ordered_edges(g);
...
static void flat_search(graph_t *g, node_t *v) {
  ...
  if (ND_flat_out(v).list)
    for (i = 0; (e = ND_flat_out(v).list[i]); i++) {
      if (hascl && NOT(agcontains(g, agtail(e)) && agcontains(g, aghead(e))))
        continue;
      ...
      if (ND_onstack(aghead(e)) == TRUE) {
        ...
        if (ND_mark(aghead(e)) == FALSE) flat_search(g, aghead(e));
      }
    }
  ND_onstack(v) = FALSE;
}
...
void install_in_rank(graph_t *g, node_t *n) {
  ...
  if (ND_order(n) > GD_rank(Root)[r].an) {
    agerr(AGERR,
          "install_in_rank, line %d: ND_order(%s) [%d] > GD_rank(Root)[%d].an "
          "[%d]\n",
  ...
  if (GD_rank(g)[r].v + ND_order(n) > GD_rank(g)[r].av + GD_rank(Root)[r].an) {
    agerr(AGERR,
          ...
          __LINE__, r, agnameof(n), GD_rank(g)[r].v + ND_order(n), r, r,
...
void build_ranks(graph_t *g, int pass) {
...
#ifdef DEBUG
  {
    ...
    for (n = GD_nlist(g); n; n = ND_next(n)) {
      ...
      for (i = 0; (e = ND_in(n).list[i]); i++) assert(MARK(agtail(e)) == FALSE);
    }
  }
#endif
  ...
  for (i = GD_minrank(g); i <= GD_maxrank(g); i++) {
    ...
    if (GD_flip(g) && (GD_rank(g)[i].n > 0)) {
      ...
      n = GD_rank(g)[i].n - 1;
      ndiv2 = n / 2;
      for (j = 0; j <= ndiv2; j++) exchange(vlist[j], vlist[n - j]);
    }
  }

  if ((g == dot_root(g)) && ncross(g) > 0) transpose(g, FALSE);
  free_queue(q);
}
    ...
    if (pos) {
      ...
      for (i = 0; i < GD_rank(g)[r].n; i++) {
        ...
        if (ND_flat_out(v).list) {
          for (j = 0; (e = ND_flat_out(v).list[j]); j++) {
            if (((GD_flip(g) == FALSE) &&
                 (ND_order(aghead(e)) < ND_order(agtail(e)))) ||
...
static int ordercmpf(int *i0, int *i1) { return (*i0) - (*i1); }
...
static int flat_mval(node_t *n) {
  ...
  if (ND_flat_in(n).size > 0) {
    ...
    nn = agtail(fl[0]);
    for (i = 1; (e = fl[i]); i++)
      if (ND_order(agtail(e)) > ND_order(nn)) nn = agtail(e);
    if (ND_mval(nn) >= 0) {
    ...
    nn = aghead(fl[0]);
    for (i = 1; (e = fl[i]); i++)
      if (ND_order(aghead(e)) < ND_order(nn)) nn = aghead(e);
    if (ND_mval(nn) > 0) {
      ND_mval(n) = ND_mval(nn) - 1;
      return FALSE;
    }
...
static boolean medians(graph_t *g, int r0, int r1) {
  ...
  for (i = 0; i < GD_rank(g)[r0].n; i++) {
    n = v[i];
    if ((ND_out(n).size == 0) && (ND_in(n).size == 0)) hasfixed |= flat_mval(n);
  }
  return hasfixed;
}
