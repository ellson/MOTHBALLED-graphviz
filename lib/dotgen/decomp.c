...
static void begin_component(graph_t *g) { Last_node = GD_nlist(g) = NULL; }
 ...
 * Since are using a stack, we need to indicate nodes on the stack. Nodes
 * unprocessed
 * in this call to decompose will have mark < Cmark; processed nodes will have
 * mark=Cmark;
 * so we use mark = Cmark+1 to indicate nodes on the stack.
 */
static void search_component(stk_t *stk, graph_t *g, node_t *n) {
  int c, i;
  elist vec[4];
  node_t *other;
  edge_t *e;
    ...
    for (c = 3; c >= 0; c--) {
      if (vec[c].list) {
        for (i = vec[c].size - 1, ep = vec[c].list + i; i >= 0; i--, ep--) {
          e = *ep;
          if ((other = aghead(e)) == n) other = agtail(e);
