...
static void setbounds(node_t *v, int *bounds, int lpos, int rpos) {
  ...
  if (ND_node_type(v) == VIRTUAL) {
    ...
    if (ND_in(v).size == 0) {/* flat */
      ...
      findlr(aghead(ND_out(v).list[0]), aghead(ND_out(v).list[1]), &l, &r);
...
static void abomination(graph_t *g) {
  ...
  GD_rank(g) = rptr + 1;
  for (r = GD_maxrank(g); r >= 0; r--) GD_rank(g)[r] = GD_rank(g)[r - 1];
...
int flat_edges(graph_t *g) {
  ...
  for (n = GD_nlist(g); n; n = ND_next(n)) {
    ...
    for (j = 0; j < ND_other(n).size; j++) {
      e = ND_other(n).list[j];
      if (ND_rank(aghead(e)) == ND_rank(agtail(e))) checkFlatAdjacent(e);
    }
  }
    ...
    if (ND_flat_out(n).list) {
      for (i = 0; (e = ND_flat_out(n).list[i]); i++) {
        if (ED_label(e)) {
          if (ED_adjacent(e)) {
            if (GD_flip(g))
              ED_dist(e) = ED_label(e)->dimen.y;
            else
              ED_dist(e) = ED_label(e)->dimen.x;
          } else {
            reset = TRUE;
            flat_node(e);
          }
      ...
      for (j = 0; j < ND_other(n).size; j++) {
        ...
        if (ED_label(e)) {
          if (ED_adjacent(e)) {
            double lw;
            if (GD_flip(g))
              lw = ED_label(e)->dimen.y;
            else
              lw = ED_label(e)->dimen.x;
