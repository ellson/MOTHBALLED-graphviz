...
static void sameport(node_t *u, elist *l, double arr_len)
    /* make all edges in L share the same port on U. The port is placed on the
       node boundary and the average angle between the edges. FIXME: this
       assumes
       naively that the edges are straight lines, which is wrong if they are
       long.
    ...
    */
{
  ...
  r = MAX(ND_lw(u) + ND_rw(u),
          ND_ht(u) + GD_ranksep(agraphof(u))); /* far away */
  x2 = x * r + ND_coord(u).x;
  y2 = y * r + ND_coord(u).y;
  ...
  the general case that the user sets a small ranksep, and requires
  repositioning
  ...
  */
  ...
  if ((arr_prt.defined = arr_len && TRUE)) {
    ...
    arr_prt.order =
        (MC_SCALE * (ND_lw_i(u) + arr_prt.p.x)) / (ND_lw_i(u) + ND_rw_i(u));
  ...
  for (i = 0; i < l->size; i++) {
    ...
    for (; e; e = ED_to_virt(e)) {/* assign to all virt edges of e */
      for (f = e; f; f = ED_edge_type(f) == VIRTUAL &&
    ...
    for (; e; e = ED_to_virt(e)) {/* assign to all virt edges of e */
      if (aghead(e) == u)
        ED_head_port(e) = arr_port.defined && !eflag ? arr_prt : prt;
      if (agtail(e) == u)
        ED_tail_port(e) = arr_port.defined && !sflag ? arr_prt : prt;
    }
#endif
  }
