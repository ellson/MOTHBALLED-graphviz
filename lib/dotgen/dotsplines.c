...
int portcmp(port p0, port p1) {
  int rv;
  if (p1.defined == FALSE) return (p0.defined ? 1 : 0);
  if (p0.defined == FALSE) return -1;
  rv = p0.p.x - p1.p.x;
...
static void edge_normalize(graph_t* g) {
  ...
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
    for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
      if (sinfo.swapEnds(e) && ED_spl(e)) swap_spline(ED_spl(e));
    }
  }
}
...
static void _dot_splines(graph_t* g, int normalize) {
  ...
  for (i = 0; i < n_edges;) {
    ...
    for (cnt = 1; i < n_edges; cnt++, i++) {
      if (le0 != (le1 = getmainedge((e1 = edges[i])))) break;
    ...
    if (et == ET_CURVED) {
      ...
      for (b = 0; b < cnt; b++) {
        e = edges[ind + b];
        if (ED_label(e)) updateBB(g, ED_label(e));
      }
  ...
  if ((E_headlabel || E_taillabel) && (E_labelangle || E_labeldistance)) {
    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
      ...
      if (E_taillabel) {
        for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
          if (ED_tail_label(e)) {
            if (place_portlabel(e, FALSE)) updateBB(g, ED_tail_label(e));
          }
        }
      }
    }
 ...
 */
void dot_splines(graph_t* g) { _dot_splines(g, 1); }
...
static int edgecmp(edge_t** ptr0, edge_t** ptr1) {
  ...
  v0 =
      ABS((int)t0); /* ugly, but explicit as to how we avoid equality tests on
                       fp numbers */
  v1 = ABS((int)t1);
  ...
  if (AGSEQ(le0) != AGSEQ(le1)) return (AGSEQ(le0) - AGSEQ(le1));
...
static void setState(graph_t* auxg, attr_state_t* attr_state) {
  ...
  if (!E_weight) E_weight = agattr(auxg, AGEDGE, "weight", "");
  E_minlen = NULL;
  E_fontcolor = NULL;
 ...
 */
static void make_flat_adj_edges(graph_t* g, path* P, edge_t** edges, int ind,
                                int cnt, edge_t* e0, int et) {
  node_t* n;
  node_t* tn, *hn;
  edge_t* e;
...
static void makeFlatEnd(graph_t* g, spline_info_t* sp, path* P, node_t* n,
                        edge_t* e, pathend_t* endp, boolean isBegin) {
  boxf b;
  ...
  if (b.LL.x < b.UR.x && b.LL.y < b.UR.y) endp->boxes[endp->boxn++] = b;
}
/* makeBottomFlatEnd;
 */
...
static void make_flat_labeled_edge(graph_t* g, spline_info_t* sp, path* P,
  ...
  if (et == ET_LINE) {
    ...
    ydelta = ND_coord(ln).y - GD_rank(g)[ND_rank(tn)].ht1 - ND_coord(tn).y +
             GD_rank(g)[ND_rank(tn)].ht2;
    ydelta /= 6.;
...
static void make_flat_bottom_edges(graph_t* g, spline_info_t* sp, path* P,
                                   edge_t** edges, int ind, int cnt, edge_t* e,
                                   int splines) {
  node_t* tn, *hn;
  int j, i, r;
...
static void make_flat_edge(graph_t* g, spline_info_t* sp, path* P,
                           edge_t** edges, int ind, int cnt, int et) {
  node_t* tn, *hn;
...
static void make_regular_edge(graph_t* g, spline_info_t* sp, path* P,
  ...
  if ((et == ET_LINE) && (pointn = makeLineEdge(g, fe, pointfs, &hn))) {
    ...
    if (b.LL.x < b.UR.x && b.LL.y < b.UR.y) tend.boxes[tend.boxn++] = b;
    longedge = 0;
    smode = FALSE, si = -1;
    while (ND_node_type(hn) == VIRTUAL && !sinfo.splineMerge(hn)) {
      ...
      if (b.LL.x < b.UR.x && b.LL.y < b.UR.y) hend.boxes[hend.boxn++] = b;
    ...
    if (b.LL.x < b.UR.x && b.LL.y < b.UR.y) hend.boxes[hend.boxn++] = b;
    completeregularpath(P, segfirst, e, &tend, &hend, boxes, boxn, longedge);
    if (splines)
      ps = routesplines(P, &pn);
    else
      ps = routepolylines(P, &pn);
  ...
  for (i = 1; i < pointn - 1; i++) pointfs[i].x -= dx;

  if (numpts > numpts2) {
    numpts2 = numpts;
    pointfs2 = RALLOC(numpts2, pointfs2, pointf);
  }
  for (i = 0; i < pointn; i++) pointfs2[i] = pointfs[i];
  ...
  for (j = 1; j < cnt; j++) {
    ...
    for (i = 1; i < pointn - 1; i++) pointfs[i].x += sp->Multisep;
    for (i = 0; i < pointn; i++) pointfs2[i] = pointfs[i];
...
static void completeregularpath(path* P, edge_t* first, edge_t* last,
  ...
  for (i = 0; i < tendp->boxn; i++) add_box(P, tendp->boxes[i]);
  fb = P->nbox + 1;
  lb = fb + boxn - 3;
  for (i = 0; i < boxn; i++) add_box(P, boxes[i]);
  for (i = hendp->boxn - 1; i >= 0; i--) add_box(P, hendp->boxes[i]);
  adjustregularpath(P, fb, lb);
}
#else
void refineregularends(edge_t* left, edge_t* right, pathend_t* endp, int dir,
                       boxf b, boxf* boxes, int* boxnp);
...
static void completeregularpath(path* P, edge_t* first, edge_t* last,
  ...
  if (ND_rank(agtail(first)) + 1 == ND_rank(aghead(last))) {
    ...
    for (i = 0; i < uboxn; i++) add_box(P, uboxes[i]);
  } else {
    for (i = 0; i < uboxn; i++) add_box(P, uboxes[i]);
    fb = P->nbox;
    lb = fb + boxn - 3;
    for (i = 1; i < boxn - 1; i++) add_box(P, boxes[i]);
    for (i = 0; i < lboxn; i++) add_box(P, lboxes[i]);
  }
  for (i = hendp->boxn - 1; i >= 0; i--) add_box(P, hendp->boxes[i]);
  adjustregularpath(P, fb, lb);
}
...
void refineregularends(left, right, endp, dir, b, boxes, boxnp) edge_t* left,
    *right;
pathend_t* endp;
int dir;
box b;
...
int* boxnp;
{
  ...
  *boxnp = nsub;
  ...
  if (left) {
    ...
    pp = spline_at_y(lspls,
                     (dir == 1) ? endp->boxes[1].UR.y : endp->boxes[1].LL.y);
    for (i = 1; i < endp->boxn; i++) {
      cp = spline_at_y(lspls,
                       (dir == 1) ? endp->boxes[i].LL.y : endp->boxes[i].UR.y);
  ...
  if (right) {
    ...
    pp = spline_at_y(rspls,
                     (dir == 1) ? endp->boxes[1].UR.y : endp->boxes[1].LL.y);
    for (i = 1; i < endp->boxn; i++) {
      cp = spline_at_y(rspls,
                       (dir == 1) ? endp->boxes[i].LL.y : endp->boxes[i].UR.y);
...
static int straight_len(node_t* n) {
  ...
  while (1) {
    ...
    if (ND_node_type(v) != VIRTUAL) break;
    if ((ND_out(v).size != 1) || (ND_in(v).size != 1)) break;
    if (ND_coord(v).x != ND_coord(n).x) break;
    cnt++;
  }
  return cnt;
}
...
static void recover_slack(edge_t* e, path* p) {
  ...
  for (vn = aghead(e); ND_node_type(vn) == VIRTUAL && !sinfo.splineMerge(vn);
    ...
    while ((b < p->nbox) && (p->boxes[b].LL.y > ND_coord(vn).y)) b++;
    if (b >= p->nbox) break;
    if (p->boxes[b].UR.y < ND_coord(vn).y) continue;
    if (ND_label(vn))
...
static edge_t* top_bound(edge_t* e, int side) {
  ...
  for (i = 0; (f = ND_out(agtail(e)).list[i]); i++) {
	...
	if (ED_tail_port(e).p.x != ED_tail_port(f).p.x)
	    continue;
#endif
    if (side * (ND_order(aghead(f)) - ND_order(aghead(e))) <= 0) continue;
    if ((ED_spl(f) == NULL) &&
...
static edge_t* bot_bound(edge_t* e, int side) {
  ...
  for (i = 0; (f = ND_in(aghead(e)).list[i]); i++) {
	...
	if (ED_head_port(e).p.x != ED_head_port(f).p.x)
	    continue;
#endif
    if (side * (ND_order(agtail(f)) - ND_order(agtail(e))) <= 0) continue;
    if ((ED_spl(f) == NULL) &&
        ((ED_to_orig(f) == NULL) || (ED_spl(ED_to_orig(f)) == NULL)))
      continue;
    if ((ans == NULL) ||
...
edge_t* ie1, *oe1;
{
  ...
  if ((ND_out(n0).size != 1) && (ND_out(n0).size != 1)) return FALSE;
  e1 = oe1;
  if (ND_out(n0).size == 1 && e1) {
    e0 = ND_out(n0).list[0];
    for (cnt = 0; cnt < 2; cnt++) {
      if ((na = aghead(e0)) == (nb = aghead(e1))) break;
      if (order != (ND_order(na) > ND_order(nb))) return TRUE;
      if ((ND_out(na).size != 1) || (ND_node_type(na) == NORMAL)) break;
      e0 = ND_out(na).list[0];
      if ((ND_out(nb).size != 1) || (ND_node_type(nb) == NORMAL)) break;
      e1 = ND_out(nb).list[0];
    }
  }
  e1 = ie1;
  if (ND_in(n0).size == 1 && e1) {
    e0 = ND_in(n0).list[0];
    for (cnt = 0; cnt < 2; cnt++) {
      if ((na = agtail(e0)) == (nb = agtail(e1))) break;
      if (order != (ND_order(na) > ND_order(nb))) return TRUE;
      if ((ND_in(na).size != 1) || (ND_node_type(na) == NORMAL)) break;
      e0 = ND_in(na).list[0];
      if ((ND_in(nb).size != 1) || (ND_node_type(nb) == NORMAL)) break;
      e1 = ND_in(nb).list[0];
    }
  }
  return FALSE;
