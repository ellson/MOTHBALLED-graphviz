...
/*
 * position(g): set ND_coord(n) (x and y) for all nodes n of g, using
 * GD_rank(g).
...
static int nsiter2(graph_t *g) {
  ...
  if ((s = agget(g, "nslimit"))) maxiter = atof(s) * agnnodes(g);
  return maxiter;
}
...
static int canreach(node_t *u, node_t *v) { return go(u, v); }
...
edge_t *make_aux_edge(node_t *u, node_t *v, double len, int wt) {
  ...
  agtail(e) = u;
  aghead(e) = v;
  if (len > USHRT_MAX) len = largeMinlen(len);
    ...
    for (j = 0; j < rank[i].n; j++) {
      ...
      if ((e = (edge_t *)ND_alg(u))) {
        ...
        if (canreach(agtail(e0), aghead(e0)) == FALSE)
          make_aux_edge(aghead(e0), agtail(e0), m1, ED_weight(e));
        m1 = m0 + ND_rw(agtail(e1)) + ND_lw(aghead(e1));
        if (canreach(aghead(e1), agtail(e1)) == FALSE)
          make_aux_edge(agtail(e1), aghead(e1), m1, ED_weight(e));
      }
      ...
      for (k = 0; k < ND_flat_out(u).size; k++) {
        ...
        if ((e0 = find_fast_edge(t0, h0))) {
          ...
          if (m0 > USHRT_MAX) m0 = largeMinlen(m0);
  ...
  for (n = GD_nlist(g); n; n = ND_next(n)) {
    if (ND_save_out(n).list)
      for (i = 0; (e = ND_save_out(n).list[i]); i++) {
        ...
        ND_rank(sn) =
            MIN(ND_rank(agtail(e)) - m0 - 1, ND_rank(aghead(e)) - m1 - 1);
      }
  }
}
...
static void contain_clustnodes(graph_t *g) {
  ...
  for (c = 1; c <= GD_n_cluster(g); c++) contain_clustnodes(GD_clust(g)[c]);
}
...
static int vnode_not_related_to(graph_t *g, node_t *v) {
  ...
  for (e = ND_save_out(v).list[0]; ED_to_orig(e); e = ED_to_orig(e))
    ;
  if (agcontains(g, agtail(e))) return FALSE;
  if (agcontains(g, aghead(e))) return FALSE;
  return TRUE;
}
...
static void adjustSimple(graph_t *g, int delta, int margin_total) {
  ...
  if (delbottom > 0) {
    ...
    deltop = GD_ht2(g) + (delta - bottom) + delbottom -
...
static void set_ycoords(graph_t *g) {
    ...
    for (i = 0; i < rank[r].n; i++) {
      ...
      if (ND_other(n).list)
        for (j = 0; (e = ND_other(n).list[j]); j++) {
          if (agtail(e) == aghead(e)) {
            if (ED_label(e)) ht2 = MAX(ht2, ED_label(e)->dimen.y / 2);
          }
        }
  ...
  /* If there are cluster labels and the drawing is rotated, we need special
   * processing to
   * allocate enough room. We use adjustRanks for this, and then recompute the
   * maxht if
   * the ranks are to be equally spaced. This seems simpler and appears to work
   * better than
...
static void dot_compute_bb(graph_t *g, graph_t *root) {
  ...
  if (g == dot_root(g)) {
    ...
    for (r = GD_minrank(g); r <= GD_maxrank(g); r++) {
      ...
      for (c = rnkn - 2; ND_node_type(v) != NORMAL; c--) v = GD_rank(g)[r].v[c];
...
static void scale_bb(graph_t *g, graph_t *root, double xf, double yf) {
  ...
  for (c = 1; c <= GD_n_cluster(g); c++) scale_bb(GD_clust(g)[c], root, xf, yf);
  GD_bb(g).LL.x *= xf;
  GD_bb(g).LL.y *= yf;
  GD_bb(g).UR.x *= xf;
  GD_bb(g).UR.y *= yf;
...
static void adjustAspectRatio(graph_t *g, aspect_t *asp) {
  ...
  if (Verbose) {
    fprintf(stderr, "AR=%0.4lf\t Area= %0.4lf\t", AR,
            (double)(GD_bb(g).UR.x - GD_bb(g).LL.x) *
                (GD_bb(g).UR.y - GD_bb(g).LL.y) / 10000.0);
    fprintf(stderr, "Dummy=%d\n", countDummyNodes(g));
  }
  if (AR > 1.1 * asp->targetAR) {
    asp->nextIter =
        (int)(asp->targetAR *
              (double)(asp->curIterations - asp->prevIterations) / (AR));
  ...
  if (Verbose) fprintf(stderr, "next#iter=%d\n", asp->nextIter);
}
  ...
  for (n = GD_nlist(g); n; n = ND_next(n)) {
    ...
    if (ND_other(n).list)
      for (i = 0; (e = ND_other(n).list[i]); i++) {
        if ((d = ND_rank(aghead(e)) - ND_rank(aghead(e))) == 0) continue;
        f = ED_to_orig(e);
