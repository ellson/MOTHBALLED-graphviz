...
static void safe_delete_fast_edge(edge_t *e) {
  ...
  assert(e != NULL);
  for (i = 0; (f = ND_out(agtail(e)).list[i]); i++)
    if (f == e) zapinlist(&(ND_out(agtail(e))), e);
  for (i = 0; (f = ND_in(aghead(e)).list[i]); i++)
    if (f == e) zapinlist(&(ND_in(aghead(e))), e);
}

void other_edge(edge_t *e) { elist_append(e, ND_other(agtail(e))); }

void safe_other_edge(edge_t *e) { safe_list_append(e, &(ND_other(agtail(e)))); }
...
void fast_nodeapp(node_t *u, node_t *v) {
  ...
  ND_next(v) = ND_next(u);
  if (ND_next(u)) ND_prev(ND_next(u)) = v;
  ND_prev(v) = u;
  ND_next(u) = v;
}
...
void delete_fast_node(graph_t *g, node_t *n) {
  ...
  if (ND_next(n)) ND_prev(ND_next(n)) = ND_prev(n);
  if (ND_prev(n))
...
static char *NAME(node_t *n) {
  static char buf[20];
  if (ND_node_type(n) == NORMAL) return agnameof(n);
  sprintf(buf, "V%p", n);
  return buf;
}
...
static void basic_merge(edge_t *e, edge_t *rep) {
  if (ED_minlen(rep) < ED_minlen(e)) ED_minlen(rep) = ED_minlen(e);
  while (rep) {
...
#ifdef OBSOLETET
static int is_fast_node(graph_t *g, node_t *v) {
  node_t *n;
