...
void class2(graph_t *g) {
  ...
  mark_clusters(g);
  for (c = 1; c <= GD_n_cluster(g); c++) build_skeleton(g, GD_clust(g)[c]);
  for (n = agfstnode(g); n; n = agnxtnode(g, n))
    for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
      if (ND_weight_class(aghead(e)) <= 2) ND_weight_class(aghead(e))++;
      if (ND_weight_class(agtail(e)) <= 2) ND_weight_class(agtail(e))++;
    }
  ...
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
    ...
    for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
      ...
      /* backward edges */
        ...
        if ((opp = agfindedge(g, aghead(e), agtail(e))) &&
            (aghead(opp) != aghead(e))) {
