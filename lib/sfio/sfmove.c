...
#endif
{
  ...
  for (n_move = 0; n != 0;) {/* get the streams into the right mode */
    if (fr->mode != SF_READ && _sfmode(fr, SF_READ, 0) < 0) goto done;
        ...
        if (n > 0 && fr->extent < 0 && (fr->flags & SF_SHARE)) {
          if (rc >= 0) {/* try peeking a large buffer */
            ...
            if ((r = SFFILBUF(fr, -1)) > 0)
              ...
              r = sfpkrd(fr->file, (Void_t *)fr->data, fr->size, rc, -1,
                         (int)(-n));
              if (r <= 0) goto one_r;
              fr->next = fr->data;
    ...
    if (rc < 0) {
      ...
      reg int rdwr =
          (fr->flags & SF_MALLOC) || (fr->bits & (SF_BOTH | SF_MMAP));
      if (rdwr) {
        w = endb[-1];
        endb[-1] = rc;
      } else
    ...
    if (fw) {
      if (direct == SF_WRITE)
        ...
        if (fr->extent >= 0) (void)SFSEEK(fr, (Sfoff_t)(-r), 1);
        break;
      }
    }
