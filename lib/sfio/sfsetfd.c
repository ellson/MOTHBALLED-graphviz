...
#else
static int _sfdup(fd, newfd) reg int fd;
reg int newfd;
#endif
{
  reg int dupfd;
  ...
  if ((dupfd = dup(fd)) < 0 || dupfd >= newfd) return dupfd;
...
#else
int sfsetfd(f, newfd) reg Sfio_t *f;
reg int newfd;
#endif
{
  reg int oldfd;
  ...
  /* notify changes */
  if (_Sfnotify) (*_Sfnotify)(f, SF_SETFD, newfd);
