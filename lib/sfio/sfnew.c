...
#else
Sfio_t *sfnew(oldf, buf, size, file,
              flags) Sfio_t *oldf; /* old stream to be reused */
...
#endif
{
  ...
  if (!(flags & SF_RDWR)) return NIL(Sfio_t *);
  ...
  if ((f = oldf)) {
    if (flags & SF_EOF) {
      ...
      oldf = NIL(Sfio_t *);
               ...
               SF_AVAIL) {/* only allow SF_STATIC to be already closed */
      if (!(f->flags & SF_STATIC)) return NIL(Sfio_t *);
      sflags = f->flags;
      oldf = NIL(Sfio_t *);
  ...
  if (!f) {/* reuse a standard stream structure if possible */
    ...
    if (!f) {
      if (!(f = (Sfio_t *)malloc(sizeof(Sfio_t)))) return NIL(Sfio_t *);
...
/* create a mutex */
#if vt_threaded
  if (!f->mutex) f->mutex = vtmtxopen(NIL(Vtmutex_t *), VT_INIT);
#endif
  ...
  if (_Sfnotify) (*_Sfnotify)(f, SF_NEW, f->file);
