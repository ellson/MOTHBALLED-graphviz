...
#endif
{
  ...
  for (; f; f = f->push) {
    ...
    if ((f->flags & SF_STRING) || (f->mode & SF_SYNCED)) goto next;
    ...
    if ((f->mode & SF_WRITE) &&
      ...
      if (!SFISNULL(f) &&
          (f->bits & SF_HOLE)) {/* realize a previously created hole of 0's */
    ...
    if ((f->mode & SF_READ) && f->extent >= 0 &&
        ((f->bits & SF_MMAP) ||
         f->next <
             f->endb)) {/* make sure the file pointer is at the right place */
