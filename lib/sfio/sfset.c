...
#else
int sfset(f, flags, set) reg Sfio_t *f;
reg int flags;
reg int set;
#endif
{
  ...
  if (flags == 0 && set == 0) SFMTXRETURN(f, (f->flags & SF_FLAGS));

  if ((oflags = (f->mode & SF_RDWR)) != (int)f->mode &&
      _sfmode(f, oflags, 0) < 0)
    SFMTXRETURN(f, 0);

  if (flags == 0) SFMTXRETURN(f, (f->flags & SF_FLAGS));
  ...
  if (f->extent < 0) f->flags &= ~SF_APPENDWR;
