...
static Sfdisc_t Rmdisc = {NIL(Sfread_f), NIL(Sfwrite_f), NIL(Sfseek_f),
                          _tmprmfile,    NIL(Sfdisc_t *)};
...
#endif
{
  ...
  if (!(path = getenv(path))) return NIL(char **);
...
#endif
{
  ...
  for (t = 0; t < 10; ++t) {/* compute a random name */
    ...
    Key = A * Key + 987654321;
    file = sfprints("%s/sf%3.3.32lu.%3.3.32lu", Tmpcur[0], (Key >> 15) & 0x7fff,
                    Key & 0x7fff);
#else
    ...
    if (!file) return -1;
#if _has_oflags
    if ((fd = open(file, O_RDWR | O_CREAT | O_EXCL | O_TEMPORARY,
                   SF_CREATMODE)) >= 0)
      break;
#else
    if ((fd = open(file, O_RDONLY)) >= 0) {/* file already exists */
      CLOSE(fd);
      fd = -1;
    } else if ((fd = creat(file, SF_CREATMODE)) >=
               0) {/* reopen for read and write */
      CLOSE(fd);
      if ((fd = open(file, O_RDWR)) >= 0) break;
...
#endif
{
  ...
  if (notifyf) (*notifyf)(f, SF_NEW, f->file);
...
#endif
{
  ...
  if (!(f = sfnew(NIL(Sfio_t *), NIL(char *), s, -1,
