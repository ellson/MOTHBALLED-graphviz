...
#if __STD_C
static ssize_t streamio(Sfio_t *f, Void_t *buf, size_t n, Sfdisc_t *disc,
                        int type)
#else
...
#else
int sfdcsubstream(f, parent, offset, extent) Sfio_t *f; /* stream */
Sfio_t *parent;                                         /* parent stream */
...
#endif
{
  ...
  if (!(su = (Subfile_t *)malloc(sizeof(Subfile_t)))) return -1;
