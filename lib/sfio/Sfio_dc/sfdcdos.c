...
#else
static void addmapping(dp) register Dosdisc_t *dp;
#endif
{
  register int n;
...
#endif
{
  ...
  while (1) {
    if ((n = sfrd(iop, buff, size, disc)) <= 0) return (n);
    dp->plast = dp->phere;
    dp->phere += n;
    dp->llast = dp->lhere;
    cpmax = cp + n - 1;
  ...
  while (1) {
    ...
    if (cp == cpmax)
      ...
      while (*cp++ != '\r')
        ;
    if (cp <= cpmax && *cp != '\n') continue;
    if ((m = (cp - first) - 1) > 0) memcpy(first - count, first, m);
    if (cp > cpmax) break;
    count++;
  }
  cpmax[-count] = dp->last;
...
static off_t cur_offset(Dosdisc_t *dp, off_t offset, Sfio_t *iop,
                        register int whence)
#else
static off_t cur_offset(dp, offset, iop, whence) Dosdisc_t *dp;
off_t offset;
Sfio_t *iop;
register int whence;
#endif
...
retry:
  switch (whence) {
    case SEEK_CUR:
      ...
      if (offset <= dp->begin) return (offset);
    ...
    case SEEK_SET:
      ...
      break;
    case SEEK_END:
      if (!dp->maptable) return (sfsk(iop, offset, SEEK_END, disc));
      mp = &dummy;
  ...
  if (mp) {
    ...
    dp->lhere = mp->logical;
    if ((*disc->readf)(iop, iop->data, size, disc) < 0) return (-1);
  }
  while (1) {
    if (whence == SEEK_CUR && dp->phere >= offset) break;
    if (whence == SEEK_SET && dp->lhere >= offset) break;
...
#endif
{
  ...
  if (sfset(f, 0, 0) & SF_WRITE) return (-1);

  if (!(dos = (Dosdisc_t *)malloc(sizeof(Dosdisc_t)))) return -1;
