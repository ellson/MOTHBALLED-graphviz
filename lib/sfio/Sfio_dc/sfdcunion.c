...
#endif
{
  ...
  f = un->f[un->c].f;
  while (1) {
    if ((r = sfread(f, buf, m)) < 0 || (r == 0 && un->c == un->n - 1)) break;
    ...
    buf = (char *)buf + r;
    if (sfeof(f) && un->c < un->n - 1) f = un->f[un->c += 1].f;
  }
  return n - m;
}
...
#endif
{
  ...
  un = (Union_t *)disc;
  if (un->type & UNSEEKABLE) return -1L;

  if (type == 2) {
    extent = 0;
    for (i = 0; i < un->n; ++i) extent += (sfsize(un->f[i].f) - un->f[i].lower);
    addr += extent;
  } else if (type == 1)
    addr += un->here;
  ...
  for (i = 0; i < un->n - 1; ++i) {
    ...
    if (addr < extent + s) break;
    extent += s;
  }

  s = (addr - extent) + un->f[i].lower;
  if (sfseek(un->f[i].f, s, 0) != s) return -1;

  un->c = i;
  un->here = addr;

  for (i += 1; i < un->n; ++i) sfseek(un->f[i].f, un->f[i].lower, 0);
...
#endif
{
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (!(un->type & UNSEEKABLE)) {
      ...
      if (un->f[i].lower < 0) un->type |= UNSEEKABLE;
    }
  }
