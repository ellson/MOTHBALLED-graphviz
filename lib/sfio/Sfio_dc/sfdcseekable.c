...
#endif
{
  ...
  sk = (Seek_t *)disc;
  sf = sk->shadow;
  if (sk->eof) return sfread(sf, buf, n);

  addr = sfseek(sf, (Sfoff_t)0, 1);
  extent = sfsize(sf);

  if (addr + n <= extent) return sfread(sf, buf, n);

  if ((r = (ssize_t)(extent - addr)) > 0) {
    if ((w = sfread(sf, buf, r)) != r) return w;
    buf = (char *)buf + r;
    n -= r;
  }
...
#endif
{
  ...
  if (type < 0 || type > 2) return (Sfoff_t)(-1);
  ...
  if (addr < 0)
    ...
    if (addr > extent) return (Sfoff_t)(-1);
  }
...
#endif
{
  ...
  if (sfseek(f, (Sfoff_t)0, 1) >= 0) return 0;

  if (!(sk = (Seek_t *)malloc(sizeof(Seek_t)))) return -1;
