...
#endif
{
  ...
  while (n > 0) {
    ...
    if (s >= _Sfpage) {
      s = (s / _Sfpage) * _Sfpage;
      if (SFSK(f, (Sfoff_t)s, SEEK_CUR, NIL(Sfdisc_t *)) < 0) break;
      w += s;
      n -= s;
      wbuf = (buf += s);
      f->bits |= SF_HOLE;
...
ssize_t sfwr(reg Sfio_t *f, reg const Void_t *buf, reg size_t n,
             reg Sfdisc_t *disc)
#else
ssize_t sfwr(f, buf, n, disc) reg Sfio_t *f;
reg Void_t *buf;
reg size_t n;
reg Sfdisc_t *disc;
#endif
    ...
    if (f->flags & SF_STRING) /* total required buffer */
      ...
      if (dc && dc->writef) {
        ...
        goto do_write;
               ...
               !(f->flags & (SF_SHARE | SF_APPENDWR)) && f->here == f->extent &&
               (f->here % _Sfpage) == 0) {
        if ((w = sfoutput(f, (char *)buf, n)) <= 0) goto do_write;
      } else {
      do_write:
        if ((w = write(f->file, (char *)buf, n)) > 0) f->bits &= ~SF_HOLE;
      }
