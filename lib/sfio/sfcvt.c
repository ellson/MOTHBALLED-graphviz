...
#else
char *_sfcvt(dv, n_digit, decpt, sign,
             format) Void_t *dv; /* value to convert             */
...
#endif
{
  ...
  /* set up local buffer */
  if (!Buf && !(Buf = (char *)malloc(SF_MAXDIGITS))) return SF_INFINITE;
  ...
  *sign = *decpt = 0;
  ...
  if (format & SFFMT_LDOUBLE) {
    ...
    n = 0;
    if (dval >= (Sfdouble_t)SF_MAXLONG) {/* scale to a small enough number to
                                            fit an int */
      v = SF_MAXEXP10 - 1;
      do {
        if (dval < _Sfpos10[v])
          ...
          dval *= _Sfneg10[v];
          if ((n += (1 << v)) >= SF_IDIGITS) return SF_INFINITE;
        }
    ...
    *decpt = (int)n;
    ...
    if ((v = (int)dval) != 0) {/* translate the integer part */
      ...
      n = buf - sp;
      if ((*decpt += (int)n) >= SF_IDIGITS) return SF_INFINITE;
      buf = sp;
      sp = Buf + SF_INTPART;
    } else
      n = 0;
    ...
    if (n_digit > 0) n += n_digit;

    if ((ep = (sp + n)) > (endsp = Buf + (SF_MAXDIGITS - 2))) ep = endsp;
    if (sp > ep)
      sp = ep;
    else {
...
#endif
  {
    ...
    n = 0;
    if (dval >=
        (double)SF_MAXLONG) {/* scale to a small enough number to fit an int */
      v = SF_MAXEXP10 - 1;
      do {
        if (dval < _Sfpos10[v])
          v -= 1;
