...
#else
Void_t *sfsetbuf(f, buf, size) reg Sfio_t *f; /* stream to be buffered */
...
#endif
{
  ...
  if ((init = f->mode & SF_INIT)) {
    if (!f->pool && _sfsetpool(f) < 0) SFMTXRETURN(f, NIL(Void_t *));
  ...
  if (init)
    ...
    rv = SFSYNC(f);
    SFOPEN(f, local);
    if (rv < 0) SFMTXRETURN(f, NIL(Void_t *));
  ...
  okmmap = (buf || (f->flags & SF_STRING) || (f->flags & SF_RDWR) == SF_RDWR)
               ? 0
               : 1;
...
#endif
#ifdef MAP_TYPE
      if (S_ISDIR(st.st_mode) || (int)st.st_size < SF_GRAIN) okmmap = 0;
#endif
...
#if O_TEXT /* no memory mapping with O_TEXT because read()/write() alter data \
              stream */
#ifdef MAP_TYPE
    ...
    unseekable:
      ...
      if (init) {
        if (S_ISCHR(st.st_mode)) {
          ...
          if (!(f->flags & SF_LINE) && isatty(f->file)) f->flags |= SF_LINE;
#if _sys_stat
          else {/* special case /dev/null */
            reg int dev, ino;
            dev = (int)st.st_dev;
            ino = (int)st.st_ino;
            if (stat(DEVNULL, &st) >= 0 && dev == (int)st.st_dev &&
                ino == (int)st.st_ino)
              SFSETNULL(f);
          }
#endif
        ...
        if (!f->proc && (f->bits & SF_BOTH)) (void)_sfpopen(f, -1, -1, 0);
      }
    }
...
/* get buffer space */
setbuf:
  if (size == (size_t)SF_UNBOUND) {/* define a default size suitable for block
                                      transfer */
    if (init && osize > 0)
      size = osize;
  ...
  if (f->flags &
      SF_STRING) {/* these fields are used to test actual size - see sfseek() */
    f->extent = (!sf_malloc && ((f->flags & SF_READ) || (f->bits & SF_BOTH)))
                    ? size
                    : 0;
    f->here = 0;
