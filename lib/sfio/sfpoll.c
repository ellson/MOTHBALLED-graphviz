...
retry:
  for (r = c = 0; r < n; ++r) {
    ...
    for (;;) {/* check accessibility */
      m = f->mode & SF_RDWR;
      if ((int)f->mode != m && _sfmode(f, m, 0) < 0) goto do_never;

      /* clearly ready */
      if (f->next < f->endb) goto do_ready;
      ...
      switch (_sfexcept(f, f->mode & SF_RDWR, 0, f->disc)) {
        case SF_EDONE:
          if (f->flags & SF_STRING)
            ...
            goto do_ready;
        case SF_EDISC:
          if (f->flags & SF_STRING) goto do_ready;
        case SF_ESTACK:
        case SF_ECONT:
          continue;
      }
  ...
  if (c > 0) {
    ...
    if (!(fds = (struct pollfd *)malloc(c * sizeof(struct pollfd)))) return -1;
    for (r = 0; r < c; r++) {
