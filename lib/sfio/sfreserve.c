...
#else
Void_t *sfreserve(f, size, type) reg Sfio_t *f; /* file to peek */
...
#endif
{
  ...
  if ((sz = size) == 0 &&
      type != 0) {/* only return the current status and possibly lock stream */
    ...
    if (!f->data && type > 0) rsrv = _sfrsrv(f, 0);
  ...
  for (;;) {/* prefer read mode so that data is always valid */
    ...
    if ((f->mode & mode) != 0) break;
  }

  if (n > 0 && n < sz &&
      (f->mode & mode) != 0) {/* try to accomodate request size */
