...
#else
Sfio_t *sfopen(f, file, mode) reg Sfio_t *f; /* old stream structure */
...
#endif
{
  ...
  if ((sflags = _sftype(mode, &oflags, NIL(int *))) == 0) return NIL(Sfio_t *);
...
/* usually used on the standard streams to change control flags */
    ...
    if (f->mode & SF_INIT) {/* paranoia in case another thread snuck in */
      if (f->file >= 0 && !(f->flags &SF_STRING) &&
          (oflags &= (O_TEXT | O_BINARY | O_APPEND)) !=
              0) {/* set the wanted file access control flags */
  ...
  if (sflags & SF_STRING) {
    f = sfnew(f, (char *)file,
              file ? (size_t)strlen((char *)file) : (size_t)SF_UNBOUND, -1,
              sflags);
  } else {
    if (!file) return NIL(Sfio_t *);
    ...
    if (fd >= 0) {
      ...
      if (oflags & O_TRUNC) {/* truncate file */
        reg int tf;
        while ((tf = creat(file, SF_CREATMODE)) < 0 && errno == EINTR)
          errno = 0;
        CLOSE(tf);
      }
    } else if (oflags & O_CREAT) {
      while ((fd = creat(file, SF_CREATMODE)) < 0 && errno == EINTR) errno = 0;
      if (!(oflags &
...
#else
int _sftype(mode, oflagsp, uflagp) reg char *mode;
int *oflagsp;
int *uflagp;
#endif
{
