...
static int _uexcept(reg Sfio_t *f, reg int type, Void_t *val,
                    reg Sfdisc_t *disc)
#else
static int _uexcept(f, type, val, disc) reg Sfio_t *f;
reg int type;
Void_t *val;
reg Sfdisc_t *disc;
#endif
{
  ...
  if (type != SF_CLOSING) (void)sfclose((*_Sfstack)(f, NIL(Sfio_t *)));
...
#else
int sfungetc(f, c) reg Sfio_t *f; /* push back one byte to this stream */
...
#endif
{
  ...
  if (f->disc != _Sfudisc) {
    if (!(uf = sfnew(NIL(Sfio_t *), NIL(char *), (size_t)SF_UNBOUND, -1,
                     SF_STRING | SF_READ))) {
      c = -1;
      goto done;
    }
