...
#endif
{
  ...
  for (s = (char *)argcmd; *s; ++s)
    if (Meta[(uchar)s[0]]) goto do_interp;

  /* try to construct argv */
  if (!(cmd = (char *)malloc(strlen(argcmd) + 1))) goto do_interp;
  strcpy(cmd, argcmd);
  if (!(argv = (char **)malloc(16 * sizeof(char *)))) goto do_interp;
  for (n = 0, s = cmd;;) {
    while (isspace(s[0])) s += 1;
    if (s[0] == 0) break;
    ...
    while (s[0] && !isspace(s[0])) s += 1;
    if (!s[0]) *s++ = 0;
  }
  if (n == 0) goto do_interp;
  argv[n] = NIL(char *);
  ...
  if (cmd[0] == '/' || (cmd[0] == '.' && cmd[1] == '/') ||
    ...
    for (p = Path; *p; ++p) {
      ...
      if (access(s, X_OK) == 0) execv(s, argv);
    }
  }
...
#endif
{
  ...
  if (!command || !command[0] || !(sflags = _sftype(mode, NiL, NiL))) return 0;
  ...
  if (pipe(parent) < 0) goto error;
  if ((sflags & SF_RDWR) == SF_RDWR && pipe(child) < 0) goto error;
  ...
  switch ((pid = fork())) {
    default: /* in parent process */
      ...
      /* make the streams */
      if (!(f = sfnew(f, NIL(Void_t *), (size_t)SF_UNBOUND, parent[pkeep],
                      sflags)))
        goto error;
      CLOSE(parent[!pkeep]);
    ...
    case 0: /* in child process */
      ...
      CLOSE(parent[!pkeep]);
      if ((sflags & SF_RDWR) == SF_RDWR) CLOSE(child[!ckeep]);
      ...
      if ((sflags & SF_RDWR) == SF_RDWR && pkeep == child[ckeep])
        if ((child[ckeep] = dup(pkeep)) < 0) _exit(EXIT_NOTFOUND);

      if (parent[pkeep] != pkeep) {
        sf.file = parent[pkeep];
        CLOSE(pkeep);
        if (sfsetfd(&sf, pkeep) != pkeep) _exit(EXIT_NOTFOUND);
      }

      if ((sflags & SF_RDWR) == SF_RDWR && child[ckeep] != ckeep) {
        sf.file = child[ckeep];
        CLOSE(ckeep);
        if (sfsetfd(&sf, ckeep) != ckeep) _exit(EXIT_NOTFOUND);
      }
