...
static double unweighted_common_fraction(v_data *graph, int v, int u,
                                         float *v_vector) {
  ...
  // more natural
  return ((double)num_shared_neighbors) /
         (graph[v].nedges + graph[u].nedges - num_shared_neighbors);
}
...
static float fill_neighbors_vec(v_data *graph, int vtx, float *vtx_vec) {
  ...
  if (graph[0].ewgts != NULL) {
    for (j = 0; j < graph[vtx].nedges; j++) {
      sum_weights += (vtx_vec[graph[vtx].edges[j]] = (float)fabs(
                          graph[vtx].ewgts[j]));  // use fabs for the self loop
    }
  } else {
    for (j = 0; j < graph[vtx].nedges; j++) {
...
static void fill_neighbors_vec_unweighted(v_data *graph, int vtx,
                                          float *vtx_vec) {
...
static int dist3(v_data *graph, int node1, int node2) {
  // succeeds if the graph theoretic distance between the nodes is no more than
  // 3
  int i, j, k;
  int u, v;
...
static int maxmatch(v_data *graph,           /* array of vtx data for graph */
    ...
    /*
        ...
        but on the wider edge list of 'geom_graph' (which includes 'graph''s
       edges)
        ...
        if (graph[i].ewgts[j] * inv_size /
          ...
          max_norm_edge_weight =
              (float)(graph[i].ewgts[j] * inv_size /
                      sqrt((double)geom_graph[graph[i].edges[j]].size));
        }
      }
    }
  } else {
...
#endif
// Option 2: sort the nodes begining with the ones highly approriate for
// matching
  ...
  for (i = 0; i < nvtxs; i++) {
    vtx = order[i];
    matchability[vtx] =
        graph[vtx].nedges;  // we less want to match high degree nodes
    matchability[vtx] +=
        geom_graph[vtx].size;  // we less want to match large sized nodes
    min_edge_len = 1e99;
    for (j = 1; j < graph[vtx].nedges; j++) {
      min_edge_len =
          MIN(min_edge_len,
              ddist(geom_graph, vtx, graph[vtx].edges[j]) / avg_edge_len);
    }
  ...
  for (i = 0; i < nvtxs; i++) {
    ...
    /*
       ...
       (2) AvgDeg*AvgDeg/(deg(vtx)*deg(neighbor)) (degrees measured on orig.
       graph)
       (3) AvgEdgeLen/dist(vtx,neighbor)
       (4) Weight of normalized direct connection between nodes (measured on
       orig. graph)
     */
    ...
    for (j = 1; j < geom_graph[vtx].nedges; j++) {
      ...
      // (1):
      val = A * unweighted_common_fraction(graph, vtx, neighbor, vtx_vec);

      if (val == 0 && (dist2_limit || !dist3(graph, vtx, neighbor))) {
        // graph theoretical distance is larger than 3 (or 2 if '!dist3(graph,
        // vtx, neighbor)' is commented)
        // nodes cannot be matched
        continue;
      }
      // (2)
      val += B * avg_deg_2 / (graph[vtx].nedges * graph[neighbor].nedges);

      // (3)
      val += C * avg_edge_len / ddist(geom_graph, vtx, neighbor);

      // (4)
      val += (weighted_vtx_vec[neighbor] * inv_size /
    ...
    // and use it to contract these pairs and build a coarse graph
{
  ...
  int maxCnedges = nedges;  // do not subtract (nvtxs-cnvtxs) because we do not
                            // contract only along edges
  int *edges;
  float *eweights;
#ifdef STYLES
  int styled_edges;
  ...
  if (graph[0].ewgts != NULL) {
    ...
    for (cv = 0; cv < cnvtxs; cv++) {
      ...
      for (j = 1; j < graph[v].nedges; j++) {
        ...
        if (neighbor == cv) {
          intra_weight =
              2 * graph[v].ewgts[j];  // count both directions of the intra-edge
          continue;
        }
...
#ifdef STYLES
          if (styled_edges &&
              graph[v].styles[j] != cgraph[cv].styles[index[neighbor]]) {
      ...
      for (j = 1; j < cgraph[cv].nedges; j++) index[cgraph[cv].edges[j]] = 0;
    }
    ...
    for (cv = 0; cv < cnvtxs; cv++) {
      ...
      cgraph[cv].ewgts[0] =
          (float)graph[v].edges[0];  // this is our trick to store the weights
                                     // on the diag in an unweighted graph
      if ((v = cv2v[2 * cv + 1]) != -1) {
        for (j = 1; j < graph[v].nedges; j++) {
          neighbor = v2cv[graph[v].edges[j]];
          if (neighbor == cv) continue;
...
static int make_coarse_ex_graph(
    ...
    // and use it to contract these pairs and build a coarse ex_graph
{
  ...
  for (cv = 0; cv < cnvtxs; cv++) {
    ...
    if ((v = cv2v[2 * cv + 1]) != -1) {
      ...
      cgraph[cv].x_coord = (cgraph[cv].size * cgraph[cv].x_coord +
                            graph[v].size * graph[v].x_coord) /
                           (cgraph[cv].size + graph[v].size);
      cgraph[cv].y_coord = (cgraph[cv].size * cgraph[cv].y_coord +
                            graph[v].size * graph[v].y_coord) /
                           (cgraph[cv].size + graph[v].size);
      cgraph[cv].size += graph[v].size;
    }
    cgraph[cv].nedges = cv_nedges;
     ...
     */
{
  ...
  *cnedges =
      make_coarse_graph(graph, nvtxs, nedges, cgraph, cnvtxs, v2cv, cv2v);
  *cgeom_nedges = make_coarse_ex_graph(geom_graph, nvtxs, geom_nedges,
                                       cgeom_graph, cnvtxs, v2cv, cv2v);
}
...
Hierarchy *create_hierarchy(v_data *graph, int nvtxs, int nedges,
  ...
  int nlevels =
      MAX(5, 10 * (int)log((float)(nvtxs / min_nvtxs)));  // just an estimate
  ...
  for (cur_level = 0;
       hierarchy->nvtxs[cur_level] > min_nvtxs && cur_level < 50 /*nvtxs/10 */;
       cur_level++) {
    if (cur_level == nlevels - 1) {  // we have to allocate more space
      nlevels *= 2;
      hierarchy->graphs = RALLOC(nlevels, hierarchy->graphs, v_data *);
    ...
    coarsen_match(
        hierarchy->graphs[cur_level], hierarchy->geom_graphs[cur_level],
        hierarchy->nvtxs[cur_level], hierarchy->nedges[cur_level], ngeom_edges,
        &hierarchy->graphs[cur_level + 1],
        &hierarchy->geom_graphs[cur_level + 1],
        &hierarchy->nvtxs[cur_level + 1], &hierarchy->nedges[cur_level + 1],
        &cngeom_edges, &hierarchy->v2cv[cur_level],
        &hierarchy->cv2v[cur_level + 1], parms->dist2_limit);
  }
...
static double dist_from_foci(ex_vtx_data *graph, int node, int *foci,
                             int num_foci) {
 ...
 */
void set_active_levels(Hierarchy *hierarchy, int *foci_nodes, int num_foci,
                       levelparms_t *parms) {
  int n, i;
  int *nodes;
  double *distances;
  ...
  for (level = min_level + 1; level < hierarchy->nlevels; level++) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (u >= 0) {  // cv is decomposed from 2 fine nodes
        if (graph[v].active_level < level || graph[u].active_level < level) {
          ...
          graph[v].active_level = MIN(graph[v].active_level, level - 1);
          graph[u].active_level = MIN(graph[u].active_level, level - 1);
        }
        // The node with the finer (lower) active level determines the coarse
        // active level
 ...
 */
static double findClosestActiveNode(Hierarchy *hierarchy, int node, int level,
                                    double x, double y, double closest_dist,
                                    int *closest_node,
  ...
  closest_dist = findClosestActiveNode(
      hierarchy, hierarchy->cv2v[level][2 * node], level - 1, x, y,
      closest_dist, closest_node, closest_node_level);

  if (hierarchy->cv2v[level][2 * node + 1] >= 0) {
    closest_dist = findClosestActiveNode(
        hierarchy, hierarchy->cv2v[level][2 * node + 1], level - 1, x, y,
        closest_dist, closest_node, closest_node_level);
  }
  return closest_dist;
}
 ...
 */
static int find_leftmost_descendant(Hierarchy *hierarchy, int node, int level,
                                    int cur_level) {
 ...
 * distance squared.
 */
double find_closest_active_node(Hierarchy *hierarchy, double x, double y,
  ...
  for (i = 0; i < hierarchy->nvtxs[top_level]; i++) {
    min_dist = findClosestActiveNode(hierarchy, i, top_level, x, y, min_dist,
                                     &closest_node, &closest_node_level);
  }
  *closest_fine_node = find_leftmost_descendant(hierarchy, closest_node,
...
int init_ex_graph(v_data *graph1, v_data *graph2, int n, double *x_coords,
                  double *y_coords, ex_vtx_data **gp) {
...
int extract_active_logical_coords(Hierarchy *hierarchy, int node, int level,
  ...
  counter =
      extract_active_logical_coords(hierarchy, hierarchy->cv2v[level][2 * node],
                                    level - 1, x_coords, y_coords, counter);

  if (hierarchy->cv2v[level][2 * node + 1] >= 0) {
    counter = extract_active_logical_coords(
        hierarchy, hierarchy->cv2v[level][2 * node + 1], level - 1, x_coords,
        y_coords, counter);
  }
  return counter;
}
 ...
 * of all active nodes
 */
int set_active_physical_coords(Hierarchy *hierarchy, int node, int level,
                               double *x_coords, double *y_coords,
                               int counter) {
  ...
  counter =
      set_active_physical_coords(hierarchy, hierarchy->cv2v[level][2 * node],
                                 level - 1, x_coords, y_coords, counter);

  if (hierarchy->cv2v[level][2 * node + 1] >= 0) {
    counter = set_active_physical_coords(
        hierarchy, hierarchy->cv2v[level][2 * node + 1], level - 1, x_coords,
        y_coords, counter);
  }
  return counter;
...
static int countActiveNodes(Hierarchy *hierarchy, int node, int level) {
  ...
  if (graph[node].active_level == level) {  // node is active
#ifdef DEBUG
    fprintf(stderr, "(%d,%d) (%f,%f)\n", level, node, graph[node].x_coord,
            graph[node].y_coord);
#endif
    return 1;
  }
  cnt =
      countActiveNodes(hierarchy, hierarchy->cv2v[level][2 * node], level - 1);
...
int locateByIndex(Hierarchy *hierarchy, int index, int *lp) {
  ...
  if (level < nlevels && index >= 0 &&
      hierarchy->geom_graphs[level][index].globalIndex == globalIndex) {
    *lp = level;
    return index;
  } else {
    // index not found
 ...
 */
static int isActiveAncestorOfNeighbors(Hierarchy *hierarchy, int node,
                                       int level, int activeAncestorIdx) {
  int i, active_level;
  ...
  for (i = 1; i < neighborsInLevel.nedges; i++) {
    ...
    active_level = hierarchy->geom_graphs[level][neighbor].active_level;
    if (active_level > level) {
      ...
      if (hierarchy->geom_graphs[neighborLevel][neighbor].globalIndex ==
          activeAncestorIdx) {
        return 1;
      }
    }
 ...
 * Return -1 on error
 */
int findGlobalIndexesOfActiveNeighbors(Hierarchy *hierarchy, int index,
                                       int **np) {
  int numNeighbors = 0;
  int *neighbors;
  int i, j;
  ...
  if ((node = locateByIndex(hierarchy, index, &level)) < 0) node = 0;
  ...
  for (i = 1; i < neighborsInLevel.nedges; i++) {
    ...
    if (active_level == level) {
      ...
      for (j = 0; j < numNeighbors && !found; j++) {
        if (neighbors[j] ==
            hierarchy->geom_graphs[neighborLevel][neighbor].globalIndex) {
          found = 1;
        }
      }
      if (!found) {
      ...
      // descendants of neighbor are active - add those of them that really
      // point back
      ...
      while (stackHeight > 0) {
        ...
        if (hierarchy->geom_graphs[neighborLevel][neighbor].active_level ==
            neighborLevel) {
          if (numNeighbors >= nAllocNeighbors) {
            nAllocNeighbors = 2 * nAllocNeighbors + 1;
            neighbors = RALLOC(nAllocNeighbors, neighbors, int);
          ...
          numNeighbors++;
                       ...
                       .active_level < level) {
          // check if node points back to original node (or just was clustered
          // with neighbors)

          if (isActiveAncestorOfNeighbors(
                  hierarchy, hierarchy->cv2v[neighborLevel][2 * neighbor],
                  neighborLevel - 1, index)) {
            stack[stackHeight] = neighborLevel - 1;
            stackHeight++;
            stack[stackHeight] = hierarchy->cv2v[neighborLevel][2 * neighbor];
            stackHeight++;
          }
          if (hierarchy->cv2v[neighborLevel][2 * neighbor + 1] >= 0) {

            if (isActiveAncestorOfNeighbors(
                    hierarchy, hierarchy->cv2v[neighborLevel][2 * neighbor + 1],
                    neighborLevel - 1, index)) {
              stack[stackHeight] = neighborLevel - 1;
              stackHeight++;
              stack[stackHeight] =
                  hierarchy->cv2v[neighborLevel][2 * neighbor + 1];
              stackHeight++;
            }
          }
        }
 ...
 */
void find_physical_coords(Hierarchy *hierarchy, int level, int node, double *x,
                          double *y) {
...
void find_active_ancestor_info(Hierarchy *hierarchy, int level, int node,
                               int *levell, int *nodee) {
  int active_level = hierarchy->geom_graphs[level][node].active_level;
  while (active_level > level) {
    node = hierarchy->v2cv[level][node];
    level++;
