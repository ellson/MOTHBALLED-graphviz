...
static Agnode_t *model_vnode(Agraph_t *modelgraph, Agedge_t *longedge,
                             int level) {
  Agnode_t *rep;
...
vpath_t *model_path(Agraph_t *modelgraph, Agnode_t *tx, Agnode_t *hx,
                    Agedge_t *orig) {
  vpath_t *p;
...
static Agraph_t *cluster_model(Agraph_t *ug) {
  ...
  for (n = agfstnode(ug); n; n = agnxtnode(ug, n)) {
    if (ND_cluster(n) == ug)
      (void)model_realnode(model, n);
    else
      clusterchains(model, subclustof(n, ug));
  }
  ...
  for (n = agfstnode(model); n; n = agnxtnode(model, n)) ranksize[ND_rank(n)]++;
...
static void clusterchains(Agraph_t *model, Agraph_t *ug) {
  ...
  if (is_a_cluster(ug)) (void)model_skel(model, ug);
