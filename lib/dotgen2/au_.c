...
int gvround(double arg) { return (int)(arg + (arg > 0 ? .5 : -.5)); }
...
int gvgetint(void *obj, char *str, int defval) {
  ...
  if (valstr && valstr[0]) {
    if (sscanf(valstr, "%lf%%", &frac))
      rv = gvround(frac * defval);
    else if (!sscanf(valstr, "%d", &rv))
      rv = defval;
  } else
    rv = defval;
  return rv;
...
void agautoinit(Agraph_t *g, int graphinfo_size, int nodeinfo_size,
                int edgeinfo_size) {
  int *s;
