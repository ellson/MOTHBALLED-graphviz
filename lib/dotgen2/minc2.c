...
static void add_node(Agraph_t *g, Agedge_t *e, int labels) {
  ...
  if (tempC1 == tempC2) cluster = tempC1;

  rankDiff = MND_rank(head) - MND_rank(tail);
  if ((rankDiff == 1) || (rankDiff == -1)) return;
...
static void addNodes(Agraph_t *g, int hasLabels) {
  ...
  Agedge_t *ee = NULL;
  Agedge_t **eList =
      NULL; /*for technical reasons i add dummy edges after the loop */
  int tempEdgeCnt = 0;
  int id = 0;

  for (v = agfstnode(g); v; v = agnxtnode(g, v)) {
    int type = MND_type(v);
    if (type != STDNODE) continue;
    for (e = agfstout(g, v); e; e = agnxtout(g, e)) {
      if (MED_type(e) == VIRTUAL) continue;
      eList = RALLOC(tempEdgeCnt + 1, eList, Agedge_t *);
      eList[tempEdgeCnt] = e;
      tempEdgeCnt++;
    }
  }
  for (id = 0; id < tempEdgeCnt; id++) add_node(g, eList[id], hasLabels);
...
static void double_ranks(Agraph_t *g) {
  Agnode_t *v;
  for (v = agfstnode(g); v; v = agnxtnode(g, v)) MND_rank(v) = MND_rank(v) * 2;
}
...
static void free_mcEdge(mcEdge *e) { free(e); }

static void free_mcNode(mcNode *n) {
  int i;

  if (n->isLeader) free_mcGroup(n->group);
  for (i = 0; i < n->low_edgs_cnt; i++) free_mcEdge(n->low_edgs[i]);
  for (i = 0; i < n->high_edgs_cnt; i++) free_mcEdge(n->high_edgs[i]);
  free(n->low_edgs);
  free(n->high_edgs);
  free(n->name);
  free(n);
...
static void free_mcLevel(mcLevel *l) {
  ...
  for (i = 0; i < l->nodecnt; i++) free_mcNode(l->nodes[i]);
  l->nodecnt++;
  free(l->nodes);
  free(l);
}
...
static void free_mcGraph(mcGraph *mcg) {
  ...
  for (idx = 0; idx < mcg->lvl_cnt; idx++) free_mcLevel(mcg->lvls[idx]);
...
static mcNode *initMcNode(Agnode_t *n) {
  ...
  rv = NEW(mcNode);
  rv->group = NULL;
  if (MND_orderr(n) < 0) MND_orderr(n) = 0;
...
static mcGraph *newMcGraph(Agraph_t *g) {
  ...
  for (v = agfstnode(g); v; v = agnxtnode(g, v)) {
    if (MND_rank(v) > maxRank) maxRank = MND_rank(v);
  }
...
Agnode_t *nodeByName(Agraph_t *g, char *name) {
  Agnode_t *v;
  for (v = agfstnode(g); v; v = agnxtnode(g, v)) {
    if (strcmp(agnameof(v), name) == 0) return v;
  }
  return NULL;
}
...
static int transposeAllL2H(mcGraph *mcG) {
  ...
  int improved = 0;
  int idx;
  for (idx = 0; idx < mcG->lvl_cnt; idx++) mcG->lvls[idx]->candidate = 1;
  rank = 0;
  l = mcG->lvls[0];
  idx = 0;
  do {
    if (l->candidate) {
      ...
      if (a < 0) {
        l->candidate = 1;
        if (idx > 0) mcG->lvls[idx - 1]->candidate = 1;
        if (idx < (mcG->lvl_cnt - 1)) mcG->lvls[idx + 1]->candidate = 1;
...
#define ARRAY_SZ 37
  ...
  mcGraph *mcG;
  int id = 0;
  char *nodes[] = {"d0", "e1", "e2", "f0", "f1", "g0", "a1", "b1", "b2", "c2",
                   "c5", "c8", "c3", "c4", "c6", "c7", "d2", "d4", "d6", "d5",
                   "e3", "e4", "e5", "e6", "e8", "e7", "f4", "f2", "f3", "f5",
                   "a0", "b0", "c0", "c1", "d1", "d3", "e0"};
  ...
  for (v = GD_nlist(g); v; v = ND_next(v)) {
    if ((inEdgeCnt(v, g) == 0) && (MND_inBucket(v) != 1)) {
      ...
      while ((v0 = dequeue(b))) {
        if (Verbose) fprintf(stderr, "installing %s\n", agnameof(v0));
        install_node(v0, mcG);
  ...
  */
  ...
  free_queue(b);
  for (v = agfstnode(g); v; v = agnxtnode(g, v)) addAdjEdges(v);
  id = countAllX(mcG);
#ifdef DEBUG
#ifdef WIN32
...
static void reSetNodePos(mcGraph *mcG, mcLevel *l, int lowToHigh) {
  ...
  for (id = 0; id < l->nodecnt; id++) {
    ...
    if (order == -1) order = n->order;
    if (order2 == -1) order2 = order;
    if (MAGIC_NUMBER != 0)
...
static int writer(void *closure, const unsigned char *data,
                  unsigned int length) {
...
static void drawMatrix(mcGraph *mcG, char *fileName, char *label) {
  ...
  cairo_surface_t *surface = cairo_image_surface_create(
      CAIRO_FORMAT_ARGB32, DOC_WIDTH + 100, DOC_HEIGHT);
...
static int in_cross(mcNode *v, mcNode *w) {
  ...
  for (id = 0; id < w->low_edgs_cnt; id++) {
    ...
    for (id2 = 0; id2 < v->low_edgs_cnt; id2++) {
      ...
      if ((t > 0) || ((t == 0) && (ED_tail_port(e1->lowerN->node).p.x >
                                   ED_tail_port(e2->lowerN->node).p.x)))
...
static int out_cross(mcNode *v, mcNode *w) {
  ...
  for (id = 0; id < w->high_edgs_cnt; id++) {
    ...
    for (id2 = 0; id2 < v->high_edgs_cnt; id2++) {
      ...
      if ((t > 0) || ((t == 0) && (ED_head_port(e1->higherN->node).p.x >
                                   ED_head_port(e2->higherN->node).p.x)))
...
static int transpose_step(mcGraph *mcG, int r, int reverse) {
  ...
  for (i = 0; i < l->nodecnt - 1; i++) {
    ...
    if ((c1 < c0) || ((c0 > 0) && reverse && (c1 == c0))) {
      ...
      rv += (c0 - c1);
      l->candidate = TRUE;
      if (r > 0) mcG->lvls[r - 1]->candidate = TRUE;
      if (r < mcG->lvl_cnt - 1) mcG->lvls[r + 1]->candidate = TRUE;
    }
  }
...
static void _transpose(mcGraph *mcG, int reverse) {
  ...
  for (r = 0; r <= mcG->lvl_cnt - 1; r++) mcG->lvls[r]->candidate = TRUE;
  do {
    delta = 0;
#ifdef NOTDEF
...
static void fast_node(graph_t *g, Agnode_t *n) {
  ...
  if (ND_next(n)) ND_prev(ND_next(n)) = n;
  GD_nlist(g) = n;
  ND_prev(n) = NULL;
  assert(n != ND_next(n));
}
