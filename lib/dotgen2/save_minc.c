...
/* given:
 *		ND_level(v) 		int			integer level
 *assignments
 *		ND_something(v) int			number of levels (if >
 *1)
 ...
 *		GD_maxrank(g)		int
 *		GD_rank(g)			rank_t[]
 *		GD_skel(g)			vpath_t*	path representing g in
 *its parent
 *		ND_component(v) int				connected component
 *number
 */

*ND_vrep(user node) < ->ND_vrep(model node
                                : REAL) *
                          ND_erep(model node
                                  : XNODE) ==
    user edge *ND_erep(model node
                       : VNODE) == user edge *ND_grep(model node
                                                      : CNODE) ==
    user cluster * /
...
static void installglob(Agraph_t *user, Agraph_t *fromgraph, rank_t *globr,
                        int r);
...
static Agnode_t *model_extnode(Agraph_t *modelgraph, Agnode_t *orig,
                               Agedge_t *ue) {
  char name[1024];
  Agnode_t *rep;
...
static Agnode_t *model_cnode(Agraph_t *modelgraph, Agraph_t *uclust,
                             int level) {
  Agnode_t *rep;
  rep = agnode(modelgraph, 0);
  ND_rank(rep) = level;
...
static Agedge_t *model_vedge(Agraph_t *modelgraph, Agnode_t *tx, Agnode_t *hx,
                             Agedge_t *orig) {
  Agedge_t *e;
...
static Dtdisc_t Vpathdisc = {0,  /* pass whole object as key */
                             0,  /* key size and type */
                             -1, /* link offset */
                             (Dtmake_f)0, (Dtfree_f)0,   (Dtcompar_f)pathcmp,
...
static vpath_t *pathsearch(Agraph_t *modelgraph, Agnode_t *origlow,
                           Agnode_t *orighigh) {
  Dict_t *dict;
  vpath_t path, *p;
...
vpath_t *model_path(Agraph_t *modelgraph, Agnode_t *tv, Agnode_t *hv,
                    Agedge_t *orig) {
  vpath_t *p;
  ...
  if (!p) {
    ...
    for (i = p->low; i <= p->high; i++) {
      if (i == p->low)
        ...
        p->v[i] = model_vnode(modelgraph, orig, i);
      if (i > p->low)
  ...
  tx = tp.p.x;
  if (tx > PORTCLAMP) tx = PORTCLAMP;
  if (tx < -PORTCLAMP) tx = -PORTCLAMP;
  hx = hp.p.x;
  if (hx > PORTCLAMP) hx = PORTCLAMP;
  if (hx < -PORTCLAMP) hx = -PORTCLAMP;
...
static void rec_skel(Agraph_t *uclust) {
  ...
  if (GD_parent(uclust)) (void)model_skel(GD_model(GD_parent(uclust)), uclust);
  ...
  if (is_a_cluster(ug)) {
    ...
    for (n = agfstnode(ug); n; n = agnxtnode(ug, n)) {
      for (e = agfstedge(ug->root, n); e; e = agnxtedge(ug->root, e, n)) {
        ...
        if ((ND_type(tx) == NODETYPE_CNODE) &&
            (ND_type(hx) == NODETYPE_CNODE) &&
            (ND_cluster(tx) == ND_cluster(hx)))
          continue;
...
static void search_component(Agraph_t *g, Agnode_t *n, int c) {
  ...
  for (e = agfstout(g, n); e; e = agnxtout(g, e))
    if (ND_component(e->head) < 0) search_component(g, e->head, c);
  for (e = agfstin(g, n); e; e = agnxtin(g, e))
    if (ND_component(e->tail) < 0) search_component(g, e->tail, c);
}
...
static rank_t *globalize(Agraph_t *user, Agraph_t *topmodel) {
  ...
  /* installation */
  for (r = minr; r <= maxr; r++) installglob(user, topmodel, globrank, r);
...
static void installglob(Agraph_t *ug, Agraph_t *fromgraph, rank_t *globr,
                        int r) {
  rank_t *myrank;
  int i;
  Agnode_t *v;
...
void build_main_graph(Agraph_t *g) {
  ...
  for (v = agfstnode(g); v; v = agnxtnode(g, v)) {
    ...
    for (e = agfstout(v); e; e = agxntout(g, e)) {
      tail = e->tail;
      head = e->head;
      if (ND_rank(tail) > ND_rank(head)) {
        Agnode_t *t = head;
        head = tail;
        tail = t;
      }
      if (!(eprev = agfindedge(gprime, tail, head)))
        eprev = agedge(gprime, tail, head);
      merge(eprev, e);
      associate(e, eprev);
    }
  }
