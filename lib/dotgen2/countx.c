...
static int count_crossings(rank_t *north, rank_t *south) {
  ...
  for (k = 0; k < north->ne; k++) {/* insert edge k */
    index =
        ND_order(aghead(((Agedge_t *)(north->edgelist[k].data)))) + firstindex;
    north->tree[index]++;
    while (index > 0) {
...
int crossings_below(Agraph_t *g, int northlevel) {
  ...
  if (r->crossing_cache.valid) return r->crossing_cache.count;

  if (northlevel == GD_maxrank(g))
    c = 0;
  else {
    if (!r->edgelist) {/* set up edgelist */
      ...
      for (v = r->v; *v; v++)
        for (e = agfstout(g, *v); e; e = agnxtout(g, e)) {
          r->edgelist[c].key =
              (ND_order(agtail(e)) << 16) + ND_order(aghead(e));
          r->edgelist[c].data = e;
          c++;
        }
      r->ne = c;
