 ...
 *		GD_level				int			distance from
 *layout root
 *
 * model graph objects
 *		GD_usergraph(g)	graph*		original graph or one of
 *its clusters
 *		GD_minrank(g)		int				lowest rank
 *index incl. external edges
 *		GD_maxrank(g)		int				highest rank
 *index incl. external edges
 ...
 *internal nodes
 *		GD_rank(g)			rank_t[]
 *		GD_repdict(g)		Dict_t*		local representatives
 *of user objects
 *		ND_component(v) int				connected component
 *number
 *		ND_order(v)			int				order
 *within rank
...
static void model_edge(Agraph_t *model, Agedge_t *orig) {
  ...
  if (rep.type == 0) {
    ...
    high = orig->head;
    getlowhigh(&low, &high);
    u = association(model, low).p;
    assert(u);
    v = association(model, high).p;
    assert(v);
...
static rep_t model_clust(Agraph_t *model, Agraph_t *origclust) {
  ...
  rep.p = newpath(model, NILnode, GD_minrank(origclust), NILnode,
...
static void cluster_contents(Agraph_t *ug) {
  ...
  for (n = agfstnode(ug); n; n = agnxtnode(ug, n)) {
    for (e = agfstout(root, n); e; e = agnxtout(root, e)) {
      if (!agcontains(ug, e->tail)) model_extnode(mg, e->tail);
      if (!agcontains(ug, e->head)) model_extnode(mg, e->head);
    }
  }
