...
static int init_query_structure(int segnum, segment_t* seg, trap_t* tr,
                                qnode_t* qs) {
 ...
 */
static int locate_endpoint(pointf* v, pointf* vo, int r, segment_t* seg,
                           qnode_t* qs) {
  qnode_t* rptr = &qs[r];
 ...
 */
static void merge_trapezoids(int segnum, int tfirst, int tlast, int side,
                             trap_t* tr, qnode_t* qs) {
  int t, tnext, cond;
  int ptnext;
...
static int add_segment(int segnum, segment_t* seg, trap_t* tr, qnode_t* qs) {
          ...
          : !inserted(segnum, seg, FIRSTPT)) /* insert v0 in the tree */
  {
    ...
    if (((tmp_d = tr[tl].d0) > 0) && (tr[tmp_d].u0 == tu)) tr[tmp_d].u0 = tl;
    if (((tmp_d = tr[tl].d0) > 0) && (tr[tmp_d].u1 == tu)) tr[tmp_d].u1 = tl;

    if (((tmp_d = tr[tl].d1) > 0) && (tr[tmp_d].u0 == tu)) tr[tmp_d].u0 = tl;
    if (((tmp_d = tr[tl].d1) > 0) && (tr[tmp_d].u1 == tu)) tr[tmp_d].u1 = tl;
      ...
      /* traverse from top to bot */
  {
    ...
    if (_equal_to(&tr[t].lo, &tr[tlast].lo)) tlastr = tn;
    ...
    /* the sole lower trapezoid */
      ...
      if ((tr[t].u0 > 0) &&
          (tr[t].u1 > 0)) {  /* continuation of a chain from abv. */
      ...
      t = tr[t].d0;
               ...
               (tr[t].d1 > 0)) {/* Only one trapezoid below */
      if ((tr[t].u0 > 0) &&
          (tr[t].u1 > 0)) {  /* continuation of a chain from abv. */
        if (tr[t].usave > 0) /* three upper neighbours */
        {
 ...
 */
static void find_new_roots(int segnum, segment_t* seg, trap_t* tr,
                           qnode_t* qs) {
  segment_t* s = &seg[segnum];
...
int construct_trapezoids(int nseg, segment_t* seg, int* permute, int ntraps,
                         trap_t* tr) {
  int i;
  int root, h;
  int segi = 1;
  ...
  for (h = 1; h <= math_logstar_n(nseg); h++) {
    ...
    for (i = 1; i <= nseg; i++) find_new_roots(i, seg, tr, qs);
  }
