...
void gsave(sgraph* G) {
  ...
  for (i = 0; i < G->nnodes; i++) G->nodes[i].save_n_adj = G->nodes[i].n_adj;
}
...
void reset(sgraph* G) {
  ...
  for (i = 0; i < G->nnodes; i++) G->nodes[i].n_adj = G->nodes[i].save_n_adj;
  for (; i < G->nnodes + 2; i++) G->nodes[i].n_adj = 0;
}
