...
static void genSegments(cell* cells, int ncells, boxf bb, segment_t* seg,
                        int flip) {
  int j = 0, i = 1;
  pointf pts[4];
...
static int inside_polygon(trap_t* t, segment_t* seg) {
  ...
  if (t->state == ST_INVALID) return 0;
...
static int get_vertex_positions(int v0, int v1, int* ip, int* iq) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if ((temp = get_angle(&vp0->pt, &(vert[vp0->vnext[i]].pt), &vp1->pt)) >
        angle) {
      angle = temp;
      tp = i;
    }
  ...
  *ip = tp;
  ...
  for (i = 0; i < 4; i++) {
    ...
    if ((temp = get_angle(&vp1->pt, &(vert[vp1->vnext[i]].pt), &vp0->pt)) >
        angle) {
      angle = temp;
      tq = i;
    }
...
static int traverse_polygon(int* visited, boxf* decomp, int size,
                            segment_t* seg, trap_t* tr, int mcur, int trnum,
  ...
  if ((trnum <= 0) || visited[trnum]) return size;
  ...
  if ((t->u0 <= 0) && (t->u1 <= 0)) {
    if ((t->d0 > 0) && (t->d1 > 0)) /* downward opening triangle */
    {
      ...
      if (from == t->d1) {
        ...
        size = traverse_polygon(visited, decomp, size, seg, tr, mcur, t->d1,
                                trnum, flip, TR_FROM_UP);
        size = traverse_polygon(visited, decomp, size, seg, tr, mnew, t->d0,
                                trnum, flip, TR_FROM_UP);
      } else {
        mnew = make_new_monotone_poly(mcur, v0, v1);
        size = traverse_polygon(visited, decomp, size, seg, tr, mcur, t->d0,
                                trnum, flip, TR_FROM_UP);
        size = traverse_polygon(visited, decomp, size, seg, tr, mnew, t->d1,
                                trnum, flip, TR_FROM_UP);
      }
    } else {
      retval = SP_NOSPLIT; /* Just traverse all neighbours */
      size = traverse_polygon(visited, decomp, size, seg, tr, mcur, t->u0,
                              trnum, flip, TR_FROM_DN);
      size = traverse_polygon(visited, decomp, size, seg, tr, mcur, t->u1,
                              trnum, flip, TR_FROM_DN);
      size = traverse_polygon(visited, decomp, size, seg, tr, mcur, t->d0,
                              trnum, flip, TR_FROM_UP);
      size = traverse_polygon(visited, decomp, size, seg, tr, mcur, t->d1,
    ...
    if ((t->u0 > 0) && (t->u1 > 0)) /* upward opening triangle */
    {
      ...
      if (from == t->u1) {
        ...
        size = traverse_polygon(visited, decomp, size, seg, tr, mcur, t->u1,
                                trnum, flip, TR_FROM_DN);
        size = traverse_polygon(visited, decomp, size, seg, tr, mnew, t->u0,
                                trnum, flip, TR_FROM_DN);
      } else {
        mnew = make_new_monotone_poly(mcur, v0, v1);
        size = traverse_polygon(visited, decomp, size, seg, tr, mcur, t->u0,
                                trnum, flip, TR_FROM_DN);
        size = traverse_polygon(visited, decomp, size, seg, tr, mnew, t->u1,
                                trnum, flip, TR_FROM_DN);
      }
    } else {
      retval = SP_NOSPLIT; /* Just traverse all neighbours */
...
static int monotonate_trapezoids(int nsegs, segment_t* seg, trap_t* tr,
  ...
  if (tr[tr_start].u0 > 0)
    size = traverse_polygon(visited, decomp, 0, seg, tr, 0, tr_start,
                            tr[tr_start].u0, flip, TR_FROM_UP);
  else if (tr[tr_start].d0 > 0)
    size = traverse_polygon(visited, decomp, 0, seg, tr, 0, tr_start,
                            tr[tr_start].d0, flip, TR_FROM_DN);
...
static int rectIntersect(boxf* d, const boxf* r0, const boxf* r1) {
  ...
  if ((d->LL.x >= d->UR.x) || (d->LL.y >= d->UR.y)) return 0;

  return 1;
}

#ifdef DEBUG
static void dumpTrap(trap_t* tr, int n) {
  int i;
  for (i = 1; i <= n; i++) {
    tr++;
    fprintf(stderr, "%d : %d %d (%f,%f) (%f,%f) %d %d %d %d\n", i, tr->lseg,
            tr->rseg, tr->hi.x, tr->hi.y, tr->lo.x, tr->lo.y, tr->u0, tr->u1,
            tr->d0, tr->d1);
    fprintf(stderr, "    %d %d %d %d\n", tr->sink, tr->usave, tr->uside,
            tr->state);
  }
  fprintf(stderr, "====\n");
}

static void dumpSegs(segment_t* sg, int n) {
  int i;
  for (i = 1; i <= n; i++) {
    sg++;
    fprintf(stderr, "%d : (%f,%f) (%f,%f) %d %d %d %d %d\n", i, sg->v0.x,
            sg->v0.y, sg->v1.x, sg->v1.y, sg->is_inserted, sg->root0, sg->root1,
            sg->next, sg->prev);
  }
  fprintf(stderr, "====\n");
}
...
for (i = 1; i<= nsegs; i++) {
  ...
  for (i = 0; i < vd_size; i++)
    for (j = 0; j < hd_size; j++)
      if (rectIntersect(&rs[cnt], &vert_decomp[i], &hor_decomp[j])) cnt++;
