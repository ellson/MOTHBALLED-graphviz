...
static void emitGraph(FILE* fp, maze* mp, int n_edges, route* route_list,
                      epair_t[]);
int odb_flags;
#endif
 ...
 * Assume b1 != b2
 */
static void setSeg(segment* sp, int dir, double fix, double b1, double b2,
                   int l1, int l2) {
  sp->isVert = dir;
  sp->comm_coord = fix;
  if (b1 < b2) {
...
static route convertSPtoRoute(sgraph* g, snode* fst, snode* lst) {
  ...
  for (i = 0; i < rte.n; i++) {
    if (i > 0) rte.segs[i].prev = rte.segs + (i - 1);
    if (i < rte.n - 1) rte.segs[i].next = rte.segs + (i + 1);
  }
...
static Dtdisc_t chanDisc = {
    offsetof(channel, p), sizeof(paird),         offsetof(channel, link),
    0,                    (Dtfree_f)freeChannel, (Dtcompar_f)chancmpid,
    0,                    0,                     0};

static Dtdisc_t chanItemDisc = {
    offsetof(chanItem, v), sizeof(double),         offsetof(chanItem, link),
    0,                     (Dtfree_f)freeChanItem, (Dtcompar_f)dcmpid,
    0,                     0,                      0};
...
/* addLoop:
 * Add two temporary nodes to sgraph corresponding to two ends of a loop at cell
 * cp, i
...
static int overlapSeg(segment* S1, segment* S2, bend T1, bend T2) {
  if (S1->p.p2 < S2->p.p2) {
    if (S1->l2 == T1 && S2->l1 == T2)
      return (-1);
    else if (S1->l2 == T2 && S2->l1 == T1)
      return (1);
    else
      return (0);
    ...
    if (S2->l1 == T2 && S2->l2 == T2)
      return (-1);
    else if (S2->l1 == T1 && S2->l2 == T1)
      return (1);
    else
      return (0);
  }
    ...
    if (S1->p.p2 == S2->p.p2) {
      if ((S1->l1 == S2->l1) && (S1->l2 == S2->l2))
        ...
        if (S2->l1 == T1)
          ...
          return (-1);
        else if ((S1->l1 != T1) && (S1->l2 != T1))
          return (1);
        else if ((S1->l1 != T2) && (S1->l2 != T2))
          return (-1);
        else
          return 0;
      } else if ((S2->l1 == T1) && (S2->l2 == T2)) {
        if ((S1->l1 != T1) && (S1->l2 == T2))
          return 1;
        else if ((S1->l1 == T1) && (S1->l2 != T2))
          return -1;
        else
          return 0;
      } else if ((S2->l2 == T1) && (S2->l1 == T2)) {
        if ((S1->l2 != T1) && (S1->l1 == T2))
          return 1;
        else if ((S1->l2 == T1) && (S1->l1 != T2))
          return -1;
        else
          return 0;
 ...
 */
static void set_parallel_edges(segment* seg1, segment* seg2, int dir1, int dir2,
                               int hops, maze* mp) {
  int x;
  channel* chan;
  channel* nchan;
        ...
        if (is_parallel(segs[i], segs[j])) {
          ...
          switch (prec1) {
            case -1:
              ...
              if (prec2 == 1) removeEdge(segs[i], segs[j], 1 - dir, mp);
              break;
            case 0:
              switch (prec2) {
                case -1:
...
static void attachOrthoEdges(Agraph_t* g, maze* mp, int n_edges,
                             route* route_list, splineInfo* sinfo, epair_t es[],
                             int doLbls) {
  int irte = 0;
  int i, ipt, npts;
  pointf* ispline = 0;
  ...
  for (; irte < n_edges; irte++) {
    ...
    if (doLbls && (lbl = ED_label(e)) && !lbl->set) addEdgeLabels(g, e, p1, q1);
  }
  free(ispline);
}
...
static int edgecmp(epair_t* e0, epair_t* e1) { return (e0->d - e1->d); }
...
void orthoEdges(Agraph_t* g, int doLbls) {
  ...
  if (doLbls) {
    agerr(AGWARN,
          "Orthogonal edges do not currently handle edge labels. Try using "
          "xlabels.\n");
    doLbls = 0;
  }
  mp = mkMaze(g, doLbls);
  ...
  if (setjmp(jbuf)) goto orthofinish;
...
orthofinish:
  if (Concentrate) freePS(ps);

  for (i = 0; i < n_edges; i++) free(route_list[i].segs);
  free(route_list);
  freeMaze(mp);
  free(es);
}
...
static boxf emitEdge(FILE* fp, Agedge_t* e, route rte, maze* m, int ix,
                     boxf bb) {
  int i, x, y;
