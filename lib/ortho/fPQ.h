...
static void PQupdate(snode* n, int d) {
  N_VAL(n) = d;
  PQupheap(n->n_idx);
  PQcheck();
}
...
static void PQprint() {
  ...
  for (i = 1; i <= PQcnt; i++) {
    n = pq[i];
    fprintf(stderr, "%s(%d:%d) ", n->index, N_IDX(n), N_VAL(n));
  }
  fprintf(stderr, "\n");
}
#else
