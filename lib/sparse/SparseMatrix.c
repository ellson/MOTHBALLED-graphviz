...
SparseMatrix SparseMatrix_transpose(SparseMatrix A) {
  int *ia = A->ia, *ja = A->ja, *ib, *jb, nz = A->nz, m = A->m, n = A->n,
      type = A->type, format = A->format;
  SparseMatrix B;
  int i, j;
...
int SparseMatrix_is_symmetric(SparseMatrix A, int test_pattern_symmetry_only) {
  ...
  if (test_pattern_symmetry_only && SparseMatrix_known_strucural_symmetric(A))
    return TRUE;
  ...
  switch (type) {
    ...
    case MATRIX_TYPE_COMPLEX: {
      ...
      for (i = 0; i < m; i++) {
        ...
        for (j = ib[i]; j < ib[i + 1]; j++) {
          if (ABS(b[2 * j] - a[2 * mask[jb[j]]]) > SYMMETRY_EPSILON)
            goto RETURN;
          if (ABS(b[2 * j + 1] - a[2 * mask[jb[j]] + 1]) > SYMMETRY_EPSILON)
            goto RETURN;
        }
      }
      res = TRUE;
...
static SparseMatrix SparseMatrix_init(int m, int n, int type, size_t sz,
                                      int format) {
  SparseMatrix A;
...
static SparseMatrix SparseMatrix_alloc(SparseMatrix A, int nz) {
  int format = A->format;
  size_t nz_t = (size_t)nz; /* size_t is 64 bit on 64 bit machine. Using
                               nz*A->size can overflow. */
...
SparseMatrix SparseMatrix_new(int m, int n, int nz, int type, int format) {
  /* return a sparse matrix skeleton with row dimension m and storage nz. If nz
     == 0,
...
SparseMatrix SparseMatrix_general_new(int m, int n, int nz, int type, size_t sz,
  ...
  /* return a sparse matrix skeleton with row dimension m and storage nz. If nz
     == 0,
     only row pointers are allocated. this is more general and allow elements to
     be
...
SparseMatrix SparseMatrix_from_coordinate_format(SparseMatrix A) {
  ...
  irn = A->ia;
  jcn = A->ja;
  return SparseMatrix_from_coordinate_arrays(A->nz, A->m, A->n, irn, jcn, a,
                                             A->type, A->size);
}
SparseMatrix SparseMatrix_from_coordinate_format_not_compacted(
    SparseMatrix A, int what_to_sum) {
  /* convert a sparse matrix in coordinate form to one in compressed row form.*/
  int *irn, *jcn;
  ...
  return SparseMatrix_from_coordinate_arrays_not_compacted(
      A->nz, A->m, A->n, irn, jcn, a, A->type, A->size, what_to_sum);
}

static SparseMatrix SparseMatrix_from_coordinate_arrays_internal(
    int nz, int m, int n, int *irn, int *jcn, void *val0, int type, size_t sz,
    int sum_repeated) {
  /* convert a sparse matrix in coordinate form to one in compressed row form.
...
SparseMatrix SparseMatrix_from_coordinate_arrays(int nz, int m, int n, int *irn,
                                                 int *jcn, void *val0, int type,
                                                 size_t sz) {
  return SparseMatrix_from_coordinate_arrays_internal(
      nz, m, n, irn, jcn, val0, type, sz, SUM_REPEATED_ALL);
}

SparseMatrix SparseMatrix_from_coordinate_arrays_not_compacted(
    int nz, int m, int n, int *irn, int *jcn, void *val0, int type, size_t sz,
    int what_to_sum) {
  return SparseMatrix_from_coordinate_arrays_internal(nz, m, n, irn, jcn, val0,
                                                      type, sz, what_to_sum);
}
...
SparseMatrix SparseMatrix_add(SparseMatrix A, SparseMatrix B) {
  ...
  assert(A->format == B->format &&
         A->format == FORMAT_CSR); /* other format not yet supported */
...
static void dense_transpose(real *v, int m, int n) {
  /* transpose an m X n matrix in place. Well, we do no really do it without
   * xtra memory. This is possibe, but too complicated for ow */
  int i, j;
  real *u;
...
static void SparseMatrix_multiply_dense1(SparseMatrix A, real *v, real **res,
                                         int dim, int transposed,
                                         int res_transposed) {
  /* A v or A^T v where v a dense matrix of second dimension dim. Real only for
   * now. */
  int i, j, k, *ia, *ja, n, m;
  real *a, *u;
...
static void SparseMatrix_multiply_dense2(SparseMatrix A, real *v, real **res,
                                         int dim, int transposed,
                                         int res_transposed) {
  /* A v^T or A^T v^T where v a dense matrix of second dimension n or m. Real
     only for now.
     transposed = FALSE: A*V^T, with A dimension m x n, V dimension dim x n,
     v[i*n+j] gives V[i,j]. Result of dimension m x dim
     transposed = TRUE: A^T*V^T, with A dimension m x n, V dimension dim x m.
     v[i*m+j] gives V[i,j]. Result of dimension n x dim
  */
  real *u, *rr;
  int i, m, n;
  assert(A->format == FORMAT_CSR);
...
void SparseMatrix_multiply_dense(SparseMatrix A, int ATransposed, real *v,
                                 int vTransposed, real **res,
                                 int res_transposed, int dim) {
  /* depend on value of {ATranspose, vTransposed}, assume res_transposed ==
     FALSE
     {FALSE, FALSE}: A * V, with A dimension m x n, with V of dimension n x dim.
     v[i*dim+j] gives V[i,j]. Result of dimension m x dim
     {TRUE, FALSE}: A^T * V, with A dimension m x n, with V of dimension m x
     dim. v[i*dim+j] gives V[i,j]. Result of dimension n x dim
     {FALSE, TRUE}: A*V^T, with A dimension m x n, V dimension dim x n, v[i*n+j]
     gives V[i,j]. Result of dimension m x dim
     {TRUE, TRUE}: A^T*V^T, with A dimension m x n, V dimension dim x m.
     v[i*m+j] gives V[i,j]. Result of dimension n x dim

     furthermore, if res_transpose d== TRUE, then the result is transposed.
     Hence if res_transposed == TRUE

     {FALSE, FALSE}: V^T A^T, with A dimension m x n, with V of dimension n x
     dim. v[i*dim+j] gives V[i,j]. Result of dimension dim x dim
     {TRUE, FALSE}: V^T A, with A dimension m x n, with V of dimension m x dim.
     v[i*dim+j] gives V[i,j]. Result of dimension dim x n
     {FALSE, TRUE}: V*A^T, with A dimension m x n, V dimension dim x n, v[i*n+j]
     gives V[i,j]. Result of dimension dim x m
     {TRUE, TRUE}: V A, with A dimension m x n, V dimension dim x m. v[i*m+j]
     gives V[i,j]. Result of dimension dim x n
 */
...
#ifdef DEBUG
    printf(
        "in SparseMatrix_multiply, the matrix types do not match, right now "
        "only multiplication of matrices of the same type is supported\n");
#endif
    return NULL;
  }
  type = A->type;
        ...
        for (j = ia[i]; j < ia[i + 1]; j++) {
          ...
          for (k = ib[jj]; k < ib[jj + 1]; k++) {
            if (mask[jb[k]] < ic[i]) {
              ...
              jc[nz] = jb[k];
              c[2 * nz] = a[2 * j] * b[2 * k] -
                          a[2 * j + 1] * b[2 * k + 1]; /*real part */
              c[2 * nz + 1] = a[2 * j] * b[2 * k + 1] +
                              a[2 * j + 1] * b[2 * k]; /*img part */
              nz++;
            } else {
              assert(jc[mask[jb[k]]] == jb[k]);
              c[2 * mask[jb[k]]] += a[2 * j] * b[2 * k] -
                                    a[2 * j + 1] * b[2 * k + 1]; /*real part */
              c[2 * mask[jb[k]] + 1] += a[2 * j] * b[2 * k + 1] +
                                        a[2 * j + 1] * b[2 * k]; /*img part */
            }
          }
        }
...
SparseMatrix SparseMatrix_multiply3(SparseMatrix A, SparseMatrix B,
  ...
  SparseMatrix D = NULL;
  int *mask = NULL;
  int *ia = A->ia, *ja = A->ja, *ib = B->ia, *jb = B->ja, *ic = C->ia,
      *jc = C->ja, *id, *jd;
          ...
          for (l = ib[jj]; l < ib[jj + 1]; l++) {
            ...
            for (k = ic[ll]; k < ic[ll + 1]; k++) {
              if (mask[jc[k]] < id[i]) {
                ...
                jd[nz] = jc[k];
                d[2 * nz] =
                    (a[2 * j] * b[2 * l] - a[2 * j + 1] * b[2 * l + 1]) *
                        c[2 * k] -
                    (a[2 * j] * b[2 * l + 1] + a[2 * j + 1] * b[2 * l]) *
                        c[2 * k + 1]; /*real part */
                d[2 * nz + 1] =
                    (a[2 * j] * b[2 * l + 1] + a[2 * j + 1] * b[2 * l]) *
                        c[2 * k] +
...
/* For complex matrix:
   ...
   if what_to_sum = SUM_REPEATED_REAL_PART, we find entries {i,j,x + i y} and
   sum the y's if {i,j,Round(x)} are the same
   so a matrix like {{1,1,2+3i},{1,2,3+4i},{1,1,5+3i},{1,2,4+5i},{1,2,4+4i}}
   becomes
...
SparseMatrix SparseMatrix_sum_repeat_entries(SparseMatrix A, int what_to_sum) {
  /* sum repeated entries in the same row, i.e., {1,1}->1, {1,1}->2 becomes
   * {1,1}->3 */
...
SparseMatrix SparseMatrix_coordinate_form_add_entries(SparseMatrix A,
  ...
  MEMCPY((char *)A->ia + ((size_t)nz) * sizeof(int) / sizeof(char), irn,
         sizeof(int) * ((size_t)nentries));
  MEMCPY((char *)A->ja + ((size_t)nz) * sizeof(int) / sizeof(char), jcn,
         sizeof(int) * ((size_t)nentries));
  if (A->size)
    MEMCPY((char *)A->a + ((size_t)nz) * A->size / sizeof(char), val,
           A->size * ((size_t)nentries));
...
SparseMatrix SparseMatrix_divide_row_by_degree(SparseMatrix A) {
  ...
  switch (A->type) {
    ...
    case MATRIX_TYPE_PATTERN: { break; }
    case MATRIX_TYPE_UNKNOWN:
      return NULL;
    default:
      return NULL;
...
SparseMatrix SparseMatrix_to_complex(SparseMatrix A) {
  ...
  switch (A->type) {
    ...
    case MATRIX_TYPE_COMPLEX: { break; }
    case MATRIX_TYPE_INTEGER: {
      int *a = (int *)A->a;
      int nz = A->nz;
...
SparseMatrix SparseMatrix_apply_fun_general(SparseMatrix A,
  ...
  if (!A) return A;
  if (A->format != FORMAT_CSR ||
      (A->type != MATRIX_TYPE_REAL && A->type != MATRIX_TYPE_COMPLEX)) {
#ifdef DEBUG
    printf(
        "SparseMatrix_apply_fun: only CSR and real/complex matrix "
        "supported.\n");
#endif
    return A;
  }
...
SparseMatrix SparseMatrix_crop(SparseMatrix A, real epsilon) {
  ...
  switch (A->type) {
    ...
    case MATRIX_TYPE_COMPLEX: {
      ...
      for (i = 0; i < A->m; i++) {
        for (j = sta; j < ia[i + 1]; j++) {
          if (sqrt(a[2 * j] * a[2 * j] + a[2 * j + 1] * a[2 * j + 1]) >
              epsilon) {
            ja[nz] = ja[j];
            a[2 * nz] = a[2 * j];
            a[2 * nz + 1] = a[2 * j + 1];
...
void SparseMatrix_level_sets_internal(int khops, SparseMatrix A, int root,
  ...
  /* mask is assumed to be initialized to negative if provided.
     . On exit, mask = levels for visited nodes (1 for root, 2 for its
     neighbors, etc),
...
void SparseMatrix_level_sets(SparseMatrix A, int root, int *nlevel,
  ...
  return SparseMatrix_level_sets_internal(khops, A, root, nlevel, levelset_ptr,
                                          levelset, mask, reinitialize_mask);
}
...
void SparseMatrix_weakly_connected_components(SparseMatrix A0, int *ncomp,
  ...
  *ncomp = 0;
  ...
  for (i = 0; i < m; i++) {
    if (i == 0 || mask[i] < 0) {
      SparseMatrix_level_sets(A, i, &nlevel, &levelset_ptr, &levelset, &mask,
                              FALSE);
...
static int Dijkstra_internal(SparseMatrix A, int root, real *dist, int *nlist,
                             int *list, real *dist_max, int *mask) {
  /* Find the shortest path distance of all nodes to root. If khops >= 0, the
     shortest ath is of distance <= khops,

     A: the nxn connectivity matrix. Entries are assumed to be nonnegative.
     Absolute value will be taken if
     .  entry value is negative.
     dist: length n. On on exit contain the distance from root to every other
     node. dist[root] = 0. dist[i] = distance from root to node i.
     .     if the graph is disconnetced, unreachable node have a distance -1.
     .     note: ||root - list[i]|| =!= dist[i] !!!, instead, ||root - list[i]||
     == dist[list[i]]
     nlist: number of nodes visited
     list: length n. the list of node in order of their extraction from the
     heap.
     ...
     mask: if NULL, not used. Othewise, only nodes i with mask[i] > 0 will be
     considered
  ...
  while ((ndata_min = BinaryHeap_extract_min(h))) {
    ...
    for (j = ia[i]; j < ia[i + 1]; j++) {
      ...
      if (heap_id == UNVISITED) {
        ...
        // fprintf(stderr," set neighbor id=%d, dist=%f, hid = %d, a[%d]=%f,
        ...
        // fprintf(stderr," reset neighbor id=%d, dist=%f, hid = %d, a[%d]=%f,
...
static int Dijkstra(SparseMatrix A, int root, real *dist, int *nlist, int *list,
                    real *dist_max) {
  return Dijkstra_internal(A, root, dist, nlist, list, dist_max, NULL);
}

static int Dijkstra_masked(SparseMatrix A, int root, real *dist, int *nlist,
                           int *list, real *dist_max, int *mask) {
  /* this makes the algorithm only consider nodes that are masked.
     nodes are masked as 1, 2, ..., mask_max, which is (the number of hops from
     root)+1.
...
real SparseMatrix_pseudo_diameter_weighted(SparseMatrix A0, int root,
                                           ...
                                           int *connectedQ) {
  /* weighted graph. But still assume to be undirected. unsymmetric matrix ill
   * be symmetrized */
  SparseMatrix A = A0;
  ...
  *end2 = list[nlist - 1];
  ...
  if (aggressive) {
    ...
    for (i = 0; i < iroots; i++) {
      ...
      dist_max = SparseMatrix_pseudo_diameter_weighted(A, root, FALSE, &end11,
                                                       &end22, connectedQ);
      if (dist_max > dist0) {
        *end1 = end11;
        *end2 = end22;
...
real SparseMatrix_pseudo_diameter_unweighted(SparseMatrix A0, int root,
                                             int aggressive, int *end1,
                                             int *end2, int *connectedQ) {
  ...
  SparseMatrix_level_sets(A, root, &nlevel, &levelset_ptr, &levelset, &mask,
                          TRUE);
  //  fprintf(stderr,"after level set, {%d,%d}=%d\n",levelset[0],
  // levelset[levelset_ptr[nlevel]-1], nlevel);
  ...
  *end2 = levelset[levelset_ptr[nlevel] - 1];
  ...
  if (aggressive) {
    nlevel0 = nlevel;
    iroots = 0;
    for (i = levelset_ptr[nlevel - 1];
         i < MIN(levelset_ptr[nlevel], levelset_ptr[nlevel - 1] + 5); i++) {
      iroots++;
      roots[i - levelset_ptr[nlevel - 1]] = levelset[i];
    }
    for (i = 0; i < iroots; i++) {
      root = roots[i];
      nlevel = (int)SparseMatrix_pseudo_diameter_unweighted(
          A, root, FALSE, &enda, &endb, connectedQ);
      if (nlevel > nlevel0) {
        nlevel0 = nlevel;
        *end1 = enda;
        *end2 = endb;
...
real SparseMatrix_pseudo_diameter_only(SparseMatrix A) {
  ...
  return SparseMatrix_pseudo_diameter_unweighted(A, 0, FALSE, &end1, &end2,
                                                 &connectedQ);
}

int SparseMatrix_connectedQ(SparseMatrix A0) {
  int root = 0, nlevel, *levelset_ptr = NULL, *levelset = NULL, *mask = NULL,
      connected;
  SparseMatrix A = A0;
...
void SparseMatrix_decompose_to_supervariables(SparseMatrix A, int *ncluster,
  ...
  /* nodes for a super variable if they share exactly the same neighbors. This
     is know as modules in graph theory.
     We work on columns only and columns with the same pattern are grouped as a
     super variable
   */
  ...
  for (i = 0; i < m; i++) {
    ...
    for (j = ia[i]; j < ia[i + 1]; j++) {
      ...
      if (mask[isuper] < i) {
        mask[isuper] = i;
        if (nsuper[isuper] ==
            0) {/* all nodes in the isuper group exist in this row */
#ifdef DEBUG_PRINT1
...
SparseMatrix SparseMatrix_get_augmented(SparseMatrix A) {
  ...
  if (A->a) {
    ...
    MEMCPY((void *)(((char *)val) + ((size_t)nz) * A->size), A->a,
           A->size * ((size_t)nz));
  }
...
SparseMatrix SparseMatrix_get_submatrix(SparseMatrix A, int nrow, int ncol,
  ...
  /* get the submatrix from row/columns indices[0,...,l-1].
     ...
     if rindices = NULL, it is assume that 1 -- nrow is needed. Same for
     cindices/ncol.
   */
...
SparseMatrix SparseMatrix_largest_component(SparseMatrix A) {
  ...
  if (ncomp == 1) {
    ...
    B = SparseMatrix_get_submatrix(A, nmax, nmax, &comps[comps_ptr[imax]],
                                   &comps[comps_ptr[imax]]);
  }
  FREE(comps);
  FREE(comps_ptr);
  return B;
...
SparseMatrix SparseMatrix_delete_sparse_columns(SparseMatrix A, int threshold,
                                                ...
                                                int inplace) {
  /* delete sparse columns of threshold or less entries in A. After than number
     of columns will be nnew, and
...
SparseMatrix SparseMatrix_complement(SparseMatrix A, int undirected) {
   ...
   * E(A). Only structural matrix is returned. */
  SparseMatrix B = A;
  int *ia, *ja;
  int m = A->m, n = A->n;
  int *mask, nz = 0;
  ...
  B = SparseMatrix_from_coordinate_arrays(nz, m, n, irn, jcn, NULL,
                                          MATRIX_TYPE_PATTERN, 0);
  FREE(irn);
  FREE(jcn);
  return B;
}

int SparseMatrix_k_centers(SparseMatrix D0, int weighted, int K, int root,
                           int **centers, int centering, real **dist0) {
  /*
    Input:
    ...
    centering: whether the distance should be centered so that sum_k dist[n*k+i]
    = 0
    Output:
    centers: the list of nodes that form the k-centers. If centers = NULL on
    input, it will be allocated.
    dist: of dimension k*n, dist[k*n: (k+1)*n) gives the distance of every node
    to the k-th center.
  ...
  */
  ...
  if (!weighted) {
    ...
    SparseMatrix_pseudo_diameter_unweighted(D, root, aggressive, &end1, &end2,
                                            &connectedQ);
    ...
    for (k = 0; k < K; k++) {
      ...
      SparseMatrix_level_sets(D, root, &nlevel, &levelset_ptr, &levelset, &mask,
                              TRUE);
      ...
      root = 0;
      for (i = 0; i < n; i++) {
        if (!check_connected && dist_min[i] < 0)
          continue; /* if the graph is disconnected, then we can not count on
                       every node to be in level set.
                       Usee dist_min<0 to identify those not in level set */
        if (dmax < dist_min[i] ||
            (dmax == dist_min[i] &&
             dsum < dist_sum[i])) {/* tie break with avg dist */
          dmax = dist_min[i];
          dsum = dist_sum[i];
          root = i;
        }
    ...
    SparseMatrix_pseudo_diameter_weighted(D, root, aggressive, &end1, &end2,
                                          &connectedQ);
...
int SparseMatrix_k_centers_user(SparseMatrix D0, int weighted, int K,
                                int *centers_user, int centering,
                                real **dist0) {
  /*
    Input:
    D: the graph. If weighted, the entry values is used.
...
SparseMatrix SparseMatrix_from_dense(int m, int n, real *x) {
  /* wrap a mxn matrix into a sparse matrix. the {i,j} entry of the matrix is in
   * x[i*n+j], 0<=i<m; 0<=j<n */
  int i, j, *ja;
  real *a;
...
int SparseMatrix_distance_matrix(SparseMatrix D0, int weighted, real **dist0) {
  ...
  */
  ...
  if (!weighted) {
    for (k = 0; k < n; k++) {
      SparseMatrix_level_sets(D, k, &nlevel, &levelset_ptr, &levelset, &mask,
                              TRUE);
      assert(levelset_ptr[nlevel] == n);
      for (i = 0; i < nlevel; i++) {
        for (j = levelset_ptr[i]; j < levelset_ptr[i + 1]; j++) {
...
SparseMatrix SparseMatrix_distance_matrix_k_centers(int K, SparseMatrix D,
                                                    int weighted) {
  /* return a sparse matrix whichj represent teh k-center and distance from
     every node to them.
...
SparseMatrix SparseMatrix_distance_matrix_khops(int khops, SparseMatrix D0,
                                                int weighted) {
  /*
    Input:
    khops: number of hops allow. If khops < 0, this will give a dense distances.
    Otherwise it gives a sparse matrix that represent the k-neighborhood graph
    D: the graph. If weighted, the entry values is used.
    weighted: whether to treat the graph as weighted
    Output:
    DD: of dimension nxn. DD[i,j] gives the shortest path distance, subject to
    the fact that the short oath must be of <= khops.
    return: flag. if not zero, the graph is not connected, or out of memory.
  */
  ...
  if (!weighted) {
    for (k = 0; k < n; k++) {
      SparseMatrix_level_sets_khops(khops, D, k, &nlevel, &levelset_ptr,
                                    &levelset, &mask, TRUE);
      for (i = 0; i < nlevel; i++) {
        for (j = levelset_ptr[i]; j < levelset_ptr[i + 1]; j++) {
          itmp = levelset[j];
          dtmp = i;
          if (k != itmp)
            B = SparseMatrix_coordinate_form_add_entries(B, 1, &k, &itmp,
                                                         &dtmp);
        }
      }
    }
    ...
    */
    ...
    for (k = 0; k < n; k++) {
      ...
      for (j = 0; j < nlist; j++) {
        ...
        dtmp = dist[itmp];
        if (k != itmp)
          B = SparseMatrix_coordinate_form_add_entries(B, 1, &k, &itmp, &dtmp);
      }
    }
  }
  ...
  if (D != D0) SparseMatrix_delete(D);
  if (list) FREE(list);
  /* I can not find a reliable way to make the matrix symmetric. Right now I use
     a mask array to
     limit consider of only nodes with in k hops, but even this is not
     symmetric. e.g.,
     . 10  10    10  10
     .A---B---C----D----E
     .           2 |    | 2
     .             G----F
     .               2
     If we set hops = 4, and from A, it can not see F (which is 5 hops), hence
     distance(A,E) =40
     but from E, it can see all nodes (all within 4 hops), so distance(E, A)=36.
     .
     may be there is a better way to ensure symmetric, but for now we just
     symmetrize it
  */
...
void SparseMatrix_kcore_decomposition(SparseMatrix A, int *coreness_max0,
  ...
  /* give an undirected graph A, find the k-coreness of each vertex
     ...
     coreness_ptr: array of size (coreness_max + 2), element [corness_ptr[i],
     corness_ptr[i+1])
  ...
  */
  ...
  while (PriorityQueue_pop(pq, &k, &gain)) {
    ...
    coreness_list[nz++] = k;
    mask[k] = coreness_now;
    // fprintf(stderr,"=== \nremove node %d with gain %d, mask with %d,
  ...
  if (Verbose) {
    for (i = 0; i <= coreness_now; i++) {
      if (coreness_ptr[i + 1] - coreness_ptr[i] > 0) {
        fprintf(stderr, "num_in_core[%d] = %d: ", i,
                coreness_ptr[i + 1] - coreness_ptr[i]);
#if 0
...
void SparseMatrix_kcoreness(SparseMatrix A, int **coreness) {
  ...
  SparseMatrix_kcore_decomposition(A, &coreness_max, &coreness_ptr,
                                   &coreness_list);
...
void SparseMatrix_khair_decomposition(SparseMatrix A, int *hairness_max0,
                                      int **hairness_ptr0,
                                      int **hairness_list0) {
  /* define k-hair as the largest subgraph of the graph such that the degree of
     each node is <=k.
     Give an undirected graph A, find the k-hairness of each vertex
     A: a graph. Will be made undirected and self loop removed
     hairness_max: max hair number.
     hairness_ptr: array of size (hairness_max + 2), element [corness_ptr[i],
     corness_ptr[i+1])
  ...
  while (PriorityQueue_pop(pq, &k, &gain)) {
    ...
    if (deg < deg_old) {
      ...
      for (jj = hairness_ptr[deg_old]; jj < hairness_ptr[deg_old + 1]; jj++) {
        l = hairness_list[jj];
        // fprintf(stderr,"=== \nremove node hairness_list[%d]= %d, mask with
        // %d, nelement=%d\n",jj, l, deg_old, pq->count);
        for (j = ia[l]; j < ia[l + 1]; j++) {
          neighb = ja[j];
          if (neighb == k)
            deg--; /* k was masked. But we do need to update ts degree */
          if (mask[neighb] < 0) {
            gain = PriorityQueue_get_gain(pq, neighb);
            // fprintf(stderr,"update node %d with deg %d,
      ...
      mask[k] = 0; /* because a bunch of nodes are removed, k may not be the
                      best node! Unmask */
  ...
  if (Verbose) {
    for (i = 0; i <= deg_max; i++) {
      if (hairness_ptr[i + 1] - hairness_ptr[i] > 0) {
        fprintf(stderr, "num_in_hair[%d] = %d: ", i,
                hairness_ptr[i + 1] - hairness_ptr[i]);
#if 0
...
void SparseMatrix_khairness(SparseMatrix A, int **hairness) {
  ...
  SparseMatrix_khair_decomposition(A, &hairness_max, &hairness_ptr,
                                   &hairness_list);
...
void SparseMatrix_page_rank(SparseMatrix A, real teleport_probablity,
  ...
  /* A(i,j)/Sum_k A(i,k) gives the probablity of the random surfer walking from
     i to j
     A: n x n  square matrix
     weighted: whether to use the wedge weights (matrix entries)
     page_rank: array of length n. If *page_rank was null on entry, will be
     assigned.
