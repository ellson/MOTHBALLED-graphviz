...
/* take m elements v[p[i]]],i=1,...,m and oput in u. u will be assigned if *u =
 * NULL */
void vector_take(int n, real *v, int m, int *p, real **u);
void vector_float_take(int n, float *v, int m, int *p, float **u);

/* give the position of the lagest, second largest etc in vector v if ascending
   = TRUE
   or
   give the position of the smallest, second smallest etc  in vector v if
   ascending = TRUE.
...
real vector_percentile(int n, real *x,
                       real y); /* find the value such that y% of element of
                                   vector x is <= that value.*/
...
void scale_to_box(real xmin, real ymin, real xmax, real ymax, int n, int dim,
                  real *x);

/* check to see if this is a string is integer (that can be casted into an
 * integer variable hence very long list of digits are not valid, like
 * 123456789012345. Return 1 if true, 0 if false. */
