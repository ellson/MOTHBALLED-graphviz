...
static Multilevel_Modularity_Clustering Multilevel_Modularity_Clustering_init(
    SparseMatrix A, int level) {
...
static void Multilevel_Modularity_Clustering_delete(
    Multilevel_Modularity_Clustering grid) {
  if (!grid) return;
  if (grid->A) {
    if (grid->level == 0) {
...
static Multilevel_Modularity_Clustering
Multilevel_Modularity_Clustering_establish(
    Multilevel_Modularity_Clustering grid, int ncluster_target) {
  ...
  */
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = ia[i]; j < ia[i + 1]; j++) {
      ...
      if (jj == i) continue;
      if ((jc = matching[jj]) == UNMATCHED) {
        /* the first 2 is due to the fact that deg_iter gives edge weight from i
         * to jj, but there are also edges from jj to i */
        gain =
            (2 * a[j] - 2 * deg[i] * deg[jj] * inv_deg_total) * inv_deg_total;
      } else {
        if (deg_inter[jc] > 0) {
          /* the first 2 is due to the fact that deg_iter gives edge weight from
           * i to jc, but there are also edges from jc to i */
          gain =
              (2 * deg_inter[jc] - 2 * deg[i] * deg_new[jc] * inv_deg_total) *
              inv_deg_total;
          //	  printf("mod = %f deg_inter[jc] =%f, deg[i] = %f,
          //deg_new[jc]=%f, gain = %f\n",modularity,
          //deg_inter[jc],deg[i],deg_new[jc],gain);
          deg_inter[jc] = -1; /* so that we do not redo the calulation when we
                                 hit another neighbor in cluster jc */
        } else {
          gain = -1;
        }
      }
    ...
    if (maxgain > 0 || grid->agglomerate_regardless) {
      ...
      if (jc == UNMATCHED) {
        ...
        // fprintf(stderr, "maxgain=%f, merge with existing cluster %d,
        // %d\n",maxgain, i, jc);
        deg_new[jc] += deg[i];
        matching[i] = jc;
      }
  ...
  if (Verbose)
    fprintf(stderr,
            "modularity = %f new modularity = %f level = %d, n = %d, nc = %d, "
            "gain = %g\n",
            modularity, modularity + total_gain, level, n, nc, total_gain);
  ...
  if (ncluster_target > 0) {
    if (nc <= ncluster_target && n >= ncluster_target) {
      if (n - ncluster_target > ncluster_target - nc) {/* ncluster = nc */
                 ...
                 ncluster_target - nc) {/* ncluster_target close to n */
  ...
  if (nc >= 1 && (total_gain > 0 || nc < n)) {
    ...
    /* if we want a small number of cluster but right now we have too many, we
     * will force agglomeration */
    if (ncluster_target > 0 && nc > ncluster_target &&
...
static Multilevel_Modularity_Clustering Multilevel_Modularity_Clustering_new(
    SparseMatrix A0, int ncluster_target) {
  /* ncluster_target is used to specify the target number of cluster desired,
     e.g., ncluster_target=10 means that around 10 clusters
     is desired. The resulting clustering will give as close to this number as
     possible.
     If this number != the optimal number of clusters, the resulting modularity
     may be lower, or equal to, the optimal modularity.
     .  Agglomeration will be forced even if that reduces the modularity when
     there are too many clusters. It will stop when nc <= ncluster_target <=
     nc2,
     .  where nc and nc2 are the number of clusters in the current and next
     level of clustering. The final cluster number will be
...
void modularity_clustering(SparseMatrix A, int inplace, int ncluster_target,
  ...
  *flag = 0;
  ...
  hierachical_modularity_clustering(B, ncluster_target, nclusters, assignment,
                                    modularity, flag);
