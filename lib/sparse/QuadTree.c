...
void check_or_realloc_arrays(int dim, int *nsuper, int *nsupermax,
...
void QuadTree_get_supernodes_internal(QuadTree qt, real bh, real *point,
  ...
  if (l) {
    while (l) {
      check_or_realloc_arrays(dim, nsuper, nsupermax, center, supernode_wgts,
                              distances);
  ...
  if (qt->qts) {
    ...
    if (qt->width < bh * dist) {
      ...
      for (i = 0; i < 1 << dim; i++) {
        QuadTree_get_supernodes_internal(qt->qts[i], bh, point, nodeid, nsuper,
...
void QuadTree_get_supernodes(QuadTree qt, real bh, real *point, int nodeid,
                             int *nsuper, int *nsupermax, real **center,
                             real **supernode_wgts, real **distances,
                             double *counts, int *flag) {
  int dim = qt->dim;
  ...
  *nsupermax = 10;
  ...
  QuadTree_get_supernodes_internal(qt, bh, point, nodeid, nsuper, nsupermax,
                                   center, supernode_wgts, distances, counts,
                                   flag);
}

static real *get_or_assign_node_force(real *force, int i, SingleLinkedList l,
                                      int dim) {
...
static void QuadTree_repulsive_force_interact(QuadTree qt1, QuadTree qt2,
                                              real *x, real *force, real bh,
                                              real p, real KP, real *counts) {
  /* calculate the all to all reopulsive force and accumulate on each node of
     the quadtree if an interaction is possible.
  ...
  if (qt1 == qt2) {
    for (i = 0; i < 1 << dim; i++) {
      ...
      for (j = i; j < 1 << dim; j++) {
        qt12 = qt1->qts[j];
        QuadTree_repulsive_force_interact(qt11, qt12, x, force, bh, p, KP,
                                          counts);
      }
    }
  } else {
    ...
    if (qt1->width > qt2->width && !l1) {
      for (i = 0; i < 1 << dim; i++) {
        qt11 = qt1->qts[i];
        QuadTree_repulsive_force_interact(qt11, qt2, x, force, bh, p, KP,
                                          counts);
      }
    } else if (qt2->width > qt1->width && !l2) {
      for (i = 0; i < 1 << dim; i++) {
        qt11 = qt2->qts[i];
        QuadTree_repulsive_force_interact(qt11, qt1, x, force, bh, p, KP,
                                          counts);
      }
      ...
      assert(0); /* can be both at the leaf level since that should be catched
                    at the beginning of this func. */
    }
  }
}

static void QuadTree_repulsive_force_accumulate(QuadTree qt, real *force,
                                                real *counts) {
...
void QuadTree_get_repulsive_force(QuadTree qt, real *force, real *x, real bh,
                                  real p, real KP, real *counts, int *flag) {
  /* get repulsice force by a more efficient algortihm: we consider two cells,
     if they are well separated, we
     calculate the overall repulsive force on the cell level, if not well
     separated, we divide one of the cell.
     If both cells are at the leaf level, we calcuaulate repulsicve force among
     individual nodes. Finally
     we accumulate forces at the cell levels to teh node level
     qt: the quadtree
     x: current coordinates for node i is x[i*dim+j], j = 0, ..., dim-1
     force: the repulsice force, an array of length dim*nnodes, the force for
     node i is at force[i*dim+j], j = 0, ..., dim - 1
     bh: Barnes-Hut coefficient. If width_cell1+width_cell2 <
     bh*dist_between_cells, we treat each cell as a supernode.
...
static int QuadTree_get_quadrant(int dim, real *center, real *coord) {
  /* find the quadrant that a point of coordinates coord is going into with
     reference to the center.
     if coord - center == {+,-,+,+} = {1,0,1,1}, then it will sit in the
     i-quadrant where
...
QuadTree QuadTree_new_in_quadrant(int dim, real *center, real width,
                                  int max_level, int i) {
  /* a new quadtree in quadrant i of the original cell. The original cell is
     centered at 'center".
  ...
  for (k = 0; k < dim;
       k++) {/* decompose child id into binary, if {1,0}, say, then
...
static QuadTree QuadTree_add_internal(QuadTree q, real *coord, real weight,
                                      int id, int level) {
  int i, dim = q->dim, ii;
  node_data nd = NULL;
  ...
  for (i = 0; i < q->dim; i++) {
    if (coord[i] < q->center[i] - q->width - 1.e5 * MACHINEACC * q->width ||
        coord[i] > q->center[i] + q->width + 1.e5 * MACHINEACC * q->width) {
#ifdef DEBUG_PRINT
      fprintf(stderr,
              "coordinate %f is outside of the box:{%f, %f}, \n(q->center[i] - "
              "q->width) - coord[i] =%g, coord[i]-(q->center[i] + q->width) = "
              "%g\n",
              coord[i], (q->center[i] - q->width), (q->center[i] + q->width),
              (q->center[i] - q->width) - coord[i],
  ...
  if (q->n == 0) {
    ...
    q->total_weight += weight;
    for (i = 0; i < q->dim; i++)
      q->average[i] = ((q->average[i]) * q->n + coord[i]) / (q->n + 1);
    if (!q->qts) {
      q->qts = MALLOC(sizeof(QuadTree) * (1 << dim));
      for (i = 0; i < 1 << dim; i++) q->qts[i] = NULL;
    } /* done adding new quadtree, now add points to them */

    /* insert the old node (if exist) and the current node into the appropriate
     * child quadtree */
    ii = QuadTree_get_quadrant(dim, q->center, coord);
    assert(ii < 1 << dim && ii >= 0);
    if (q->qts[ii] == NULL)
      q->qts[ii] = QuadTree_new_in_quadrant(q->dim, q->center, (q->width) / 2,
                                            max_level, ii);

    q->qts[ii] =
        QuadTree_add_internal(q->qts[ii], coord, weight, id, level + 1);
    assert(q->qts[ii]);
    ...
    if (q->l) {
      ...
      q->qts[ii] =
          QuadTree_add_internal(q->qts[ii], coord, weight, idd, level + 1);
      assert(q->qts[ii]);
...
static void QuadTree_get_nearest_internal(QuadTree qt, real *x, real *y,
                                          real *min, int *imin, int tentative,
                                          int *flag) {
...
void QuadTree_get_nearest(QuadTree qt, real *x, real *ymin, int *imin,
                          real *min, int *flag) {
