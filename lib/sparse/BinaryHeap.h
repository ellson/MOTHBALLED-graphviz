...
/* binary heap code.
   Caution: items inserted should be kept untouched, e.g., the value of the item
   should be kepted unchanged while the heap is still in use!
...
struct BinaryHeap_struct {
  ...
  void** heap;
  int* id_to_pos; /* this array store the position of an item with ID. For ID
                     that was not in use,
                     i.e., no in pos_to_id[1:len],
                     id_to_pos[id] = -1
                   */
  int* pos_to_id; /* this array stores the ID of item at position pos.
                     pos_to_id[id_to_pos[i]] = i, for i not in the id_stack & i
                     < length(id_stack)+len
...
void BinaryHeap_delete(BinaryHeap h,
                       void (*del)(void* item)); /* delete not just the heap
                                                    data structure, but also the
                                                    data items
                                                          through a user
                                                    supplied del function. */

int BinaryHeap_insert(BinaryHeap h, void* item); /* insert an item, and return
                                                    its ID.
                                                    This ID can be used later to
                                                    extract the item. ID
                                                    are always nonnegative. If
                                                    the return value is
...
void* BinaryHeap_extract_item(
    BinaryHeap h, int id); /* extract an item with ID out and delete it */

void* BinaryHeap_get_item(BinaryHeap h, int id); /* get an item with ID out,
                                                    without deleting */

int BinaryHeap_reset(BinaryHeap h, int id, void* item); /* reset value of an
                                                           item with specified
                                                           id. Return new pos.
                                                           If ID is invalue,
                                                           return -1 */
