...
Vector Vector_assign(Vector v, void *stuff, int i) {
  memcpy(((char *)v->v) + (v->size_of_elem) * i / sizeof(char), stuff,
         v->size_of_elem);
  return v;
}
...
Vector Vector_reset(Vector v, void *stuff, int i) {
  ...
  if (v->deallocator)
    (v->deallocator)((char *)v->v + (v->size_of_elem) * i / sizeof(char));
...
int Vector_get_length(Vector v) { return v->len; }
...
Vector IntegerVector_add(Vector v, int i) { return Vector_add(v, (void *)&i); }

void IntegerVector_delete(Vector v) { return Vector_delete(v); }
...
int IntegerVector_get_length(Vector v) { return Vector_get_length(v); }
...
void nulldealloactor(void *v) { return; }
...
Vector StringVector_new(int len, int delete_element_strings) {
  /* delete_element_strings decides whether we need to delete each string in the
   * vector or leave it to be cleaned by other handles */
...
Vector StringVector_add(Vector v, char *s) { return Vector_add(v, (void *)&s); }

void StringVector_delete(Vector v) { return Vector_delete(v); }
...
int StringVector_get_length(Vector v) { return Vector_get_length(v); }
