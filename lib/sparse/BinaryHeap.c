...
void BinaryHeap_delete(BinaryHeap h, void (*del)(void *item)) {
  ...
  if (del)
    for (i = 0; i < h->len; i++) del((h->heap)[i]);
  FREE(h->heap);
  FREE(h);
}
...
static int siftUp(BinaryHeap h, int nodePos) {
  ...
  if (nodePos != 0) {
    ...
    if ((h->cmp)(heap[parentPos], heap[nodePos]) ==
        1) {/* if smaller than parent, swap */
...
static int siftDown(BinaryHeap h, int nodePos) {
  ...
  if (childPos1 == h->len - 1) {
    ...
    if ((h->cmp)(heap[childPos1], heap[childPos2]) ==
        1) {/* pick the smaller of the two child */
      childPos = childPos2;
    } else {
      childPos = childPos1;
    }
...
int BinaryHeap_insert(BinaryHeap h, void *item) {
  ...
  /* insert an item, and return its ID. This ID can be used later to extract the
   * item */
  if (len > h->max_len - 1) {
    if (BinaryHeap_realloc(h) == NULL) return BinaryHeap_error_malloc;
  }

  /* check if we have IDs in the stack to reuse. If no, then assign the last pos
   * as the ID */
...
void *BinaryHeap_extract_item(BinaryHeap h, int id) {
  ...
  if (pos < h->len - 1) {/* move the last item to occupy the position of
                            extracted item */
    swap(h, pos, h->len - 1);
    (h->len)--;
    pos = siftUp(h, pos);
...
void BinaryHeap_sanity_check(BinaryHeap h) {
  int i, key_spare, parentPos, *id_to_pos = h->id_to_pos,
                               *pos_to_id = h->pos_to_id;
  void **heap = h->heap;
  int *mask;
...
void BinaryHeap_print(BinaryHeap h, void (*pnt)(void *)) {
  ...
  fprintf(stderr, "\nSpare keys =");
  for (i = 0; i <= h->id_stack->last; i++) {
    fprintf(stderr, "%d(%d) ", h->id_stack->stack[i],
