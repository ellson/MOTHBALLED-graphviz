...
struct Multilevel_MQ_Clustering_struct {
  ...
  */
  ...
  real *deg_intra; /* dimension n. deg[i] equal to the sum of edge weights
                      within cluster i */
  real *dout; /* dimension n, dout[i] = \sum_{j -- i} a(i,j)/|j| is the scaled
                 sum of outdegree */
...
/* find a clustering of vertices by maximize modularity quality
   ...
   .   is desired. this may not always be realized, and modularity quality may
   be low when this is specified. Default: maxcluster = 0 (no limit)
