...
#endif
{
  ...
  if ((pid = getpid()) < 0) return NIL(char *);

  s = ends;
  do {
    if (s == begs) return NIL(char *);
    *--s = '0' + pid % 10;
  } while ((pid /= 10) > 0);
  while (s < ends) *begs++ = *s++;
...
#endif
{
  ...
  while (*file) {
    if (*file == '%') {
      switch (file[1]) {
        case 'p':
          if (!(next = insertpid(next, endb))) return -1;
          file += 2;
          break;
        default:
          goto copy;
...
#endif
{
  if (Vmregion->meth.meth == VM_MTPROFILE) vmprofile(Vmregion, _Vmpffd);
}
...
#endif
{
  ...
  if ((!vm || vm->meth.meth == VM_MTDEBUG) && (env = getenv("VMDEBUG")) &&
      env[0]) {
    if (vm || (vm = vmopen(Vmdcsbrk, Vmdebug, 0))) {
      reg int setcheck = 0;

      while (*env) {
        if (*env == 'a') vmset(vm, VM_DBABORT, 1);

        if (*env < '0' || *env > '9')
          env += 1;
        else if (env[0] == '0' && (env[1] == 'x' || env[1] == 'X')) {
          if ((addr = atou(&env)) != 0) vmdbwatch((Void_t *)addr);
        } else {
          _Vmdbcheck = atou(&env);
          setcheck = 1;
        }
      }
      if (!setcheck) _Vmdbcheck = 1;
    }
  }

  if ((!vm || vm->meth.meth == VM_MTPROFILE) && (env = getenv("VMPROFILE")) &&
      env[0]) {
    _Vmpffd = createfile(env);
    if (!vm) vm = vmopen(Vmdcsbrk, Vmprofile, 0);
  }
...
#else
Void_t *malloc(size) reg size_t size;
#endif
{
  VMFLINIT();
...
#else
Void_t *realloc(data, size) reg Void_t *data; /* block to be reallocated      */
...
#else
void free(data) reg Void_t *data;
#endif
{
  VMFLINIT();
...
#else
Void_t *calloc(n_obj, s_obj) reg size_t n_obj;
reg size_t s_obj;
#endif
{
  VMFLINIT();
  return (*Vmregion->meth.resizef)(Vmregion, NIL(Void_t *), n_obj * s_obj,
                                   VM_RSZERO);
}
...
#else
void cfree(data) reg Void_t *data;
#endif
{
  VMFLINIT();
  (void)(*Vmregion->meth.freef)(Vmregion, data);
...
#else
Void_t *memalign(align, size) reg size_t align;
reg size_t size;
#endif
{
  VMFLINIT();
...
#else
Void_t *valloc(size) reg size_t size;
#endif
{
  VMFLINIT();
...
#else
Void_t *alloca(size) size_t size;
#endif
{
  char array[ALIGN];
  char *file;
