...
#else
static Pfobj_t *pfsearch(vm, file,
                         line) Vmalloc_t *vm; /* region allocating from */
...
#endif
{
  ...
  if (!Vmpf && !(Vmpf = vmopen(Vmdcheap, Vmpool, 0))) return NIL(Pfobj_t *);

  /* make hash table; PFTABLE'th slot hold regions' records */
  if (!Pftable) {
    if (!(Pftable =
              (Pfobj_t **)vmalloc(Vmheap, (PFTABLE + 1) * sizeof(Pfobj_t *))))
      return NIL(Pfobj_t *);
    for (n = PFTABLE; n >= 0; --n) Pftable[n] = NIL(Pfobj_t *);
  }

  /* see if it's there with a combined hash value of vm,file,line */
  h = line + (((Vmulong_t)vm) >> 4);
  for (cp = file; *cp; ++cp) h += (h << 7) + ((*cp) & 0377) + 987654321L;
  n = (int)(h % PFTABLE);
  ...
  if (!pf) {
    ...
    hn = 0;
    for (cp = file; *cp; ++cp) hn += (hn << 7) + ((*cp) & 0377) + 987654321L;
    n = (int)(hn % PFTABLE);
    for (fn = Pftable[n]; fn; fn = fn->next)
      if (PFLINE(fn) < 0 && strcmp(PFNAME(fn), file) == 0) break;
    if (!fn) {
      reg size_t s;
      s = sizeof(Pfobj_t) - sizeof(Pfdata_t) + strlen(file) + 1;
      if (!(fn = (Pfobj_t *)vmalloc(Vmheap, s))) return NIL(Pfobj_t *);
      fn->next = Pftable[n];
      Pftable[n] = fn;
      PFLINE(fn) = -1;
...
#else
static void pfclose(vm) Vmalloc_t *vm;
#endif
{
  reg int n;
...
#if __STD_C
static void pfsetinfo(Vmalloc_t *vm, Vmuchar_t *data, size_t size, char *file,
                      int line)
#else
static void pfsetinfo(vm, data, size, file, line) Vmalloc_t *vm;
Vmuchar_t *data;
size_t size;
char *file;
int line;
#endif
{
  ...
  if (pf) {/* update region statistics */
    ...
    PFALLOC(pf) += size;
    PFNALLOC(pf) += 1;
    if ((s = PFALLOC(pf) - PFFREE(pf)) > PFMAX(pf)) PFMAX(pf) = s;
  }
}
...
#else
static Pfobj_t *pfsort(pf) Pfobj_t *pf;
#endif
{
  reg Pfobj_t *one, *two, *next;
  reg int cmp;

  if (!pf->next) return pf;
  ...
  for (pf = next = NIL(Pfobj_t *);
    ...
    if (PFLINE(one) == 0 && PFLINE(two) == 0)
      ...
      if (cmp == 0) cmp = PFVM(one) > PFVM(two) ? 1 : -1;
    }
...
#if __STD_C
static char *pfsummary(char *buf, Vmulong_t na, Vmulong_t sa, Vmulong_t nf,
                       Vmulong_t sf, Vmulong_t max, Vmulong_t size)
#else
static char *pfsummary(buf, na, sa, nf, sf, max, size) char *buf;
Vmulong_t na;
Vmulong_t sa;
Vmulong_t nf;
Vmulong_t sf;
...
#else
int vmprofile(vm, fd) Vmalloc_t *vm;
int fd;
#endif
{
...
#define FLSBUF() (bufp > buf ? write(fd, buf, bufp - buf) : 0)
  ...
  for (pf = Pftable[PFTABLE]; pf; pf = pf->next) {
    if (vm && PFVM(pf) != vm) continue;
    alloc = 0;
    for (seg = PFVM(pf)->data->seg; seg; seg = seg->next) alloc += seg->extent;
    bufp = (*_Vmstrcpy)(bufp, "region", '=');
    bufp = (*_Vmstrcpy)(bufp, (*_Vmitoa)(VLONG(PFVM(pf)), 0), ':');
    bufp = pfsummary(bufp, PFNALLOC(pf), PFALLOC(pf), PFNFREE(pf), PFFREE(pf),
                     PFMAX(pf), alloc);
  }
  ...
  for (pf = list; pf;) {/* compute summary for file */
    ...
    for (last = pf; last; last = last->next) {
      if (strcmp(PFFILE(last), PFFILE(pf)) != 0) break;
      nalloc += PFNALLOC(pf);
      alloc += PFALLOC(last);
      nfree += PFNFREE(last);
      free += PFFREE(last);
...
#else
static Void_t *pfalloc(vm, size) Vmalloc_t *vm;
size_t size;
#endif
{
  reg size_t s;
  ...
  if (!(vd->mode & VM_TRUST) && ISLOCK(vd, 0)) return NIL(Void_t *);
  SETLOCK(vd, 0);

  s = ROUND(size, ALIGN) + PF_EXTRA;
  if (!(data = KPVALLOC(vm, s, (*(Vmbest->allocf))))) goto done;
...
#else
static int pffree(vm, data) Vmalloc_t *vm;
Void_t *data;
#endif
{
  reg Pfobj_t *pf;
...
#else
static Void_t *pfresize(vm, data, size, type) Vmalloc_t *vm;
Void_t *data;
size_t size;
int type;
#endif
{
  ...
  if ((addr =
           KPVRESIZE(vm, data, news, (type & ~VM_RSZERO), Vmbest->resizef))) {
    if (pf) {
      PFFREE(pf) += s;
      PFNFREE(pf) += 1;
      pf = PFREGION(pf);
  ...
  if (addr && (type & VM_RSZERO) && oldsize < size) {
    reg Vmuchar_t *d = (Vmuchar_t *)addr + oldsize,
                  *ed = (Vmuchar_t *)addr + size;
    do {
      *d++ = 0;
    } while (d < ed);
  }
...
#else
static long pfsize(vm, addr) Vmalloc_t *vm;
Void_t *addr;
#endif
{
...
#else
static long pfaddr(vm, addr) Vmalloc_t *vm;
Void_t *addr;
#endif
{
...
#else
static int pfcompact(vm) Vmalloc_t *vm;
#endif
{
...
#else
static Void_t *pfalign(vm, size, align) Vmalloc_t *vm;
size_t size;
size_t align;
#endif
{
  ...
  if (!(data = KPVALIGN(vm, s, align, Vmbest->alignf))) goto done;
...
static Vmethod_t _Vmprofile = {pfalloc, pfresize,  pffree,  pfaddr,
                               pfsize,  pfcompact, pfalign, VM_MTPROFILE};
