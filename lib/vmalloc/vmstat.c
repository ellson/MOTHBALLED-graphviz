...
#endif
{
  ...
  if (vd->mode & VM_MTLAST)
    ...
    s = ROUND(s, ALIGN);
    for (b = vd->free; b; b = SEGLINK(b)) st->n_free += 1;
  }
  ...
  for (seg = vd->seg; seg; seg = seg->next) {
    ...
    if (vd->mode & (VM_MTDEBUG | VM_MTBEST | VM_MTPROFILE)) {
      ...
      if (seg->free) st->n_free += (SIZE(seg->free) + sizeof(Head_t)) / s;
  ...
  if ((vd->mode & VM_MTPOOL) && s > 0) {
    st->n_busy -= st->n_free;
    if (st->n_busy > 0) st->s_busy = (st->m_busy = vd->pool) * st->n_busy;
    if (st->n_free > 0) st->s_free = (st->m_free = vd->pool) * st->n_free;
  }
