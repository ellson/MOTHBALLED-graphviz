...
#else
static Void_t *lastalloc(vm, size) Vmalloc_t *vm;
size_t size;
#endif
{
  reg Block_t *tp, *next;
  ...
  for (;;) {
    for (last = NIL(Seg_t *), seg = vd->seg; seg; last = seg, seg = seg->next) {
      if (!(tp = seg->free) || (SIZE(tp) + sizeof(Head_t)) < size) continue;
      if (last) {
        last->next = seg->next;
        seg->next = vd->seg;
        vd->seg = seg;
...
#else
static int lastfree(vm, data) Vmalloc_t *vm;
reg Void_t *data;
#endif
{
  reg Seg_t *seg;
  ...
  SEG(fp) = seg;
  SIZE(fp) =
      ((Vmuchar_t *)BLOCK(seg->baddr) - (Vmuchar_t *)data) - sizeof(Head_t);
  seg->free = fp;
...
static Void_t *lastresize(Vmalloc_t *vm, reg Void_t *data, size_t size,
                          int type)
#else
static Void_t *lastresize(vm, data, size, type) Vmalloc_t *vm;
reg Void_t *data;
size_t size;
int type;
#endif
{
  ...
  if (data == (Void_t *)vd->free)
    ...
    for (seg = vd->seg; seg; seg = seg->next)
      if (data >= seg->addr && data < (Void_t *)seg->baddr) break;
  ...
  if (s < 0 || (ssize_t)size > s) {
    if (s >= 0) {/* amount to extend */
      ds = size - s;
      ds = ROUND(ds, vd->incr);
      addr = (*vm->disc->memoryf)(vm, seg->addr, seg->extent, seg->extent + ds,
                                  vm->disc);
      if (addr == seg->addr) {
        s += ds;
        seg->size += ds;
...
#else
static long lastaddr(vm, addr) Vmalloc_t *vm;
Void_t *addr;
#endif
{
  reg Vmdata_t *vd = vm->data;

  if (!(vd->mode & VM_TRUST) && ISLOCK(vd, 0)) return -1L;
...
#else
static long lastsize(vm, addr) Vmalloc_t *vm;
Void_t *addr;
#endif
{
  reg Vmdata_t *vd = vm->data;
  ...
  if (!vd->free || addr != (Void_t *)vd->free)
    ...
    return (Vmuchar_t *)vd->seg->baddr - (Vmuchar_t *)addr - sizeof(Head_t);
}

#if __STD_C
static int lastcompact(Vmalloc_t *vm)
#else
static int lastcompact(vm) Vmalloc_t *vm;
#endif
{
  reg Block_t *fp;
  reg Seg_t *seg, *next;
...
#else
static Void_t *lastalign(vm, size, align) Vmalloc_t *vm;
size_t size;
size_t align;
#endif
{
  ...
  s = size + align;
  if (!(data = (Vmuchar_t *)KPVALLOC(vm, s, lastalloc))) goto done;

  /* find the segment containing this block */
  for (seg = vd->seg; seg; seg = seg->next)
    if (seg->last == (Block_t *)data) break;
  /**/ ASSERT(seg);

  /* get a suitably aligned address */
  if ((s = (size_t)(VLONG(data) % align)) != 0) data += align - s;
...
static Vmethod_t _Vmlast = {lastalloc, lastresize,  lastfree,  lastaddr,
                            lastsize,  lastcompact, lastalign, VM_MTLAST};
