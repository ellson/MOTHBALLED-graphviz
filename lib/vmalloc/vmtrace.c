...
#else
static char *trstrcpy(to, from, endc) char *to;
char *from;
int endc;
#endif
{
...
#else
static char *tritoa(v, type) Vmulong_t v; /* value to convert */
...
#if __STD_C
static void trtrace(Vmalloc_t *vm, Vmuchar_t *oldaddr, Vmuchar_t *newaddr,
...
#endif
{
  ...
  for (seg = vd->seg; seg; seg = seg->next) {
    ...
    for (b = SEGBLOCK(seg), endb = BLOCK(seg->baddr); b < endb;) {
      if (ISJUNK(SIZE(b)) || !ISBUSY(SIZE(b))) continue;
