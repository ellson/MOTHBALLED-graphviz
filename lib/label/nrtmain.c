...
static int printData(object_t *objs, int n_objs, xlabel_t *lbls, int n_lbls,
  ...
  for (i = 0; i < n_objs; i++) {
    if (objs[i].lbl && objs[i].lbl->lbl)
      fprintf(stderr, " [%d] %p %p (%.02f, %.02f) (%.02f, %.02f) %s\n", i,
              &objs[i], objs[i].lbl, objs[i].pos.x, objs[i].pos.y, objs[i].sz.x,
              objs[i].sz.y, ((textlabel_t *)objs[i].lbl->lbl)->text);
    else
  ...
  for (i = 0; i < n_lbls; i++) {
    fprintf(stderr, " [%d] %p (%.02f, %.02f) (%.02f, %.02f) %s\n", i, &lbls[i],
            lbls[i].pos.x, lbls[i].pos.y, lbls[i].sz.x, lbls[i].sz.y,
...
int doxlabel(opts_t *opts) {
  ...
  for (np = agfstnode(gp); np; np = agnxtnode(gp, np)) {
    if (ND_xlabel(np)) n_nlbls++;
    for (ep = agfstout(gp, np); ep; ep = agnxtout(gp, ep)) {
      if (ED_xlabel(ep)) n_elbls++;
    }
  }
...
int checkOpt(char *l, char *legal[], int n) {
  int i;
  for (i = 0; i < n; i++) {
    if (strcmp(l, legal[i]) == 0) return 1;
  }
  return 0;
}
