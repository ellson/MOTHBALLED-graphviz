...
static void GetBranches(RTree_t *rtp, Node_t *n, Branch_t *b) {
  ...
  for (i = 1; i < NODECARD + 1; i++) {
    rtp->split.CoverSplit =
        CombineRect(&rtp->split.CoverSplit, &rtp->split.BranchBuf[i].rect);
  }
...
static void MethodZero(RTree_t *rtp) {
  ...
  while (rtp->split.Partitions[0].count[0] + rtp->split.Partitions[0].count[1] <
             NODECARD + 1 &&
         rtp->split.Partitions[0].count[0] < NODECARD + 1 - rtp->MinFill &&
         rtp->split.Partitions[0].count[1] < NODECARD + 1 - rtp->MinFill) {
    biggestDiff = -1;
    for (i = 0; i < NODECARD + 1; i++) {
      if (!rtp->split.Partitions[0].taken[i]) {
        ...
        growth0 = RectArea(&rect) - rtp->split.Partitions[0].area[0];
        rect = CombineRect(r, &rtp->split.Partitions[0].cover[1]);
        growth1 = RectArea(&rect) - rtp->split.Partitions[0].area[1];
  ...
  if (rtp->split.Partitions[0].count[0] + rtp->split.Partitions[0].count[1] <
      NODECARD + 1) {
    group = 0;
    if (rtp->split.Partitions[0].count[0] >= NODECARD + 1 - rtp->MinFill)
      group = 1;
    for (i = 0; i < NODECARD + 1; i++) {
      if (!rtp->split.Partitions[0].taken[i]) Classify(rtp, i, group);
    }
  }
  ...
  for (i = 0; i < NODECARD; i++) {
    for (j = i + 1; j < NODECARD + 1; j++) {
      ...
      /* waste = RectArea(&CombineRect(&rtp->split.BranchBuf[i].rect,
         //                  &rtp->split.BranchBuf[j].rect)) - area[i] -
         area[j];
       */
...
static void Classify(RTree_t *rtp, int i, int group) {
  ...
  if (rtp->split.Partitions[0].count[group] == 0)
    rtp->split.Partitions[0].cover[group] = rtp->split.BranchBuf[i].rect;
  else
    rtp->split.Partitions[0].cover[group] = CombineRect(
        &rtp->split.BranchBuf[i].rect, &rtp->split.Partitions[0].cover[group]);
...
static void InitPVars(RTree_t *rtp) {
  ...
  rtp->split.Partitions[0].count[0] = rtp->split.Partitions[0].count[1] = 0;
  rtp->split.Partitions[0].cover[0] = rtp->split.Partitions[0].cover[1] =
      NullRect();
  rtp->split.Partitions[0].area[0] = rtp->split.Partitions[0].area[1] = 0;
  for (i = 0; i < NODECARD + 1; i++) {
...
PrintPVars(RTree_t *rtp) {
  ...
  fprintf(stderr, "count[0] = %d  area = %d\n",
          rtp->split.Partitions[0].count[0], rtp->split.Partitions[0].area[0]);
  fprintf(stderr, "count[1] = %d  area = %d\n",
          rtp->split.Partitions[0].count[1], rtp->split.Partitions[0].area[1]);
  if (rtp->split.Partitions[0].area[0] + rtp->split.Partitions[0].area[1] > 0) {
    fprintf(
        stderr, "total area = %d  effectiveness = %3.2f\n",
        rtp->split.Partitions[0].area[0] + rtp->split.Partitions[0].area[1],
        (float)rtp->split.CoverSplitArea / (rtp->split.Partitions[0].area[0] +
                                            rtp->split.Partitions[0].area[1]));
  }
