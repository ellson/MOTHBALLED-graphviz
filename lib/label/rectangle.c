...
void InitRect(Rect_t* r) {
  register int i;
  for (i = 0; i < NUMSIDES; i++) r->boundary[i] = 0;
}
...
Rect_t NullRect() {
  ...
  for (i = 1; i < NUMDIMS; i++) r.boundary[i] = r.boundary[i + NUMDIMS] = 0;
  return r;
}
...
Rect_t CombineRect(Rect_t* r, Rect_t* rr) {
  ...
  if (Undefined(r)) return *rr;
  if (Undefined(rr)) return *r;
