...
#define mbgetchar(p)                                                 \
  ((ast.locale.set &AST_LC_multibyte)                                \
       ? ((ast.tmp_int = mbtowc(&ast.tmp_wchar, p, MB_CUR_MAX)) >= 0 \
...
static char *gobble(Match_t *mp, register char *s, register int sub, int *g,
                    int clear) {
  register int p = 0;
  register char *b = 0;
  int c = 0;
  ...
  for (;;) switch (mbgetchar(s)) {
      ...
      case '(':
        if (!b) {
          ...
          if (clear) {
            if (!sub) n++;
            if (n < MAXGROUP) mp->current.beg[n] = mp->current.end[n] = 0;
          }
        }
        break;
      case ')':
...
static int onematch(Match_t *mp, int g, char *s, char *p, char *e, char *r,
                    int flags) {
  register int pc;
  register int sc;
  register int n;
...
#if _DEBUG_MATCH
  error_info.indent++;
  error(-1, "onematch g=%d s=%-.*s p=%s r=%p flags=%o", g, e - s, s, p, r,
        flags);
#endif
  ...
  do {
    olds = s;
    sc = getsource(s, e);
    if (icase && isupper(sc)) sc = tolower(sc);
    oldp = p;
    switch (pc = mbgetchar(p)) {
      ...
      case '!':
        if (pc == '(' || *p == '(') {
          ...
          if (!(p = gobble(mp, subp, 0, &g, !r))) RETURN(0);
          if (pc == '*' || pc == '?' || (pc == '+' && oldp == r)) {
            if (onematch(mp, g, s, p, e, NiL, flags)) RETURN(1);
            if (!sc || !getsource(s, e)) {
          ...
          do {
            if (grpmatch(mp, n, olds, subp, s, flags) == pc) {
              if (n < MAXGROUP) {
                ...
                if (s > mp->current.end[n]) mp->current.end[n] = s;
#if _DEBUG_MATCH
                error(-4, "subgroup#%d n=%d beg=%p end=%p len=%d", __LINE__, n,
                      mp->current.beg[n], mp->current.end[n],
              ...
              if (onematch(mp, sc, s, p, e, oldp, flags)) {
                if (p == oldp && n < MAXGROUP) {
                  ...
                  if (s > mp->current.end[n]) mp->current.end[n] = s;
#if _DEBUG_MATCH
                  error(-4, "subgroup#%d n=%d beg=%p end=%p len=%d", __LINE__,
                        n, mp->current.beg[n], mp->current.end[n],
                        mp->current.end[n] - mp->current.beg[n]);
#endif
                }
                RETURN(1);
          ...
          switch (pc = mbgetchar(p)) {
            ...
            case ')':
              ...
              mp->current.groups = g;
              if (!pc &&
                  (!mp->best.next_s || ((flags & STR_MAXIMAL) &&
            ...
            case '\\':
              ...
              if (pc >= '0' && pc <= '9') {
                n = pc - '0';
#if _DEBUG_MATCH
                error(-2, "backref#%d n=%d g=%d beg=%p end=%p len=%d", __LINE__,
                      n, g, mp->current.beg[n], mp->current.end[n],
                      mp->current.end[n] - mp->current.beg[n]);
#endif
                if (n <= g && mp->current.beg[n]) pc = *mp->current.beg[n];
              }
            /*FALLTHROUGH*/ default:
              if (icase && isupper(pc)) pc = tolower(pc);
              n = 0;
              break;
          }
          p = oldp;
          for (;;) {
            ...
            olds = s;
            sc = getsource(s, e);
            if ((flags & STR_ICASE) && isupper(sc)) sc = tolower(sc);
          }
        } else if (pc != '?' && pc != sc)
          RETURN(0);
        break;
      ...
      case ')':
        ...
        if (!pc && (!mp->best.next_s ||
          ...
          mp->best.groups = g;
#if _DEBUG_MATCH
          error(-3, "best#%d groups=%d next=\"%s\"", __LINE__, mp->best.groups,
                mp->best.next_s);
#endif
        }
        RETURN(!sc);
        ...
        for (;;) {
          ...
          if (!(pc = mbgetchar(p))) {
            ...
            if (ok)
              ...
              switch (
                  HASHNKEY5(x, oldp[0], oldp[1], oldp[2], oldp[3], oldp[4])) {
                ...
                case HASHNKEY5(5, 'u', 'p', 'p', 'e', 'r') :
                  if (icase ? islower(sc) : isupper(sc)) ok = 1;
                  break;
            ...
            if (ok < 0) {
              ...
              if (p - range > sizeof(pat) - 2) RETURN(0);
              memcpy(pat, range, p - range);
              pat[p - range] = '*';
              pat[p - range + 1] = 0;
              if (fnmatch(pat, olds, 0)) RETURN(0);
              pat[p - range] = 0;
              ok = 0;
          ...
          getrange:
            ...
            if (!ok)
              ...
              if (icase && isupper(pc)) pc = tolower(pc);
            x = mbgetchar(range);
            if (icase && isupper(x)) x = tolower(x);
            if (sc == x || sc == pc || (sc > x && sc < pc)) ok = 1;
            if (*p == '-' && *(p + 1) != ']') {
              (void)mbgetchar(p);
              range = oldp;
            } else
      ...
      case '\\':
        ...
        if (pc >= '0' && pc <= '9') {
          n = pc - '0';
#if _DEBUG_MATCH
          error(-2, "backref#%d n=%d g=%d beg=%p end=%p len=%d", __LINE__, n, g,
                mp->current.beg[n], mp->current.end[n],
                mp->current.end[n] - mp->current.beg[n]);
#endif
          if (n <= g && (oldp = mp->current.beg[n])) {
            while (oldp < mp->current.end[n])
              if (!*olds || *olds++ != *oldp++) RETURN(0);
            s = olds;
            break;
          }
        }
...
static int grpmatch(Match_t *mp, int g, char *s, register char *p, char *e,
                    int flags) {
  register char *a;
...
int strgrpmatch(const char *b, const char *p, int *sub, int n, int flags) {
  ...
  if ((flags & STR_RIGHT) && match.current.next_s != e) return 0;
  if (!sub) return 1;
  match.current.beg[0] = s;
  s = (char *)b;
  if (n > match.current.groups) n = match.current.groups;
  for (i = 0; i < n; i++) {
...
char *strsubmatch(const char *s, const char *p, int flags) {
  ...
  return strgrpmatch(s, p, match, 1, (flags ? STR_MAXIMAL : 0) | STR_LEFT)
             ? (char *)s + match[1]
             : (char *)0;
}
