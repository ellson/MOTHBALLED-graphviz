...
long strton(const char *a, char **e, char *basep, int m) {
  ...
  if (!basep || (base = *basep) < 0 || base > 64) base = 0;
  while (isspace(*s)) s++;
  if ((negative = (*s == '-')) || *s == '+') s++;
  p = s;
  if (base <= 1) {
  ...
  if (base == 10) {
    ...
    if ((base & ~(base - 1)) == base) {
      if (base < 8)
        ...
        shift = base < 64 ? 5 : 6;
      while ((c = cv[*s++]) < base) n = (n << shift) + c;
    } else
      while ((c = cv[*s++]) < base) n = (n * base) + c;
    c = *(s - 1);
  }
