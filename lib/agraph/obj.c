  ...
  switch (AGTYPE(obj)) {
    case AGRAPH:
      ...
      if (agmapnametoid(agroot(g), AGTYPE(obj), newname, &new_id, FALSE) == 0)
        return FAILURE;
      if (new_id == old_id) return SUCCESS;
      if (agmapnametoid(agroot(g), AGTYPE(obj), newname, &new_id, TRUE) == 0)
        return FAILURE;
      if (agparent(g) && agidsubg(agparent(g), new_id, 0)) return FAILURE;
...
int agisarootobj(void *obj) { return (agraphof(obj)->desc.maingraph); }
...
int agpopdisc(Agraph_t *g, Agcbdisc_t *cbd) {
  ...
  if (stack_ent) {
    if (stack_ent->f == cbd)
      ...
      if (stack_ent && stack_ent->prev) stack_ent->prev = stack_ent->prev->prev;
    }
    if (stack_ent) {
      agfree(g, stack_ent);
      return SUCCESS;
...
Dtdisc_t Ag_obj_id_disc = {0, /* pass object ptr      */
                           0, /* size (ignored)       */
                           offsetof(Agobj_t, id_link), /* link offset */
                           NIL(Dtmake_f),              NIL(Dtfree_f),
                           agobjidcmpf,                NIL(Dthash_f),
                           agdictobjmem,               NIL(Dtevent_f)};

Dtdisc_t Ag_obj_seq_disc = {0, /* pass object ptr      */
                            0, /* size (ignored)       */
                            offsetof(Agobj_t, seq_link), /* link offset */
                            NIL(Dtmake_f),               NIL(Dtfree_f),
                            agobjseqcmpf,                NIL(Dthash_f),
                            agdictobjmem,                NIL(Dtevent_f)};
