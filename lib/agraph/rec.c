...
Agrec_t *aggetrec(void *obj, char *name, int mtf) {
  ...
  if (d) {
    if (hdr->tag.mtflock) {
      if (mtf && (hdr->data != d)) agerror(AGERROR_MTFLOCK, "");
    } else {
 ...
 */
void *agrealbindrec(void *arg_obj, char *recname, unsigned int recsize, int mtf,
                    int norecur) {
  Agraph_t *g;
  Agobj_t *obj;
  Agrec_t *rec;
    ...
    case AGINEDGE:
      for (n = agfstnode(g); n; n = agnxtnode(n))
        if (kind == AGNODE)
          ...
          for (e = agfstout(n); e; e = agnxtout(e)) agdelrec(e, rec_name);
        }
      break;
    default:
      break;
