...
Dtdisc_t AgDataDictDisc = {offsetof(Agsym_t, name), /* use symbol name as key */
                           -1,                      offsetof(Agsym_t, link),
                           NIL(Dtmake_f),           freesym,
...
Agsym_t *agattr(Agraph_t *g, int kind, char *name, char *value) {
   ...
   */
  if (lsym && value && (kind == AGRAPH)) agxset(g, lsym, value);
  return lsym;
}
...
void agnodeattr_init(Agnode_t *n, int norecur) {
  ...
  data = agattrrec(n);
  if ((!data) || (!data->dict)) (void)agmakeattrs(n, norecur);
}
...
void agedgeattr_init(Agedge_t *e, int norecur) {
  ...
  data = agattrrec(e);
  if ((!data) || (!data->dict)) (void)agmakeattrs(e, norecur);
}
