...
static void indent(Agraph_t *g, iochan_t *ofile) {
  int i;
  for (i = Level; i > 0; i--) ioput(g, ofile, "\t");
}
 ...
 */
static char *_agcanonstr(char *arg, char *buf) {
  char *s, *p;
  unsigned char uc;
  int cnt = 0;
 ...
 * must agree with strings in scan.l
 */
char *agcanonStr(char *str) { return agcanonstr(str, getoutputbuffer(str)); }
...
static void write_node(Agnode_t *n, iochan_t *ofile, Dict_t *d) {
  ...
  if (NOT(attrs_written(n))) write_nondefault_attrs(n, ofile, d);
  ioput(g, ofile, ";\n");
}
...
static int write_node_test(Agraph_t *g, Agnode_t *n, unsigned long pred_id) {
  if (NOT(node_in_subg(g, n)) && has_no_predecessor_below(n, pred_id)) {
    if (has_no_edges(n) || not_default_attrs(g, n)) return TRUE;
  }
  return FALSE;
}
...
static void write_body(Agraph_t *g, iochan_t *ofile) {
  ...
  for (n = agfstnode(g); n; n = agnxtnode(n)) {
    if (write_node_test(g, n, AGSEQ(n))) write_node(n, ofile, dd->dict.n);
    for (e = agfstout(n); e; e = agnxtout(e)) {
      if (write_node_test(g, e->node, AGSEQ(n)))
        write_node(e->node, ofile, dd->dict.n);
      if (write_edge_test(g, e)) write_edge(e, ofile, dd->dict.e);
    }
  }
}
...
static void set_attrwf(Agraph_t *g, int toplevel, int value) {
  ...
  if (toplevel) {
    for (n = agfstnode(g); n; n = agnxtnode(n)) {
      AGATTRWF(n) = value;
      for (e = agfstout(n); e; e = agnxtout(e)) AGATTRWF(e) = value;
    }
  }
}
