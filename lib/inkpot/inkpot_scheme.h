...
typedef struct inkpot_scheme_s {
  IDX_STRINGS_t string_idx; /* The scheme name in TAB_STRINGS.  (e.g. "x11") */
} inkpot_scheme_t;

typedef struct inkpot_range_s {
  IDX_INDEXES_t size; /* 1,0 for alts,  all but last alt=1, last alt is 0,
                       * n>=2 for ranges
                       */
  MSK_SCHEMES_t scheme_bits; /* A bit for each inkpot_scheme_name_t
                              * that includes this inkpot_name_t
                              */
  IDX_INDEXES_t first_value_idx; /* index of first in range, or first
                                    alternative */
} inkpot_range_t;
...
typedef struct inkpot_name_s {/* Color_name used by one or more
  ...
  IDX_STRINGS_t string_idx; /* The color name in TAB_STRINGS.
                         * (e.g. "green") */
...
struct inkpot_s {            /* The Ink Pot */
  MSK_SCHEMES_t scheme_bits, /* One bit per inkpot_scheme_name_t */
      out_scheme_bit;        /* One scheme only for output. */

  IDX_INDEXES_t index, /* The index for the current value in
                          active_schemes_index */
      out_index; /* The index for the current value in active_out_schemes_index
                    */

  int active_schemes,     /* The number of active index schemes. */
      active_out_schemes; /* The number of active index schemes. */

  inkpot_name_t *out_name; /* The current output name, or NULL. */

  char *canon; /* malloc'ed, reused storage for canonicalizing color request
                  strings */
  int canon_alloc;
  ...
  inkpot_cache_element_t cache
      [SZT_MRU_CACHE]; /* MRU chache of successful color lookups */
  IDX_MRU_CACHE_t most_recently_used_idx;

  inkpot_write_disc_t write_disc; /* User changeable function for writing output
                           *    Defaults to an internal function that writes
                           *    to FILE* streams */
  void *write_closure; /* A parameter to pass to the writer function.
