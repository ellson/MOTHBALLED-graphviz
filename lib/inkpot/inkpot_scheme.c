...
static int string_cmpf(const char *k, const char *b) {
  ...
  if (*k)
    return 1; /* if the next char in key is not '\0', then key is longer */
  if (*b)
    return -1; /* if the next char in base is not '\0', then base is longer */
  return 0;
}
...
static inkpot_scheme_t *inkpot_find_scheme(const char *scheme) {
  ...
  return (inkpot_scheme_t *)bsearch((void *)scheme, (void *)TAB_SCHEMES,
...
static inkpot_icolor_t *inkpot_find_icolor(const char *icolor) {
  if (icolor == NULL) return NULL;

  return (inkpot_icolor_t *)bsearch((void *)icolor, (void *)TAB_ICOLORS,
...
static inkpot_status_t inkpot_scheme(inkpot_t *inkpot, const char *scheme) {
  ...
  if (inkpot_scheme == NULL) return ((inkpot->status = INKPOT_SCHEME_UNKNOWN));
...
inkpot_status_t inkpot_schemes_put(inkpot_t *inkpot, const char *schemes) {
  ...
  if (schemes == NULL) return ((inkpot->status = INKPOT_SCHEME_UNKNOWN));
  ...
  if (len >= inkpot->canon_alloc) {
    ...
    if (!inkpot->canon) return ((inkpot->status = INKPOT_MALLOC_FAIL));
  }
  ...
  for (c = *q; c;) {
    ...
    *p++ = '\0';
    if (*s) {
      ...
      if (rc != INKPOT_SUCCESS) return rc;
    }
  }
  return rc;
}
...
inkpot_status_t inkpot_icolor_get(inkpot_t *inkpot, const char *icolor) {
  ...
  if (!inkpot_icolor) return ((inkpot->status = INKPOT_SCHEME_UNKNOWN));
...
static inkpot_status_t inkpot_put_value_idx(inkpot_t *inkpot,
...
static inkpot_status_t inkpot_cache_get(inkpot_t *inkpot) {
   ...
   *
   * Its not sufficient for the scheme of the cached value to still be present
   *in the current
   * schemes since the correct resolution may have the same name but different
   *value.
   */
  color = inkpot->canon;
  for (i = 0; i < SZT_MRU_CACHE; i++) {
    if (inkpot->cache[i].scheme_bits != inkpot->scheme_bits) continue;
    ...
    if (i) {/* if it is not already MRU then reorder to make it so */
      inkpot->cache[i - 1].next_recently_used_idx =
          inkpot->cache[i].next_recently_used_idx;
...
static inkpot_status_t inkpot_cache_put(inkpot_t *inkpot,
                                        IDX_NAMES_t name_idx) {
  IDX_MRU_CACHE_t i;
...
static inkpot_status_t inkpot_put_name(inkpot_t *inkpot) {
  ...
  name = (inkpot_name_t *)bsearch((void *)color, (void *)TAB_NAMES, SZT_NAMES,
                                  sizeof(inkpot_name_t), inkpot_name_cmpf);
  if (name == NULL) return ((inkpot->status = INKPOT_COLOR_UNKNOWN));
  ...
  if (name->scheme_bits) {
    ...
    for (i = name->value_idx; i < SZT_ALTS; i++) {
      ...
      if (!TAB_ALTS[i].string_idx) break;
    }
  }
  return ((inkpot->status = INKPOT_COLOR_UNKNOWN));
}
...
inkpot_status_t inkpot_put_rgba(inkpot_t *inkpot, double rgba[4]) {
  ...
  if (rc == INKPOT_SUCCESS) inkpot->value = value;
...
inkpot_status_t inkpot_put(inkpot_t *inkpot, const char *color) {
  ...
  if (!color) return ((inkpot->status = INKPOT_COLOR_UNKNOWN));
  ...
  for (p = color, q = inkpot->canon; (c = *p) && (c == ' ' || c == '\t'); p++) {
  }; /* remove leading ws */
  ...
  *q = '\0';
  ...
  if (*inkpot->canon == '#') {
    ...
    hsva[3] = 1.0;
    if (sscanf(inkpot->canon, "%lf%lf%lf%lf", &hsva[0], &hsva[1], &hsva[2],
               &hsva[3]) >= 3)
      rc = inkpot_put_hsva(inkpot, hsva);              /* FIXME */
    else if (sscanf(inkpot->canon, "%d", &index) == 1) /* simple indexes */
      rc = inkpot_put_index(inkpot, index);
  }
  if (rc != INKPOT_SUCCESS &&
      inkpot->active_schemes) {/* pseudo names from scheme_name+index */
  ...
  if (rc != INKPOT_SUCCESS) {
    ...
    for (p = q = inkpot->canon; (c = *p) && !(c == ' ' || c == '\t'); p++) {
      *q++ = tolower(c);
    };
    *q = '\0';
    ...
    if (value_idx < SZT_VALUES) {
      for (t = TAB_VALUE_TO[value_idx]; t < SZT_TO_NAMES; t++) {
        ...
        if (out_name->scheme_bits) {
          if (out_name->value_idx != value_idx) break;
...
inkpot_status_t inkpot_get_hsva(inkpot_t *inkpot, double hsva[4]) {
  ...
  if (rc == INKPOT_SUCCESS) rgba2hsva(rgba, hsva);
...
inkpot_status_t inkpot_get_cmyk(inkpot_t *inkpot, double cmyk[4]) {
  ...
  rc = inkpot_get_rgba(inkpot, rgba);
  if (rc == INKPOT_SUCCESS) rgba2cmyk(rgba, cmyk);
...
inkpot_status_t inkpot_write_rgba8(inkpot_t *inkpot) {
  ...
  rc = inkpot_get_rgba_i(inkpot, rgba);
  if (rc == INKPOT_SUCCESS) {
    len = sprintf(buf, "%02x%02x%02x%02x", rgba[0] >> 8, rgba[1] >> 8,
                  rgba[2] >> 8, rgba[3] >> 8);
    assert(len == 8);
    inkpot->write_disc.writer(inkpot->write_closure, buf, len);
  }
...
inkpot_status_t inkpot_write_rgb8(inkpot_t *inkpot) {
  ...
  if (rc == INKPOT_SUCCESS) {
    len =
        sprintf(buf, "%02x%02x%02x", rgba[0] >> 8, rgba[1] >> 8, rgba[2] >> 8);
    assert(len == 6);
    inkpot->write_disc.writer(inkpot->write_closure, buf, len);
  }
  return rc;
...
inkpot_status_t inkpot_debug_schemes(inkpot_t *inkpot) {
  ...
  for (i = 0; i < SZT_SCHEMES; i++) {
    ...
    if (found) inkpot_puts(inkpot, "\n");
  }
  for (j = 0; j < inkpot->active_schemes; j++) {
    inkpot_puts(inkpot, " ");
    inkpot_puts(inkpot,
                &TAB_STRINGS[TAB_ICOLORS[inkpot->scheme_list[j]].string_idx]);
    inkpot_puts(inkpot, "#(in)");
  }
  for (j = 0; j < inkpot->active_out_schemes; j++) {
    ...
    inkpot_puts(
        inkpot,
        &TAB_STRINGS[TAB_ICOLORS[inkpot->out_scheme_list[j]].string_idx]);
    inkpot_puts(inkpot, "#(out)");
  }
  inkpot_puts(inkpot, "\n");
