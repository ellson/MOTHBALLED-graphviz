...
static Agnode_t *pop(stk_t *sp) {
  if (sp->curp == sp->curblk->data) {
    if (sp->curblk == sp->fstblk) return 0;
...
static int dfs(Agraph_t *g, Agnode_t *n, void *state, stk_t *stk) {
  ...
  while ((n = pop(stk))) {
    ...
    for (e = agfstedge(g, n); e; e = agnxtedge(g, e, n)) {
      if ((other = agtail(e)) == n) other = aghead(e);
      if (!MARKED(stk, other)) push(stk, other);
    }
  }
  return cnt;
}
...
Agraph_t **pccomps(Agraph_t *g, int *ncc, char *pfx, boolean *pinned) {
  ...
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) UNMARK(&stk, n);
  ...
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
    if (MARKED(&stk, n) || !isPinned(n)) continue;
    if (!out) {
...
static void unionNodes(Agraph_t *dg, Agraph_t *g) {
  ...
  for (dn = agfstnode(dg); dn; dn = agnxtnode(dg, dn)) {
    if (AGTYPE(ptrOf(dn)) == AGNODE) {
      ...
      clust = clustOf(dn);
      for (n = agfstnode(clust); n; n = agnxtnode(clust, n)) agsubnode(g, n, 1);
    }
  }
}
 ...
 * Returns the number of edges added.
 */
int nodeInduce(Agraph_t *g) { return node_induce(g, g->root); }
