 ...
 */
static void fillEdge(Agedge_t *e, point p, PointSet *ps, int dx, int dy,
                     int ssize, int doS) {
  int j, k;
  bezier bz;
  pointf pt, hpt;
 ...
 * the graph.
 */
static void genBox(boxf bb0, ginfo *info, int ssize, int margin, point center,
                   char *s) {
  PointSet *ps;
  int W, H;
  point UR, LL;
  ...
  for (x = LL.x; x <= UR.x; x++)
    for (y = LL.y; y <= UR.y; y++) addPS(ps, x, y);
  ...
  if (pinfo->mode == l_clust) {
    ...
    for (i = 1; i <= GD_n_cluster(g); i++) {
      ...
      if ((bb.UR.x > bb.LL.x) && (bb.UR.y > bb.LL.y)) {
        ...
        for (x = bb.LL.x; x <= bb.UR.x; x++)
          for (y = bb.LL.y; y <= bb.UR.y; y++) addPS(ps, x, y);

        /* note which nodes are in clusters */
        for (n = agfstnode(subg); n; n = agnxtnode(subg, n)) ND_clust(n) = subg;
      }
    }
    ...
    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
      ...
      if (!ND_clust(n)) {/* n is not in a top-level cluster */
        ...
        CELL(pt, ssize);
        for (e = agfstout(eg, n); e; e = agnxtout(eg, e)) {
          if (ND_clust(n) == ND_clust(aghead(e))) continue;
          fillEdge(e, pt, ps, dx, dy, ssize, doSplines);
        }
      }
    }
 ...
 */
static int fits(int x, int y, ginfo *info, PointSet *ps, point *place, int step,
                boxf *bbs) {
 ...
 */
static void placeGraph(int i, ginfo *info, PointSet *ps, point *place, int step,
                       int margin, boxf *bbs) {
  int x, y;
  int W, H;
  int bnd;
  ...
  if (i == 0) {
    ...
    if (fits(-W / 2, -H / 2, info, ps, place, step, bbs)) return;
  }

  if (fits(0, 0, info, ps, place, step, bbs)) return;
  ...
  if (W >= H) {
    for (bnd = 1;; bnd++) {
      ...
      y = -bnd;
      for (; x < bnd; x++)
        if (fits(x, y, info, ps, place, step, bbs)) return;
      for (; y < bnd; y++)
        if (fits(x, y, info, ps, place, step, bbs)) return;
      for (; x > -bnd; x--)
        if (fits(x, y, info, ps, place, step, bbs)) return;
...
static point *polyRects(int ng, boxf *gs, pack_info *pinfo) {
  ...
  if (Verbose) fprintf(stderr, "step size = %d\n", stepSize);
  ...
  free(sinfo);
  for (i = 0; i < ng; i++) free(info[i].cells);
  free(info);
  freePS(ps);
 ...
 */
static point *polyGraphs(int ng, Agraph_t **gs, Agraph_t *root,
                         pack_info *pinfo) {
  int stepSize;
  ginfo *info;
  ginfo **sinfo;
  ...
  /* calculate grid size */
  bbs = N_GNEW(ng, boxf);
  for (i = 0; i < ng; i++) bbs[i] = GD_bb(gs[i]);
...
point *putGraphs(int ng, Agraph_t **gs, Agraph_t *root, pack_info *pinfo) {
  ...
  if (pinfo->mode <= l_graph) return polyGraphs(ng, gs, root, pinfo);
  ...
  if (pinfo->mode == l_array) {
    if (pinfo->flags & PK_USER_VALS) {
      ...
      for (i = 0; i < ng; i++) {
        s = agget(gs[i], "sortv");
        if (s && (sscanf(s, "%d", &v) > 0) && (v >= 0)) pinfo->vals[i] = v;
      }
    }
    pts = arrayRects(ng, bbs, pinfo);
    if (pinfo->flags & PK_USER_VALS) free(pinfo->vals);
  }
...
static void shiftEdge(Agedge_t *e, int dx, int dy) {
  ...
  if (ED_label(e)) MOVEPT(ED_label(e)->pos);
  if (ED_xlabel(e)) MOVEPT(ED_xlabel(e)->pos);
  if (ED_head_label(e)) MOVEPT(ED_head_label(e)->pos);
  if (ED_tail_label(e)) MOVEPT(ED_tail_label(e)->pos);

  if (ED_spl(e) == NULL) return;

  for (j = 0; j < ED_spl(e)->size; j++) {
    bz = ED_spl(e)->list[j];
    for (k = 0; k < bz.size; k++) MOVEPT(bz.list[k]);
    if (bz.sflag) MOVEPT(ED_spl(e)->list[j].sp);
    if (bz.eflag) MOVEPT(ED_spl(e)->list[j].ep);
  }
}
...
static void shiftGraph(Agraph_t *g, int dx, int dy) {
  ...
  if (GD_label(g) && GD_label(g)->set) MOVEPT(GD_label(g)->pos);
 ...
 */
int shiftGraphs(int ng, Agraph_t **gs, point *pp, Agraph_t *root,
                int doSplines) {
  int i;
  int dx, dy;
  double fx, fy;
    ...
    switch (*p) {
      case 'a':
        if (strneq(p, ARRAY, SLEN(ARRAY))) {
          ...
          p += SLEN(ARRAY);
          p = chkFlags(p, pinfo);
          if ((sscanf(p, "%d", &i) > 0) && (i > 0)) pinfo->sz = i;
      ...
      case 'b':
        if (streq(p, "bisect")) pinfo->mode = l_bisect;
        break;
#endif
      case 'c':
        if (streq(p, "cluster")) pinfo->mode = l_clust;
        break;
      case 'g':
        if (streq(p, "graph")) pinfo->mode = l_graph;
        break;
#ifdef NOT_IMPLEMENTED
      case 'h':
        if (streq(p, "hull")) pinfo->mode = l_hull;
        break;
#endif
      case 'n':
        if (streq(p, "node")) pinfo->mode = l_node;
        break;
#ifdef NOT_IMPLEMENTED
      case 't':
        if (streq(p, "tile")) pinfo->mode = l_tile;
        break;
#endif
    }
  }
