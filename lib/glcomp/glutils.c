...
int GetFixedOGLPos(int x, int y, float kts, GLfloat *X, GLfloat *Y,
  ...
  gluUnProject(winX, winY, wwinZ, modelview, projection, viewport, &posX, &posY,
               &posZ);
  *X = (GLfloat)posX;
  *Y = (GLfloat)posY;
  *Z = (GLfloat)posZ;
...
/*transforms 2d windows location to 3d gl coords but depth is calculated unlike
...
float GetOGLDistance(int l) {
  ...
  glEnd();
  gluProject(10.0, 10.0, 1.00, modelview, projection, viewport, &wwinX, &wwinY,
             &wwinZ);
  x = 50;
  y = 50;
  winX = (float)x;
  ...
  winX = (float)x;
  winY = (float)viewport[3] - (float)y;
  gluUnProject(winX, winY, wwinZ, modelview, projection, viewport, &posXX,
               &posYY, &posZZ);
  //    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
/*
        functions def: returns opengl coordinates of firt hit object by using
   screen coordinates
...
void to3D(int x, int y, GLfloat *X, GLfloat *Y, GLfloat *Z) {
  ...
  for (idx = 0; idx < WIDTH * WIDTH; idx++) {
    ...
    if ((winZ[idx] > comp) && (winZ[idx] < 1)) comp = winZ[idx];
  }
  //      printf ("\n");

  gluUnProject(winX, winY, comp, modelview, projection, viewport, &posX, &posY,
               &posZ);
...
static double len(glCompPoint p) { return sqrt(dot(p, p)); }
...
static double dist(glCompPoint p, glCompPoint q) { return (len(sub(p, q))); }
...
void glCompCalcWidget(glCompCommon *parent, glCompCommon *child,
  ...
  if (child->align != glAlignNone)  // if alignment, make sure width and height
                                    // is no greater than parent
  {
  ...
  switch (child->align) {
    ...
    case glAlignTop:
      ref->pos.y =
          parent->refPos.y + parent->height - child->height - borderWidth;
  ...
  if (child->align == glAlignNone)  // No alignment , chekc anchors
  {
    ...
    if (child->anchor.topAnchor)
      ref->height = parent->refPos.y + parent->height - ref->pos.y -
                    child->anchor.top - borderWidth;
    if (child->anchor.rightAnchor)
      ref->width = parent->refPos.x + parent->width - ref->pos.x -
...
GLfloat distBetweenPts(glCompPoint A, glCompPoint B, float R) {
  GLfloat rv = 0;
  rv = (A.x - B.x) * (A.x - B.x) + (A.y - B.y) * (A.y - B.y) +
       (A.z - B.z) * (A.z - B.z);
  rv = sqrt(rv);
  if (rv <= R) return 0;
  return rv;
