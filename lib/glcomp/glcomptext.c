...
void init_gl_vars(glCompText *f) {
  ...
  if (f->glcache.poly[0] != GL_FILL) glPolygonMode(GL_FRONT, GL_FILL);
  if (f->glcache.poly[1] != GL_FILL) glPolygonMode(GL_BACK, GL_FILL);

  f->glcache.istextureon = glIsEnabled(GL_TEXTURE_2D);
  if (!f->glcache.istextureon) glEnable(GL_TEXTURE_2D);
  f->glcache.isdepthon = glIsEnabled(GL_DEPTH_TEST);
  if (f->glcache.isdepthon) glDisable(GL_DEPTH_TEST);
  f->glcache.islightingon = glIsEnabled(GL_LIGHTING);
  if (f->glcache.islightingon) glDisable(GL_LIGHTING);
  ...
  if (&f->glcache.isblendon) glEnable(GL_BLEND);
...
void restore_gl_vars(glCompText *f) {
  ...
  if (f->glcache.poly[1] != GL_FILL) glPolygonMode(GL_BACK, f->glcache.poly[1]);

  if (f->glcache.islightingon) glEnable(GL_LIGHTING);
  ...
  if (f->glcache.isdepthon) glEnable(GL_DEPTH_TEST);
  if (!f->glcache.istextureon) glDisable(GL_TEXTURE_2D);
...
static void glPrintf(glCompText *font, GLfloat xpos, GLfloat ypos, GLfloat zpos,
  ...
  for (; *bf;
       *bf++, x += charGap)  // size*0.7 is the distance between2 characters
  {
...
void glprintf(glCompText *font, GLfloat xpos, GLfloat ypos, GLfloat zpos,
              GLfloat width, char *bf) {
  glPrintf(font, xpos, ypos, zpos, width, bf, 0);
}

void glprintfz(glCompText *font, GLfloat xpos, GLfloat ypos, GLfloat zpos,
               GLfloat width, char *bf) {
  glPrintf(font, xpos, ypos, zpos, width, bf, 0);
}
...
static int fontId(fontset_t *fontset, char *fontdesc) {
  ...
  for (ind = 0; ind < fontset->count; ind++) {
    if (strcmp(fontset->fonts[ind]->fontdesc, fontdesc) == 0) return ind;
  }
  return -1;
}
...
static int glutfontId(fontset_t *fontset, void *glutfont) {
  int ind = 0;
  for (ind = 0; ind < fontset->count; ind++) {
    if (fontset->fonts[ind]->glutfont == glutfont) return ind;
  }
  return -1;
}
...
void copy_font(glCompText *targetfont, const glCompText *sourcefont) {
  ...
  if (targetfont->fontdesc) free(targetfont->fontdesc);
...
void free_font_set(fontset_t *fontset) {
  ...
  free(fontset->fonts);
#ifndef _WIN32
  if (fontset->font_directory) rmdir(fontset->font_directory);
#endif
  if (fontset->font_directory) free(fontset->font_directory);
  free(fontset);
}
