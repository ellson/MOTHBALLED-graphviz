...
glCompButton *glCompButtonNew(glCompObj *par, GLfloat x, GLfloat y, GLfloat w,
                              GLfloat h, char *caption) {
  glCompButton *p;
...
int glCompButtonAddPngGlyph(glCompButton *b, char *fileName) {
  ...
  if (b->image) glCompImageDelete(b->image);
...
void glCompButtonHide(glCompButton *p) {
  p->common.visible = 0;
  if (p->label) p->label->common.visible = 0;
  if (p->image) p->image->common.visible = 0;
}

void glCompButtonShow(glCompButton *p) {
  p->common.visible = 1;
  if (p->label) p->label->common.visible = 1;
  if (p->image) p->image->common.visible = 1;
}
...
void glCompButtonDraw(glCompButton *p) {
  ...
  if (!p->common.visible) return;
  /*draw panel */
  glCompDrawRectPrism(&(ref.pos), ref.width, ref.height, p->common.borderWidth,
                      0.01, &(ref.color), !p->status);
  if (p->label) p->label->common.functions.draw((void *)p->label);
  if (p->image) p->image->common.functions.draw((void *)p->image);
  if (p->common.callbacks.draw)
    p->common.callbacks.draw(
        (void *)p); /*user defined drawing routines are called here. */
}
