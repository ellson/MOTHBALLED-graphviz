 ...
 * Return 0 on failure.
 */
glCompImage *glCompImageNewFile(glCompObj *par, GLfloat x, GLfloat y,
...
void glCompImageDelete(glCompImage *p) {
  glCompEmptyCommon(&p->common);
	...
	free(p->pngFile);
#endif
  if (p->texture) glCompDeleteTexture(p->texture);
  free(p);
}
...
int glCompImageLoad(glCompImage *i, unsigned char *data, int width, int height,
  ...
  if (data != NULL) {/*valid image data */
    ...
    i->texture =
        glCompSetAddNewTexImage(i->common.compset, width, height, data, is2D);
    if (i->texture) {
...
void glCompImageDraw(void *obj) {
  ...
  if (p->texture->id <= 0) {
    ...
    glVertex3d(ref.pos.x, ref.pos.y, d);
    glTexCoord2d(1.0f, 1.0f);
    glVertex3d(ref.pos.x + w, ref.pos.y, d);
    glTexCoord2d(1.0f, 0.0f);
    glVertex3d(ref.pos.x + w, ref.pos.y + h, d);
    glTexCoord2d(0.0f, 0.0f);
    glVertex3d(ref.pos.x, ref.pos.y + h, d);
    glEnd();
...
void glCompImageClick(glCompObj *o, GLfloat x, GLfloat y, glMouseButtonType t) {
  if (o->common.callbacks.click) o->common.callbacks.click(o, x, y, t);
}
