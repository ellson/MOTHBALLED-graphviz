...
static PangoLayout *get_pango_layout(char *markup_text, char *fontdescription,
                                     int fontsize, double *width,
                                     double *height) {
  ...
  if (width) *width = (double)pango_width / PANGO_SCALE;

  if (height) *height = (double)pango_height / PANGO_SCALE;

  return layout;
}

static cairo_status_t writer(void *closure, const unsigned char *data,
                             unsigned int length) {
...
int glCompCreateFontFile(char *fontdescription, int fs, char *fontfile,
  ...
  for (c = 0; c < 256; c++) {
    ...
    layout = get_pango_layout(buf, fontdescription, fs, &width, &height);
...
unsigned char *glCompCreatePangoTexture(char *fontdescription, int fontsize,
  ...
  //      surface
  // =cairo_image_surface_create(CAIRO_FORMAT_A8,(int)width,(int)height);
