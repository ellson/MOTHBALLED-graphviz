...
static void *dflt_open(char *f) { return fopen(f, "r"); }

static int dflt_close(void *fp) { return fclose((FILE *)fp); }
...
void closeIngraph(ingraph_state *sp) {
  if (!sp->ingraphs && sp->u.Files && sp->fp) sp->fns->closef(sp->fp);
  free(sp->fns);
  if (sp->heap) free(sp);
}
