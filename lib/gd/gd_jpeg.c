...
static void fatal_jpeg_error(j_common_ptr cinfo) {
  ...
  if (jmpbufw != 0) {
    ...
    fprintf(stderr,
            "gd-jpeg: EXTREMELY fatal error: longjmp"
            " returned control; terminating\n");
  } else {
    fprintf(stderr,
            "gd-jpeg: EXTREMELY fatal error: jmpbuf"
...
BGD_DECLARE(void) gdImageJpegCtx(gdImagePtr im, gdIOCtx *outfile, int quality) {
...
#ifdef JPEG_DEBUG
    printf(
        "gd-jpeg: interlace set, outputting progressive"
        " JPEG image\n");
#endif
  ...
  row = (JSAMPROW)gdCalloc(
      1, cinfo.image_width * cinfo.input_components * sizeof(JSAMPLE));
  if (row == 0) {
    fprintf(stderr,
            "gd-jpeg: error: unable to allocate JPEG row "
  ...
  if (im->trueColor) {
    ...
    for (i = 0; i < im->sy; i++) {
      ...
      if (nlines != 1)
        fprintf(stderr,
                "gd_jpeg: warning: jpeg_write_scanlines"
...
BGD_DECLARE(gdImagePtr) gdImageCreateFromJpegCtx(gdIOCtx *infile) {
  ...
  if (retval != JPEG_HEADER_OK)
    fprintf(stderr,
            "gd-jpeg: warning: jpeg_read_header returns"
            " %d, expected %d\n",
            retval, JPEG_HEADER_OK);

  if (cinfo.image_height > INT_MAX)
    fprintf(stderr,
            "gd-jpeg: warning: JPEG image height (%u) is"
  ...
  if (cinfo.image_width > INT_MAX)
    fprintf(stderr,
            "gd-jpeg: warning: JPEG image width (%u) is"
            " greater than INT_MAX (%d) (and thus greater than"
            " gd can handle)\n",
  ...
  if (im == 0) {
    fprintf(stderr,
            "gd-jpeg error: cannot allocate gdImage"
            " struct\n");
    goto error;
  }
  ...
  if (jpeg_start_decompress(&cinfo) != TRUE)
    fprintf(stderr,
            "gd-jpeg: warning: jpeg_start_decompress"
            " reports suspended data source\n");

#ifdef JPEG_DEBUG
  printf("gd-jpeg: JPEG image information:");
  if (cinfo.saw_JFIF_marker)
    printf(" JFIF version %d.%.2d", (int)cinfo.JFIF_major_version,
  ...
  printf(" %ux%u (raw) / %ux%u (scaled) %d-bit", cinfo.image_width,
         cinfo.image_height, cinfo.output_width, cinfo.output_height,
...
/* REMOVED by TBB 2/12/01. This field of the structure is
  ...
  if (cinfo.out_color_space == JCS_RGB) {
    if (cinfo.output_components != 3) {
      fprintf(stderr,
              "gd-jpeg: error: JPEG color quantization"
  ...
  if (row == 0) {
    fprintf(stderr,
            "gd-jpeg: error: unable to allocate row for"
  ...
  if (cinfo.out_color_space == JCS_CMYK) {
    for (i = 0; i < cinfo.output_height; i++) {
      ...
      if (nrows != 1) {
        fprintf(stderr,
                "gd-jpeg: error: jpeg_read_scanlines"
                " returns %u, expected 1\n",
                nrows);
        goto error;
      }
  ...
  if (jpeg_finish_decompress(&cinfo) != TRUE)
    fprintf(stderr,
            "gd-jpeg: warning: jpeg_finish_decompress"
            " reports suspended data source\n");
