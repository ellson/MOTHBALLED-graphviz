...
static void do_convert(unsigned char **to_p, unsigned char **from_p,
                       const char *code) {
  ...
  if (strcmp(code, NEWJISSTR) == 0 || strcmp(code, OLDJISSTR) == 0) {
    ...
    for (i = 0; from[i] != '\0' && j < BUFSIZ; i++) {
      ...
      if (p1 < 127)
        to[j++] = p1;
      else if ((p1 >= 161) && (p1 <= 223)) {/* Hankaku Kana */
        to[j++] = SS2;
        to[j++] = p1;
      } else {
        p2 = from[++i];
...
static int do_check_and_conv(unsigned char *to, unsigned char *from) {
  ...
  switch (DetectKanjiCode(from)) {
    ...
    case ESCI:
      debug(
          "This string includes Hankaku-Kana (jisx0201) escape sequence [ESC] "
          "+ ( + I.");
      do_convert(&tmp_p, &from, NEWJISSTR);
      break;
    case NEC:
