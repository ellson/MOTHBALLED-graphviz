...
static void ReadImage(gdImagePtr im, gdIOCtx *fd, int len, int height,
                      unsigned char (*cmap)[256], int interlace,
...
BGD_DECLARE(gdImagePtr) gdImageCreateFromGifCtx(gdIOCtxPtr fd) {
  ...
  for (;;) {
    ...
    if (!useGlobalColormap) {
      ...
      ReadImage(im, fd, width, height, localColorMap, BitSet(buf[8], INTERLACE),
                &ZeroDataBlock);
    } else {
      if (!haveGlobalColormap) {
        gdImageDestroy(im);
      ...
      ReadImage(im, fd, width, height, ColorMap, BitSet(buf[8], INTERLACE),
                &ZeroDataBlock);
    }
    if (Transparent != (-1)) {
...
#endif
      if ((buf[0] & 0x1) != 0) *Transparent = buf[3];
...
static int LWZReadByte_(gdIOCtx *fd, LZW_STATIC_DATA *sd, char flag,
  ...
  if (flag) {
    ...
    for (; i < (1 << MAX_LWZ_BITS); ++i) sd->table[0][i] = sd->table[1][0] = 0;
  ...
  if (sd->sp > sd->stack) return *--sd->sp;

  while ((code = GetCode(fd, &sd->scd, sd->code_size, FALSE, ZeroDataBlockP)) >=
         0) {
