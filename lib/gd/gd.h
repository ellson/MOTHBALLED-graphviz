...
#ifdef NETWARE
/* default fontpath for netware systems, could probably be handled in configure
 * for 2.1 */
...
BGD_DECLARE(gdImagePtr) gdImageCreateFromGd2PartCtx(gdIOCtxPtr in, int srcx,
...
BGD_DECLARE(gdImagePtr)
    gdImageCreatePaletteFromTrueColor(gdImagePtr im, int ditherFlag,
                                      int colorsWanted);
...
BGD_DECLARE(void) gdImageGifAnimBeginCtx(gdImagePtr im, gdIOCtx *out,
                                         int GlobalCM, int Loops);
BGD_DECLARE(void) gdImageGifAnimAddCtx(gdImagePtr im, gdIOCtx *out, int LocalCM,
...
BGD_DECLARE(void)
    gdImageEllipse(gdImagePtr im, int cx, int cy, int w, int h, int color);
BGD_DECLARE(void) gdImageFilledEllipse(gdImagePtr im, int cx, int cy, int w,
                                       int h, int color);
BGD_DECLARE(void)
