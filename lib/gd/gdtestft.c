...
int main(int argc, char *argv[]) {
  ...
  if (argc == 2) {
    ...
    fprintf(stderr,
            "If fontfilename is not a full or relative path, GDFONTPATH is "
            "searched for\n"
            "it. If GDFONTPATH is not set, /usr/share/fonts/truetype is "
            "searched.\n");
    exit(1);
  }
...
/* If destination is not truecolor, convert the JPEG to a
   ...
   colors for text smoothing. */
      ...
      gdImageCopyResampled(im, imb, 0, 0, 0, 0, sx * 3, sy, gdImageSX(imb),
                           gdImageSY(imb));
    } else {
