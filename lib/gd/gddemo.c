...
int main(void) {
  ...
  gdImageFilledRectangle(im_out, 0, 0, gdImageSX(im_out), gdImageSY(im_out),
                         white);
  ...
  if (!in) {
    ...
    for (a = 0; (a < 360); a += 45) {
      ...
      gdImageCopyRotated(im_out, im_in, 256 + 192 + cx, 192 + cy, 0, 0,
                         gdImageSX(im_in), gdImageSY(im_in), a);
    }
  }
  ...
  if (im_in) {
    ...
    gdImageCopyResized(brush, im_in, 0, 0, 0, 0, gdImageSX(brush),
                       gdImageSY(brush), gdImageSX(im_in), gdImageSY(im_in));
