  ...
  if (0xA1 <= byte && byte <= 0xFE) {
    ...
    *chPtr = (Tcl_UniChar)byte;
    ...
    if (((str[1] & 0xC0) == 0x80) && ((str[2] & 0xC0) == 0x80)) {
      ...
      *chPtr = (Tcl_UniChar)(((byte & 0x0F) << 12) | ((str[1] & 0x3F) << 6) |
                             (str[2] & 0x3F));
      return 3;
    }
    /*
...
static void *fontFetch(char **error, void *key) {
  ...
  *error = font_path(&(a->fontpath), a->fontlist);
  ...
  if (*error || !a->fontpath || !a->fontpath[0]) {
    ...
    free(a->fontlist);
    if (a->fontpath) free(a->fontpath);
    gdFree(a);

    if (!*error) *error = "font_path() returned an empty font pathname";
...
fprintf(stderr,"fontpathname=%s\n",fullname);
  ...
  if (!err && ((suffix = strstr(a->fontpath, ".pfa")) ||
  ...
  if (fg < 0) {
    ...
    if (im->trueColor) {
      ...
      a->tweencolor = gdImageColorResolve(
          im, (pixel * im->red[fg] + npixel * im->red[bg]) / NUMCOLORS,
...
static char *gdft_draw_bitmap(gdCache_head_t *tc_cache, gdImage *im, int fg,
  ...
  for (row = 0; row < bitmap.rows; row++) {
    ...
    for (col = 0; col < bitmap.width; col++, pc++) {
      if (bitmap.pixel_mode == ft_pixel_mode_grays) {
         ...
         */
        tc_key.pixel =
            ((bitmap.buffer[pc] * NUMCOLORS) + bitmap.num_grays / 2) /
            (bitmap.num_grays - 1);
      } else if (bitmap.pixel_mode == ft_pixel_mode_mono) {
        tc_key.pixel =
            ((bitmap.buffer[pc / 8] << (pc % 8)) & 128) ? NUMCOLORS : 0;
        /* 2.0.5: mode_mono fix from Giuliano Pochini */
        tc_key.pixel =
...
BGD_DECLARE(char *)
    gdImageStringFTEx(gdImage *im, int *brect, int fg, char *fontlist,
                      ...
                      gdFTStringExtraPtr strex) {
  FT_Matrix matrix;
  FT_Vector penf, oldpenf, delta, total_min = {0, 0}, total_max = {0, 0},
                                  glyph_min, glyph_max;
  FT_Face face;
  ...
  if (fg < 0) render_mode |= FT_LOAD_MONOCHROME;
  ...
  for (i = 0; i < face->num_charmaps; i++) {
    charmap = face->charmaps[i];
     ...
     (((FREETYPE_MAJOR == 2) &&                             \
       (((FREETYPE_MINOR == 1) && (FREETYPE_PATCH >= 3)) || \
        (FREETYPE_MINOR > 1))) ||                           \
      (FREETYPE_MAJOR > 2)))
...
#define FT_ENCODING_SJIS FT_ENCODING_MS_SJIS
    ...
    if (encoding == gdFTEX_Unicode) {
      if ((charmap->platform_id =
               3 && charmap->encoding_id == 1) /* Windows Unicode */
          ||
          (charmap->platform_id == 3 &&
           charmap->encoding_id == 0) /* Windows Symbol */
          ||
          (charmap->platform_id == 2 &&
           charmap->encoding_id == 1) /* ISO Unicode */
          ||
      ...
      if (charmap->platform_id == 3 &&
          charmap->encoding_id == 4) /* Windows Big5 */
      {
        encodingfound++;
        break;
      }
      ...
      if (charmap->platform_id == 3 &&
          charmap->encoding_id == 2) /* Windows Sjis */
      {
        encodingfound++;
        break;
      }
...
/* 2.0.24: David R. Morrison: use the more complete ifdef here. */
    ...
    if (brect) {/* only if need brect */
      ...
      if (i == 0) {
        ...
        if (glyph_min.x < total_min.x) total_min.x = glyph_min.x;
        if (glyph_min.y < total_min.y) total_min.y = glyph_min.y;
        if (glyph_max.x > total_max.x) total_max.x = glyph_max.x;
        if (glyph_max.y > total_max.y) total_max.y = glyph_max.y;
      }
    }
    ...
    if (render) {
      ...
      /* load glyph again into the slot (erase previous one)  - this time with
       * scaling */
      err = FT_Load_Glyph(face, glyph_index, render_mode);
      if (err) {
        gdCacheDelete(tc_cache);
  ...
  if (render) FT_Done_Size(platform_specific);
  ...
  for (i = 0; i < sizeof(postscript_alias) / sizeof(*postscript_alias); i++) {
    if (strcmp(fontname, postscript_alias[i].name) == 0) {
      ...
      FcPattern *pattern = FcPatternBuild(
          0, FC_FAMILY, FcTypeString, postscript_alias[i].family, FC_STYLE,
          FcTypeString, postscript_alias[i].style, NULL);
      ...
      if (strcmp((const char *)family, postscript_alias[i].family) != 0) {
        ...
        return "fontconfig: Didn't find expected font family. Perhaps URW Type "
               "1 fonts need installing?";
      }
      break;
    }
  }
 ...
 * else return NULL.
 */
static char *chkWinFontPath(char *envv) {
  char *root;
  char *path;
  root = getenv(envv);
  if (!root) return NULL;
...
static char *font_path(char **fontpath, char *name_list) {
  ...
  *fontpath = NULL;
...
#ifdef WIN32
  if (!fontsearchpath) fontsearchpath = getWinFontPath();
#endif
  ...
  for (name = gd_strtok_r(fontlist, LISTSEPARATOR, &strtok_ptr); name;
    ...
    fullname = gdRealloc(fullname, strlen(fontsearchpath) + strlen(name) + 8);
