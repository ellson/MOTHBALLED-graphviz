...
int main(int argc, char *argv[]) {
  ...
  /* 2.0.12 */
  fprintf(stderr,
          "annotate is not useful without freetype.\n"
  ...
  if (argc != 3) {
    ...
    fprintf(stderr,
            "If the file 'paris.ttf' exists in /usr/share/fonts/truetype or in "
            "a\n");
    fprintf(stderr,
            "location specified in the GDFONTPATH environment variable, 'font "
            "paris' is\n");
    fprintf(stderr,
            "sufficient. You may also specify the full, rooted path of a font "
            "file.\n");
    exit(1);
  }
  iin = argv[1];
