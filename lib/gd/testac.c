...
void testDrawing(gdImagePtr im_in, double scale, int blending, int palette,
                 char *filename);
...
void testDrawing(gdImagePtr im_in, double scale, int blending, int palette,
  ...
  gdImageFill(im_out, (int)(gdImageSX(im_in) * scale / 2),
              (int)(gdImageSY(im_in) * scale / 2), gdTrueColor(192, 192, 255));
  ...
  gdImageCopyResampled(
      im_out, im_in, 0, 0, 0, 0, (int)(gdImageSX(im_in) * scale),
