...
int Pshortestpath(Ppoly_t *polyp, Ppoint_t *eps, Ppolyline_t *output) {
  ...
  for (pi = 0, minx = HUGE_VAL, minpi = -1; pi < polyp->pn; pi++) {
    if (minx > polyp->ps[pi].x) minx = polyp->ps[pi].x, minpi = pi;
  }
  p2 = polyp->ps[minpi];
  ...
  for (trii = 0; trii < tril; trii++)
    for (trij = trii + 1; trij < tril; trij++) connecttris(trii, trij);

  /* find first and last triangles */
  for (trii = 0; trii < tril; trii++)
    if (pointintri(trii, &eps[0])) break;
  if (trii == tril) {
  ...
  ftrii = trii;
  for (trii = 0; trii < tril; trii++)
    if (pointintri(trii, &eps[1])) break;
  if (trii == tril) {
  ...
  while (trii != -1) {
    ...
    for (ei = 0; ei < 3; ei++)
      if (trip->e[ei].rtp && trip->e[ei].rtp->mark == 1) break;
...
static int isdiagonal(int pnli, int pnlip2, pointnlink_t **pnlps, int pnln) {
  ...
  if (ccw(pnlps[pnlim1]->pp, pnlps[pnli]->pp, pnlps[pnlip1]->pp) == ISCCW)
    res =
        (ccw(pnlps[pnli]->pp, pnlps[pnlip2]->pp, pnlps[pnlim1]->pp) == ISCCW) &&
  ...
  for (pnlj = 0; pnlj < pnln; pnlj++) {
    pnljp1 = (pnlj + 1) % pnln;
    if (!((pnlj == pnli) || (pnljp1 == pnli) || (pnlj == pnlip2) ||
          (pnljp1 == pnlip2)))
      if (intersects(pnlps[pnli]->pp, pnlps[pnlip2]->pp, pnlps[pnlj]->pp,
                     pnlps[pnljp1]->pp))
        return FALSE;
  }
  return TRUE;
...
static void loadtriangle(pointnlink_t *pnlap, pointnlink_t *pnlbp,
  ...
  for (ei = 0; ei < 3; ei++) trip->e[ei].ltp = trip;
}
...
static int marktripath(int trii, int trij) {
  ...
  for (ei = 0; ei < 3; ei++)
    if (tris[trii].e[ei].rtp && marktripath(tris[trii].e[ei].rtp - tris, trij))
      return TRUE;
  tris[trii].mark = 0;
  return FALSE;
}
...
static int between(Ppoint_t *pap, Ppoint_t *pbp, Ppoint_t *pcp) {
  ...
  if (ccw(pap, pbp, pcp) != ISON) return FALSE;
...
static void growtris(int newtrin) {
  ...
  if (!tris) {
    ...
    if (!(tris = (triangle_t *)realloc((void *)tris, TRIANGLESIZE * newtrin))) {
