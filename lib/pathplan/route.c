...
static int reallyroutespline(Pedge_t *, int, Ppoint_t *, int, Ppoint_t,
                             Ppoint_t);
static int mkspline(Ppoint_t *, int, tna_t *, Ppoint_t, Ppoint_t, Ppoint_t *,
                    Ppoint_t *, Ppoint_t *, Ppoint_t *);
static int splinefits(Pedge_t *, int, Ppoint_t, Pvector_t, Ppoint_t, Pvector_t,
                      Ppoint_t *, int);
...
static int reallyroutespline(Pedge_t *edges, int edgen, Ppoint_t *inps,
  ...
  if (tnan < inpn) {
    if (!tnas) {
      if (!(tnas = malloc(sizeof(tna_t) * inpn))) return -1;
    } else {
      if (!(tnas = realloc(tnas, sizeof(tna_t) * inpn))) return -1;
    }
    tnan = inpn;
  }
  tnas[0].t = 0;
  ...
  for (i = 1; i < inpn; i++) tnas[i].t /= tnas[inpn - 1].t;
  for (i = 0; i < inpn; i++) {
    tnas[i].a[0] = scale(ev0, B1(tnas[i].t));
    tnas[i].a[1] = scale(ev1, B2(tnas[i].t));
  }
  if (mkspline(inps, inpn, tnas, ev0, ev1, &p1, &v1, &p2, &v2) == -1) return -1;
  if (splinefits(edges, edgen, p1, v1, p2, v2, inps, inpn)) return 0;
...
static int mkspline(Ppoint_t *inps, int inpn, tna_t *tnas, Ppoint_t ev0,
                    Ppoint_t ev1, Ppoint_t *sp0, Ppoint_t *sv0, Ppoint_t *sp1,
                    Ppoint_t *sv1) {
  Ppoint_t tmp;
...
static double dist_n(Ppoint_t *p, int n) {
  ...
  rv = 0.0;
  for (i = 1; i < n; i++) {
    rv += sqrt((p[i].x - p[i - 1].x) * (p[i].x - p[i - 1].x) +
...
static int splineisinside(Pedge_t *edges, int edgen, Ppoint_t *sps) {
  ...
  for (ei = 0; ei < edgen; ei++) {
    ...
    /* if ((rootn = splineintersectsline (sps, lps, roots)) == 4)
       return 1; */
    if ((rootn = splineintersectsline(sps, lps, roots)) == 4) continue;
    for (rooti = 0; rooti < rootn; rooti++) {
      if (roots[rooti] < EPSILON2 || roots[rooti] > 1 - EPSILON2) continue;
      t = roots[rooti];
      td = t * t * t;
      tc = 3 * t * t * (1 - t);
      tb = 3 * t * (1 - t) * (1 - t);
      ta = (1 - t) * (1 - t) * (1 - t);
      ip.x = ta * sps[0].x + tb * sps[1].x + tc * sps[2].x + td * sps[3].x;
      ip.y = ta * sps[0].y + tb * sps[1].y + tc * sps[2].y + td * sps[3].y;
    ...
    if (ycoeff[1] == 0) {
      ...
      if (xrootn == 4)
        if (yrootn == 4)
          ...
          for (j = 0; j < yrootn; j++) addroot(yroots[j], roots, &rootn);
      else if (yrootn == 4)
        for (i = 0; i < xrootn; i++) addroot(xroots[i], roots, &rootn);
      else
        for (i = 0; i < xrootn; i++)
          for (j = 0; j < yrootn; j++)
            if (xroots[i] == yroots[j]) addroot(xroots[i], roots, &rootn);
      return rootn;
    } else {
      points2coeff(sps[0].x, sps[1].x, sps[2].x, sps[3].x, scoeff);
      scoeff[0] -= xcoeff[0];
      ...
      for (i = 0; i < xrootn; i++) {
        ...
        if (tv >= 0 && tv <= 1) {
          ...
          if ((0 <= sv) && (sv <= 1)) addroot(tv, roots, &rootn);
        }
      }
      return rootn;
    }
...
static void addroot(double root, double *roots, int *rootnp) {
  if (root >= 0 && root <= 1) roots[*rootnp] = root, (*rootnp)++;
}
...
static void growops(int newopn) {
  ...
  if (!ops) {
    ...
    if (!(ops = (Ppoint_t *)realloc((void *)ops, POINTSIZE * newopn))) {
