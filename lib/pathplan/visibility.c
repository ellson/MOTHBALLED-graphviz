...
static array2 allocArray(int V, int extra) {
  ...
  for (i = V; i < V + extra; i++) arr[i] = (COORD *)0;
...
int wind(Ppoint_t a, Ppoint_t b, Ppoint_t c) {
  ...
  /* need to allow for small math errors.  seen with "gcc -O2 -mcpu=i686
   * -ffast-math" */
 ...
 */
static COORD dist(Ppoint_t a, Ppoint_t b) { return sqrt(dist2(a, b)); }
 ...
 */
static int clear(Ppoint_t pti, Ppoint_t ptj, int start, int end, int V,
                 Ppoint_t pts[], int nextPt[], int prevPt[]) {
  int k;

  for (k = 0; k < start; k++) {
    if (INTERSECT(pti, ptj, pts[k], pts[nextPt[k]], pts[prevPt[k]])) return 0;
  }
  for (k = end; k < V; k++) {
    if (INTERSECT(pti, ptj, pts[k], pts[nextPt[k]], pts[prevPt[k]])) return 0;
  }
...
int directVis(Ppoint_t p, int pp, Ppoint_t q, int qp, vconfig_t *conf) {
  ...
  for (k = 0; k < s1; k++) {
    if (INTERSECT(p, q, pts[k], pts[nextPt[k]], pts[prevPt[k]])) return 0;
  }
  for (k = e1; k < s2; k++) {
    if (INTERSECT(p, q, pts[k], pts[nextPt[k]], pts[prevPt[k]])) return 0;
  }
