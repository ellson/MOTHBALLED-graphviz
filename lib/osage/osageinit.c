...
static void layout(Agraph_t* g, int depth) {
  ...
  if ((pinfo.mode == l_array) && (pinfo.flags & PK_USER_VALS)) {
    ...
    if (cattr || vattr)
      ...
      agerr(AGWARN,
            "Graph %s has array packing with user values but no \"sortv\" "
  ...
  if (Verbose > 1) {
    indent(depth);
    fprintf(stderr, "%s : %f %f %f %f\n", agnameof(g), rootbb.LL.x, rootbb.LL.y,
            rootbb.UR.x, rootbb.UR.y);
  }

  /* Translate so that rootbb.LL is origin.
   * This makes the repositioning simpler; we just translate the clusters and
   * nodes in g by
...
static void reposition(Agraph_t* g, int depth) {
  ...
  for (i = 1; i <= GD_n_cluster(g); i++) {
    ...
    if (depth) {
      ...
      if (Verbose > 1) {
        indent(depth);
        fprintf(stderr, "%s : %f %f %f %f\n", agnameof(subg), sbb.LL.x,
                sbb.LL.y, sbb.UR.x, sbb.UR.y);
      }
      GD_bb(subg) = sbb;
    }
