...
static Dtdisc_t constr = {
    offsetof(nitem, val), sizeof(int),     offsetof(nitem, link),
    NIL(Dtmake_f),        NIL(Dtfree_f),   (Dtcompar_f)cmpitem,
...
#ifdef DEBUG
static int indegree(graph_t *g, node_t *n) {
  edge_t *e;
  int cnt = 0;
...
static int outdegree(graph_t *g, node_t *n) {
  edge_t *e;
  int cnt = 0;
...
static void validate(graph_t *g) {
  node_t *n;
  edge_t *e;
  int i, cnt;
...
static graph_t *mkNConstraintG(graph_t *g, Dt_t *list, intersectfn intersect,
  ...
  agbindrec(cg, "Agraphinfo_t", sizeof(Agraphinfo_t),
  ...
  for (p = (nitem *)dtflatten(list); p;
    ...
    for (nxp = (nitem *)dtlink(link, (Dtlink_t *)p); nxp;
      ...
      if (intersect(p, nxp)) {
        ...
        agbindrec(e, "Agedgeinfo_t", sizeof(Agedgeinfo_t),
                  TRUE);  // edge custom data
        assert(delta <= 0xFFFF);
        ED_minlen(e) = delta;
        ED_weight(e) = 1;
      }
  ...
  for (p = (nitem *)dtflatten(list); p;
    ...
    if (oldval != p->val) {/* new pos: reset nxt */
      ...
      for (nxt = (nitem *)dtlink(link, (Dtlink_t *)p); nxt;
        ...
        if (nxt->val != oldval) break;
      }
      if (!nxt) break;
    }
    for (nxp = nxt; nxp; nxp = (nitem *)dtlink(list, (Dtlink_t *)nxp)) {
      if (intersect(p, nxp)) agedge(vg, p->vnode, nxp->vnode, NULL, 1);
    }
  }
...
#define overlap(pb, qb)                                                    \
  ((pb.LL.x <= qb.UR.x) && (qb.LL.x <= pb.UR.x) && (pb.LL.y <= qb.UR.y) && \
   (qb.LL.y <= pb.UR.y))
...
static int overlaps(nitem *p, int cnt) {
  ...
  for (i = 0; i < cnt - 1; i++) {
    pj = pi + 1;
    for (j = i + 1; j < cnt; j++) {
      if (overlap(pi->bb, pj->bb)) return 1;
      pj++;
    }
    pi++;
  }
