...
int stress_majorization_with_hierarchy(
  ...
  *************************************************/
  ...
  if (!directionalityExist) {
    return stress_majorization_kD_mkernel(graph, n, nedges_graph, d_coords,
                                          nodes, dim, opts, model, maxi);
  }
  ...
  ******************************************************************/
  ...
  if (smart_ini) {
    ...
    if (dim > 2) {
      ...
      if (stress_majorization_kD_mkernel(graph, n, nedges_graph, d_coords + 1,
                                         nodes, dim - 1, opts, model, 15) < 0)
        return -1;
    ...
    if (levels_gap > 0) {
      ...
      for (i = 0; i < num_levels; i++) {
        displacement +=
            MAX((double)0, levels_gap - (y[ordering[levels[i]]] + displacement -
    ...
    for (k = 0; k < dim; k++) {
      ...
      if (k == 1) {
        ...
        constrained_majorization_new_with_gaps(
            cMajEnv, b[k], coords, dim, k, localConstrMajorIterations,
            hierarchy_boundaries, levels_gap);
