...
static void heapify(heap *h, int i, int index[], Word dist[]) {
  ...
  while (1) {
    ...
    if (insideHeap(h, l) && greaterPriority(h, l, i, dist))
      ...
      largest = i;
    if (insideHeap(h, r) && greaterPriority(h, r, largest, dist)) largest = r;
...
static void initHeap(heap *h, int startVertex, int index[], Word dist[],
                     int n) {
  int i, count;
  ...
  for (j = (n - 1) / 2; j >= 0; j--) heapify(h, j, index, dist);
}
...
static void increaseKey(heap *h, int increasedVertex, Word newDist, int index[],
                        Word dist[]) {
  int placeInHeap;
  int i;

  if (dist[increasedVertex] <= newDist) return;
  ...
  i = placeInHeap;
  while (i > 0 &&
         dist[h->data[parent(i)]] >
             newDist) {/* can write here: greaterPriority(i,parent(i),dist) */
...
void dijkstra(int vertex, vtx_data *graph, int n, DistType *dist) {
  ...
  for (i = 0; i < n; i++) dist[i] = (DistType)MAX_DIST;
  dist[vertex] = 0;
...
int dijkstra_bounded(int vertex, vtx_data *graph, int n, DistType *dist,
                     int bound, int *visited_nodes)
    /* make dijkstra, but consider only nodes whose *unweighted* distance from
       'vertex'  */
    /* is at most 'bound' */
...
static void initHeap_f(heap *h, int startVertex, int index[], float dist[],
                       int n) {
  int i, count;
  int j; /* We cannot use an unsigned value in this loop */
  h->data = N_GNEW(n - 1, int);
  ...
  for (j = (n - 1) / 2; j >= 0; j--) heapify_f(h, j, index, dist);
}
