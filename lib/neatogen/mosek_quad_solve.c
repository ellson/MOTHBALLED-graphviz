...
***********************/
MosekEnv *mosek_init_hier(float *lap, int n, int *ordering, int *level_indexes,
                          int num_divisions, float separation) {
  int count = 0;
  ...
  /* int
   * lapsize=mskEnv->num_variables+(mskEnv->num_variables*(mskEnv->num_variables-1))/2;
   */
  ...
  /* constraint matrix */
  separation /=
      2.0; /* separation between each node and it's adjacent constraint */
  num_constraints =
      get_num_digcola_constraints(levels, num_levels) + num_divisions + 1;
/* constraints of the form x_i - x_j >= sep so 2 non-zero entries per constraint
 * in LHS matrix
 ...
 */
  ...
  if (mskEnv->r == MSK_RES_OK) {
    ...
    mskEnv->r = MSK_maketask(mskEnv->env, num_constraints,
                             mskEnv->num_variables, &mskEnv->task);

    if (mskEnv->r == MSK_RES_OK) {
      int c_ind = 0;
      int c_var = n - 1;
      mskEnv->r = MSK_linkfunctotaskstream(mskEnv->task, MSK_STREAM_LOG, NULL,
                                           printstr);
      /* Resize the task. */
      if (mskEnv->r == MSK_RES_OK)
        mskEnv->r = MSK_resizetask(mskEnv->task, num_constraints,
                                   mskEnv->num_variables, 0, /* no cones!! */
      ...
      for (j = 0; j < mskEnv->num_variables && mskEnv->r == MSK_RES_OK; ++j)
        mskEnv->r = MSK_putbound(mskEnv->task, 0, j, MSK_BK_RA, -MSK_INFINITY,
                                 MSK_INFINITY);
      for (j = 0; j < levels[0].num_nodes && mskEnv->r == MSK_RES_OK; j++) {
        ...
        mskEnv->r = MSK_putbound(mskEnv->task, 1, c_ind, MSK_BK_LO, separation,
                                 MSK_INFINITY);
        c_ind++;
      }
      for (i = 0; i < num_divisions && mskEnv->r == MSK_RES_OK; i++) {
        ...
        for (j = 0; j < levels[i].num_nodes && mskEnv->r == MSK_RES_OK; j++) {
          ...
          mskEnv->r = MSK_putbound(mskEnv->task, 1, c_ind, MSK_BK_LO,
                                   separation, MSK_INFINITY);
          c_ind++;
        }
        for (j = 0; j < levels[i + 1].num_nodes && mskEnv->r == MSK_RES_OK;
             j++) {
      ...
      for (i = 0; i < num_divisions + 1 && mskEnv->r == MSK_RES_OK; i++) {
        ...
        mskEnv->r = MSK_putavec(mskEnv->task, 1, c_ind, 2, subi, vali);
        mskEnv->r =
            MSK_putbound(mskEnv->task, 1, c_ind, MSK_BK_LO, 0, MSK_INFINITY);
        c_ind++;
      }
#ifdef EQUAL_WIDTH_LEVELS
      ...
      if (mskEnv->r == MSK_RES_OK) {
         ...
         */
        mskEnv->r = MSK_putqobj(mskEnv->task, nonzero_lapsize, mskEnv->qsubi,
                                mskEnv->qsubj, mskEnv->qval);
      }
    }
  }
...
*/
void mosek_quad_solve_hier(MosekEnv *mskEnv, float *b, int n, float *coords,
  ...
  fprintf(logfile, "]\n");
#endif
  if (mskEnv->r == MSK_RES_OK) mskEnv->r = MSK_optimize(mskEnv->task);

  if (mskEnv->r == MSK_RES_OK) {
    MSK_getsolutionslice(mskEnv->task, MSK_SOL_ITR, MSK_SOL_ITEM_XX, 0,
                         mskEnv->num_variables, mskEnv->xx);
...
MosekEnv *mosek_init_sep(float *lap, int num_variables, int num_dummy_vars,
  ...
  if (mskEnv->r == MSK_RES_OK) {
    ...
    if (mskEnv->r == MSK_RES_OK) {
      ...
      if (mskEnv->r == MSK_RES_OK)
        mskEnv->r =
            MSK_resizetask(mskEnv->task, num_constraints, mskEnv->num_variables,
                           0, /* no cones!! */
                              /* number of non-zero constraint matrix entries:
                            *   each constraint applies to 2 vars
                            */
                           2 * num_constraints, nonzero_lapsize);
      ...
      for (i = 0; i < num_constraints; i++) {
        ...
        if (u < 0) {
          mskEnv->r = MSK_putbound(mskEnv->task, 0, v, MSK_BK_RA, -MSK_INFINITY,
                                   -separation);
          assert(mskEnv->r == MSK_RES_OK);
        } else if (v < 0) {
          mskEnv->r = MSK_putbound(mskEnv->task, 0, u, MSK_BK_RA, separation,
                                   MSK_INFINITY);
          assert(mskEnv->r == MSK_RES_OK);
        } else {
          ...
          assert(mskEnv->r == MSK_RES_OK);
          mskEnv->r = MSK_putbound(mskEnv->task, 1, i, MSK_BK_LO, separation,
                                   MSK_INFINITY);
          assert(mskEnv->r == MSK_RES_OK);
        }
      }
