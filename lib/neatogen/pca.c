...
void PCA_alloc(DistType **coords, int dim, int n, double **new_coords,
               int new_dim) {
  ...
  for (i = 0; i < new_dim; i++) eigs[i] = N_GNEW(dim, double);
  ...
  for (i = 0; i < new_dim; i++) free(eigs[i]);
  free(eigs);
  free(evals);
  free(DD[0]);
  free(DD);
...
boolean iterativePCA_1D(double **coords, int dim, int n,
...
double dist(double **coords, int dim, int p1, int p2) {
  ...
  for (i = 0; i < dim; i++) {
    sum += (coords[i][p1] - coords[i][p2]) * (coords[i][p1] - coords[i][p2]);
  }
  return sqrt(sum);
}
  ...
  for (i = 0; i < n; i++) {
    ...
    *ewgts = 0;
    for (j = 1; j < laplacian[i].nedges; j++) {
      neighbor = edges[j];
      *ewgts -= ewgts[j] = float(-1.0 / (dist(X, dim, i, neighbor) + 1e-10));
    }
  }
}
