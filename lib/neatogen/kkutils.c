...
static DistType **compute_apsp_dijkstra(vtx_data *graph, int n) {
  ...
  for (i = 0; i < n; i++) dij[i] = storage + i * n;
...
static void split_by_place(double *place, int *nodes, int first, int last,
                           int *middle) {
...
static float *fvals;
static int fcmpf(int *ip1, int *ip2) {
  float d1 = fvals[*ip1];
  float d2 = fvals[*ip2];
  if (d1 < d2)
    return -1;
...
void quicksort_placef(float *place, int *ordering, int first, int last) {
  if (first < last) {
    fvals = place;
    qsort(ordering + first, last - first + 1, sizeof(ordering[0]),
          (qsort_cmpf)fcmpf);
  }
}
