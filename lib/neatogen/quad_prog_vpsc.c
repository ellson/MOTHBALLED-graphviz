...
/*
 * Use gradient-projection to solve Variable Placement with Separation
 * Constraints problem.
 */
int constrained_majorization_vpsc(CMajEnvVPSC *e, float *b, float *place,
                                  int max_iterations) {
  int i, j, counter;
  float *g, *old_place, *d;
...
/*
 * Set up environment and global constraints (dir-edge constraints, containment
 *constraints
 * etc).
 *
...
CMajEnvVPSC *initCMajVPSC(int n, float *packedMat, vtx_data *graph,
  ...
  e->gm = 0;
  if (diredges == 1) {
    if (Verbose) fprintf(stderr, "  generate edge constraints...\n");
    for (i = 0; i < e->nv; i++) {
    ...
    if (Verbose) fprintf(stderr, "Found %d DiG-CoLa boundaries\n", e->ndv);
    ...
    for (i = 0; i < e->ndv; i++) {
      ...
      for (j = 0; j < levels[i].num_nodes; j++) {
        e->gcs[e->gm++] =
            newConstraint(e->vs[levels[i].nodes[j]], e->vs[cvar], halfgap);
      }
      /* incoming constraints for each var in level above boundary */
      for (j = 0; j < levels[i + 1].num_nodes; j++) {
        e->gcs[e->gm++] =
            newConstraint(e->vs[cvar], e->vs[levels[i + 1].nodes[j]], halfgap);
      }
    }
  ...
  /* fprintf(stderr,"  generate edge constraints... done:
   * n=%d,m=%d\n",e->n,e->gm); */
 ...
 */
void generateNonoverlapConstraints(CMajEnvVPSC *e, float nsizeScale,
                                   float **coords, int k,
  ...
  for (i = 0; i < n; i++) {
    bb[i].LL.x =
        coords[0][i] - nsizeScale * opt->nsize[i].x / 2.0 - opt->gap.x / 2.0;
    bb[i].UR.x =
        coords[0][i] + nsizeScale * opt->nsize[i].x / 2.0 + opt->gap.x / 2.0;
    bb[i].LL.y =
        coords[1][i] - nsizeScale * opt->nsize[i].y / 2.0 - opt->gap.y / 2.0;
    bb[i].UR.y =
        coords[1][i] + nsizeScale * opt->nsize[i].y / 2.0 + opt->gap.y / 2.0;
  }
  if (genclusters) {
  ...
  if (Verbose) fprintf(stderr, "  generated %d constraints\n", e->m);
  ...
  if (opt->mosek) {
    ...
    e->mosekEnv =
        mosek_init_sep(e->packedMat, e->nv + e->nldv, e->ndv, e->cs, e->m);
  }
#endif
#ifdef WIN32
  free(bb);
...
/*
 * Statically remove overlaps, that is remove all overlaps by moving each node
 * as
 * little as possible.
 */
