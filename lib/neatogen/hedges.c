...
void ELinitialize() {
  ...
  if (ELhash == NULL) ELhash = N_GNEW(ELhashsize, Halfedge *);
...
Halfedge *ELleftbnd(Point *p) {
  ...
  if (bucket > 0 && bucket < ELhashsize - 1) {
    if (ELhash[bucket] != (Halfedge *)NULL) ELhash[bucket]->ELrefcnt -= 1;
    ELhash[bucket] = he;
...
Halfedge *ELright(Halfedge *he) { return (he->ELright); }

Halfedge *ELleft(Halfedge *he) { return (he->ELleft); }
