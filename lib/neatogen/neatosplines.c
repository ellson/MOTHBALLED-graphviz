...
static void freeitem(Dt_t *d, edgeitem *obj, Dtdisc_t *disc) { free(obj); }

static int cmpitems(Dt_t *d, edgeinfo *key1, edgeinfo *key2, Dtdisc_t *disc) {
  int x;

  if (key1->n1 > key2->n1) return 1;
  if (key1->n1 < key2->n1) return -1;
  if (key1->n2 > key2->n2) return 1;
  if (key1->n2 < key2->n2) return -1;

  if ((x = key1->p1.x - key2->p1.x)) return x;
  if ((x = key1->p1.y - key2->p1.y)) return x;
  if ((x = key1->p2.x - key2->p2.x)) return x;
  return (key1->p2.y - key2->p2.y);
}

Dtdisc_t edgeItemDisc = {
    offsetof(edgeitem, id), sizeof(edgeinfo),   offsetof(edgeitem, link),
    (Dtmake_f)newitem,      (Dtfree_f)freeitem, (Dtcompar_f)cmpitems,
    0,                      0,                  0};
...
void makeSelfArcs(path *P, edge_t *e, int stepx) {
  ...
  if ((cnt == 1) || Concentrate) {
    ...
    if (ED_label(e)) updateBB(agraphof(agtail(e)), ED_label(e));
    makePortLabels(e);
  } else {
    int i;
 ...
 */
Ppolyline_t getPath(edge_t *e, vconfig_t *vconfig, int chkPts, Ppoly_t **obs,
                    int npoly) {
  Ppolyline_t line;
  int pp, qp;
  Ppoint_t p, q;
...
/* makePolyline:
 */
static void makePolyline(graph_t *g, edge_t *e) {
 ...
 */
void makeSpline(graph_t *g, edge_t *e, Ppoly_t **obs, int npoly,
                boolean chkPts) {
...
static int _spline_edges(graph_t *g, expand_t *pmargin, int edgetype) {
  ...
  if (obs) {
    if ((legal = Plegal_arrangement(obs, npoly))) {
      ...
      if (edgetype == ET_ORTHO)
        agerr(AGWARN,
              "the bounding boxes of some nodes touch - falling back to "
              "straight line edges\n");
      else
        agerr(AGWARN,
              "some nodes with margin (%.02f,%.02f) touch - falling back to "
              "straight line edges\n",
              pmargin->x, pmargin->y);
    }
  }
  ...
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
    for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
       ...
       * agnameof(aghead(e)), ED_count(e)); */
      node_t *head = aghead(e);
      if (useEdges && ED_spl(e)) {
        addEdgeLabels(g, e, add_pointf(ND_coord(n), ED_tail_port(e).p),
                      add_pointf(ND_coord(head), ED_head_port(e).p));
      } else if (ED_count(e) == 0)
        continue;          /* only do representative */
...
#ifdef HAVE_GTS
  if (rtr) freeRouter(rtr);
#endif

  if (vconfig) Pobsclose(vconfig);
  if (P) {
    free(P->boxes);
    free(P);
  }
  if (obs) {
    for (i = 0; i < npoly; i++) free(obs[i]);
    free(obs);
  }
  return 0;
}
 ...
 */
int splineEdges(graph_t *g, int (*edgefn)(graph_t *, expand_t *, int),
                int edgetype) {
  node_t *n;
  edge_t *e;
  expand_t margin;
  ...
  if (edgefn(g, &margin, edgetype)) return 1;
...
static void scaleBB(graph_t *g, double xf, double yf) {
  ...
  for (i = 1; i <= GD_n_cluster(g); i++) scaleBB(GD_clust(g)[i], xf, yf);
}
...
static void translateG(Agraph_t *g, pointf offset) {
  ...
  for (i = 1; i <= GD_n_cluster(g); i++) translateG(GD_clust(g)[i], offset);
}
...
static boolean _neato_set_aspect(graph_t *g) {
  ...
  if (GD_drawing(g)->ratio_kind) {
    ...
    if (GD_drawing(g)->ratio_kind == R_FILL) {
      ...
      if (GD_drawing(g)->size.x <= 0) return (translated || FALSE);
