...
int power_iteration(double **square_mat, int n, int neigs, double **eigs,
  ...
  choose:
    ...
    evals[i] = angle * len; /* this is the Rayleigh quotient (up to errors due
                               to orthogonalization):
                               u*(A*u)/||A*u||)*||A*u||, where u=last_vec, and
                               ||u||=1
                             */
  }
exit:
...
void mult_dense_mat(double **A, float **B, int dim1, int dim2, int dim3,
                    float ***CC) {
  /*
...
void mult_dense_mat_d(double **A, float **B, int dim1, int dim2, int dim3,
                      double ***CC) {
  /*
    A is dim1 x dim2, B is dim2 x dim3, C = A x B
  */
...
void mult_sparse_dense_mat_transpose(vtx_data *A, double **B, int dim1,
                                     int dim2, float ***CC) {
  /*
...
void init_vec_orth1(int n, double *vec) {
  ...
  for (i = 0; i < n; i++) vec[i] = rand() % RANGE;
...
/* inline */
void right_mult_with_vector_f(float **matrix, int n, double *vector,
                              double *result) {
  int i, j;
  ...
  for (i = 0; i < n; i++) {
    res = 0;
    for (j = 0; j < n; j++) res += matrix[i][j] * vector[j];
    result[i] = res;
  }
  /* orthog1(n,vector); */
}
...
/* inline */
void vectors_subtraction(int n, double *vector1, double *vector2,
                         double *result) {
  int i;
  for (i = 0; i < n; i++) {
...
#ifdef UNUSED
/* inline */
void vectors_mult_addition(int n, double *vector1, double alpha,
                           double *vector2) {
  int i;
  for (i = 0; i < n; i++) {
...
double max_abs(int n, double *vector) {
  ...
  int i;
  for (i = 0; i < n; i++)
    if (fabs(vector[i]) > max_val) max_val = fabs(vector[i]);
...
/* inline */
void right_mult_with_vector_transpose(double **matrix, int dim1, int dim2,
  ...
  for (i = 0; i < dim1; i++) {
    res = 0;
    for (j = 0; j < dim2; j++) res += matrix[j][i] * vector[j];
    result[i] = res;
  }
}

/* inline */
void right_mult_with_vector_d(double **matrix, int dim1, int dim2,
                              double *vector, double *result) {
  ...
  for (i = 0; i < dim1; i++) {
    res = 0;
    for (j = 0; j < dim2; j++) res += matrix[i][j] * vector[j];
    result[i] = res;
  }
}
...
#ifdef UNUSED
/* inline */
void right_mult_with_vectorf(vtx_data *matrix, int n, float *vector,
                             float *result) {
  int i, j;
...
/* inline */
void right_mult_with_vector_fd(float **matrix, int n, float *vector,
                               double *result) {
  int i, j;
...
/* inline */
void vectors_substractionf(int n, float *vector1, float *vector2,
                           float *result) {
  int i;
  for (i = 0; i < n; i++) {
    result[i] = vector1[i] - vector2[i];
...
/* inline */
void vectors_mult_additionf(int n, float *vector1, float alpha,
                            float *vector2) {
  int i;
  for (i = 0; i < n; i++) {
    vector1[i] = vector1[i] + alpha * vector2[i];
...
double max_absf(int n, float *vector) {
  ...
  float max_val = -1e30f;
  for (i = 0; i < n; i++)
    if (fabs(vector[i]) > max_val) max_val = (float)(fabs(vector[i]));
...
void init_vec_orth1f(int n, float *vec) {
  ...
  for (i = 0; i < n; i++) vec[i] = (float)(rand() % RANGE);
