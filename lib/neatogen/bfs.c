  ...
  if (graph[0].ewgts == NULL) {
    ...
    while (deQueue(Q, &closestVertex)) {
      ...
      for (i = 1; i < graph[closestVertex].nedges; i++) {
        ...
        if (dist[neighbor] < -0.5) {/* first time to reach neighbor */
          dist[neighbor] =
              closestDist + (DistType)graph[closestVertex].ewgts[i];
          enQueue(Q, neighbor);
        }
      }
    }
...
int bfs_bounded(int vertex, vtx_data *graph, int n, DistType *dist, Queue *Q,
...
void freeQueue(Queue *qp) { free(qp->data); }
...
boolean deQueue(Queue *qp, int *vertex) {
  if (qp->start >= qp->end) return FALSE; /* underflow */
  *vertex = qp->data[qp->start++];
  return TRUE;
}

boolean enQueue(Queue *qp, int vertex) {
  if (qp->end >= qp->queueSize) return FALSE; /* overflow */
