  ...
  switch (cond) {
    ...
    case 2: /*     the two lines  have a common segment  */
      if (online(l, m, 0) == -1) {/* ms between ls and le */
        pt1 = ms;
        pt2 =
            (online(m, l, 1) == -1) ? ((online(m, l, 0) == -1) ? le : ls) : me;
      } else if (online(l, m, 1) == -1) {/* me between ls and le */
        pt1 = me;
        pt2 =
            (online(l, m, 0) == -1) ? ((online(m, l, 0) == -1) ? le : ls) : ms;
      } else {
        /* may be degenerate? */
 ...
 */
static int find_intersection(vertex *l, vertex *m, intersection *ilist,
                             data *input) {
  double x, y;
  pointf p;
  int i[3];
 ...
 */
static int find_ints(vertex vertex_list[], polygon polygon_list[], data *input,
                     intersection ilist[]) {
  int i, j, k, found = 0;
  ...
  for (i = 0; i < input->nvertices; i++) pvertex[i] = vertex_list + i;
...
finish:
  for (tempa = all.first, j = 0; j < all.number; j++, tempa = new) {
    new = tempa->next;
    free(tempa);
  }
  free(pvertex);
...
int Plegal_arrangement(Ppoly_t **polys, int n_polys) {
  ...
  for (i = nverts = 0; i < n_polys; i++) nverts += polys[i]->pn;
