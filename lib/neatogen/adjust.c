...
static int countOverlap(int iter) {
  ...
  for (i = 0; i < nsites; i++) nodeInfo[i].overlaps = 0;
  ...
  if (Verbose > 1) fprintf(stderr, "overlap [%d] : %d\n", iter, count);
  return count;
}
...
static double areaOf(Point a, Point b, Point c) {
  ...
  area =
      (double)(fabs(a.x * (b.y - c.y) + b.x * (c.y - a.y) + c.x * (a.y - b.y)) /
               2);
  return area;
}
...
static int vAdjust(void) {
  ...
  if (!useIter || (iterations > 0)) overlapCnt = countOverlap(iterCnt);

  if ((overlapCnt == 0) || (iterations == 0)) return 0;
...
double *getSizes(Agraph_t *g, pointf pad, int *n_elabels, int **elabels) {
  ...
  if (elabels && nedge_nodes) {
    ...
    nedge_nodes = 0;
    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
      if (IS_LNODE(n)) elabs[nedge_nodes++] = ND_id(n);
    }
    *elabels = elabs;
...
SparseMatrix makeMatrix(Agraph_t *g, int dim, SparseMatrix *D) {
  ...
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
    ...
    for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
      I[i] = row;
      J[i] = ND_id(aghead(e));
      if (!sym || (sscanf(agxget(e, sym), "%lf", &v) != 1)) v = 1;
      val[i] = v;
      /* edge length */
      if (symD) {
...
#if ((defined(HAVE_GTS) || defined(HAVE_TRIANGLE)) && defined(SFDP))
static int fdpAdjust(graph_t *g, adjust_data *am) {
  ...
  remove_overlap(Ndim, A, pos, sizes, am->value, am->scaling, ELSCHEME_NONE, 0,
                 NULL, NULL, mapBool(agget(g, "overlap_shrink"), TRUE), &flag);
...
#ifdef IPSEPCOLA
static int vpscAdjust(graph_t *G) {
  int dim = 2;
 ...
 */
static void setPrismValues(Agraph_t *g, char *s, adjust_data *dp) {
  int v;
  ...
  if ((sscanf(s, "%d", &v) > 0) && (v >= 0))
    ...
    dp->value = 1000;
  dp->scaling =
      late_double(g, agfindgraphattr(g, "overlap_scaling"), -4.0, -1.e10);
}
  ...
  if ((s == NULL) || (*s == '\0')) {
    ...
    while (ap->attrib) {
      if (!strncasecmp(s, ap->attrib, ap->len)) {
        ...
        dp->mode = ap->mode;
        dp->print = ap->print;
        if (ap->mode == AM_PRISM) setPrismValues(g, s + ap->len, dp);
        break;
      }
      ap++;
    }
    if (ap->attrib == NULL) {
      ...
      if (dp->mode == AM_PRISM) setPrismValues(g, "", dp);
    }
  }
  return dp;
}
 ...
 */
static int parseFactor(char *s, expand_t *pp, float sepfact) {
  int i;
  float x, y;
...
expand_t sepFactor(graph_t *g) {
  ...
  if ((marg = agget(g, "sep")) && parseFactor(marg, &pmargin, 1.0)) {
             ...
             parseFactor(marg, &pmargin, SEPFACT)) {
  } else {/* default */
...
expand_t esepFactor(graph_t *g) {
  ...
  if ((marg = agget(g, "esep")) && parseFactor(marg, &pmargin, 1.0)) {
             ...
             parseFactor(marg, &pmargin, 1.0 / SEPFACT)) {
  } else {
