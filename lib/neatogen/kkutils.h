...
inline double distance_kD(double **coords, int dim, int i, int j) {
  ...
  double sum = 0;
  for (int k = 0; k < dim; k++) {
    sum += (coords[k][i] - coords[k][j]) * (coords[k][i] - coords[k][j]);
  }
  return sqrt(sum);
}
void compute_apsp(vtx_data *graph, int n, DistType **(&Dij));
void compute_apsp_artifical_weights(vtx_data *graph, int n, DistType **(&Dij));

void quicksort_place(double *place, int *ordering, int first, int last);
void free_graph(vtx_data *(&graph));
#else
extern void fill_neighbors_vec_unweighted(vtx_data *, int vtx, int *vtx_vec);
extern int common_neighbors(vtx_data *, int v, int u, int *);
extern void empty_neighbors_vec(vtx_data *graph, int vtx, int *vtx_vec);
...
extern void restore_old_weights(vtx_data *graph, int n, float *old_weights);
#endif
