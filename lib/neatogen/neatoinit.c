...
static int user_spline(attrsym_t *E_pos, edge_t *e) {
  ...
  do {
    ...
    if ((n < 4) || (n % 3 != 1)) {
      ...
      if (!warned) {
        warned = 1;
        agerr(AGWARN,
              "pos attribute for edge (%s,%s) doesn't have 3n+1 points\n",
  ...
  if (ED_label(e)) set_label(e, ED_label(e), "lp");
  if (ED_xlabel(e)) set_label(e, ED_xlabel(e), "xlp");
  if (ED_head_label(e)) set_label(e, ED_head_label(e), "head_lp");
  if (ED_tail_label(e)) set_label(e, ED_tail_label(e), "tail_lp");
...
static pos_edge nop_init_edges(Agraph_t *g) {
  ...
  if (agnedges(g) == 0) return AllEdges;
 ...
 */
static void dfs(Agraph_t *subg, Agraph_t *parentg, attrsym_t *G_lp,
                attrsym_t *G_bb) {
  boxf bb;
 ...
 * it has a valid bb.
 */
static void nop_init_graphs(Agraph_t *g, attrsym_t *G_lp, attrsym_t *G_bb) {
  graph_t *subg;
  char *s;
  double x, y;
...
int init_nop(Agraph_t *g, int adjust) {
  ...
  if (!G_bb) G_bb = agattr(g, AGRAPH, "bb", "");
  ...
  for (i = 0; (np = GD_neato_nlist(g)[i]); i++) {
    ...
    if (ND_xlabel(np)) set_label(np, ND_xlabel(np), "xlp");
  }
  ...
  if (GD_drawing(g)->xdots) {
    haveBackground = 1;
    GD_drawing(g)->ratio_kind =
        R_NONE; /* Turn off any aspect change if background present */
  } else
    haveBackground = 0;

  if (adjust && (Nop == 1) && !haveBackground) didAdjust = adjustNodes(g);
  ...
  if (haveBackground) GD_bb(g) = xdotBB(g);
  ...
  if (!adjust) {
    ...
    boolean didShift;
    if (translate && !haveBackground &&
        ((GD_bb(g).LL.x != 0) || (GD_bb(g).LL.y != 0)))
      neato_translate(g);
    didShift = neato_set_aspect(g);
    /* if we have some edge positions and we either shifted or adjusted, free
     * edge positions */
...
static int neatoModel(graph_t *g) {
  ...
  if ((c == 'c') && streq(p, "circuit")) return MODEL_CIRCUIT;
  if (c == 's') {
    if (streq(p, "subset"))
      return MODEL_SUBSET;
 ...
 */
static void dfsCycle(vtx_data *graph, int i, int mode, node_t *nodes[]) {
  node_t *np, *hp;
  int j, e, f;
 ...
 */
static void acyclic(vtx_data *graph, int nv, int mode, node_t *nodes[]) {
  int i;
  node_t *np;
 ...
 *
 */
static vtx_data *makeGraphData(graph_t *g, int nv, int *nedges, int mode,
                               int model, node_t ***nodedata) {
  vtx_data *graph;
  node_t **nodes;
  ...
  if (haveLen || haveDir) ewgts = N_GNEW(2 * ne + nv, float);
  if (haveWt) eweights = N_GNEW(2 * ne + nv, float);
#ifdef DIGCOLA
  if (haveDir) edists = N_GNEW(2 * ne + nv, float);
#endif
  ...
  for (np = agfstnode(g); np; np = agnxtnode(g, np)) {
    ...
    for (ep = agfstedge(g, np); ep; ep = agnxtedge(g, ep, np)) {
      ...
      if (idx != j) {/* seen before */
        if (haveWt) graph[i].eweights[idx] += ED_factor(ep);
        if (haveLen) {
        ...
        *edges++ = ND_id(vp);
        if (haveWt) *eweights++ = ED_factor(ep);
        if (haveLen)
          *ewgts++ = ED_dist(ep);
        else if (haveDir)
          *ewgts++ = 1.0;
  ...
  if (ne != agnedges(g)) {
    ...
    if (haveLen) ewgts = RALLOC(2 * ne + nv, graph[0].ewgts, float);
    if (haveWt) eweights = RALLOC(2 * ne + nv, graph[0].eweights, float);
...
void dumpClusterData(cluster_data *dp) {
  ...
  for (i = 0; i < dp->nclusters; i++) {
    ...
    for (j = 0; j < sz; j++) fprintf(stderr, "  %d", dp->clusters[i][j]);
    fprintf(stderr, "\n");
  }
...
void dumpOpts(ipsep_options *opp, int nv) {
  ...
  if (opp->clusters) dumpClusterData(opp->clusters);
}
#endif
 ...
 */
static void majorization(graph_t *mg, graph_t *g, int nv, int mode, int model,
                         int dim, int steps, adjust_data *am) {
  double **coords;
  int ne;
  int i, rv = 0;
  node_t *v;
  ...
  if (init == INIT_SELF) opts |= opt_smart_init;
  ...
  if (mode != MODE_MAJOR) {
    double lgap =
        late_double(g, agfindgraphattr(g, "levelsgap"), 0.0, -MAXDOUBLE);
    if (mode == MODE_HIER) {
      ...
      if (mapbool(str)) {
        opt.diredges = 1;
        if (Verbose) fprintf(stderr, "Generating Edge Constraints...\n");
      ...
      if (am->mode == AM_IPSEP) {
        opt.noverlap = 1;
        if (Verbose) fprintf(stderr, "Generating Non-overlap Constraints...\n");
      } else if (am->mode == AM_VPSC) {
        opt.noverlap = 2;
        if (Verbose) fprintf(stderr, "Removing overlaps as postprocess...\n");
      } else
        opt.noverlap = 0;
#ifdef MOSEK
      str = agget(g, "mosek");
      ...
      dumpOpts(&opt, nv);
#endif
      rv = stress_majorization_cola(gp, nv, ne, coords, nodes, Ndim, model,
                                    MaxIter, &opt);
      freeClusterData(cs);
      free(nsize);
    }
...
#endif
    rv = stress_majorization_kD_mkernel(gp, nv, ne, coords, nodes, Ndim, opts,
                                        model, MaxIter);
  ...
  if (rv < 0) {
    ...
    for (v = agfstnode(g); v;
         v = agnxtnode(g, v)) {/* store positions back in nodes */
      int idx = ND_id(v);
      int i;
      for (i = 0; i < Ndim; i++) {
...
static void kkNeato(Agraph_t *g, int nG, int model) {
  if (model == MODEL_SUBSET) {
    ...
    if (!circuit_model(g, nG)) {
      agerr(AGWARN, "graph %s is disconnected. Hence, the circuit model\n",
            agnameof(g));
      agerr(AGPREV, "is undefined. Reverting to the shortest path model.\n");
      agerr(AGPREV,
            "Alternatively, consider running neato using -Gpack=true or "
            "decomposing\n");
 ...
 */
static void neatoLayout(Agraph_t *mg, Agraph_t *g, int layoutMode,
                        int layoutModel, adjust_data *am) {
  int nG;
  char *str;
...
#ifdef IPSEPCOLA
static void addCluster(graph_t *g) {
  graph_t *subg;
...
void neato_layout(Agraph_t *g) {
  ...
  if (Nop) {
    ...
    if (mode == l_undef) {
       ...
       */
      if ((Pack < 0) && layoutMode) Pack = CL_OFFSET;
      pinfo.mode = l_node;
    } else if (Pack < 0)
      Pack = CL_OFFSET;
    if (Pack >= 0) {
