...
static int splines_intersect(int dim, int u1, int v1, int u2, int v2,
                             real cos_critical,
                             int check_edges_with_same_endpoint, real *x,
  ...
  /* u1, v2 an u2, v2: the node index of the two edn points of two edges.
     ...
     .     the same end point specially in ignoring splines that exit/enter the
     same end pont at around 180
     x: the coordinates of nodes
     xsplines1,xsplines2: the first and second splines corresponding to two
     edges
  ...
  */
  ...
  /* origonally I though the two end points has to be included. But now I think
  it does not need to since
  ...
  */
  ...
  /* splines could be a list of
     1. 3n points
     2. of the form "e,x,y" followed by 3n points, where x,y is really padded to
     the end of the 3n points
     3. of the form "s,x,y" followed by 3n points, where x,y is padded to the
     start of the 3n points
  */
  if (xsplines1) {
  ...
  while (xsplines1 && sscanf(xsplines1, "%lf,%lf", &(x1[ns1 * dim]),
                             &x1[ns1 * dim + 1]) == 2) {
    if (endp1 && iter1 == 0) {
      tmp[0] = x1[ns1 * dim];
      tmp[1] = x1[ns1 * dim + 1];
  ...
  */
  ...
  while (xsplines2 && sscanf(xsplines2, "%lf,%lf", &(x2[ns2 * dim]),
                             &x2[ns2 * dim + 1]) == 2) {
    if (endp2 && iter2 == 0) {
      tmp[0] = x2[ns2 * dim];
      tmp[1] = x2[ns2 * dim + 1];
  ...
  */
  ...
  for (i = 0; i < ns1 - 1; i++) {
    for (j = 0; j < ns2 - 1; j++) {
      cos_a = intersection_angle(&(x1[dim * i]), &(x1[dim * (i + 1)]),
                                 &(x2[dim * j]), &(x2[dim * (j + 1)]));
...
Agraph_t *edge_distinct_coloring(char *color_scheme, char *lightness,
                                 Agraph_t *g, real angle, real accuracy,
                                 int check_edges_with_same_endpoint, int seed) {
  /* color the edges of a graph so that conflicting edges are as dinstrinct in
     color as possibl.
     color_scheme: rgb, lab, gray, or a list of comma separaterd RGB colors in
     hex, like #ff0000,#00ff00
     ...
     accuracy: how accurate when finding  color of an edge to be as different
     from others
     check_edges_with_same_endpoint: if TRUE, we will check edges with same end
     point and only consider them as conflict if
     .                   their angle is very small. Edges that share an end
     point and is close to 180 degree
  ...
  */
  ...
  if (Verbose) fprintf(stderr, "cos = %f, nz2 = %d\n", cos_critical, nz2);
  ...
  if (Import_dot_splines(g, &ne, &xsplines)) {
    ...
    assert(ne == nz2);
    cos_a =
        1.; /* for splines we exit conflict check as soon as we find an
               conflict, so the anle may not be representitive, hence set to
               constant */
    for (i = 0; i < nz2; i++) {
      u1 = irn[i];
      v1 = jcn[i];
      for (j = i + 1; j < nz2; j++) {
        u2 = irn[j];
        v2 = jcn[j];
        if (splines_intersect(dim, u1, v1, u2, v2, cos_critical,
                              check_edges_with_same_endpoint, x, xsplines[i],
                              xsplines[j])) {
...
#ifdef TIME
    fprintf(stderr, "cpu for dual graph =%10.3f",
...
/* no splines, justsimple edges */
    ...
    for (i = 0; i < nz2; i++) {
      ...
      for (j = i + 1; j < nz2; j++) {
        u2 = irn[j];
        v2 = jcn[j];
        cos_a = intersection_angle(&(x[dim * u1]), &(x[dim * v1]),
                                   &(x[dim * u2]), &(x[dim * v2]));
        if (!check_edges_with_same_endpoint && cos_a >= -1) cos_a = ABS(cos_a);
        if (cos_a > cos_critical) {
          B = SparseMatrix_coordinate_form_add_entries(B, 1, &i, &j, &cos_a);
...

  {
    ...
    int weightedQ = FALSE;
    int iter_max = 100;
    node_distinct_coloring(color_scheme, lightness, weightedQ, C, accuracy,
                           iter_max, seed, &cdim, &colors, &color_diff,
                           &color_diff_sum, &flag);
    if (flag) goto RETURN;
#ifdef TIME
...
/* for printing dual*/
  ...
  if (Verbose)
    fprintf(stderr,
            "The edge conflict graph has %d nodes and %d edges, final "
            "color_diff=%f color_diff_sum = %f\n",
