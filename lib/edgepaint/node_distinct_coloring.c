...
static void node_distinct_coloring_internal2(int scheme, QuadTree qt,
                                             int weightedQ, SparseMatrix A,
                                             int cdim, real accuracy,
                                             int iter_max, int seed,
                                             real *colors, real *color_diff0,
  ...
  if (n == 1) {
    if (scheme == COLOR_LAB) {
      ...
      *color_diff0 = 1000;
      *color_diff_sum0 = 1000;
    } else {
      for (i = 0; i < cdim; i++) colors[i] = 0;
      *color_diff0 = sqrt(cdim);
      *color_diff_sum0 = sqrt(cdim);
    }
    return;
  } else if (n == 2) {
    if (scheme == COLOR_LAB) {
  ...
  */
  ...
  while (iter++ < iter_max && (color_diff > color_diff_old ||
                               (color_diff == color_diff_old &&
                                color_diff_sum > color_diff_sum_old))) {
    ...
    for (i = 0; i < n; i++) {
      ...
      cc = &(colors[i * cdim]);
      if (scheme == COLOR_LAB) {
        furtherest_point_in_list(k, cdim, wgt, x, qt, max_level, mydist,
                                 &dist_max, &cc);
      } else if (scheme == COLOR_RGB || scheme == COLOR_GRAY) {
        furtherest_point(k, cdim, wgt, x, center, width, max_level, mydist,
                         &dist_max, &cc);
      } else {
        assert(0);
      }
  ...
  if (scheme == COLOR_LAB) {
    ...
    color_lab lab;
    for (i = 0; i < n; i++) {
      lab = color_lab_init(colors[i * cdim], colors[i * cdim + 1],
                           colors[i * cdim + 2]);
      rgb = LAB2RGB(lab);
...
static void node_distinct_coloring_internal(int scheme, QuadTree qt,
  ...
  if (seed < 0) {
    ...
    for (i = 0; i < iter; i++) {
      seed = irand(100000);
      node_distinct_coloring_internal2(scheme, qt, weightedQ, A, cdim, accuracy,
                                       iter_max, seed, colors, color_diff0,
                                       color_diff_sum0);
      if (color_diff_max < *color_diff0) {
        seed_max = seed;
        color_diff_max = *color_diff0;
      }
    }
    seed = seed_max;
  }
...
void node_distinct_coloring(char *color_scheme, char *lightness, int weightedQ,
                            SparseMatrix A0, real accuracy, int iter_max,
                            int seed, int *cdim0, real **colors,
                            real *color_diff0, real *color_diff_sum0,
                            int *flag) {
  /*
     for a graph A, get a distinctive color of its nodes so that the color
     distance among all neighboring nodes are maximized. Here
     color distance on a node is defined as the minimum of color differences
     between a node and its neighbors (or the minimum of weighted color
     differences if weightedQ = true,
     ...
     color_scheme: rgb, gray, lab, or one of the color palettes in
     color_palettes.h, or a list of hex rgb colors separaterd by comma like
     "#ff0000,#00ff00"
     ...
     color: On input an array of size n*cdim, if NULL, will be allocated. On
     exit the final color assignment for node i is [cdim*i,cdim*(i+1)), in RGB
     (between 0 to 1)
  ...
  *color_diff_sum0 = 0;
  for (i = 0; i < ncomps; i++) {
    ...
    B = SparseMatrix_get_submatrix(A, nn, nn, &(comps[comps_ptr[i]]),
                                   &(comps[comps_ptr[i]]));
    node_distinct_coloring_internal(scheme, qt, weightedQ, B, cdim, accuracy,
                                    iter_max, seed, ctmp, &color_diff,
                                    &color_diff_sum);
    if (i == 0) {
      *color_diff0 = color_diff;
    }
    *color_diff0 = MIN(*color_diff0, color_diff);
    ...
    for (j = comps_ptr[i]; j < comps_ptr[i + 1]; j++) {
      jj = j - comps_ptr[i];
      MEMCPY(&((*colors)[comps[j] * cdim]), &(ctmp[jj * cdim]),
             cdim * sizeof(real));
    }
