...
static Agraph_t *clone_graph(Agraph_t *ing, Agraph_t **xg) {
  ...
  agbindrec(clone, "Agraphinfo_t", sizeof(Agraphinfo_t),
  ...
  for (n = agfstnode(ing); n; n = agnxtnode(ing, n)) {
    ...
    agbindrec(xn, "Agnodeinfo_t", sizeof(Agnodeinfo_t),
              TRUE);  // node custom data
    CLONE(n) = xn;
  }
  ...
  for (n = agfstnode(ing); n; n = agnxtnode(ing, n)) {
    ...
    for (e = agfstout(ing, n); e; e = agnxtout(ing, e)) {
      ...
      agbindrec(xe, "Agedgeinfo_t", sizeof(Agedgeinfo_t),
                TRUE);  // node custom data
      ORIGE(xe) = e;
      DEGREE(xn) += 1;
      DEGREE(xh) += 1;
  ...
  if (diff > 0) {
    ...
    if (diff < no_pair_count) {
      for (mark = 0; mark < no_pair_count; mark += 2) {
        ...
        agbindrec(agedge(g, tp, hp, NULL, 1), "Agedgeinfo_t",
                  sizeof(Agedgeinfo_t), TRUE);  // edge custom data
        DEGREE(tp)++;
        DEGREE(hp)++;
        diff--;
      }
      ...
      for (mark = 0; mark < no_pair_count; mark++) {
        hp = neighbors_without[mark];
        agbindrec(agedge(g, tp, hp, NULL, 1), "Agedgeinfo_t",
                  sizeof(Agedgeinfo_t), TRUE);  // node custom data
        DEGREE(tp)++;
        DEGREE(hp)++;
      }
    }
...
static Agraph_t *remove_pair_edges(Agraph_t *ing) {
  ...
  while (counter < (nodeCount - 3)) {
    ...
    for (e = agfstedge(g, currnode); e; e = agnxtedge(g, e, currnode)) {
      adjNode = aghead(e);
      if (currnode == adjNode) adjNode = agtail(e);
...
static void measure_distance(Agnode_t *n, Agnode_t *ancestor, int dist,
                             Agnode_t *change) {
  Agnode_t *parent;
...
static nodelist_t *find_longest_path(Agraph_t *tree) {
  ...
  for (n = agfstnode(tree); n; n = agnxtnode(tree, n)) {
    ...
    if (count == 1) measure_distance(n, n, 0, NULL);
  }
...
static void dfs(Agraph_t *g, Agnode_t *n, Agraph_t *tree) {
  ...
  for (e = agfstedge(g, n); e; e = agnxtedge(g, e, n)) {
    neighbor = aghead(e);
    if (neighbor == n) neighbor = agtail(e);
...
static Agraph_t *spanning_tree(Agraph_t *g) {
  ...
  agbindrec(tree, "Agraphinfo_t", sizeof(Agraphinfo_t),
            TRUE);  // node custom data
...
static void block_graph(Agraph_t *g, block_t *sn) {
  ...
  for (n = agfstnode(subg); n; n = agnxtnode(subg, n)) {
    for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
      if (BLOCK(aghead(e)) == sn) agsubedge(subg, e, 1);
    }
  }
}
      ...
      if (EDGEORDER(e) > 0) {
        ...
        for (eitem = (edgelistitem *)dtfirst(openEdgeList); eitem;
          ...
          if (EDGEORDER(ep) > EDGEORDER(e)) {
            if ((aghead(ep) != n) && (agtail(ep) != n)) crossings++;
          }
        }
...
static nodelist_t *reduce(nodelist_t *list, Agraph_t *subg, int *cnt) {
  ...
  for (curnode = agfstnode(subg); curnode; curnode = agnxtnode(subg, curnode)) {
    ...
    for (e = agfstedge(subg, curnode); e; e = agnxtedge(subg, e, curnode)) {
      neighbor = agtail(e);
      if (neighbor == curnode) neighbor = aghead(e);
...
static nodelist_t *reduce_edge_crossings(nodelist_t *list, Agraph_t *subg) {
  ...
  for (i = 0; i < CROSS_ITER; i++) {
    ...
    if ((origCrossings == crossings) || (crossings == 0)) return list;
  }
  return list;
}
...
static double largest_nodesize(nodelist_t *list) {
  ...
  for (item = list->first; item; item = item->next) {
    n = ORIGN(item->curr);
    if (ND_width(n) > size) size = ND_width(n);
    if (ND_height(n) > size) size = ND_height(n);
  }
  return size;
}
...
static void place_node(Agraph_t *g, Agnode_t *n, nodelist_t *list) {
  ...
  for (one = neighbors->first; one; one = one->next) UNSET_NEIGHBOR(one->curr);
...
static void place_residual_nodes(Agraph_t *g, nodelist_t *list) {
  ...
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
    if (!ONPATH(n)) place_node(g, n, list);
  }
}
