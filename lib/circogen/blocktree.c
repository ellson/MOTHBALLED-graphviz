...
static Agraph_t *makeBlockGraph(Agraph_t *g, circ_state *state) {
  ...
  agbindrec(subg, "Agraphinfo_t", sizeof(Agraphinfo_t),
...
static void dfs(Agraph_t *g, Agnode_t *u, circ_state *state, int isRoot,
  ...
  for (e = agfstedge(g, u); e; e = agnxtedge(g, e, u)) {
    ...
    if (VAL(v) ==
        0) {/* Since VAL(root) == 0, it gets treated as artificial cut point */
      PARENT(v) = u;
      push(stk, e);
      dfs(g, v, state, 0, stk);
      ...
      if (LOWVAL(v) >= VAL(u)) {/* u is an articulation point */
        ...
        if (block) {/* If block != NULL, it's not empty */
          if (!BLOCK(u) && blockSize(block) > 1) addNode(block, u);
