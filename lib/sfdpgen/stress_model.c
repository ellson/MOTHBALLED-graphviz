...
void stress_model_core(int dim, SparseMatrix B, real **x, int edge_len_weighted,
                       int maxit_sm, real tol, int *flag) {
  int m;
  ...
  *flag = 0;
  ...
  if (edge_len_weighted) {
    sm = SparseStressMajorizationSmoother_new(
        A, dim, lambda, *x, WEIGHTING_SCHEME_SQR_DIST,
        TRUE); /* do not under weight the long distances */
    // sm = SparseStressMajorizationSmoother_new(A, dim, lambda, *x,
    // WEIGHTING_SCHEME_INV_DIST, TRUE);/* do not under weight the long
    // distances */
  } else {
    sm = SparseStressMajorizationSmoother_new(
        A, dim, lambda, *x, WEIGHTING_SCHEME_NONE,
        TRUE); /* weight the long distances */
  }
...
void stress_model_gv(void *data) {
  ...
  return stress_model_core(d->dim, d->D, d->x, d->edge_len_weighted,
                           d->maxit_sm, d->tol, d->flag);
}
void stress_model(int dim, SparseMatrix A, SparseMatrix D, real **x,
                  int edge_len_weighted, int maxit_sm, real tol, int *flag) {
  struct stress_model_data data = {dim,      D,   x,   edge_len_weighted,
                                   maxit_sm, tol, flag};
  ...
  argvv[0][0] = '1';
  //  gviewer_init(&argcc, argvv, 0.1, 20, 60, 2*1010, 2*770, A, dim, *x,
  // &(data), stress_model_gv);
  gviewer_set_edge_color_scheme(COLOR_SCHEME_NO);
  gviewer_toggle_bgcolor();
  gviewer_init(&argcc, argvv, 0.1, 20, 60, 720, 720, A, dim, *x, &(data),
               stress_model_gv);
  free(argvv);
}
#else
