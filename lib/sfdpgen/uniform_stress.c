...
UniformStressSmoother UniformStressSmoother_new(int dim, SparseMatrix A,
                                                real *x, real alpha, real M,
                                                int *flag) {
...
real UniformStressSmoother_smooth(UniformStressSmoother sm, int dim, real *x,
                                  int maxit_sm) {
...
SparseMatrix get_distance_matrix(SparseMatrix A, real scaling) {
  /* get a distance matrix from a graph, at the moment we just symmetrize the
   matrix. At the moment if the matrix is not real,
   we just assume distance of 1 among edges. Then we apply scaling to the entire
   matrix */
  SparseMatrix B;
  real *val;
  int i;
  ...
  val = (real *)B->a;
  if (scaling != 1)
    for (i = 0; i < B->nz; i++) val[i] *= scaling;
  return B;
}

extern void scale_to_box(real xmin, real ymin, real xmax, real ymax, int n,
                         int dim, real *x);
...
#ifdef DEBUG_PRINT
    fprintf(stderr,
            "input coordinates to uniform_stress are the same, use random "
            "coordinates as initial input");
#endif
