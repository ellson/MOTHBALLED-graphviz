...
#ifdef UNUSED
static void get_neighborhood_precision_recall(char *outfile, SparseMatrix A0,
  ...
  //  int *ia, *ja;
  int *g_order = NULL,
      *p_order = NULL; /* ordering using graph/physical distance */
  ...
  for (k = 5; k <= 50; k += 5) {
    ...
    for (i = 0; i < n; i++) {
      ...
      ng_neighbors =
          MIN(n - 1, k); /* set the number of closest neighbor in the graph
                            space to consider, excluding the node itself */
      np_neighbors =
          ng_neighbors; /* set the number of closest neighbor in the embedding
                           to consider, excluding the node itself */
      radius = pdist[p_order[np_neighbors]];
      true_positive = 0;
      for (j = 1; j <= ng_neighbors; j++) {
        node_dist =
            pdist[g_order[j]]; /* the phisical distance for j-th closest node
                                  (in graph space) */
...
void dump_distance_edge_length(char *outfile, SparseMatrix A, int dim,
                               real *x) {
  int weighted = TRUE;
  int n, i, j, nzz;
  ...
  dmax = dij[p[nz - 1]];
  nbins =
      MIN(nbins, dmax / MAX(1, dmin)); /* scale by dmin since edge length of 1
                                          is treated as 72 points in
                                          stress/maxent/full_stress */
  ...
  for (i = 0; i < nbins; i++) {
     ...
     */
    nz = 0;
    xmin = xmax = xij[p[nsta]];
    while (nsta < nzz && dij[p[nsta]] >= bsta &&
           dij[p[nsta]] <= bsta + bwidth) {
      xbin[nz++] = xij[p[nsta]];
...
real get_full_stress(SparseMatrix A, int dim, real *x, int weighting_scheme) {
  /* get the full weighted stress, \sum_ij w_ij (t ||x_i-x_j||^2-d_ij)^2, where
     t
     is the optimal scaling factor t = \sum w_ij ||x_i-x_j||^2/(\sum w_ij d_ij
     ||x_i - x_j||),

     Matrix A is assume to be sparse and a full distance matrix will be
     generated.
     We assume undirected graph and only check an undirected edge i--j, not i->j
     and j->i.
  */
  int weighted = TRUE;
  int n, i, j;
...

  {
    int K = 20;
    sne_disimilarity =
        get_sne_disimilarity(n, ideal_dist_matrix, dist_matrix, K);
  }

  get_neighborhood_precision_recall("/tmp/recall.txt", A, ideal_dist_matrix,
                                    dist_matrix);
  get_neighborhood_precision_recall("/tmp/precision.txt", A, dist_matrix,
                                    ideal_dist_matrix);

  fprintf(stderr, "sne_disimilarity = %f\n", sne_disimilarity);
  if (n > 0)
    fprintf(stderr, "sstress per edge = %f\n", stress / n / (n - 1) * 2);
...
StressMajorizationSmoother StressMajorizationSmoother2_new(
    SparseMatrix A, int dim, real lambda0, real *x, int ideal_dist_scheme) {
...
StressMajorizationSmoother SparseStressMajorizationSmoother_new(
    SparseMatrix A, int dim, real lambda0, real *x, int weighting_scheme,
...
real SparseStressMajorizationSmoother_smooth(
    SparseStressMajorizationSmoother sm, int dim, real *x, int maxit_sm,
    real tol) {
...
static void get_edge_label_matrix(relative_position_constraints data, int m,
                                  int dim, real *x, SparseMatrix *LL,
                                  real **rhs) {
  ...
  if (edge_labeling_scheme == ELSCHEME_PENALTY ||
    ...
    /* for an node with two neighbors j--i--k, and assume i needs to be between
       j and k, then the contribution to P is
       .   i    j     k
       i   1   -0.5  -0.5
       j  -0.5 0.25  0.25
       k  -0.5 0.25  0.25
    ...
    */
    ...
    Lc = SparseMatrix_from_coordinate_arrays(nz, m, m, irn, jcn, val,
                                             MATRIX_TYPE_REAL, sizeof(real));
  } else if (edge_labeling_scheme == ELSCHEME_PENALTY2 ||
             edge_labeling_scheme == ELSCHEME_STRAIGHTLINE_PENALTY2) {
    /* for an node with two neighbors j--i--k, and assume i needs to be between
       the old position of j and k, then the contribution to P is
       1/d_jk, and to the right hand side: {0,...,average_position_of_i's
       neighbor if i is an edge node,...}
    */
    if (!irn) {
      assert((!jcn) && (!val));
      nz = n_constr_nodes;
...
real get_stress(int m, int dim, int *iw, int *jw, real *w, real *d, real *x,
                real scaling, void *data, int weighted) {
  int i, j;
  real res = 0., dist;
  /* we use the fact that d_ij = w_ij*graph_dist(i,j). Also, d_ij and x are
   * scalinged by *scaling, so divide by it to get actual unscaled streee. */
  for (i = 0; i < m; i++) {
    for (j = iw[i]; j < iw[i + 1]; j++) {
      ...
      if (weighted) {
        ...
        res += (dist - distance(x, dim, i, jw[j])) *
               (dist - distance(x, dim, i, jw[j]));
      }
    }
  }
...
static void uniform_stress_augment_rhs(int m, int dim, real *x, real *y,
                                       real alpha, real M) {
  int i, j, k;
  real dist, distij;
  for (i = 0; i < m; i++) {
...
static real uniform_stress_solve(SparseMatrix Lw, real alpha, int dim, real *x0,
...
real StressMajorizationSmoother_smooth(StressMajorizationSmoother sm, int dim,
                                       real *x, int maxit_sm, real tol) {
  SparseMatrix Lw = sm->Lw, Lwd = sm->Lwd, Lwdd = NULL;
  int i, j, k, m, *id, *jd, *iw, *jw, idiag, flag = 0, iter = 0;
  real *w, *dd, *d, *y = NULL, *x0 = NULL, *x00 = NULL, diag, diff = 1,
                    *lambda = sm->lambda, res, alpha = 0., M = 0.;
  SparseMatrix Lc = NULL;
  real dij, dist;
  ...
  while (iter++ < maxit_sm && diff > tol) {
    ...
    if (sm->scheme != SM_SCHEME_STRESS_APPROX) {
      ...
      for (i = 0; i < m; i++) {
        for (j = 0; j < dim; j++) {
          y[i * dim + j] = 0; /* for stress_approx scheme, the whole rhs is
                                 calculated in stress_maxent_augment_rhs */
        }
      }
    }
    ...
    switch (sm->scheme) {
      ...
      case SM_SCHEME_UNIFORM_STRESS
          : {/* this part can be done more efficiently using octree
                approximation */
      ...
      case SM_SCHEME_MAXENT: {
        ...
        if (Gviewer) {
          ...
          sprintf(lab, "maxent. alpha=%10.2g, iter=%d",
    ...
    if (sm->scheme == SM_SCHEME_UNIFORM_STRESS) {
      res = uniform_stress_solve(Lw, alpha, dim, x, y, sm->tol_cg, sm->maxit_cg,
                                 &flag);
    } else {
      res = SparseMatrix_solve(Lw, dim, x, y, sm->tol_cg, sm->maxit_cg,
                               SOLVE_METHOD_CG, &flag);
      // res = SparseMatrix_solve(Lw, dim, x, y,  sm->tol_cg, 1,
      // SOLVE_METHOD_JACOBI, &flag);
    }
...
TriangleSmoother TriangleSmoother_new(SparseMatrix A, int dim, real lambda0,
                                      real *x, int use_triangularization) {
  TriangleSmoother sm;
...
SpringSmoother SpringSmoother_new(SparseMatrix A, int dim,
                                  spring_electrical_control ctrl, real *x) {
  SpringSmoother sm;
...
void SpringSmoother_smooth(SpringSmoother sm, SparseMatrix A,
  ...
  spring_electrical_spring_embedding(dim, A, sm->D, sm->ctrl, node_weights, x,
                                     &flag);
  assert(!flag);
}
