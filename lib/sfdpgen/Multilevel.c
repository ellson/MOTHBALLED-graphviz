...
Multilevel_control Multilevel_control_new(int scheme, int mode) {
  ...
  ctrl->maxlevel = 1 << 30;
  ctrl->randomize = TRUE;
  /* now set in spring_electrical_control_new(), as well as by command line
    argument -c
    ctrl->coarsen_scheme =
...
void Multilevel_control_delete(Multilevel_control ctrl) { FREE(ctrl); }
...
static void maximal_independent_vertex_set_RS(SparseMatrix A, int randomize,
                                              int **vset, int *nvset,
                                              int *nzc) {
  /* The Ruge-Stuben coarsening scheme. Initially all vertices are in the U set
     (with marker MAX_IND_VTX_SET_U),
     with gain equal to their degree. Select vertex with highest gain into a C
     set (with
     marker >= MAX_IND_VTX_SET_C), and their neighbors j in F set (with marker
     MAX_IND_VTX_SET_F). The neighbors of
  ...
  *nzc = 0;
  ...
  if (!randomize) {
    for (i = 0; i < m; i++) PriorityQueue_push(q, i, ia[i + 1] - ia[i]);
  } else {
    p = random_permutation(m);
    for (ii = 0; ii < m; ii++) {
      i = p[ii];
  ...
  while (PriorityQueue_pop(q, &i, &gain)) {
    ...
    for (j = ia[i]; j < ia[i + 1]; j++) {
      jj = ja[j];
      assert((*vset)[jj] == MAX_IND_VTX_SET_U ||
...
static void maximal_independent_edge_set_heavest_edge_pernode(SparseMatrix A,
                                                              int randomize,
                                                              int **matching,
                                                              int *nmatch) {
  int i, ii, j, *ia, *ja, m, n, *p = NULL;
...
static void maximal_independent_edge_set_heavest_edge_pernode_leaves_first(
    SparseMatrix A, int randomize, int **cluster, int **clusterp,
    int *ncluster) {
...
static void maximal_independent_edge_set_heavest_edge_pernode_scaled(
      ...
      for (j = ia[i]; j < ia[i + 1]; j++) {
        ...
        if ((*matching)[ja[j]] == ja[j] && (*matching)[i] == i) {
          if (first) {
            ...
            if (a[j] / (ia[i + 1] - ia[i]) / (ia[ja[j] + 1] - ia[ja[j]]) >
                amax) {
              amax = a[j] / (ia[i + 1] - ia[i]) / (ia[ja[j] + 1] - ia[ja[j]]);
              jamax = ja[j];
            }
...
SparseMatrix DistanceMatrix_restrict_cluster(int ncluster, int *clusterp,
                                             int *cluster, SparseMatrix P,
                                             SparseMatrix R, SparseMatrix D) {
#if 0
...
SparseMatrix DistanceMatrix_restrict_filtering(int *mask, int is_C, int is_F,
                                               SparseMatrix D) {
  /* max independent vtx set based coarsening. Coarsen nodes has mask >= is_C.
   * Fine nodes == is_F. */
  if (!D) return NULL;
  assert(0); /* not yet implemented! */
  return NULL;
...
static void Multilevel_coarsen_internal(SparseMatrix A, SparseMatrix *cA,
                                        SparseMatrix D, SparseMatrix *cD,
  ...
  switch (ctrl->coarsen_scheme) {
    case COARSEN_HYBRID:
      ...
      if (Verbose)
        fprintf(stderr,
                "hybrid scheme, try "
                "COARSEN_INDEPENDENT_EDGE_SET_HEAVEST_EDGE_PERNODE_LEAVES_"
                "FIRST first\n");
#endif
      *coarsen_scheme_used = ctrl->coarsen_scheme =
          COARSEN_INDEPENDENT_EDGE_SET_HEAVEST_EDGE_PERNODE_LEAVES_FIRST;
      Multilevel_coarsen_internal(A, cA, D, cD, node_wgt, cnode_wgt, P, R, ctrl,
                                  coarsen_scheme_used);
      ...
      if (!(*cA)) {
        ...
        if (Verbose)
          fprintf(stderr,
                  "switching to "
                  "COARSEN_INDEPENDENT_EDGE_SET_HEAVEST_EDGE_PERNODE_"
                  "SUPERNODES_FIRST\n");
#endif
        *coarsen_scheme_used = ctrl->coarsen_scheme =
            COARSEN_INDEPENDENT_EDGE_SET_HEAVEST_EDGE_PERNODE_SUPERNODES_FIRST;
        Multilevel_coarsen_internal(A, cA, D, cD, node_wgt, cnode_wgt, P, R,
      ...
      if (!(*cA)) {
        ...
        if (Verbose)
          fprintf(stderr,
                  "switching to "
                  "COARSEN_INDEPENDENT_EDGE_SET_HEAVEST_CLUSTER_PERNODE_LEAVES_"
                  "FIRST\n");
#endif
        *coarsen_scheme_used = ctrl->coarsen_scheme =
            COARSEN_INDEPENDENT_EDGE_SET_HEAVEST_CLUSTER_PERNODE_LEAVES_FIRST;
    ...
    case COARSEN_INDEPENDENT_EDGE_SET_HEAVEST_EDGE_PERNODE_LEAVES_FIRST:
      ...
      assert(ncluster <= n);
      nc = ncluster;
      if ((ctrl->coarsen_mode == COARSEN_MODE_GENTLE &&
           nc > ctrl->min_coarsen_factor * n) ||
          nc == n || nc < ctrl->minsize) {
#ifdef DEBUG_PRINT
        if (Verbose)
          fprintf(stderr,
                  "nc = %d, nf = %d, minsz = %d, coarsen_factor = %f "
                  "coarsening stops\n",
      ...
      *P = SparseMatrix_from_coordinate_arrays(
          nzc, n, nc, irn, jcn, (void *)val, MATRIX_TYPE_REAL, sizeof(real));
      *R = SparseMatrix_transpose(*P);

      *cD = DistanceMatrix_restrict_cluster(ncluster, clusterp, cluster, *P, *R,
                                            D);
    ...
    case COARSEN_INDEPENDENT_EDGE_SET_HEAVEST_EDGE_PERNODE:
      if (ctrl->coarsen_scheme ==
          COARSEN_INDEPENDENT_EDGE_SET_HEAVEST_EDGE_PERNODE)
        maximal_independent_edge_set_heavest_edge_pernode(A, ctrl->randomize,
                                                          &matching, &nmatch);
    case COARSEN_INDEPENDENT_EDGE_SET_HEAVEST_EDGE_PERNODE_DEGREE_SCALED:
      if (ctrl->coarsen_scheme ==
        ...
        maximal_independent_edge_set_heavest_edge_pernode_scaled(
            A, ctrl->randomize, &matching, &nmatch);
      nc = nmatch;
      if ((ctrl->coarsen_mode == COARSEN_MODE_GENTLE &&
           nc > ctrl->min_coarsen_factor * n) ||
          nc == n || nc < ctrl->minsize) {
    ...
    case COARSEN_INDEPENDENT_VERTEX_SET_RS:
      if (ctrl->coarsen_scheme == COARSEN_INDEPENDENT_VERTEX_SET) {
        ...
        maximal_independent_vertex_set_RS(A, ctrl->randomize, &vset, &nvset,
                                          &nzc);
      }
      ia = A->ia;
      ja = A->ja;
      ...
      *cD = DistanceMatrix_restrict_filtering(vset, MAX_IND_VTX_SET_C,
                                              MAX_IND_VTX_SET_F, D);
      break;
    default:
      goto RETURN;
...
void Multilevel_coarsen(SparseMatrix A, SparseMatrix *cA, SparseMatrix D,
  ...
  *R = NULL;
  *cA = NULL;
  *cnode_wgt = NULL, *cD = NULL;

  n = A->n;

  do {/* this loop force a sufficient reduction */
    node_wgt = cnode_wgt0;
    Multilevel_coarsen_internal(A, &cA0, D, &cD0, node_wgt, &cnode_wgt0, &P0,
                                &R0, ctrl, coarsen_scheme_used);
    if (!cA0) return;
    nc = cA0->n;
#ifdef DEBUG_PRINT
...
static Multilevel Multilevel_establish(Multilevel grid,
  ...
  if (Verbose) {
    ...
    fprintf(stderr, "level -- %d, n = %d, nz = %d nz/n = %f\n", grid->level,
            grid->n, grid->A->nz, grid->A->nz / (double)grid->n);
  }
#endif
  A = grid->A;
  D = grid->D;
  ...
  Multilevel_coarsen(A, &cA, D, &cD, grid->node_weights, &cnode_weights, &P, &R,
                     ctrl, &coarsen_scheme_used);
  if (!cA) return grid;
...
Multilevel Multilevel_new(SparseMatrix A0, SparseMatrix D0, real *node_weights,
                          Multilevel_control ctrl) {
  /* A: the weighting matrix. D: the distance matrix, could be NULL. If not
   * null, the two matrices must have the same sparsity pattern */
  Multilevel grid;
  ...
  if (D &&
      (!SparseMatrix_is_symmetric(D, FALSE) || D->type != MATRIX_TYPE_REAL)) {
