...
spring_electrical_control spring_electrical_control_new() {
  ...
  ctrl->p =
      AUTOP;   /*a negativve number default to -1. repulsive force = dist^p */
  ctrl->q = 1; /*a positive number default to 1. Only apply to maxent.
                 attractive force = dist^q. Stress energy =
                 (||x_i-x_j||-d_ij)^{q+1} */
  ctrl->random_start = TRUE; /* whether to apply SE from a random layout, or
                                from exisiting layout */
  ctrl->K =
      -1; /* the natural distance. If K < 0, K will be set to the average
             distance of an edge */
  ctrl->C = 0.2; /* another parameter. f_a(i,j) = C*dist(i,j)^2/K * d_ij,
                    f_r(i,j) = K^(3-p)/dist(i,j)^(-p). By default C = 0.2. */
  ...
  ctrl->multilevel_coarsen_scheme =
      COARSEN_INDEPENDENT_EDGE_SET_HEAVEST_EDGE_PERNODE_SUPERNODES_FIRST; /* pass
                                                                             on
                                                                             to
                                                                             Multilevel_control->coarsen_scheme
                                                                             */
  ctrl->multilevel_coarsen_mode =
      COARSEN_MODE_FORCEFUL; /*alternative: COARSEN_MODE_GENTLE. pass on to
                                Multilevel_control->coarsen_mode */

  ctrl->quadtree_size =
      45; /* cut off size above which quadtree approximation is used */
  ctrl->max_qtree_level = 10; /* max level of quadtree */
  ctrl->bh = 0.6; /* Barnes-Hutt constant, if width(snode)/dist[i,snode] < bh,
                     treat snode as a supernode.*/
  ctrl->tol =
      0.001; /* minimum different between two subsequence config before
                terminating. ||x-xold||_infinity < tol/K */
  ctrl->maxiter = 500;
...
void oned_optimizer_delete(oned_optimizer opt) { FREE(opt); }
...
void oned_optimizer_train(oned_optimizer opt, real work) {
  ...
  if (opt->direction == OPT_INIT) {
    ...
    /*    fprintf(stderr, "{current_level, prev_level} = {%d,%d}, {work,
     * work_prev} = {%f,%f}",i,i-1,opt->work[i], opt->work[i-1]);*/
    assert(i >= 1);
...
int oned_optimizer_get(oned_optimizer opt) { return opt->i; }
...
real average_edge_length(SparseMatrix A, int dim, real *coord) {
  ...
  for (i = 0; i < A->m; i++) {
    for (j = ia[i]; j < ia[i + 1]; j++) {
      d = 0;
      for (k = 0; k < dim; k++) {
        d += (coord[dim * i + k] - coord[dim * ja[j]]) *
             (coord[dim * i + k] - coord[dim * ja[j]]);
      }
      dist += sqrt(d);
    }
  }
...
#ifdef ENERGY
static real spring_electrical_energy(int dim, SparseMatrix A, real *x, real p,
                                     real CRK, real KP) {
  /* 1. Grad[||x-y||^k,x] = k||x-y||^(k-1)*0.5*(x-y)/||x-y|| = k/2*||x-y||^(k-2)
     (x-y)
     which should equal to -force (force = -gradient),
     hence energy for force ||x-y||^m (y-x) is ||x-y||^(m+2)*2/(m+2) where m !=
     2
     2. Grad[Log[||x-y||],x] = 1/||x-y||*0.5*(x-y)/||x-y|| =
     0.5*(x-y)/||x-y||^2,
  ...
  if (width && dim == 2) {
    for (i = 0; i < A->m; i++) {
      ...
      fprintf(fp,
              ...
              width[i * dim], width[i * dim + 1], x[i * dim], x[i * dim + 1],
...
static void sort_real(int n, real *a) { qsort(a, n, sizeof(real), comp_real); }
...
void spring_electrical_embedding_fast(int dim, SparseMatrix A0,
                                      spring_electrical_control ctrl,
                                      real *node_weights, real *x, int *flag) {
  /* x is a point to a 1D array, x[i*dim+j] gives the coordinate of the i-th
   * node at dimension j.  */
  SparseMatrix A = A0;
  int m, n;
  int i, j, k;
  real p = ctrl->p, K = ctrl->K, C = ctrl->C, CRK, tol = ctrl->tol,
       maxiter = ctrl->maxiter, cool = ctrl->cool, step = ctrl->step, KP;
  int *ia = NULL, *ja = NULL;
  real *xold = NULL;
...
// start2 = clock();
    ...
    if (Gviewer) {
      ...
      // if ((adaptive_cooling && iter%100 == 0) || (!adaptive_cooling &&
      // iter%10 == 0)) gviewer_dump_current_frame();
      FREE(lab);
    }
#endif
...
/* repulsive force */
    ...
    if (qt) {
      ...
      /*      if (Verbose) fprintf(stderr, "\r iter=%d cpu=%.2f, quadtree=%.2f
         quad_force=%.2f other=%.2f counts={%.2f,%.2f,%.2f} step=%f Fnorm=%f
         nz=%d  K=%f qtree_lev = %d",
                           iter, ((real) (clock() - start2)) / CLOCKS_PER_SEC,
         qtree_new_cpu0,
                           qtree_cpu0,((real) (clock() - start2))/CLOCKS_PER_SEC
         - qtree_cpu0 - qtree_new_cpu0,
      ...
      */
      ...
      if (Verbose) {
        fprintf(stderr,
                "\r                iter = %d, step = %f Fnorm = %f nz = %d  K "
                "= %f                                  ",
                iter, step, Fnorm, A->nz, K);
#ifdef ENERGY
...
#ifdef DEBUG_PRINT
  if (Verbose) {
    fprintf(stderr, "\n iter = %d, step = %f Fnorm = %f nz = %d  K = %f   ",
            iter, step, Fnorm, A->nz, K);
  }
#endif
...
void spring_electrical_embedding_slow(int dim, SparseMatrix A0,
                                      spring_electrical_control ctrl,
                                      real *node_weights, real *x, int *flag) {
  /* a version that does vertex moves in one go, instead of one at a time, use
   * for debugging the fast version. Quadtree is not used. */
  /* x is a point to a 1D array, x[i*dim+j] gives the coordinate of the i-th
   * node at dimension j.  */
  SparseMatrix A = A0;
  int m, n;
  ...
  real *center = NULL, *supernode_wgts = NULL, *distances = NULL, nsuper_avg,
       counts = 0, counts_avg = 0;
  real *force;
#ifdef TIME
  ...
  *flag = 0;
  ...
  do {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (USE_QT) {
        ...
        start = clock();
#endif
        QuadTree_get_supernodes(qt, ctrl->bh, &(x[dim * i]), i, &nsuper,
        ...
        for (j = 0; j < nsuper; j++) {
          ...
          for (k = 0; k < dim; k++) {
            if (p == -1) {
              f[k] += supernode_wgts[j] * KP *
                      (x[i * dim + k] - center[j * dim + k]) / (dist * dist);
            } else {
              f[k] += supernode_wgts[j] * KP *
                      (x[i * dim + k] - center[j * dim + k]) /
                      pow(dist, 1. - p);
            }
          }
        }
        ...
        if (ctrl->use_node_weights && node_weights) {
          for (j = 0; j < n; j++) {
            ...
            for (k = 0; k < dim; k++) {
              if (p == -1) {
                f[k] += node_weights[j] * KP *
                        (x[i * dim + k] - x[j * dim + k]) / (dist * dist);
              } else {
                f[k] += node_weights[j] * KP *
                        (x[i * dim + k] - x[j * dim + k]) / pow(dist, 1. - p);
              }
            }
          }
        } else {
          for (j = 0; j < n; j++) {
            ...
            for (k = 0; k < dim; k++) {
              if (p == -1) {
                ...
                f[k] +=
                    KP * (x[i * dim + k] - x[j * dim + k]) / pow(dist, 1. - p);
              }
            }
          }
        }
...
#endif
      if (Verbose && 0)
        fprintf(stderr, "nsuper_avg=%f, counts_avg = %f 2*nsuper+counts=%f\n",
    ...
    if (Verbose) {
      fprintf(stderr,
              "\r                iter = %d, step = %f Fnorm = %f nsuper = %d "
              "nz = %d  K = %f                                  ",
  ...
  if (Verbose) {
    if (USE_QT) {
      fprintf(stderr,
              "iter = %d, step = %f Fnorm = %f qt_level = %d nsuper = %d nz = "
              "%d  K = %f   ",
              iter, step, Fnorm, max_qtree_level, (int)nsuper_avg, A->nz, K);
    } else {
      fprintf(stderr,
...
//#define VIS_MULTILEVEL
    ...
    if (Gviewer) {
      char *lab;
      lab = MALLOC(sizeof(char) * 100);
      sprintf(lab, "sfdp, adaptive_cooling = %d iter=%d", adaptive_cooling,
              iter);
      gviewer_set_label(lab);
      gviewer_reset_graph_coord(A, dim, x);
      drawScene();
...
static void scale_coord(int n, int dim, real *x, int *id, int *jd, real *d,
                        real dj) {
  int i, j, k;
...
void spring_maxent_embedding(int dim, SparseMatrix A0, SparseMatrix D,
                             spring_electrical_control ctrl, real *node_weights,
                             real *x, real rho, int *flag) {
  /* x is a point to a 1D array, x[i*dim+j] gives the coordinate of the i-th
     node at dimension j.

     Minimize \Sum_{(i,j)\in E} w_ij (||x_i-x_j||-d_ij)^2 - \rho
     \Sum_{(i,j)\NotIn E} Log ||x_i-x_j||

     or

     Minimize \Sum_{(i,j)\in E} w_ij (||x_i-x_j||-d_ij)^2 - \rho
     \Sum_{(i,j)\NotIn E} ||x_i-x_j||^p

     The derivatives are

     d E/d x_i = \Sum_{(i,j)\in E} w_ij (||x_i-x_j||-d_ij) (x_i-x_j)/||x_i-x_j||
     - \rho \Sum_{(i,j)\NotIn E} (x_i-x_j)/||x_i-x_j||^2

     or

      d E/d x_i = \Sum_{(i,j)\in E} w_ij (||x_i-x_j||-d_ij)
     (x_i-x_j)/||x_i-x_j|| - \rho \Sum_{(i,j)\NotIn E} ||x_i-x_j||^(p-2)
     (x_i-x_j)
  ...
  */
  ...
  int m, n;
  int i, j, k;
  real p = ctrl->p, C = ctrl->C, tol = ctrl->tol, maxiter = ctrl->maxiter,
       cool = ctrl->cool, step = ctrl->step, w_ij, dj = 1.;
  int *ia = NULL, *ja = NULL;
  int *id = NULL, *jd = NULL;
  real *d, dmean;
  real *xold = NULL;
  ...
  *flag = 0;
  ...
  if (ctrl->random_start) {
    ...
    /* rescale x to give minimum stress:
       ...
       thus
       s = (\Sum_{(ij)\in E} w_ij d_ij ||x_i-x_j||)/(\Sum_{(i,j)\in E} w_ij
       ||x_i-x_j||^2)
    */
  }
  ...
  do {
    ...
    /*
      .    d E/d x_i = \Sum_{(i,j)\in E} w_ij (||x_i-x_j||-d_ij)
      (x_i-x_j)/||x_i-x_j|| - \rho \Sum_{(i,j)\NotIn E} (x_i-x_j)/||x_i-x_j||^2
      or
      .    d E/d x_i = \Sum_{(i,j)\in E} w_ij (||x_i-x_j||-d_ij)
      (x_i-x_j)/||x_i-x_j|| - \rho \Sum_{(i,j)\NotIn E} ||x_i-x_j||^(p-2)
      (x_i-x_j)
    */
    for (i = 0; i < n; i++) {
      ...
      /* spring (attractive or repulsive) force   w_ij (||x_i-x_j||-d_ij)
       * (x_i-x_j)/||x_i-x_j|| */
      for (j = id[i]; j < id[i + 1]; j++) {
        if (jd[j] == i) continue;
        dist = distance_cropped(x, dim, i, jd[j]);
        ...
        if (ctrl->q == 2) {
          w_ij = 1. / (dj * dj * dj);
          for (k = 0; k < dim; k++) {
            f[k] += -w_ij * (x[i * dim + k] - x[jd[j] * dim + k]) *
        ...
        /* discount repulsive force between neighboring vertices which will be
           applied next, that way there is no
           repulsive forces between neighboring vertices */
        if (ctrl->use_node_weights && node_weights) {
          for (k = 0; k < dim; k++) {
            if (p == -1) {
              f[k] -= rho * node_weights[j] *
                      (x[i * dim + k] - x[jd[j] * dim + k]) / (dist * dist);
            } else {
              f[k] -= rho * node_weights[j] *
                      (x[i * dim + k] - x[jd[j] * dim + k]) / pow(dist, 1. - p);
            }
          }
        } else {
          for (k = 0; k < dim; k++) {
            if (p == -1) {
              f[k] -=
                  rho * (x[i * dim + k] - x[jd[j] * dim + k]) / (dist * dist);
            } else {
              f[k] -= rho * (x[i * dim + k] - x[jd[j] * dim + k]) /
                      pow(dist, 1. - p);
            }
          }
        }
          ...
          for (j = 0; j < n; j++) {
            ...
            for (k = 0; k < dim; k++) {
              if (p == -1) {
                ...
                f[k] +=
                    rho * (x[i * dim + k] - x[j * dim + k]) / pow(dist, 1. - p);
              }
            }
          }
        }
    ...
    if (Verbose) {
      fprintf(stderr,
              "\r                iter = %d, step = %f Fnorm = %f nsuper = %d "
              "nz = %d stress = %f                                  ",
              iter, step, Fnorm, (int)nsuper_avg, A->nz, stress);
    }
#endif
...
void spring_electrical_spring_embedding(int dim, SparseMatrix A0,
                                        ...
                                        int *flag) {
  /* x is a point to a 1D array, x[i*dim+j] gives the coordinate of the i-th
     node at dimension j. Same as the spring-electrical except we also
  ...
  do {
    ...
    for (i = 0; i < n; i++) {
      ...
      for (j = id[i]; j < id[i + 1]; j++) {
        ...
        for (k = 0; k < dim; k++) {
          if (dist < d[j]) {
            f[k] += 0.2 * CRK * (x[i * dim + k] - x[jd[j] * dim + k]) *
                    (dist - d[j]) * (dist - d[j]) / dist;
          } else {
            f[k] -= 0.2 * CRK * (x[i * dim + k] - x[jd[j] * dim + k]) *
                    (dist - d[j]) * (dist - d[j]) / dist;
          }
...
static void prolongate(int dim, SparseMatrix A, SparseMatrix P, SparseMatrix R,
                       real *x, real *y, int coarsen_scheme_used, real delta) {
  int nc, *ia, *ja, i, j, k;
...
void pcp_rotate(int n, int dim, real *x) {
  ...
  if (y[1] == 0) {
    ...
    /*         Eigensystem[{{x0, x1}, {x1, x3}}] =
               ...
               {-(-x0 + x3 - Sqrt[x0^2 + 4*x1^2 - 2*x0*x3 + x3^2])/(2*x1), 1}}}
    */
    axis[0] = -(-y[0] + y[3] - sqrt(y[0] * y[0] + 4 * y[1] * y[1] -
                                    2 * y[0] * y[3] + y[3] * y[3])) /
              (2 * y[1]);
    axis[1] = 1;
  }
...
static void attach_edge_label_coordinates(int dim, SparseMatrix A,
                                          int n_edge_label_nodes,
                                          int *edge_label_nodes, real *x,
                                          real *x2) {
  int *mask;
  int i, ii, j, k;
  int nnodes = 0;
  ...
  for (i = 0; i < n_edge_label_nodes; i++) {
    if (edge_label_nodes[i] >= 0 && edge_label_nodes[i] < A->m)
...
static SparseMatrix shorting_edge_label_nodes(SparseMatrix A,
  ...
  B = SparseMatrix_from_coordinate_arrays(nz, id, id, irn, jcn, NULL,
                                          MATRIX_TYPE_PATTERN, sizeof(real));
...
static void multilevel_spring_electrical_embedding_core(
    int dim, SparseMatrix A0, SparseMatrix D0, spring_electrical_control ctrl,
    real *node_weights, real *label_sizes, real *x, int n_edge_label_nodes,
  ...
  *flag = 0;
  ...
  /* we first generate a layout discarding (shorting) the edge labels nodes,
   * then assign the edge label nodes at the average of their neighbors */
  if ((ctrl->edge_labeling_scheme == ELSCHEME_STRAIGHTLINE_PENALTY ||
       ctrl->edge_labeling_scheme == ELSCHEME_STRAIGHTLINE_PENALTY2) &&
      n_edge_label_nodes > 0) {
    SparseMatrix A2;

    real *x2 = MALLOC(sizeof(real) * (A->m) * dim);
    A2 = shorting_edge_label_nodes(A, n_edge_label_nodes, edge_label_nodes);
    multilevel_spring_electrical_embedding(dim, A2, NULL, ctrl, NULL, NULL, x2,
                                           0, NULL, flag);

    assert(!(*flag));
    attach_edge_label_coordinates(dim, A, n_edge_label_nodes, edge_label_nodes,
                                  x, x2);
    remove_overlap(dim, A, x, label_sizes, ctrl->overlap, ctrl->initial_scaling,
                   ctrl->edge_labeling_scheme, n_edge_label_nodes,
                   edge_label_nodes, A, ctrl->do_shrinking, flag);
  ...
  mctrl = Multilevel_control_new(ctrl->multilevel_coarsen_scheme,
                                 ctrl->multilevel_coarsen_mode);
  ...
  do {
    ...
    if (ctrl->method == METHOD_SPRING_ELECTRICAL) {
      if (ctrl->tscheme == QUAD_TREE_NONE) {
        spring_electrical_embedding_slow(dim, grid->A, ctrl, grid->node_weights,
                                         xc, flag);
      } else if (ctrl->tscheme == QUAD_TREE_FAST ||
                 (ctrl->tscheme == QUAD_TREE_HYBRID &&
                  grid->A->m > QUAD_TREE_HYBRID_SIZE)) {
        ...
        spring_electrical_embedding_fast(dim, grid->A, ctrl, grid->node_weights,
                                         xc, flag);
      } else if (ctrl->tscheme == QUAD_TREE_NORMAL) {
        spring_electrical_embedding(dim, grid->A, ctrl, grid->node_weights, xc,
                                    flag);
      } else {
        spring_electrical_embedding(dim, grid->A, ctrl, grid->node_weights, xc,
                                    flag);
      ...
      if (Multilevel_is_finest(
              grid)) {/* gradually reduce influence of entropy */
        spring_maxent_embedding(dim, grid->A, grid->D, ctrl, grid->node_weights,
                                xc, rho, flag);
  ...
  /* rotation has to be done before overlap removal, since rotation could induce
   * overlaps */
  if (dim == 2) {
    pcp_rotate(n, dim, x);
  }
...
void multilevel_spring_electrical_embedding_gv(void *data) {
  ...
  multilevel_spring_electrical_embedding_core(
      d->dim, d->A, d->D, d->ctrl, d->node_weights, d->label_sizes, d->x,
      d->n_edge_label_nodes, d->edge_label_nodes, d->flag);
  gviewer_reset_graph_coord(d->A, d->dim,
                            d->x); /* A inside spring_electrical gets deleted */
}
void multilevel_spring_electrical_embedding(
    int dim, SparseMatrix A, SparseMatrix D, spring_electrical_control ctrl,
    real *node_weights, real *label_sizes, real *x, int n_edge_label_nodes,
    int *edge_label_nodes, int *flag) {
  struct multilevel_spring_electrical_embedding_data data = {
      dim,         A, D,                  ctrl,             node_weights,
      label_sizes, x, n_edge_label_nodes, edge_label_nodes, flag};

  int argcc = 1;
  char **argvv;

  if (!Gviewer)
    return multilevel_spring_electrical_embedding_core(
        dim, A, D, ctrl, node_weights, label_sizes, x, n_edge_label_nodes,
        edge_label_nodes, flag);
  ...
  // gviewer_init(&argcc, argvv, 0.01, 20, 60, 2*1010, 2*770, A, dim, x,
  // &(data), multilevel_spring_electrical_embedding_gv);
  gviewer_init(&argcc, argvv, 0.01, 20, 60, 320, 320, A, dim, x, &(data),
               multilevel_spring_electrical_embedding_gv);
  free(argvv);
}
#else
void multilevel_spring_electrical_embedding(
