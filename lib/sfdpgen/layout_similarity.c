...
real distance_cropped(real *x, int dim, int i, int j) {
  ...
  for (k = 0; k < dim; k++)
    dist +=
        (x[i * dim + k] - x[j * dim + k]) * (x[i * dim + k] - x[j * dim + k]);
  dist = sqrt(dist);
...
static real get_r(int n, real *x, real *y, real theta, real *r, real *p,
                  real *q) {
  /* find the best scaling factor r* such that
     f[theta, r] = Sum[(x[i,1] + p - r(Sin[theta] y[i,1] + Cos[theta]
     y[i,2]))^2+(x[i,2] + q - r(Cos[theta] y[i,1] -Sin[theta] y[i,2]))^2, i = 1,
     n] is minimized. Return the value f[theta, r*],
     also return r.
  */
  real top = 0, bot = 0, c, s, f = 0, yy, yx, xxavg = 0, xyavg = 0, yxavg = 0,
       yyavg = 0;
  int i;
  ...
  *q = yyavg / n * (*r) - xyavg;
  ...
  for (i = 0; i < n; i++) {
    ...
    yx = c * y[2 * i] - s * y[2 * i + 1];
    yy = s * y[2 * i] + c * y[2 * i + 1];
    f += (x[2 * i] + *p - (*r) * yx) * (x[2 * i] + *p - (*r) * yx) +
         (x[2 * i + 1] + *q - (*r) * yy) * (x[2 * i + 1] + *q - (*r) * yy);
  }
  return f;
}

static real dispacement(int n, real *x, real *y, real *rmin, real *thetamin,
                        real *pmin, real *qmin) {
  ...
  *qmin = q;
  ...
  for (i = 0; i < n; i++)
    normx += x[2 * i] * x[2 * i] + x[2 * i + 1] * x[2 * i + 1];
...
int main(int argc, char *argv[]) {
  ...
  fprintf(stderr,
          "mean = %f std = %f disimilarity = %f area = %f badness = %f "
          "displacement = %f\n",
          mean, dev, dev / mean, ratio, (dev / mean + 1) * ratio,
          dispacement(n, x, y, &r, &theta, &p, &q));
  fprintf(stderr, "theta = %f scaling = %f, shift = {%f, %f}\n", theta, 1 / r,
          p, q);

  printf("%.2f %.2f %.2f\n", dev / mean,
         dispacement(n, x, y, &r, &theta, &p, &q), ratio / 1000000.);
