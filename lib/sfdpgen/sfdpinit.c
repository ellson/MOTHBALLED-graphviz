...
static void sfdp_init_node_edge(graph_t *g) {
  ...
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
    for (e = agfstout(g, n); e; e = agnxtout(g, e)) sfdp_init_edge(e);
  }
}
...
static real *getPos(Agraph_t *g, spring_electrical_control ctrl) {
  ...
  if (agfindnodeattr(g, "pos") == NULL) return pos;
...
static void sfdpLayout(graph_t *g, spring_electrical_control ctrl, int hops,
                       pointf pad) {
  real *sizes;
  real *pos;
  Agnode_t *n;
  ...
  if (ctrl->method ==
      METHOD_SPRING_MAXENT) /* maxent can work with distance matrix */
  ...
  switch (ctrl->method) {
    ...
    case METHOD_SPRING_MAXENT:
      multilevel_spring_electrical_embedding(Ndim, A, D, ctrl, NULL, sizes, pos,
                                             n_edge_label_nodes,
    ...
    case METHOD_STRESS: {
      ...
      if (!D) {
        D = SparseMatrix_get_real_adjacency_matrix_symmetrized(
            A); /* all distance 1 */
        weighted = FALSE;
      } else {
      ...
      if (hops > 0) {
        ...
        if (Verbose) {
          fprintf(stderr,
                  "extracted a %d-neighborhood graph of %d edges from a graph "
                  "of %d edges\n",
                  hops, (DD->nz) / 2, (D->nz / 2));
        }
...
#if UNUSED
static int late_mode(graph_t *g, Agsym_t *sym, int dflt) {
  char *s;
  int v;
  int rv;
...
static int late_quadtree_scheme(graph_t *g, Agsym_t *sym, int dflt) {
  ...
  if (isdigit(*s)) {
    ...
    if (!strcasecmp(s, "none") || !strcasecmp(s, "false")) {
      rv = QUAD_TREE_NONE;
    } else if (!strcasecmp(s, "normal") || !strcasecmp(s, "true") ||
               !strcasecmp(s, "yes")) {
      rv = QUAD_TREE_NORMAL;
...
static void tuneControl(graph_t *g, spring_electrical_control ctrl) {
  ...
  ctrl->p =
      -1.0 * late_double(g, agfindgraphattr(g, "repulsiveforce"), -AUTOP, 0.0);
  ctrl->multilevels = late_int(g, agfindgraphattr(g, "levels"), INT_MAX, 0);
  ctrl->smoothing =
      late_smooth(g, agfindgraphattr(g, "smoothing"), SMOOTHING_NONE);
  ctrl->tscheme =
      late_quadtree_scheme(g, agfindgraphattr(g, "quadtree"), QUAD_TREE_NORMAL);
  /* ctrl->method = late_mode(g, agfindgraphattr(g, "mode"),
  ...
  ctrl->rotation =
      late_double(g, agfindgraphattr(g, "rotation"), 0.0, -MAXDOUBLE);
  ctrl->edge_labeling_scheme =
      late_int(g, agfindgraphattr(g, "label_scheme"), 0, 0);
