...
void Operator_uniform_stress_matmul_delete(Operator o) { FREE(o->data); }

real *Operator_uniform_stress_matmul_apply(Operator o, real *x, real *y) {
  struct uniform_stress_matmul_data *d =
      (struct uniform_stress_matmul_data *)(o->data);
  SparseMatrix A = d->A;
  real alpha = d->alpha;
  real xsum = 0.;
  int m = A->m, i;
...
real cg(Operator Ax, Operator precond, int n, int dim, real *x0, real *rhs,
        real tol, int maxit, int *flag) {
  real *x, *b, res = 0;
  int k, i;
  x = N_GNEW(n, real);
  b = N_GNEW(n, real);
...
real *jacobi(SparseMatrix A, int dim, real *x0, real *rhs, int maxit,
             int *flag) {
...
real SparseMatrix_solve(SparseMatrix A, int dim, real *x0, real *rhs, real tol,
                        int maxit, int method, int *flag) {
  Operator Ax, precond;
  int n = A->m;
  real res = 0;
  *flag = 0;
