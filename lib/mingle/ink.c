...
static double dotPoint(point_t a, point_t b) { return a.x * b.x + a.y * b.y; }

point_t Origin;

/* sumLengths:
 */
static double sumLengths_avoid_bad_angle(point_t *points, int npoints,
  ...
  for (i = 0; i < npoints; i++) {
    ...
    sum += len;
    cos_theta =
        (diff_x0 * diff_x + diff_y0 * diff_y) / MAX((len * len0), 0.00001);
    cos_max = MAX(cos_max, cos_theta);
  }

  // distance of single line from 'meeting' to 'end'
  return sum * (cos_max +
                angle_param); /* straight line gives angle_param - 1, turning
                                 angle of 180 degree gives angle_param + 1 */
}
static double sumLengths(point_t *points, int npoints, point_t end,
                         point_t meeting) {
  int i;
  double sum = 0;
...
/* bestInk:
 */
static double bestInk(point_t *points, int npoints, point_t begin, point_t end,
                      double prec, point_t *meet, real angle_param) {
  ...
  do {
    ...
    if (angle_param < 1) {
      ...
      value1 =
          sumLengths_avoid_bad_angle(points, npoints, end, first, angle_param);
      value2 =
          sumLengths_avoid_bad_angle(points, npoints, end, second, angle_param);
      value3 =
          sumLengths_avoid_bad_angle(points, npoints, end, third, angle_param);
      value4 =
          sumLengths_avoid_bad_angle(points, npoints, end, fourth, angle_param);
    }
  ...
  } while (fabs(value1 - value4) / (MIN(value1, value4) + 1e-10) > prec &&
...
static double project_to_line(point_t pt, point_t left, point_t right,
                              real angle) {
  /* pt
     ^  ^
     .   \ \
   ...
   d .    a\    \
     ...
     .       \       \
     .   c    \  alpha \             b
     .<------left:0 ----------------------------> right:1. Find the projection
   of pt on the left--right line. If the turning angle is small,
     |                  |
     |<-------f---------
     we should get a negative number. Let a := left->pt, b := left->right, then
   we are calculating:
     c = |a| cos(a,b)/|b| b
     d = a - c
     f = -ctan(alpha)*|d|/|b| b
  ...
  */
  ...
  if (angle <= 0 || angle >= M_PI)
    return 2; /* return outside of the interval which should be handled as a
                 sign of infeasible turning angle */
  alpha = angle;
...
double ink(pedge *edges, int numEdges, int *pick, double *ink0, point_t *meet1,
  ...
  *ink0 = 0;
  ...
  for (i = 0; i < numEdges; i++) {
    ...
    (*ink0) +=
        sqrt((sources[i].x - targets[i].x) * (sources[i].x - targets[i].x) +
             (sources[i].y - targets[i].y) * (sources[i].y - targets[i].y));
  ...
  if (angle > 0 && angle < M_PI) {
    if (cbegin + cend > 1 || cbegin > 1 || cend > 1) {
      /* no point can be found that satisfies the angular constraints, so we
       * give up and set ink to a large value */
      if (Verbose && 0)
        fprintf(stderr,
                "no point satisfying any angle constraints can be found. "
                "cbeg=%f cend=%f\n",
                cbegin, cend);
      inkUsed = 1000 * (*ink0);
      *meet1 = *meet2 = mid;
    ...
    cbegin = MAX(0, cbegin); /* make sure the new adjusted point is with in
                                [begin,end] internal */
    diff = subPoint(end, begin);
    begin = addPoint(begin, scalePoint(diff, cbegin));

    cend = MAX(0, cend); /* make sure the new adjusted point is with in
                            [end,begin] internal */
  ...
  // fprintf(stderr,"beg={%f,%f}, meet1={%f,%f}, meet2={%f,%f}, mid={%f,%f},
  // end={%f,%f}\n",begin.x, begin.y, meet1->x, meet1->y, meet2->x, meet2->y,
  // mid.x, mid.y, end.x, end.y);

  // fprintf(stderr,"ink used = %f\n",inkUsed);
  //  fprintf(stderr,"{cb,ce}={%f, %f} end={%f,%f}, meet={%f,%f},
  // mid={%f,%f}\n",cbegin, cend, end.x, end.y, meet2->x, meet2->y, mid.x,
  // mid.y);
  free(sources);
  free(targets);
  return inkUsed;
