...
#define WD2(n) \
  (X_marg.doAdd ? (ND_width(n) / 2.0 + X_marg.x) : ND_width(n) * X_marg.x / 2.0)
#define HT2(n)                                    \
  (X_marg.doAdd ? (ND_height(n) / 2.0 + X_marg.y) \
                : ND_height(n) * X_marg.y / 2.0)
    static xparams xParams = {60,  /* numIters */
                              0.0, /* T0 */
                              0.3, /* K */
                              1.5, /* C */
                              0    /* loopcnt */
...
#ifdef DEBUG
  if (Verbose && ov) fprintf(stderr, "Warning: %d overlaps remain on ", ov);
  pr2graphs(g, GORIG(agroot(g)));
  fprintf(stderr, "\n");
#endif
...
void fdp_xLayout(graph_t *g, xparams *xpms) {
  ...
  if (tries && !x_layout(g, xpms, tries)) return;
