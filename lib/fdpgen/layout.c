 ...
 */
static void finalCC(graph_t *g, int c_cnt, graph_t **cc, point *pts,
...
static node_t *mkDeriveNode(graph_t *dg, char *name) {
  ...
  dn = agnode(dg, name, 1);
  agbindrec(dn, "Agnodeinfo_t", sizeof(Agnodeinfo_t), TRUE);  // node custom
                                                              // data
...
static void chkPos(graph_t *g, node_t *n, layout_info *infop, boxf *bbp) {
  ...
  if (p[0]) {
    ...
    c = '\0';
    if (sscanf(p, "%lf,%lf,%lf,%lf%c", &bb.LL.x, &bb.LL.y, &bb.UR.x, &bb.UR.y,
               &c) >= 4) {
      if (PSinputscale > 0.0) {
...
static graph_t *deriveGraph(graph_t *g, layout_info *infop) {
  ...
  for (i = 1; i <= GD_n_cluster(g); i++) {
    ...
    ND_clust(dn) = subg;
    ND_id(dn) = id++;
    if (infop->G_coord) chkPos(subg, dn, infop, &fix_bb);
  ...
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
    if (!DNODE(n)) {
      if (PARENT(n) && (PARENT(n) != GPARENT(g))) {
        agerr(AGERR,
              "node \"%s\" is contained in two non-comparable clusters \"%s\" "
              "and \"%s\"\n",
...
static void layout(graph_t *g, layout_info *infop) {
  ...
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) DNODE(n) = 0;
...
static void fdpSplines(graph_t *g) {
  ...
  if (State < GVSPLINES) spline_edges1(g, ET_LINE);
}
