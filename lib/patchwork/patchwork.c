...
void dumpTree(treenode_t* r, int ind) {
  ...
  fprintf(stderr, "%s (%f)\n",
          (r->kind == AGNODE ? agnameof(r->u.n) : agnameof(r->u.subg)),
          r->area);
  for (cp = r->leftchild; cp; cp = cp->rightsib) dumpTree(cp, ind + 1);
}
#endif
...
static void layoutTree(treenode_t* tree) {
  ...
  if (Verbose)
    fprintf(stderr, "rec %f %f %f %f\n", tree->r.x[0], tree->r.x[1],
            tree->r.size[0], tree->r.size[1]);
  for (i = 0; i < nc; i++) {
    nodes[i]->r = recs[i];
    if (Verbose)
      fprintf(stderr, "%f - %f %f %f %f = %f (%f %f %f %f)\n", areas_sorted[i],
              recs[i].x[0] - recs[i].size[0] * 0.5,
              recs[i].x[1] - recs[i].size[1] * 0.5,
              recs[i].x[0] + recs[i].size[0] * 0.5,
              recs[i].x[1] + recs[i].size[1] * 0.5,
              recs[i].size[0] * recs[i].size[1], recs[i].x[0], recs[i].x[1],
              recs[i].size[0], recs[i].size[1]);
  }
  free(nodes);
  free(areas_sorted);
  free(recs);
  ...
  cp = tree->leftchild;
  for (i = 0; i < nc; i++) {
    if (cp->kind == AGRAPH) layoutTree(cp);
    cp = cp->rightsib;
  }
}
...
static void walkTree(treenode_t* tree) {
  ...
  if (tree->kind == AGRAPH) {
    for (p = tree->leftchild; p; p = p->rightsib) walkTree(p);
    x0 = tree->r.x[0];
    y0 = tree->r.x[1];
    wd = tree->r.size[0];
    ht = tree->r.size[1];
    ...
    if (Verbose)
      fprintf(stderr, "%s coord %.5g %.5g ht %f width %f\n", agnameof(n),
              ND_coord(n).x, ND_coord(n).y, ND_ht(n), ND_xsize(n));
  }
}
