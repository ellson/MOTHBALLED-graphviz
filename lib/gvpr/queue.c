...
static void freef(Dt_t *d, nsitem *obj, Dtdisc_t *disc) { free(obj); }

static Dtdisc_t ndisc = {
    offsetof(nsitem, np), sizeof(void *),  offsetof(nsitem, link),
    (Dtmake_f)makef,      (Dtfree_f)freef, 0,
    0,                    0,               0};
...
void freeQ(queue *nq) { dtclose(nq); }
