...
int validTVT(int c) { return ((TV_flat <= c) && (c <= TV_prepostrev)); }
...
gvprbinding* findBinding(Gpr_t* state, char* fname) {
  ...
  key.name = fname;
  bp = (gvprbinding*)bsearch(&key, state->bindings, state->n_bindings,
                             sizeof(gvprbinding), bindingcmpf);
  if (!bp) error(ERROR_ERROR, "No binding for \"%s\" in call()", fname);
  return bp;
}
...
void closeGPRState(Gpr_t* state) {
  ...
  if (state->tmp) sfclose(state->tmp);
  free(state->dp);
  free(state);
}

#ifdef WIN32_DLL
int pathisrelative(char* path) { return PathIsRelative(path); }
#endif
