...
static char *case_str[] = {"BEGIN", "END", "BEG_G", "END_G",
                           "N",     "E",   "EOF",   "ERROR", };
 ...
 *   action = '{' <expr> '}'
 */
static case_t parseCase(Sfio_t *str, char **guard, int *gline, char **action,
                        int *aline) {
  case_t kind;
...
static parse_block *addBlock(parse_block *last, char *stmt, int line,
                             int n_nstmts, case_info *nodelist, int n_estmts,
                             case_info *edgelist) {
 ...
 * return new item as tail
 */
static case_info *addCase(case_info *last, char *guard, int gline, char *action,
                          int line, int *cnt) {
  case_info *item;
...
parse_prog *parseProg(char *input, int isFile) {
  ...
  while (more) {
    switch (parseCase(str, &guard, &gline, &action, &line)) {
      ...
      case BeginG:
        if (action &&
            (begg_stmt || nodelist || edgelist)) {/* non-empty block */
          blockl = addBlock(blockl, begg_stmt, l_beging, n_nstmts, nodelist,
                            n_estmts, edgelist);
          if (!blocklist) blocklist = blockl;
          n_blocks++;
  ...
  if (begg_stmt || nodelist || edgelist) {/* non-empty block */
    blockl = addBlock(blockl, begg_stmt, l_beging, n_nstmts, nodelist, n_estmts,
                      edgelist);
    if (!blocklist) blocklist = blockl;
    n_blocks++;
  }
