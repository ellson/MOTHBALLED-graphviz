...
void nodeInduce(Agraph_t *selected) {
  ...
  for (n = agfstnode(selected); n; n = agnxtnode(selected, n)) {
    for (e = agfstout(base, n); e; e = agnxtout(base, e)) {
      if (agsubnode(selected, aghead(e), FALSE)) agsubedge(selected, e, TRUE);
    }
  }
}
...
int copyAttr(Agobj_t *src, Agobj_t *tgt) {
  ...
  while ((sym = agnxtattr(srcg, skind, sym))) {
    ...
    if (!tsym) tsym = agattr(tgtg, tkind, sym->name, sym->defval);
    val = agxget(src, sym);
    if (aghtmlstr(val)) {
...
static Dtdisc_t edgepair = {
    offsetof(edgepair_t, key), sizeof(Agedge_t *), offsetof(edgepair_t, link),
    NIL(Dtmake_f),             NIL(Dtfree_f),      (Dtcompar_f)cmppair,
...
Agraph_t *cloneG(Agraph_t *g, char *name) {
  ...
  if (!name || (*name == '\0')) name = agnameof(g);
...
static void cc_dfs(Agraph_t *g, Agraph_t *comp, Agnode_t *n) {
  ...
  for (e = agfstedge(g, n); e; e = agnxtedge(g, e, n)) {
    if (agtail(e) == n)
      ...
      other = agtail(e);
    if (!CCMARKED(other)) cc_dfs(g, comp, other);
  }
}
...
Agraph_t *compOf(Agraph_t *g, Agnode_t *n) {
  ...
  for (np = agfstnode(g); np; np = agnxtnode(g, np)) CCUNMARK(np);
...
char *toLower(Expr_t *pgm, char *s, Sfio_t *tmps) {
  ...
  while ((c = *s++)) sfputc(tmps, tolower(c));
...
char *toUpper(Expr_t *pgm, char *s, Sfio_t *tmps) {
  ...
  while ((c = *s++)) sfputc(tmps, toupper(c));
...
static int colorxlate(char *str, gvcolor_t *color, color_type_t target_type) {
  ...
  if (!fake.name) return COLOR_MALLOC_FAIL;
  if ((last == NULL) || (last->name[0] != fake.name[0]) ||
      (strcmp(last->name, fake.name))) {
    last = (hsvrgbacolor_t *)bsearch((void *)&fake, (void *)color_lib,
  ...
  switch (target_type) {
    ...
    case RGBA_BYTE:
      ...
      break;
    case CMYK_BYTE:
      color->u.cmyk[0] = color->u.cmyk[1] = color->u.cmyk[2] =
          color->u.cmyk[3] = 0;
      break;
    case RGBA_WORD:
...
char *colorx(Expr_t *ex, char *incolor, char *fmt, Sfio_t *fp) {
  ...
  if (rc != COLOR_OK) return "";

  switch (type) {
    case HSVA_DOUBLE:
      sfprintf(fp, "%.03f %.03f %.03f", color.u.HSVA[0], color.u.HSVA[1],
               color.u.HSVA[2]);
      if (alpha) sfprintf(fp, " %.03f", color.u.HSVA[3]);
      break;
    case RGBA_BYTE:
      sfprintf(fp, "#%02x%02x%02x", color.u.rgba[0], color.u.rgba[1],
               color.u.rgba[2]);
      if (alpha) sfprintf(fp, "%02x", color.u.rgba[3]);
      break;
    case CMYK_BYTE:
      sfprintf(fp, "#%02x%02x%02x%02x", color.u.cmyk[0], color.u.cmyk[1],
