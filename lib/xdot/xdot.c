...
static int agxbput(char *s, agxbuf *xb) {
  ...
  if (xb->ptr + ssz > xb->eptr) agxbmore(xb, ssz);
  memcpy(xb->ptr, s, ssz);
  xb->ptr += ssz;
  return ssz;
}
...
#ifdef UNUSED
  int r, sz;
  r = sscanf(s, "%lf %lf %lf %lf%n", &(rp->x), &(rp->y), &(rp->w), &(rp->h),
             &sz);
  if (r != 4)
    return 0;
  else
  ...
  switch (*s++) {
    ...
    case 'c':
      ...
      if (clr.type == xd_none) {
        ...
        op->u.grad_color = clr;
        if (ops) op->drawfunc = ops[xop_grad_color];
      }
      break;
    ...
    case 't':
      ...
      CHK(s);
      if (ops) op->drawfunc = ops[xop_fontchar];
      break;
...
xdot *parseXDotFOn(char *s, drawfunc_t fns[], int sz, xdot *x) {
  ...
  if (!x) {
    x = NEW(xdot);
    if (sz <= sizeof(xdot_op)) sz = sizeof(xdot_op);
  ...
  if (error) x->flags |= XDOT_PARSE_ERROR;
  if (x->cnt) {
...
static void gradprint(char *s, void *v) { agxbput(s, (agxbuf *)v); }
