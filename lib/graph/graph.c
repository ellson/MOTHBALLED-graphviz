...
Dtdisc_t agNamedisc = {
    offsetof(Agnode_t, name), -1,              -1, /* link offset */
...
Dtdisc_t agNodedisc = {
    ...
    NIL(Dtmake_f),          NIL(Dtfree_f),   (Dtcompar_f)agcmpid,
    NIL(Dthash_f),          NIL(Dtmemory_f), NIL(Dtevent_f)};

Dtdisc_t agIndisc = {0,                 /* pass whole object as key */
                     0,             -1, /* link offset */
                     NIL(Dtmake_f), NIL(Dtfree_f),   (Dtcompar_f)agcmpin,
                     NIL(Dthash_f), NIL(Dtmemory_f), NIL(Dtevent_f)};

Dtdisc_t agOutdisc = {0,               /* pass whole object as key */
                      0,           -1, /* link offset */
                      (Dtmake_f)0, (Dtfree_f)0,   (Dtcompar_f)agcmpout,
...
#ifdef DEBUG
static int myinedgecmp(e0, e1) Agedge_t *e0, *e1;
{
  int rv = myinedgecmp(e0, e1);
  printf("compare (%s,%s:%s),(%s,%s:%s) = %d\n",
         e0->head ? e0->head->name : "nil", e0->tail ? e0->tail->name : "nil",
         e0->attr && e0->attr[KEYX] ? e0->attr[KEYX] : "nil",
         e1->head ? e1->head->name : "nil", e1->tail ? e1->tail->name : "nil",
         e1->attr && e1->attr[KEYX] ? e1->attr[KEYX] : "nil", rv);
  return rv;
}
#endif
...
void agpushproto(Agraph_t *g) { dup_proto(g, g->proto); }
...
static Agraph_t *agNEWgraph(char *name, Agraph_t *parent, int kind) {
  ...
  if (parent == NULL) {
    ...
    for (i = 0; i < nobj; i++) g->attr[i] = agstrdup(AG.proto_g->attr[i]);
  } else {
    g->univ = parent->univ;
    g->root = parent->root;
    ...
    for (i = 0; i < nobj; i++) g->attr[i] = agstrdup(parent->attr[i]);
  }
...
static int reach0(Dict_t *m, Agnode_t *from, Agnode_t *to) {
  ...
  for (e = agfstout(from->graph, from); e; e = agnxtout(from->graph, e))
    if ((dtsearch(m, e->head) == NULL) && reach0(m, e->head, to)) return TRUE;
  return FALSE;
}
...
void agINSgraph(Agraph_t *g, Agraph_t *subg) {
  ...
  t = g->meta_node;
  h = subg->meta_node;
  if (t && h && (reach(h, t) == FALSE)) agedge(t->graph, t, h);
}
...
void agclose(Agraph_t *g) {
  ...
  if (is_meta == FALSE) {
    nobj = dtsize(g->univ->globattr->dict);
    for (i = 0; i < nobj; i++) agstrfree(g->attr[i]);
  }
  if (g->attr) free(g->attr);
  if (g->didset) free(g->didset);
  if (g == g->root) {
...
int agnnodes(Agraph_t *g) { return dtsize(g->nodes); }

int agnedges(Agraph_t *g) { return dtsize(g->outedges); }
