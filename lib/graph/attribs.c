...
static void add_node_attr(Agraph_t *g, Agsym_t *attr, int isnew) {
  ...
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) obj_init_attr(n, attr, isnew);
  if (g->meta_node) {
    for (n = agfstnode(g->meta_node->graph); n;
         n = agnxtnode(g->meta_node->graph, n))
...
Agraph_t *agprotograph() { return AG.proto_g; }

Agnode_t *agprotonode(Agraph_t *g) { return g->proto->n; }

Agedge_t *agprotoedge(Agraph_t *g) { return g->proto->e; }
...
static int initproto(void) {
  ...
  if (a->index != KEYX) return 1;
  a = agattr(g->proto->e, TAIL_ID, "");
  if (a->index != TAILX) return 1;
  a->printed = FALSE;
  a = agattr(g->proto->e, HEAD_ID, "");
  if (a->index != HEADX) return 1;
  a->printed = FALSE;
  return 0;
}
...
void agFREEdict(Agraph_t *g, Agdict_t *dict) {
  ...
  if (dict->list) {
    i = 0;
    while ((a = dict->list[i++])) agfreesym(a);
    free(dict->list);
  }
  free(dict);
}
...
Agdict_t *agNEWdict(char *name) {
  ...
  static Dtdisc_t symdisc = {
      ...
      -1,                                                    /* size */
      -1,                                                    /* link */
      (Dtmake_f)0,             (Dtfree_f)0,   (Dtcompar_f)0, /* use strcmp */
...
int aginitlib(int gs, int ns, int es) {
  ...
  } else if ((AG.graph_nbytes != gs) || (AG.node_nbytes != ns) ||
             (AG.edge_nbytes != es))
    agerr(AGWARN, "aginit() called multiply with inconsistent args\n");
  return rv;
}

char *agget(void *obj, char *attr) { return agxget(obj, agindex(obj, attr)); }
