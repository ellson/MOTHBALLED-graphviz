...
Agnode_t *agfstnode(Agraph_t *g) { return (Agnode_t *)dtfirst(g->nodes); }

Agnode_t *agnxtnode(Agraph_t *g, Agnode_t *n) {
  return (Agnode_t *)dtnext(g->nodes, n);
}

Agnode_t *aglstnode(Agraph_t *g) { return (Agnode_t *)dtlast(g->nodes); }
...
Agnode_t *agNEWnode(Agraph_t *subg, char *name, Agnode_t *proto) {
  ...
  for (i = 0; i < nobj; i++)
    n->attr[i] =
        agstrdup(proto ? proto->attr[i] : subg->univ->nodeattr->list[i]->value);
  return n;
}
...
void agFREEnode(Agnode_t *n) {
  ...
  if (AG_IS_METAGRAPH(n->graph) == FALSE) {
    ...
    for (i = 0; i < nobj; i++) agstrfree(n->attr[i]);
  }
  free(n->attr);
  free(n->didset);
  free(n);
