...
int aglinenumber() { return Line_number; }
...
void aglexinit(FILE *fp, gets_f mygets) {
  if (Lexer_fp != fp) LexPtr = NULL;
  Lexer_fp = fp;
  if (mygets) AG.fgets = mygets;
  if (AG.fgets == NULL) AG.fgets = fgets;
  if (AG.linebuf == NULL) {
    LineBufSize = BUFSIZ;
...
static char *lex_gets(void) {
  ...
  do {
    ...
    clp = AG.fgets(AG.linebuf + curlen + 1, LineBufSize - curlen - 1, Lexer_fp);
    if (clp == NULL) break;
...
int aglex(void) {
  ...
  do {
    ...
    if ((Line_number == 1) && !strncmp(LexPtr, (char *)BOM, 3)) LexPtr += 3;
...
static void error_context(void) {
  ...
  for (p = LexPtr - 1; (p > buf) && (!isspace(*(unsigned char *)p)); p--)
    ;
  if (buf < p) {
    c = *p;
    *p = '\0';
...
void agseterr(agerrlevel_t lvl) { agerrlevel = lvl; }

int agerrors(void) { return MAX(agmaxerr, AG.syntax_errors); }
