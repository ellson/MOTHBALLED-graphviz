...
static Agraph_t *finish(int rv, Agraph_t *g) {
  if (rv) {
    if (g) agclose(g);
    return NULL;
  } else
...
static int _is_number_char(char c) { return (isdigit(c) || (c == '.')); }
  ...
  *p++ = '\"';
  ...
  while (uc) {
    ...
    *p++ = (char)uc;
    uc = *(unsigned char *)s++;
    cnt++;
    if (uc && backslash_pending &&
        !((_is_number_char(p[-1]) || isalpha(p[-1]) || (p[-1] == '\\')) &&
          (_is_number_char(uc) || isalpha(uc)))) {
      *p++ = '\\';
      *p++ = '\n';
      has_special = TRUE;
      backslash_pending = FALSE;
    } else if (uc && cnt % SMALLBUF == 0) {
      if (!((_is_number_char(p[-1]) || isalpha(p[-1]) || (p[-1] == '\\')) &&
            (_is_number_char(uc) || isalpha(uc)))) {
        *p++ = '\\';
        *p++ = '\n';
        has_special = TRUE;
...
void agsetiodisc(char *(*myfgets)(char *s, int size, FILE *stream),
                 size_t (*myfwrite)(const void *ptr, size_t size, size_t nmemb,
                                    FILE *stream),
 ...
 */
char *agcanonical(char *str) { return agstrcanon(str, getoutputbuffer(str)); }

/* agcanon:
 * Safe version of agstrcanon but using only double quotes.
 * Any string can be used, but there is no check for html strings.
 */
char *agcanon(char *str) { return _agstrcanon(str, getoutputbuffer(str)); }
...
Dtdisc_t agEdgedisc = {
    offsetof(Agedge_t, id), sizeof(int),     -1,
    NIL(Dtmake_f),          NIL(Dtfree_f),   (Dtcompar_f)agcmpid,
    NIL(Dthash_f),          NIL(Dtmemory_f), NIL(Dtevent_f)};
static void write_subg(Agraph_t *g, FILE *fp, Agraph_t *par, int indent,
                       printdict_t *state) {
  Agraph_t *subg, *meta;
  Agnode_t *n, *pn;
  Agedge_t *e, *pe;
...
#ifdef ferror
/* if ferror is a macro (__SUNPRO_C __CYGWIN__ __MINGW32__ __FreeBSD__ and poss
 * others)
 * then wrap it in a function */
static int agferror(FILE *stream) { return ferror(stream); }
#endif
