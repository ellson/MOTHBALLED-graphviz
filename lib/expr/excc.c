...
static void print(Excc_t* cc, Exnode_t* expr) {
  ...
  if ((x = expr->data.print.args)) {
    sfprintf(
        cc->ccdisc->text, "sfprintf(%s, \"%s",
        expr->data.print.descriptor->op == CONSTANT &&
                expr->data.print.descriptor->data.constant.value.integer == 2
            ? "sfstderr"
            : "sfstdout",
...
static void gen(Excc_t* cc, register Exnode_t* expr) {
  ...
  switch (expr->op) {
    ...
    case FOR:
      ...
      sfprintf(cc->ccdisc->text, ") {");
      if (expr->data.operand.right) gen(cc, expr->data.operand.right);
      sfprintf(cc->ccdisc->text, "}");
      return;
    case ID:
      if (cc->ccdisc->ccf)
    ...
    case ITERATER:
      if (expr->op == DYNAMIC) {
        ...
        sfprintf(
            ...
            cc->id, cc->tmp,
            expr->data.generate.array->data.variable.symbol->name, cc->id,
            cc->tmp, expr->data.generate.index->name, cc->id, cc->tmp, cc->id,
            cc->tmp, expr->data.generate.array->data.variable.symbol->name,
            cc->id, cc->tmp);
    ...
    case SWITCH:
      ...
      while ((x = x->data.select.next)) {
        if (n) sfprintf(cc->ccdisc->text, "else ");
        if (!(p = x->data.select.constant))
          ...
          while ((v = *p++)) {
            ...
            if (t == STRING)
              sfprintf(cc->ccdisc->text, "strmatch(%stmp_%d, \"%s\")", cc->id,
              ...
              switch (t) {
                case INTEGER:
                case UNSIGNED:
                  sfprintf(cc->ccdisc->text, "%I*u", sizeof(v->integer),
                           v->integer);
                  break;
                default:
    ...
    case I2F:
      ...
      return;
    case S2I:
      /* sfprintf(cc->ccdisc->text, "strto%s(", sizeof(intmax_t) > sizeof(long)
       * ? "ll" : "l"); */
  ...
  if (x->type == STRING) {
    ...
    if (!y) sfprintf(cc->ccdisc->text, "%s", exopname(expr->op));
    sfprintf(cc->ccdisc->text, "(");
    gen(cc, x);
    if (y) {
...
static int global(Dt_t* table, void* object, void* handle) {
...
Excc_t* exccopen(Expr_t* expr, Exccdisc_t* disc) {
  ...
  if (!(id = disc->id)) id = "";
  if (!(cc = newof(0, Excc_t, 1, strlen(id) + 2))) return 0;
  cc->expr = expr;
  cc->disc = expr->disc;
  cc->id = (char*)(cc + 1);
  cc->ccdisc = disc;
  if (!(disc->flags & EX_CC_DUMP)) {
    ...
    sfprintf(disc->text, "\n#include <ast.h>\n");
    if (*id) sfsprintf(cc->id, strlen(id) + 2, "%s_", id);
    sfprintf(disc->text, "\n");
...
int exdump(Expr_t* expr, Exnode_t* node, Sfio_t* sp) {
  ...
  ccdisc.text = sp;
  if (!(cc = exccopen(expr, &ccdisc))) return -1;
  if (node)
    gen(cc, node);
  else
