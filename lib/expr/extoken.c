...
static void trace(Expr_t* ex, int lev, char* op, int c) {
  ...
  switch (c) {
    ...
    case '=':
      s = t = buf;
      *t++ = ' ';
      if (!lev && exlval.op != c) *t++ = exlval.op;
      *t++ = c;
      *t = 0;
      break;
    case AND:
    ...
    case INTEGER:
      s = " INTEGER ";
      sfsprintf(t = buf, sizeof(buf), "%I*d", sizeof(exlval.integer),
                exlval.integer);
      break;
    case LABEL:
      s = " LABEL ";
    ...
    case UNSIGNED:
      s = " UNSIGNED ";
      sfsprintf(t = buf, sizeof(buf), "%I*u", sizeof(exlval.integer),
                exlval.integer);
      break;
    case BREAK:
      s = " break";
  ...
  if (x)
    error(TRACE_lex + lev, "%s: [%d] %04d%s%s (%x)", op, ex->input->nesting, c,
          s, t, x);

  else
    error(TRACE_lex + lev, "%s: [%d] %04d%s%s", op, ex->input->nesting, c, s,
          t);
}
...
again:
  for (;;) switch (c = lex(ex)) {
      ...
      case '+':
      case '-':
        if ((q = lex(ex)) == c) return exlval.op = c == '+' ? INC : DEC;
        goto opeq;
      case '*':
      case '%':
      case '^':
      ...
      case '"':
        ...
        if (q == '"' || (ex->disc->flags & EX_CHARSTRING)) {
          if (!(exlval.string = vmstrdup(ex->vm, s))) goto eof;
        ...
        if ((c = lex(ex)) == 'x' || c == 'X') {
          ...
          if (c == '#') {
            ...
            floating:
              q = FLOATING;
              sfputc(ex->tmp, c);
              while (isdigit(c = lex(ex))) sfputc(ex->tmp, c);
            }
            if (c == 'e' || c == 'E') {
              q = FLOATING;
              sfputc(ex->tmp, c);
        ...
        if (isalpha(c) || c == '_' || c == '$') {
          ...
          switch (exlval.id->lex) {
            ...
            case NAME:
              ...
              if (c == ':' && !expr.nolabel) return LABEL;
              break;
            case PRAGMA:
              /*
