...
static char *LibInfo[] = {"graphviz",      /* Program */
...
void gvFinalize(GVC_t *gvc) {
  if (gvc->active_jobs) gvrender_end_job(gvc->active_jobs);
}
...
int gvFreeContext(GVC_t *gvc) {
  ...
  gvjobs_delete(gvc);
  if (gvc->config_path) free(gvc->config_path);
  if (gvc->input_filenames) free(gvc->input_filenames);
