...
int gvLayout(GVC_t *gvc, graph_t *g, const char *engine) {
  ...
  if (gvLayoutJobs(gvc, g) == -1) return -1;

  /* set bb attribute for basic layout.
   * doesn't yet include margins, scaling or page sizes because
   * those depend on the renderer being used. */
  if (GD_drawing(g)->landscape)
    sprintf(buf, "%d %d %d %d", ROUND(GD_bb(g).LL.y), ROUND(GD_bb(g).LL.x),
            ROUND(GD_bb(g).UR.y), ROUND(GD_bb(g).UR.x));
  else
    sprintf(buf, "%d %d %d %d", ROUND(GD_bb(g).LL.x), ROUND(GD_bb(g).LL.y),
...
int gvRender(GVC_t *gvc, graph_t *g, const char *format, FILE *out) {
  ...
  job->output_file = out;
  if (out == NULL) job->flags |= OUTPUT_NOT_REQUIRED;
...
int gvRenderFilename(GVC_t *gvc, graph_t *g, const char *format,
                     const char *filename) {
  int rc;
  GVJ_t *job;
...
/* gvFreeRenderData:
 * Utility routine to free memory allocated in gvRenderData, as the application
 * code may use
 * a different runtime library.
 */
void gvFreeRenderData(char *data) { free(data); }
