...
static const int PAGE_ALIGN = 4095; /* align to a 4K boundary (less one),
                                       typical for Linux, Mac OS X and Windows
                                       memory allocation */
  ...
  if (job->output_data) {
    if (len > job->output_data_allocated - (job->output_data_position + 1)) {
      ...
      job->output_data_allocated =
          (job->output_data_position + len + 1 + PAGE_ALIGN) & ~PAGE_ALIGN;
...
static void auto_output_filename(GVJ_t *job) {
  ...
  if (job->graph_index)
    ...
    gidx[0] = '\0';
  if (!(fn = job->input_filename)) fn = "noname.gv";
...
int gvdevice_initialize(GVJ_t *job) {
  ...
  /* if the device has no initialization then it uses file output */
    ...
    if (gvc->common.auto_outfile_names) auto_output_filename(job);
...
int gvferror(FILE *stream) {
  ...
  if (!job->gvc->write_fn && !job->output_data) return ferror(job->output_file);
...
void gvdevice_format(GVJ_t *job) {
  ...
  if (gvde && gvde->format) gvde->format(job);
  gvflush(job);
}
...
/* gvprintf:
 * Unless vsnprintf is available, this function is unsafe due to the fixed
 * buffer size.
...
static char *gvprintnum(size_t *len, double number) {
  static char tmpbuf
      [sizeof(maxnegnumstr)]; /* buffer big enough for worst case */
  char *result =
      tmpbuf + sizeof(maxnegnumstr); /* init result to end of tmpbuf */
  long int N;
  ...
  if (number > -maxnegnum) {/* +ve limit */
    *len =
        sizeof(maxnegnumstr) - 2; /* len doesn't include terminator or sign */
...
int main(int argc, char *argv[]) {
  ...
  double test[] = {
      -maxnegnum * 1.1, -maxnegnum * .9, 1e8,            10.008, 10,
      1,                .1,              .01,            .006,   .005,
      .004,             .001,            1e-8,           0,      -0,
      -1e-8,            -.001,           -.004,          -.005,  -.006,
      -.01,             -.1,             -1,             -10,    -10.008,
      -1e8,             maxnegnum * .9,  maxnegnum * 1.1};
