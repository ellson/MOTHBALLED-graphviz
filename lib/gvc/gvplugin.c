...
api_t gvplugin_api(char *str) {
  ...
  for (api = 0; api < ARRAY_SIZE(api_names); api++) {
    if (strcmp(str, api_names[api]) == 0) return (api_t)api;
  }
...
char *gvplugin_api_name(api_t api) {
  if (api >= ARRAY_SIZE(api_names)) return NULL;
...
#define TYPSIZ 63
  ...
  while (*pnext) {
    ...
    if ((p = strchr(pnxt, ':'))) *p = '\0';
    if (strcmp(pins, pnxt) <= 0) break;
    pnext = &((*pnext)->next);
  }
  ...
  while (*pnext) {
    ...
    if ((p = strchr(pnxt, ':'))) *p = '\0';
    if (strcmp(pins, pnxt) != 0) break;
    if (quality >= (*pnext)->quality) break;
    pnext = &((*pnext)->next);
  }
 ...
 */
static boolean gvplugin_activate(GVC_t *gvc, api_t api, const char *typestr,
                                 char *name, char *path,
                                 gvplugin_installed_t *typeptr) {
...
gvplugin_library_t *gvplugin_library_load(GVC_t *gvc, char *path) {
  ...
  if (!gvc->common.demand_loading) return NULL;
  ...
  if (gvc->common.verbose >= 2) fprintf(stderr, "Loading %s\n", p);
...
#define TYPBUFSIZ 64
  ...
  for (pnext = &(gvc->apis[api]); *pnext; pnext = &((*pnext)->next)) {
    ...
    dep = strchr(typ, ':');
    if (dep) *dep++ = '\0';
    if (strcmp(typ, reqtyp)) continue; /* types empty or mismatched */
    ...
    if (!reqpkg || strcmp(reqpkg, (*pnext)->package->name) == 0) {
      ...
      if (dep && (apidep != api)) /* load dependency if needed, continue if
                                     can't find */
        if (!(gvplugin_load(gvc, apidep, dep))) continue;
      break;
    }
  }
  rv = *pnext;
...
char *gvplugin_list(GVC_t *gvc, api_t api, const char *str) {
  ...
  if (new) {/* if the type was not found, or if str without ':',
    ...
    for (pnext = plugin; *pnext; pnext = &((*pnext)->next)) {
      ...
      if (!typestr_last) free(typestr_last);
      typestr_last = q;
    }
    if (!typestr_last) free(typestr_last);
  }
...
char **gvPluginList(GVC_t *gvc, char *kind, int *sz, const char *str) {
  ...
  if (!kind) return NULL;
  for (api = 0; api < ARRAY_SIZE(api_names); api++) {
    if (!strcasecmp(kind, api_names[api])) break;
  }
...
Agraph_t *gvplugin_graph(GVC_t *gvc) {
  Agraph_t *g, *sg, *ssg;
  Agnode_t *n, *m, *loadimage_n, *renderer_n, *device_n, *textlayout_n,
      *layout_n;
  Agedge_t *e;
  Agsym_t *a;
      ...
      for (pnext = &(gvc->apis[api]); *pnext; pnext = &((*pnext)->next)) {
        if ((*pnext)->package == package) {
          ...
          switch (api) {
            case API_device:
            case API_loadimage:
              /* draw device as box - record last device in plugin  (if any) in
               * device_n */
              /* draw loadimage as box - record last loadimage in plugin  (if
               * any) in loadimage_n */
              ...
              break;
            case API_render:
              /* draw renderers as ellipses - record last renderer in plugin (if
               * any) in renderer_n */
              ...
              break;
            case API_textlayout:
              /* draw textlayout  as invtriangle - record last textlayout in
               * plugin  (if any) in textlayout_n */
              ...
              break;
            case API_layout:
              /* draw textlayout  as hexagon - record last layout in plugin  (if
               * any) in layout_n */
              strcpy(bufb, api_names[api]);
              strcat(bufb, "_");
              strcat(bufb, q);
        ...
        if ((*pnext)->package == package) {
          ...
          switch (api) {
            case API_device:
              ...
              e = agfindedge(g, n, m);
              if (!e) e = agedge(g, n, m, NULL, 1);
              if (p && *p) {
                strcpy(bufb, "render_");
                strcat(bufb, p);
                m = agfindnode(ssg, bufb);
                if (!m) m = agnode(g, bufb, 1);
                e = agfindedge(g, m, n);
                if (!e) e = agedge(g, m, n, NULL, 1);
              }
              break;
            case API_loadimage:
              strcpy(buf2, q);
