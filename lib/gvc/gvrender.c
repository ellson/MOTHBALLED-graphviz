...
int gvrender_begin_job(GVJ_t *job) {
  ...
  if (gvdevice_initialize(job)) return 1;
  if (gvre) {
    if (gvre->begin_job) gvre->begin_job(job);
  }
  return 0;
}
...
void gvrender_end_job(GVJ_t *job) {
  ...
  if (gvre) {
    if (gvre->end_job) gvre->end_job(job);
  }
...
void gvrender_begin_graph(GVJ_t *job, graph_t *g) {
  ...
  if (gvre) {
    ...
    if (gvre->begin_graph) gvre->begin_graph(job);
...
void gvrender_end_graph(GVJ_t *job) {
  ...
  if (gvre) {
    if (gvre->end_graph) gvre->end_graph(job);
  }
  gvdevice_format(job);
}
...
void gvrender_begin_page(GVJ_t *job) {
  ...
  if (gvre) {
    if (gvre->begin_page) gvre->begin_page(job);
  }
}
...
void gvrender_end_page(GVJ_t *job) {
  ...
  if (gvre) {
    if (gvre->end_page) gvre->end_page(job);
  }
}
...
void gvrender_end_layer(GVJ_t *job) {
  ...
  if (gvre) {
    if (gvre->end_layer) gvre->end_layer(job);
  }
}
...
void gvrender_begin_cluster(GVJ_t *job, graph_t *sg) {
  ...
  if (gvre) {
    if (gvre->begin_cluster) gvre->begin_cluster(job);
  }
}
...
void gvrender_end_cluster(GVJ_t *job, graph_t *g) {
  ...
  if (gvre) {
    if (gvre->end_cluster) gvre->end_cluster(job);
  }
}
...
void gvrender_begin_nodes(GVJ_t *job) {
  ...
  if (gvre) {
    if (gvre->begin_nodes) gvre->begin_nodes(job);
  }
}
...
void gvrender_end_nodes(GVJ_t *job) {
  ...
  if (gvre) {
    if (gvre->end_nodes) gvre->end_nodes(job);
  }
}
...
void gvrender_begin_edges(GVJ_t *job) {
  ...
  if (gvre) {
    if (gvre->begin_edges) gvre->begin_edges(job);
  }
}
...
void gvrender_end_edges(GVJ_t *job) {
  ...
  if (gvre) {
    if (gvre->end_edges) gvre->end_edges(job);
  }
}
...
void gvrender_begin_node(GVJ_t *job, node_t *n) {
  ...
  if (gvre) {
    if (gvre->begin_node) gvre->begin_node(job);
  }
}
...
void gvrender_end_node(GVJ_t *job) {
  ...
  if (gvre) {
    if (gvre->end_node) gvre->end_node(job);
  }
}
...
void gvrender_begin_edge(GVJ_t *job, edge_t *e) {
  ...
  if (gvre) {
    if (gvre->begin_edge) gvre->begin_edge(job);
  }
}
...
void gvrender_end_edge(GVJ_t *job) {
  ...
  if (gvre) {
    if (gvre->end_edge) gvre->end_edge(job);
  }
}
...
void gvrender_begin_anchor(GVJ_t *job, char *href, char *tooltip, char *target,
  ...
  if (gvre) {
    if (gvre->begin_anchor) gvre->begin_anchor(job, href, tooltip, target, id);
  }
}
...
void gvrender_end_anchor(GVJ_t *job) {
  ...
  if (gvre) {
    if (gvre->end_anchor) gvre->end_anchor(job);
  }
}
...
void gvrender_begin_label(GVJ_t *job, label_type type) {
  ...
  if (gvre) {
    if (gvre->begin_label) gvre->begin_label(job, type);
  }
}
...
void gvrender_end_label(GVJ_t *job) {
  ...
  if (gvre) {
    if (gvre->end_label) gvre->end_label(job);
  }
}
...
void gvrender_textspan(GVJ_t *job, pointf p, textspan_t *span) {
  ...
  if (span->str && span->str[0] &&
    ...
    if (gvre) {
      if (gvre->textspan) gvre->textspan(job, PF, span);
    }
  }
}
...
void gvrender_set_pencolor(GVJ_t *job, char *name) {
  ...
  if (gvre) {
    ...
    if (gvre->resolve_color) gvre->resolve_color(job, color);
  }
    ...
    if (s)
      while ((p = line = *s++)) {
        if (streq(line, "solid"))
          ...
          agerr(AGWARN, "gvrender_set_style: unsupported style %s - ignoring\n",
                line);
        }
      }
  }
...
void gvrender_polygon(GVJ_t *job, pointf *af, int n, int filled) {
  ...
  if (gvre) {
    if (gvre->polygon && job->obj->pen != PEN_NONE) {
      ...
      if (noPoly) job->obj->pencolor = save_pencolor;
    }
  }
}
...
void gvrender_beziercurve(GVJ_t *job, pointf *af, int n, int arrow_at_start,
                          int arrow_at_end, boolean filled) {
  gvrender_engine_t *gvre = job->render.engine;
...
void gvrender_comment(GVJ_t *job, char *str) {
  ...
  if (gvre) {
    if (gvre->comment) gvre->comment(job, str);
  }
}
