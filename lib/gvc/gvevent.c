...
static void gv_graph_state(GVJ_t *job, graph_t *g) {
  ...
  if (!a) a = agfindgraphattr(g, s_URL);
  if (a) job->selected_href = strdup_and_subst_obj(agxget(g, a), (void *)g);
}
...
static void gv_node_state(GVJ_t *job, node_t *n) {
  ...
  if (!a) a = agfindnodeattr(agraphof(n), s_URL);
  if (a) job->selected_href = strdup_and_subst_obj(agxget(n, a), (void *)n);
}
...
static void gv_edge_state(GVJ_t *job, edge_t *e) {
  ...
  gv_argvlist_set_item(nlist, j++,
                       agisdirected(agraphof(agtail(e))) ? "->" : "--");
  ...
  if (!a) a = agfindedgeattr(agraphof(aghead(e)), s_URL);
  if (a) job->selected_href = strdup_and_subst_obj(agxget(e, a), (void *)e);
}
  ...
  if (obj) {
    switch (agobjkind(obj)) {
      case AGRAPH:
        ...
        if (a) job->active_tooltip = strdup_and_subst_obj(agxget(g, a), obj);
        break;
      case AGNODE:
        n = (node_t *)obj;
        ND_gui_state(n) |= GUI_STATE_ACTIVE;
        a = agfindnodeattr(agraphof(n), s_tooltip);
        if (a) job->active_tooltip = strdup_and_subst_obj(agxget(n, a), obj);
        break;
      case AGEDGE:
        e = (edge_t *)obj;
        ED_gui_state(e) |= GUI_STATE_ACTIVE;
        a = agfindedgeattr(agraphof(aghead(e)), s_tooltip);
        if (a) job->active_tooltip = strdup_and_subst_obj(agxget(e, a), obj);
        break;
    }
  }
}
...
static void gvevent_button_press(GVJ_t *job, int button, pointf pointer) {
  switch (button) {
    ...
    case 4:
      ...
      job->fit_mode = 0;
      if (job->rotation) {
        job->focus.x -= (pointer.y - job->height / 2.) * (ZOOMFACTOR - 1.) /
                        (job->zoom * job->devscale.y);
        job->focus.y += (pointer.x - job->width / 2.) * (ZOOMFACTOR - 1.) /
                        (job->zoom * job->devscale.x);
      } else {
        job->focus.x += (pointer.x - job->width / 2.) * (ZOOMFACTOR - 1.) /
                        (job->zoom * job->devscale.x);
        job->focus.y += (pointer.y - job->height / 2.) * (ZOOMFACTOR - 1.) /
                        (job->zoom * job->devscale.y);
      }
      job->zoom *= ZOOMFACTOR;
      job->needs_refresh = 1;
...
static void gvevent_motion(GVJ_t *job, pointf pointer) {
  ...
  if (fabs(dx) < EPSILON &&
      fabs(dy) < EPSILON) /* ignore motion events with no motion */
    return;
...
static int quit_cb(GVJ_t *job) { return 1; }
...
static int toggle_fit_cb(GVJ_t *job) {
  ...
  if (job->fit_mode) {
    ...
    job->zoom = MIN((double)job->width / (double)dflt_width,
...
static void gvevent_delete(GVJ_t *job) { /* FIXME */ }
...
static void gvevent_read(GVJ_t *job, const char *filename, const char *layout) {
  ...
  if (!filename) {
    ...
    f = fopen(filename, "r");
    if (!f) return; /* FIXME - need some error handling */
    g = agread(f, NIL(Agdisc_t *));
    fclose(f);
  }
  if (!g) return; /* FIXME - need some error handling */

  if (gvc->g) {
    gvle = gvc->layout.engine;
    if (gvle && gvle->cleanup) gvle->cleanup(gvc->g);
    graph_cleanup(gvc->g);
    agclose(gvc->g);
  }
...
static void gvevent_render(GVJ_t *job, const char *format,
                           const char *filename) {
  gvRenderFilename(job->gvc, job->gvc->g, format, filename);
}

gvevent_key_binding_t gvevent_key_binding[] = {{"Q", quit_cb},
                                               {"Left", left_cb},
                                               {"KP_Left", left_cb},
                                               {"Right", right_cb},
                                               {"KP_Right", right_cb},
...
gvdevice_callbacks_t gvdevice_callbacks = {
    gvevent_refresh, gvevent_button_press, gvevent_button_release,
    gvevent_motion,  gvevent_modify,       gvevent_delete,
    gvevent_read,    gvevent_layout,       gvevent_render, };
