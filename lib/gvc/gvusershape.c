...
static knowntype_t knowntypes[] = {
    ...
    /*    { SVG_MAGIC,  sizeof(SVG_MAGIC)-1,  FT_SVG,  "svg",  },  - viewers
       expect xml preamble */
...
static unsigned int svg_units_convert(double n, char *u) {
  if (strcmp(u, "in") == 0) return ROUND(n * POINTS_PER_INCH);
  if (strcmp(u, "px") == 0) return ROUND(n * POINTS_PER_INCH / 96);
  if (strcmp(u, "pc") == 0) return ROUND(n * POINTS_PER_INCH / 6);
  if (strcmp(u, "pt") == 0 || strcmp(u,
                                     "\"") == 0) /* ugly!!  - if there are no
                                                    inits then the %2s get the
                                                    trailing '"' */
    return ROUND(n);
  if (strcmp(u, "cm") == 0) return ROUND(n * POINTS_PER_CM);
  if (strcmp(u, "mm") == 0) return ROUND(n * POINTS_PER_MM);
  return 0;
}
...
static void jpeg_size(usershape_t *us) {
  ...
  while (TRUE) {
     ...
     * a parsing error.
     */
    if (!get_int_msb_first(us->f, 1, &marker)) return;
    ...
    /* A stand-alone... */
    if (strchr((char *)standalone_markers, marker)) continue;
    ...
    if (marker == 0xc2) {
      ...
      if (!get_int_msb_first(us->f, 3, &junk)) return;
    ...
    if (!get_int_msb_first(us->f, 2, &length)) return;
...
static void ps_size(usershape_t *us) {
  ...
  while (fgets(line, sizeof(line), us->f)) {
     ...
     */
    if (!(linep = strstr(line, "%%BoundingBox:"))) continue;
...
static void usershape_close(Dict_t *dict, Void_t *p, Dtdisc_t *disc) {
  ...
  if (us->f) fclose(us->f);
  if (us->data && us->datafree) us->datafree(us);
  free(us);
}

static Dtdisc_t ImageDictDisc = {offsetof(usershape_t, name), /* key */
                                 -1,                          /* size */
                                 0,                           /* link offset */
                                 NIL(Dtmake_f),               usershape_close,
...
static usershape_t *gvusershape_open(const char *name) {
  ...
  if (!(us = gvusershape_find(name))) {
    if (!(us = zmalloc(sizeof(usershape_t)))) return NULL;
