...
/*
   ...
   plugin_api.
    If multiple plugins of the same type are found then the highest quality
   wins.
...
static gvplugin_package_t *gvplugin_package_record(GVC_t *gvc, char *path,
                                                   char *name) {
      ...
      do {
        if (nest == 2) {
          ...
          if (nest == 2)
            ...
            quality = 0;
          rc = gvplugin_install(gvc, gv_api, type, quality, package, NULL);
          if (!rc) {
...
void gvconfig_plugin_install_from_library(GVC_t *gvc, char *path,
...
#ifdef ENABLE_LTDL
static void gvconfig_write_library_config(GVC_t *gvc, char *path,
                                          gvplugin_library_t *library,
                                          FILE *f) {
  gvplugin_api_t *apis;
  gvplugin_installed_t *types;
  ...
  for (apis = library->apis; (types = apis->types); apis++) {
    ...
    for (i = 0; types[i].type; i++) {
      ...
      if (!(gvplugin_load(gvc, apis->api, types[i].type))) fprintf(f, "#FAILS");
      ...
      for (i = 0; i < c; ++i) {
        ...
        if (tmp) {
          if (tmp > path) {
            ...
            while ((*s != '/') && (s > path)) s--;
            if (strncmp(s, DOTLIBS, STRLEN(DOTLIBS)) == 0) continue;
          }
      ...
      if (f) {
        while (!feof(f)) {
          ...
          if (tmp) {
            *tmp = 0;
            /* Check for real /lib dir. Don't accept pre-install /.libs */
            if (strcmp(strrchr(path, '/'), "/.libs") == 0) continue;
            strcpy(line, path); /* use line buffer for result */
            strcat(line,
                   "/graphviz"); /* plugins are in "graphviz" subdirectory */
            libdir = line;
            break;
          }
        }
...
static void config_rescan(GVC_t *gvc, char *config_path) {
  ...
  if (config_path) {
    ...
    fprintf(f,
            "# You may temporarily disable a plugin by removing or commenting "
            "out\n");
    fprintf(f,
            "# a line in this file, or you can modify its \"quality\" value to "
            "affect\n");
    ...
    for (i = 0; i < globbuf.gl_pathc; i++) {
      ...
      if (re_status == 0) {
        ...
        if (library) {
          ...
          if (path) path++;
          if (f && path) gvconfig_write_library_config(gvc, path, library, f);
        }
      }
    }
  }
...
#define MAX_SZ_CONFIG 100000
  ...
  if (gvc->common.demand_loading) {
    ...
    if (!gvc->config_path) {
      gvc->config_path =
          gmalloc(strlen(libdir) + 1 + strlen(config_file_name) + 1);
...
static int glob(GVC_t *gvc, char *pattern, int flags,
  ...
  do {
    if (cnt >= arrsize - 1) {
      arrsize += 512;
      if (str)
        str = (char **)realloc(str, arrsize * sizeof(char *));
      else
...
static void globfree(glob_t *pglob) {
  ...
  if (pglob->gl_pathv) free(pglob->gl_pathv);
}
#endif
#endif
